...
fn demo_transform() {
    ...
    for v in &translated.vertices {
        assert!(
            v.point.x >= 4.99 && v.point.x <= 6.01,
            "expected x in [5,6], got {}",
            v.point.x
        );
    }
    ...
    for v in &box_brep.vertices {
        assert!(
            ...
            "original x should be in [0,1], got {}",
            v.point.x
        );
    }
    ...
    let mut box2 = BRep::from_primitive(PrimitiveSolid::Box {
        width: 2.0,
        height: 2.0,
        depth: 2.0,
    });
...
fn demo_extrema() {
    ...
    let c1 = Curve3::Line(Line3 {
        origin: DVec3::ZERO,
        direction: DVec3::X,
    });
    let c2 = Curve3::Line(Line3 {
    ...
    println!(
        "Line(x=5,z-axis) ↔ Circle(r=2,xy-plane): distance={:.6}",
        d3
    );
    ...
    let c_inner = Curve3::Circle(Circle3 {
        center: DVec3::ZERO,
        normal: DVec3::Z,
        radius: 2.0,
    });
    let c_outer = Curve3::Circle(Circle3 {
        center: DVec3::ZERO,
        normal: DVec3::Z,
        radius: 5.0,
    });
    ...
    let c_same1 = Curve3::Circle(Circle3 {
        center: DVec3::ZERO,
        normal: DVec3::Z,
        radius: 3.0,
    });
    let c_same2 = Curve3::Circle(Circle3 {
        center: DVec3::ZERO,
        normal: DVec3::Z,
        radius: 3.0,
    });
...
fn demo_color_import() {
    ...
    assert!(
        step_str.contains("COLOUR_RGB"),
        "STEP output should contain COLOUR_RGB"
    );
    assert!(
        step_str.contains("STYLED_ITEM"),
        "STEP output should contain STYLED_ITEM"
    );
    println!(
        "Write colored STEP: {} bytes, contains color entities",
        step_str.len()
    );
    ...
    match result {
        Ok((brep, color_opt)) => {
            ...
            match color_opt {
                Some(color) => {
                    println!(
                        "Color map returned: {} face colors, solid_color={:?}",
                        color.face_colors.len(),
                        color.solid_color
                    );
    ...
    let plain_step = StepWriter::write_string(
        &box_brep,
        ExportSelection {
            selected_faces: &[],
            selected_edges: &[],
    ...
    let (_, no_color) =
        StepReader::parse_string_with_color(&plain_step).expect("plain STEP should parse");
    println!(
        "Plain STEP (no color entities) → color=None: {}",
        no_color.is_none()
    );
