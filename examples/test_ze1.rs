...
use rcad_algorithms::{BooleanOpType, boolean_op_with_retry, total_surface_area};
use rcad_modeling::make_cylinder_brep;

fn main() {
    // Match the generated test: both at center (0,0,2)
    let b1 =
        make_cylinder_brep(DVec3::new(0.0, 0.0, 2.0), DVec3::Z, DVec3::X, 1.0, 4.0).expect("b1");

    let pivot = DVec3::new(0.0, 0.0, 2.0);
    let mut b2 =
        make_cylinder_brep(DVec3::new(0.0, 0.0, 2.0), DVec3::Z, DVec3::X, 1.0, 4.0).expect("b2");
