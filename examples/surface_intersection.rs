...
use rcad_algorithms::{intersect_surfaces, SurfaceCurve, SurfaceSurfaceIntersection};
...
fn demo_intss() {
    ...
    // 3. Plane × Sphere — great circle
    {
        ...
        if let SurfaceCurve::Circle(c) = &r.curves[0] {
            assert!((c.radius - 3.0).abs() < 1e-6, "expected r=3, got {}", c.radius);
    ...
    {
        let p = Surface3::Plane(Plane { origin: DVec3::new(0.0,0.0,2.0), normal: DVec3::Z });
        let s = Surface3::Sphere(SphericalSurface { center: DVec3::ZERO, axis: DVec3::Z, radius: 3.0 });
        let r = intersect_surfaces(&p, &s);
        ...
        if let SurfaceCurve::Circle(c) = &r.curves[0] {
            ...
            assert!((c.radius - expected).abs() < 1e-6, "expected r≈{expected:.4}, got {}", c.radius);
            println!("  PASS: circle r={:.4}", c.radius);
        } else { panic!("expected Circle"); }
    }
    ...
    {
        let p = Surface3::Plane(Plane { origin: DVec3::new(0.0,0.0,3.0), normal: DVec3::Z });
        let s = Surface3::Sphere(SphericalSurface { center: DVec3::ZERO, axis: DVec3::Z, radius: 3.0 });
        let r = intersect_surfaces(&p, &s);
    ...
    {
        let p = Surface3::Plane(Plane { origin: DVec3::new(0.0,0.0,2.0), normal: DVec3::Z });
        let c = Surface3::Cylinder(CylindricalSurface { origin: DVec3::ZERO, axis: DVec3::Z, radius: 2.0 });
    ...
    {
        let s1 = Surface3::Sphere(SphericalSurface { center: DVec3::ZERO, axis: DVec3::Z, radius: 1.0 });
        let s2 = Surface3::Sphere(SphericalSurface { center: DVec3::new(10.0,0.0,0.0), axis: DVec3::Z, radius: 1.0 });
        let r = intersect_surfaces(&s1, &s2);
...
fn demo_trimmed() {
    ...
    // 2. TrimmedSurface wraps a Sphere
    {
        ...
        assert!((north.z - 5.0).abs() < 1e-6, "expected z=5 at north pole, got {}", north.z);
        println!("Sphere northern hemisphere trim: north pole at z={:.4}  PASS", north.z);

        // Equator (v=π/2): should be at radius 5 in XY plane
        let equator = s.point_at(0.0, PI / 2.0);
        assert!((equator.length() - 5.0).abs() < 1e-6);
        assert!(equator.z.abs() < 1e-6, "equator should be at z=0, got {}", equator.z);
        println!("Sphere equator: ({:.4},{:.4},{:.4})  PASS", equator.x, equator.y, equator.z);
    }
    ...
    //     that the reader handles RECTANGULAR_TRIMMED_SURFACE gracefully)
    {
        ...
        println!("STEP round-trip box: {} faces  PASS", brep2.solids[0].shells[0].faces.len());
    }
}
