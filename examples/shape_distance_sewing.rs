...
use rcad_kernel::{
    ...
    min_distance, point_to_shape_distance, BRep,
};
use rcad_algorithms::{section_curves, section_polylines, SectionCurve};
...
fn demo_shape_distance() {
    ...
    println!("  closest point: ({:.4},{:.4},{:.4})", d.point_on_b.x, d.point_on_b.y, d.point_on_b.z);
    println!("  distance: {:.6}", d.distance);
    assert!((d.distance - 4.0).abs() < 0.01, "expected ~4.0, got {}", d.distance);
    println!("  PASS");

    // 2. Point to cylinder
    let cyl = BRep::from_primitive(PrimitiveSolid::Cylinder { radius: 2.0, height: 4.0 });
    let q2 = DVec3::new(10.0, 0.0, 2.0);
    let d2 = point_to_shape_distance(q2, &cyl);
    println!("\nPoint ({},{},{}) → Cylinder(r=2, h=4):", q2.x, q2.y, q2.z);
    println!("  closest point: ({:.4},{:.4},{:.4})", d2.point_on_b.x, d2.point_on_b.y, d2.point_on_b.z);
    println!("  distance: {:.6}", d2.distance);
    assert!(d2.distance > 0.0 && d2.distance < 20.0, "distance out of range: {}", d2.distance);
    println!("  PASS");
    ...
    assert!(d3.distance < 0.01, "same-position spheres should have ~0 distance, got {}", d3.distance);
    println!("  PASS");

    // 4. Sphere vs torus (both at origin)
    let torus = BRep::from_primitive(PrimitiveSolid::Torus { major_radius: 3.0, minor_radius: 0.5 });
...
fn demo_sewing() {
    ...
    assert_eq!(r1.stitched_pairs, 0, "single closed box should have no stitched pairs");
    assert_eq!(r1.free_edges.len(), 0, "single closed box should have no free edges");
    println!("  PASS");
    ...
    println!("  result faces: {}", r2.brep.solids[0].shells[0].faces.len());
    assert!(r2.stitched_pairs > 0, "expected stitched pairs for two coincident boxes");
    println!("  PASS: {} edges stitched", r2.stitched_pairs);

    // 3. Two different-size boxes (no coincident vertices) → no stitching
    let big = BRep::from_primitive(PrimitiveSolid::Box { width: 5.0, height: 5.0, depth: 5.0 });
    let small = BRep::from_primitive(PrimitiveSolid::Box { width: 1.0, height: 1.0, depth: 1.0 });
    let r3 = sew_shells(&[big, small], 1e-6);
    println!("\nDifferent boxes (no shared vertices):");
    println!("  stitched pairs: {}", r3.stitched_pairs);
    println!("  result faces: {}", r3.brep.solids[0].shells[0].faces.len());
    // 6 + 6 = 12 faces
    assert_eq!(r3.brep.solids[0].shells[0].faces.len(), 12, "expected 12 merged faces");
...
fn demo_section_curves() {
    ...
    let cyl = BRep::from_primitive(PrimitiveSolid::Cylinder { radius: 1.5, height: 4.0 });
    let plane_mid = Plane { origin: DVec3::new(0.0, 0.0, 2.0), normal: DVec3::Z };
    let curves_cyl = section_curves(&cyl, &plane_mid);
    let (a_cyl, p_cyl) = count(&curves_cyl);
    println!("\nCylinder(r=1.5, h=4) ∩ z=2 plane (perpendicular):");
    println!("  total curves: {} (analytic={}, polyline={})", curves_cyl.len(), a_cyl, p_cyl);
    let has_analytic = curves_cyl.iter().any(|c| matches!(c, SectionCurve::Analytic(_)));
    if has_analytic {
    ...
    let mut box_brep = BRep::from_primitive(PrimitiveSolid::Box { width: 2.0, height: 2.0, depth: 2.0 });
    populate_box_geom(&mut box_brep);
    let plane_box = Plane { origin: DVec3::new(0.0, 0.0, 1.0), normal: DVec3::Z };
    let curves_box = section_curves(&box_brep, &plane_box);
    let (a_box, p_box) = count(&curves_box);
    println!("\nBox(2×2×2) with analytic planes ∩ z=1 plane:");
    println!("  total curves: {} (analytic={}, polyline={})", curves_box.len(), a_box, p_box);
    ...
    assert!(!poly.is_empty(), "section_polylines should return at least one loop");
    for (i, loop_pts) in poly.iter().enumerate() {
        println!("  loop {}: {} points", i, loop_pts.len());
        for &p in loop_pts {
            assert!((p.z - 1.0).abs() < 1e-5, "all points should be at z=1, got z={}", p.z);
        }
    }
    println!("  PASS: all section points at z=1.0");

    // 5. Torus → polyline fallback (quartic section)
    let torus = BRep::from_primitive(PrimitiveSolid::Torus { major_radius: 3.0, minor_radius: 1.0 });
    let plane_torus = Plane { origin: DVec3::ZERO, normal: DVec3::Z };
    let curves_torus = section_curves(&torus, &plane_torus);
    let (a_t, p_t) = count(&curves_torus);
    println!("\nTorus(R=3, r=1) ∩ z=0 plane:");
    println!("  total curves: {} (analytic={}, polyline={})", curves_torus.len(), a_t, p_t);
