use rcad_algorithms::{boolean_op_with_options, boolean_op_with_retry, BooleanOpType, BooleanOptions, total_surface_area, extrude_polygon_solid};
use glam::DVec3;

fn main() {
    // Match I3 test case
    let pr1 = extrude_polygon_solid(
        &[DVec3::new(0.0, 50.0, 40.0), DVec3::new(0.0, 150.0, 40.0), DVec3::new(150.0, 150.0, 40.0), DVec3::new(150.0, 50.0, 40.0)],
        DVec3::new(0.0, 0.0, -1.0), 40.0
    ).expect("pr1");
    eprintln!("pr1 SA: {}", total_surface_area(&pr1));

    let pr2 = extrude_polygon_solid(
        &[DVec3::new(25.0, 25.0, 50.0), DVec3::new(100.0, 25.0, 50.0), DVec3::new(100.0, 125.0, 50.0), DVec3::new(25.0, 125.0, 50.0)],
        DVec3::new(0.0, 0.0, -1.0), 30.0
    ).expect("pr2");
    eprintln!("pr2 SA: {}", total_surface_area(&pr2));

    let po1 = boolean_op_with_retry(BooleanOpType::Difference, &pr1, &pr2).expect("po1 = pr1 - pr2");
    ...
    for (i, face) in shell.faces.iter().enumerate() {
        if let Some(surf_idx) = po1.geom.face_surface[i] {
            if let Some(surf) = po1.geom.surfaces.get(surf_idx) {
                eprintln!("  face {}: {:?} at surface idx {}", i, std::mem::discriminant(surf), surf_idx);
            }
        } else {
    ...
    let pr3 = extrude_polygon_solid(
        &[DVec3::new(50.0, 75.0, 50.0), DVec3::new(125.0, 75.0, 50.0), DVec3::new(125.0, 175.0, 50.0), DVec3::new(50.0, 175.0, 50.0)],
        DVec3::new(0.0, 0.0, -1.0), 30.0
    ).expect("pr3");
    eprintln!("pr3 SA: {}", total_surface_area(&pr3));

    let result = boolean_op_with_retry(BooleanOpType::Difference, &po1, &pr3).expect("result = po1 - pr3");

    // Also try without BVH
    let opts_no_bvh = BooleanOptions { use_bvh: false, ..Default::default() };
    ...
    eprintln!("Result faces (no BVH): {}", result_no_bvh.0.solids[0].shells[0].faces.len());
