...
fn main() {
    ...
    println!("Expected A-B: A with a rectangular notch cut from x=1.5..3 side");
    println!();
    ...
    for (si, solid) in result.solids.iter().enumerate() {
        for (shi, shell) in solid.shells.iter().enumerate() {
            ...
            for (fi, face) in shell.faces.iter().enumerate() {
                ...
                for (wi, iw) in face.inner_wires.iter().enumerate() {
                    let mut ipts: Vec<DVec3> = Vec::new();
                    for we in &iw.edges {
                        if let Some(e) = result.edges.get(we.idx) {
                            let vi = if we.forward { e.start } else { e.end };
                    ...
                    let ix_max = ipts.iter().map(|p| p.x).fold(f64::NEG_INFINITY, f64::max);
                    let iy_min = ipts.iter().map(|p| p.y).fold(f64::INFINITY, f64::min);
