...
use rcad_kernel::{centroid, surface_area, volume, BRep, PrimitiveSolid};
...
fn main() {
    ...
    let b1 = BRep::from_primitive(PrimitiveSolid::Box { width: 1.0, height: 1.0, depth: 1.0 });
    add(&mut rows, "Box 1×1×1",   "SA=6, V=1",       &b1);

    let b2 = BRep::from_primitive(PrimitiveSolid::Box { width: 2.0, height: 3.0, depth: 4.0 });
    add(&mut rows, "Box 2×3×4",   "SA=52, V=24",     &b2);

    let b3 = BRep::from_primitive(PrimitiveSolid::Box { width: 5.0, height: 0.5, depth: 0.5 });
    ...
    add(&mut rows, "Square pillar 1×1×3", "SA=14, V=3",
        &extrude(&sq, 0, DVec3::Z, 3.0).unwrap());

    let wide = polygon_profile(&[
        DVec3::new(0.0, 0.0, 0.0), DVec3::new(4.0, 0.0, 0.0),
        DVec3::new(4.0, 0.2, 0.0), DVec3::new(0.0, 0.2, 0.0),
    ]);
    add(&mut rows, "Flat plank 4×0.2×1", "SA=11.2, V=0.8",
    ...
    add(&mut rows, "Eq. prism side=2 h=4", "SA≈27.46, V≈6.93",
    ...
    add(&mut rows, "Hex bolt head r=1 h=0.6", "SA≈8.80, V≈1.56",
    ...
    add(&mut rows, "L-beam 3×3×5 t=0.5", "SA=65.5, V=13.75",
    ...
    let t_pts = [
        ...
        DVec3::new( 2.0, 0.5, 0.0), DVec3::new( 0.25, 0.5, 0.0),
        DVec3::new( 0.25, 4.0, 0.0), DVec3::new(-0.25, 4.0, 0.0),
        DVec3::new(-0.25, 0.5, 0.0), DVec3::new(-2.0, 0.5, 0.0),
    ];
    add(&mut rows, "T-beam 4×4×6 t=0.5", "cross≈3.5",
    ...
    let i_pts = [
        DVec3::new(-hw,  0.0, 0.0), DVec3::new( hw,  0.0, 0.0),
        DVec3::new( hw,  ft,  0.0), DVec3::new( wh,  ft,  0.0),
        DVec3::new( wh,  tot-ft, 0.0), DVec3::new( hw,  tot-ft, 0.0),
        DVec3::new( hw,  tot, 0.0), DVec3::new(-hw,  tot, 0.0),
        DVec3::new(-hw,  tot-ft, 0.0), DVec3::new(-wh,  tot-ft, 0.0),
        DVec3::new(-wh,  ft,  0.0), DVec3::new(-hw,  ft,  0.0),
    ];
    add(&mut rows, "I-beam h=3 len=8", "sym. section",
        &extrude(&polygon_profile(&i_pts), 0, DVec3::Z, 8.0).unwrap());

    // ── Regular polygon prisms ─────────────────────────────────────────
    for (n, label) in [(5, "Pentagon prism r=1 h=2"),
    ...
    let r_out = 1.5_f64;
    let r_in  = 0.75_f64;
    let star_pts: Vec<DVec3> = (0..12).map(|i| {
        let a = std::f64::consts::TAU * i as f64 / 12.0;
        let r = if i % 2 == 0 { r_out } else { r_in };
        DVec3::new(r * a.cos(), r * a.sin(), 0.0)
    }).collect();
    add(&mut rows, "Star (12-pt) h=1", "",
        &extrude(&polygon_profile(&star_pts), 0, DVec3::Z, 1.0).unwrap());

    // ── Print table ────────────────────────────────────────────────────
    let w = 92;
    println!("{:<28}  {:>10}  {:>10}  {:>26}  {:<16}  {}",
             "Shape", "Area", "Volume", "Centroid (x,y,z)", "Expected", "Valid?");
    println!("{}", "─".repeat(w));
    for r in &rows {
        println!("{:<28}  {:>10.4}  {:>10.4}  ({:>6.3},{:>6.3},{:>6.3})  {:<16}  {}",
            r.name, r.area, r.vol, r.c.x, r.c.y, r.c.z,
            r.expected, if r.valid { "✓" } else { "✗" });
    }
    println!("{}", "─".repeat(w));
    println!("{} shapes. All geometry computed from triangulated faces.", rows.len());
}

struct Row { name: &'static str, area: f64, vol: f64, c: DVec3, expected: &'static str, valid: bool }
