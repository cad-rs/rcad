...
use glam::DVec3;
use rcad_kernel::{
    approximate_points, closest_point_on_curve, closest_point_on_surface, interpolate_points,
    geom::{
        BSplineSurface, Circle3, Curve3, CurveEval, CylindricalSurface, Line3,
        PrimitiveSolid, SphericalSurface, Surface3, SurfaceEval, ToroidalSurface,
    },
    BRep,
};
...
fn demo_curve_fitting() {
    ...
    println!("Interpolated {} arc points with degree-{} B-spline", n_pts, curve.degree);
    ...
    println!("  max interior interpolation error: {:.2e}", max_interior_err);
    assert!(max_interior_err < 1e-4, "interior error too large: {max_interior_err}");
    ...
    println!("\nApproximated {} points with {} control points (degree {})",
        n_pts, approx.control_points.len(), approx.degree);
    ...
    assert!((a1 - pts[n_pts - 1]).length() < 1e-6, "approx end not pinned");
    println!("  endpoint pinning: {:.2e} / {:.2e}", (a0 - pts[0]).length(), (a1 - pts[n_pts - 1]).length());
    ...
    println!("  degree: {}, control points: {}", sine_curve.degree, sine_curve.control_points.len());
    ...
    assert!(matches!(interpolate_points(&[DVec3::ZERO; 3]), Err(FitError::DegeneratePoints)));
...
fn demo_projection() {
    ...
    println!("  query ({:.1},{:.1},{:.1}) → point ({:.6},{:.6},{:.6}), dist={:.6}",
        q.x, q.y, q.z, r.point.x, r.point.y, r.point.z, r.distance);
    assert!((r.point - DVec3::X).length() < 1e-5, "expected (1,0,0), got {}", r.point);
    assert!((r.distance - 2.0).abs() < 1e-5, "expected distance 2, got {}", r.distance);
    println!("  PASS");
    ...
    println!("  query ({:.1},{:.1},{:.1}) → point ({:.6},{:.6},{:.6}), dist={:.6}",
        q_line.x, q_line.y, q_line.z, r_line.point.x, r_line.point.y, r_line.point.z, r_line.distance);
    assert!((r_line.point - DVec3::new(3.0, 0.0, 0.0)).length() < 1e-4,
        "expected (3,0,0), got {}", r_line.point);
    assert!((r_line.distance - 4.0).abs() < 1e-4, "expected distance 4, got {}", r_line.distance);
    println!("  PASS");
    ...
    println!("\nSphere projection:");
    println!("  query ({:.1},{:.1},{:.1}) → point ({:.6},{:.6},{:.6}), dist={:.6}",
        q_sphere.x, q_sphere.y, q_sphere.z, r_sphere.point.x, r_sphere.point.y, r_sphere.point.z, r_sphere.distance);
    assert!((r_sphere.point - DVec3::X).length() < 1e-6, "expected (1,0,0), got {}", r_sphere.point);
    assert!((r_sphere.distance - 4.0).abs() < 1e-6, "expected dist=4, got {}", r_sphere.distance);
    println!("  PASS");
    ...
    assert!((r_cyl.point - DVec3::new(1.0, 3.0, 0.0)).length() < 1e-6,
        "expected (1,3,0), got {}", r_cyl.point);
    println!("  PASS");
    ...
    println!("  query ({:.1},{:.1},{:.1}) → point ({:.6},{:.6},{:.6}), dist={:.6}",
        q_torus.x, q_torus.y, q_torus.z, r_torus.point.x, r_torus.point.y, r_torus.point.z, r_torus.distance);
    assert!((r_torus.point - DVec3::new(4.0, 0.0, 0.0)).length() < 1e-5,
        "expected (4,0,0), got {}", r_torus.point);
    println!("  PASS");
    ...
    println!("  query ({:.1},{:.1},{:.1}) → point ({:.6},{:.6},{:.6}), dist={:.6}",
        q_bspline.x, q_bspline.y, q_bspline.z, r_bspline.point.x, r_bspline.point.y, r_bspline.point.z, r_bspline.distance);
    assert!((r_bspline.point - DVec3::new(0.5, 0.5, 0.0)).length() < 1e-4,
        "expected (0.5, 0.5, 0), got {}", r_bspline.point);
    assert!((r_bspline.distance - 3.0).abs() < 1e-4, "expected dist=3, got {}", r_bspline.distance);
    println!("  PASS");
}
...
fn demo_analytic_booleans() {
    ...
    fn make_box(w: f64, h: f64, d: f64) -> BRep {
        let mut brep = BRep::from_primitive(PrimitiveSolid::Box { width: w, height: h, depth: d });
    ...
    println!("  sphere faces: {}", sphere_brep.solids[0].shells[0].faces.len());
    ...
    match boolean_op(BooleanOpType::Union, &box_brep, &sphere_brep) {
        ...
        Err(e) => {
            println!("  Note: union returned Err({:?}) — boolean ops on curved faces are work-in-progress", e);
        }
    }
    ...
    match boolean_op(BooleanOpType::Difference, &box_brep2, &sphere_brep2) {
        ...
        Err(e) => {
            println!("  Note: difference returned Err({:?}) — boolean ops on curved faces are work-in-progress", e);
        }
    }
    ...
    let cyl_brep = BRep::from_primitive(PrimitiveSolid::Cylinder { radius: 1.5, height: 10.0 });
    ...
    let sp = SphericalSurface { center: DVec3::ZERO, axis: DVec3::Z, radius: 2.0 };
    let q_on_sphere = DVec3::new(0.0, 0.0, 5.0);
    let r_sp = closest_point_on_surface(&Surface3::Sphere(sp), q_on_sphere, 16);
    assert!((r_sp.point - DVec3::new(0.0, 0.0, 2.0)).length() < 1e-6,
        "sphere projection wrong: {}", r_sp.point);
    println!("  Sphere(r=2) projection check: PASS");

    let cyl = CylindricalSurface { origin: DVec3::ZERO, axis: DVec3::Z, radius: 1.5 };
    let q_on_cyl = DVec3::new(10.0, 0.0, 5.0);
    let r_cyl = closest_point_on_surface(&Surface3::Cylinder(cyl), q_on_cyl, 16);
    assert!((r_cyl.point - DVec3::new(1.5, 0.0, 5.0)).length() < 1e-6,
        "cylinder projection wrong: {}", r_cyl.point);
