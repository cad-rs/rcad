...
fn main() {
    ...
    section_report("x=0.5 cut", &box1, Plane { origin: DVec3::new(0.5, 0.0, 0.0), normal: DVec3::X });
    section_report("y=0.5 cut", &box1, Plane { origin: DVec3::new(0.0, 0.5, 0.0), normal: DVec3::Y });
    section_report("z=0.5 cut", &box1, Plane { origin: DVec3::new(0.0, 0.0, 0.5), normal: DVec3::Z });

    // ── 2. 2×3×4 box — diagonal cut ───────────────────────────────────
    println!("\n=== 2. 2×3×4 Box Diagonal Section ===");
    let box2 = BRep::from_primitive(PrimitiveSolid::Box { width: 2.0, height: 3.0, depth: 4.0 });
    ...
    section_report("diagonal x+z=3", &box2,
        Plane { origin: DVec3::new(1.5, 0.0, 1.5), normal: diag_normal });
    // Horizontal mid-height
    section_report("y=1.5 (mid height)", &box2,
        Plane { origin: DVec3::new(0.0, 1.5, 0.0), normal: DVec3::Y });
    ...
    section_report("z=2.5 (mid)", &pillar, Plane { origin: DVec3::new(0.0,0.0,2.5), normal: DVec3::Z });
    section_report("x=0.5", &pillar, Plane { origin: DVec3::new(0.5,0.0,0.0), normal: DVec3::X });
    section_report("45° through pillar", &pillar,
        Plane { origin: DVec3::new(0.5,0.5,2.5), normal: DVec3::new(1.0,1.0,0.0).normalize() });
    ...
    section_report("z=2 (mid height)", &prism, Plane { origin: DVec3::new(0.0,0.0,2.0), normal: DVec3::Z });
    section_report("x=1 (vertical)", &prism, Plane { origin: DVec3::new(1.0,0.0,0.0), normal: DVec3::X });
    section_report("tilted through base edge", &prism,
        Plane { origin: DVec3::new(1.0,0.5,2.0), normal: DVec3::new(0.0,1.0,1.0).normalize() });

    // ── 5. Hexagonal bolt head ─────────────────────────────────────────
    println!("\n=== 5. Hexagonal Bolt Head Sections ===");
    let hex_pts: Vec<DVec3> = (0..6).map(|i| {
    ...
    section_report("z=0.3 (mid)", &bolt, Plane { origin: DVec3::new(0.0,0.0,0.3), normal: DVec3::Z });
    section_report("x=0 (bisect)", &bolt, Plane { origin: DVec3::ZERO, normal: DVec3::X });

    // ── 6. L-beam cross section ────────────────────────────────────────
    println!("\n=== 6. L-Beam Sections ===");
    let l_pts = [
        DVec3::new(0.0, 0.0, 0.0), DVec3::new(3.0, 0.0, 0.0),
        DVec3::new(3.0, 0.5, 0.0), DVec3::new(0.5, 0.5, 0.0),
        DVec3::new(0.5, 3.0, 0.0), DVec3::new(0.0, 3.0, 0.0),
    ];
    ...
    section_report("z=3 (cross section)", &l_beam,
        Plane { origin: DVec3::new(0.0,0.0,3.0), normal: DVec3::Z });
    section_report("y=0.25 (through flange)", &l_beam,
        Plane { origin: DVec3::new(0.0,0.25,0.0), normal: DVec3::Y });
    section_report("x=0.25 (through web)", &l_beam,
        Plane { origin: DVec3::new(0.25,0.0,0.0), normal: DVec3::X });

    // ── 7. Quarter-turn elbow (revolve 90°) ───────────────────────────
    println!("\n=== 7. Quarter Elbow Sections ===");
    let r_prof = polygon_profile(&[
        DVec3::new(3.0, -0.25, 0.0), DVec3::new(3.5, -0.25, 0.0),
        DVec3::new(3.5,  0.25, 0.0), DVec3::new(3.0,  0.25, 0.0),
    ]);
    ...
    section_report("x=3.25 (start face)", &elbow,
        Plane { origin: DVec3::new(3.25, 0.0, 0.0), normal: DVec3::X });
    // Cut perpendicular to Y at elbow end (y=3.25)
    section_report("y=3.25 (end face)", &elbow,
        Plane { origin: DVec3::new(0.0, 3.25, 0.0), normal: DVec3::Y });
    // Oblique cut through the arc at 45°
    section_report("45° bisect", &elbow,
        Plane { origin: DVec3::new(2.3, 2.3, 0.0), normal: DVec3::new(1.0, 1.0, 0.0).normalize() });

    // ── 8. Half-pipe (revolve 180°) — sections ───────────────────────
    println!("\n=== 8. Half-Pipe (Revolve 180°) Sections ===");
    let ring_prof = polygon_profile(&[
        DVec3::new(2.0, -0.3, 0.0), DVec3::new(2.6, -0.3, 0.0),
        DVec3::new(2.6,  0.3, 0.0), DVec3::new(2.0,  0.3, 0.0),
    ]);
    ...
    section_report("x=2.3 (outer wall)", &half_pipe,
        Plane { origin: DVec3::new(2.3, 0.0, 0.0), normal: DVec3::X });
    section_report("z=-2.3 (at 90° rotation)", &half_pipe,
        Plane { origin: DVec3::new(0.0, 0.0, -2.3), normal: DVec3::Z });
    section_report("y=0 (mid-height)", &half_pipe,
        Plane { origin: DVec3::ZERO, normal: DVec3::Y });

    // ── 9. Stacked assembly — union of 3 extruded layers ──────────────
    println!("\n=== 9. Stacked Assembly Sections ===");
    let base_sq = polygon_profile(&[
        DVec3::new(-1.5,-1.5,0.0), DVec3::new( 1.5,-1.5,0.0),
        DVec3::new( 1.5, 1.5,0.0), DVec3::new(-1.5, 1.5,0.0),
    ]);
    ...
    let top_sq = polygon_profile(&[
        DVec3::new(-0.5,-0.5,0.0), DVec3::new( 0.5,-0.5,0.0),
        DVec3::new( 0.5, 0.5,0.0), DVec3::new(-0.5, 0.5,0.0),
    ]);
    ...
    section_report("z=0.25 (base level)", &stack,
        Plane { origin: DVec3::new(0.0,0.0,0.25), normal: DVec3::Z });
    section_report("z=0.75 (mid level)", &stack,
        Plane { origin: DVec3::new(0.0,0.0,0.75), normal: DVec3::Z });
    section_report("z=1.25 (top level)", &stack,
        Plane { origin: DVec3::new(0.0,0.0,1.25), normal: DVec3::Z });
    ...
    let arm = 1.5_f64;
    let plus_pts = [
        DVec3::new(-th,-arm,0.0), DVec3::new( th,-arm,0.0),
        DVec3::new( th,-th, 0.0), DVec3::new(arm,-th, 0.0),
        DVec3::new(arm, th, 0.0), DVec3::new( th, th, 0.0),
        DVec3::new( th, arm,0.0), DVec3::new(-th, arm,0.0),
        DVec3::new(-th, th, 0.0), DVec3::new(-arm,th, 0.0),
        DVec3::new(-arm,-th,0.0), DVec3::new(-th,-th,0.0),
    ];
    ...
    section_report("z=3 (cross section)", &plus_beam,
        Plane { origin: DVec3::new(0.0,0.0,3.0), normal: DVec3::Z });
    section_report("y=0 (horizontal slice)", &plus_beam,
        Plane { origin: DVec3::ZERO, normal: DVec3::Y });
    section_report("45° oblique", &plus_beam,
        Plane { origin: DVec3::new(0.0,0.0,3.0), normal: DVec3::new(1.0,1.0,0.0).normalize() });
...
fn section_report(label: &str, brep: &BRep, plane: Plane) {
    ...
    println!("  {:<30}  {} loop(s), {} pts total", label, loops.len(), total_pts);
}
