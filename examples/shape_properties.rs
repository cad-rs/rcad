...
fn main() {
    ...
    println!("\n=== 6. Section of Unit Box at z=0.5 ===");
    let plane_z = Plane { origin: DVec3::new(0.0, 0.0, 0.5), normal: DVec3::Z };
    ...
    println!("\n=== 7. Section of 2×3×4 Box at y=1.5 ===");
    let plane_y = Plane { origin: DVec3::new(0.0, 1.5, 0.0), normal: DVec3::Y };
    ...
    let plane_eq = Plane { origin: DVec3::new(0.0, 0.0, 0.0), normal: DVec3::Z };
    ...
    println!("  Faces        : {}",
        sphere.solids.iter().flat_map(|s| &s.shells).flat_map(|sh| &sh.faces).count());
...
fn polygon_profile(pts: &[DVec3]) -> BRep {
    ...
    let surface = Surface3::Plane(rcad_kernel::geom::Plane { origin: pts[0], normal: DVec3::Z });
