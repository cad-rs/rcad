...
use glam::DVec2;
use rcad_kernel::{
    ANGULAR, APPROXIMATION, BRep, CONFUSION, Curve2dEval, PrimitiveSolid, edge_tolerance,
    face_tolerance,
    geom::{BSplineCurve2, Curve2d, Line2d},
    model_tolerance, vertex_tolerance,
};
...
fn demo_bspline_curve2() {
    ...
    println!(
        "    t=0.5  → ({:.4}, {:.4})  (midpoint by symmetry)",
        c_half.x, c_half.y
    );
...
fn demo_curve2d_in_geomstore() {
    ...
    println!(
        "    t=0.0  → ({:.4}, {:.4})  (expect 0.0, 0.0)",
        start.x, start.y
    );
    println!(
        "    t=0.5  → ({:.4}, {:.4})  (expect 0.5, 0.5 — apex at midpoint)",
        mid.x, mid.y
    );
    println!(
        "    t=1.0  → ({:.4}, {:.4})  (expect 1.0, 0.0)",
        end.x, end.y
    );
...
fn demo_tolerance() {
    ...
    println!(
        "    CONFUSION    = {:.0e}  (point coincidence, OCCT default)",
        CONFUSION
    );
    println!("    ANGULAR      = {:.0e}  (angular, radians)", ANGULAR);
    println!(
        "    APPROXIMATION= {:.0e}  (tessellation approximation)",
        APPROXIMATION
    );
    ...
    println!(
        "    vertex_tolerance(0) = {:.2e}  (expect {:.2e})",
        vertex_tolerance(&brep_empty, 0),
        CONFUSION
    );
    println!(
        "    edge_tolerance(0)   = {:.2e}  (expect {:.2e})",
        edge_tolerance(&brep_empty, 0),
        CONFUSION
    );
    println!(
        "    face_tolerance(0)   = {:.2e}  (expect {:.2e})",
        face_tolerance(&brep_empty, 0),
        CONFUSION
    );
    println!(
        "    model_tolerance     = {:.2e}  (expect {:.2e})",
        model_tolerance(&brep_empty),
        CONFUSION
    );
    ...
    println!(
        "    vertex_tolerance(0) = {:.2e}  (expect 1.00e-6)",
        vertex_tolerance(&brep, 0)
    );
    println!(
        "    edge_tolerance(3)   = {:.2e}  (expect 1.00e-5)",
        edge_tolerance(&brep, 3)
    );
    println!(
        "    face_tolerance(5)   = {:.2e}  (expect 1.00e-4)",
        face_tolerance(&brep, 5)
    );
    println!(
        "    model_tolerance     = {:.2e}  (expect 1.00e-4, max)",
        model_tolerance(&brep)
    );
    ...
    println!(
        "    vertex_tolerance(0) = {:.2e}  (expect {:.2e}, fallback)",
        vertex_tolerance(&brep, 0),
        CONFUSION
    );
    ...
    println!(
        "    edge_tolerance(999) = {:.2e}  (expect {:.2e}, fallback)",
        edge_tolerance(&brep, 999),
        CONFUSION
    );
}
