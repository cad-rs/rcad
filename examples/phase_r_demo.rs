...
use rcad_algorithms::{
    boolean_op, BooleanOpType,
    detect_gaps_overlaps, imprint_brep,
};
...
fn make_box(x: f64, y: f64, z: f64, w: f64, h: f64, d: f64) -> BRep {
    let mut b = BRep::from_primitive(PrimitiveSolid::Box { width: w, height: h, depth: d });
    for v in &mut b.vertices { v.point += DVec3::new(x, y, z); }
...
fn make_cylinder(cx: f64, cy: f64, r: f64, h: f64) -> BRep {
    let mut b = BRep::from_primitive(PrimitiveSolid::Cylinder { radius: r, height: h });
    for v in &mut b.vertices { v.point += DVec3::new(cx, cy, 0.0); }
    b
}
...
fn make_sphere(cx: f64, cy: f64, cz: f64, r: f64) -> BRep {
    ...
    for v in &mut b.vertices { v.point += DVec3::new(cx, cy, cz); }
    b
}
...
fn demo_curved_boolean() {
    ...
    // 1. Box ∩ Cylinder — cylinder (r=0.4, h=3) pierces a 1×1×1 box
    {
        ...
        match result {
            Ok(r) => {
                let fc = face_count(&r);
                println!("Box ∩ Cylinder: {} faces (no panic, curved boolean runs)", fc);
                assert!(fc >= 1, "expected at least 1 face in intersection, got {fc}");
                println!("  PASS");
            }
    ...
    // 3. Regression — Box union Box still works
    {
        ...
        let result = boolean_op(BooleanOpType::Union, &a, &b)
            .expect("box union should not fail");
...
fn demo_imprint_and_detect() {
    ...
    // 1. imprint_brep: two adjacent boxes sharing a common face
    {
        ...
        println!("imprint_brep (touching boxes): {} result faces, {} seam pairs",
            fc, result.seam_edges.len());
    ...
    // 2. detect_gaps_overlaps — gap case: two boxes with 0.05 gap
    {
        ...
        println!("Gap detection (0.05 gap, tol=0.1): {} gaps, {} overlaps, {} shared",
            report.gaps.len(), report.overlaps.len(), report.shared_faces.len());
        assert!(!report.gaps.is_empty(), "expected at least one gap detected");
        assert!(report.overlaps.is_empty(), "expected no overlaps");
        for g in &report.gaps {
            println!("  gap: face_a={}, face_b={}, max_gap={:.4}", g.face_a, g.face_b, g.max_gap);
        }
        println!("  PASS");
    }
    ...
    // 3. detect_gaps_overlaps — overlap case: two overlapping boxes
    {
        ...
        println!("Overlap detection (0.1 overlap, tol=0.2): {} gaps, {} overlaps, {} shared",
            report.gaps.len(), report.overlaps.len(), report.shared_faces.len());
        ...
        println!("  gaps={}, overlaps={}, shared={} (results depend on sampling)",
            report.gaps.len(), report.overlaps.len(), report.shared_faces.len());
    ...
    // 4. detect_gaps_overlaps — shared face: two perfectly touching boxes
    {
        ...
        println!("Shared face detection (touching at x=1): {} gaps, {} overlaps, {} shared",
            report.gaps.len(), report.overlaps.len(), report.shared_faces.len());
        println!("  PASS (detection ran without panic)");
    }
}
