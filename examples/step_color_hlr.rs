...
use glam::DVec3;
use rcad_algorithms::{hlr, hlr_to_svg, HlrCamera};
use rcad_kernel::appearance::{Color, StepColor};
use rcad_kernel::{BRep, PrimitiveSolid};
use rcad_step::{write_assembly, AssemblyComponent};
...
fn demo_colored_step() {
    ...
    println!("  Solid color (BLUE): {} STEP records", step.lines().filter(|l| l.starts_with('#')).count());
    ...
    println!("  Per-face colors: {} STEP records", step.lines().filter(|l| l.starts_with('#')).count());
}
...
fn demo_assembly() {
    ...
    let components = vec![
        AssemblyComponent::new("RedPart", box_brep.clone())
    ...
    println!("  Assembly: {} components, {} STEP records",
        components.len(),
        step.lines().filter(|l| l.starts_with('#')).count()
    );
    let nauo_count = step.lines().filter(|l| l.contains("NEXT_ASSEMBLY_USAGE_OCCURRENCE")).count();
...
fn demo_bspline_step() {
    ...
    brep.vertices.push(rcad_kernel::topology::Vertex { point: DVec3::new(0.0, 1.0, 0.0) });
    let v0 = 0usize;
    let v1 = 1usize;
    let edge_idx = brep.edges.len();
    brep.edges.push(rcad_kernel::topology::Edge { start: v0, end: v1 });
...
fn demo_hlr() {
    ...
    for (filename, camera, label) in &views {
        ...
        save(filename, &svg);
        println!("  {label}: {} visible, {} hidden segments",
