...
use glam::DVec3;
use rcad_algorithms::{
    BooleanOpType, SimplifyOptions, boolean_op, boolean_op_simplified,
...
fn main() {
    ...
    let raw_union =
        boolean_op(BooleanOpType::Union, &left, &right).expect("raw boolean union failed");
    println!(
...
fn make_box_at(x: f64, y: f64, z: f64, size: f64) -> BRep {
    let mut brep =
        make_box_brep(DVec3::ZERO, DVec3::X, DVec3::Y, size, size, size).expect("make box");
