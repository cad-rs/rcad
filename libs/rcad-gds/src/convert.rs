...
pub fn boundary_to_wire(boundary: &GdsBoundary) -> Result<Wire> {
    if boundary.points.len() < 3 {
        return Err(GdsError::GeometryError(
            "Boundary has fewer than 3 points".to_string(),
        ));
    }
    ...
    Ok(Wire { edges: wire_edges })
}

/// Build a flat-faced BRep from a polygon at a given Z offset.
fn polygon_to_brep_face(points: &[glam::DVec2], z_offset: f64) -> Result<BRep> {
    if points.len() < 3 {
        return Err(GdsError::GeometryError(
    ...
    for boundary in &structure.boundaries {
        ...
        if layer_settings.thickness > 0.0 {
            ...
            let face_idx = flat
                .tshapes
                .iter()
                .position(|ts| matches!(&**ts, topods::TShape::Face(_)))
                .ok_or_else(|| GdsError::GeometryError("No face in flat polygon".to_string()))?;
