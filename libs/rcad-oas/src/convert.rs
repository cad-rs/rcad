...
fn polygon_to_brep_face(points: &[glam::DVec2], z_offset: f64) -> Result<BRep, OasError> {
    if points.len() < 3 {
...
fn merge_into(dst: &mut BRep, src: &BRep) {
    let solid_indices: Vec<usize> = src
        .tshapes
        .iter()
        .enumerate()
        .filter_map(|(i, ts)| match &**ts {
            TShape::Solid(_) => Some(i),
            _ => None,
        })
        .collect();
...
impl OasConverter {
    ...
    pub fn path_to_face(&self, path: &OasPath) -> Result<Face, OasError> {
        if path.points.len() < 2 {
            return Err(OasError::GeometryError(
                "Path has fewer than 2 points".to_string(),
            ));
        }
    ...
    fn path_to_polygon(path: &OasPath) -> OasPolygon {
        ...
        if path.points.len() == 2 {
            ...
            for i in 0..path.points.len() - 1 {
                ...
                if i == 0 {
                    left_points.push(glam::DVec2::new(
                        p0.x + nx * half_width,
                        p0.y + ny * half_width,
                    ));
                    right_points.push(glam::DVec2::new(
                        p0.x - nx * half_width,
                        p0.y - ny * half_width,
                    ));
                }
    ...
    pub fn cell_to_brep(&self, cell: &OasCell) -> Result<BRep, OasError> {
        ...
        for polygon in &cell.polygons {
            ...
            if ls.thickness > 0.0 {
                match rcad_modeling::builder::ops::extrude(&flat, 0, glam::DVec3::Z, ls.thickness) {
...
mod tests {
    ...
    fn test_layer_config_thickness() {
        let polygon = create_test_polygon();
        let config = LayerConfig::new().with_layer(
            1,
            LayerSettings {
                thickness: 5.0,
                z_offset: 10.0,
