//! OCCT Extrema_GGenExtCC instantiated for 2D curves — the alias
//! `Extrema_ECC2d` (TKGeomBase/Extrema/Extrema_ECC2d.hxx L28-34):
//! `Extrema_GGenExtCC<Adaptor2d_Curve2d, Extrema_Curve2dTool,
//! Adaptor2d_Curve2d, Extrema_Curve2dTool, Extrema_POnCurv2d, gp_Pnt2d,
//! Extrema_ExtPC2d>` — the template body of Extrema_GGenExtCC.hxx L55-920 that
//! computes the extremal distances between two 2D curves with Evtushenko's
//! global optimization solver.
//!
//! Also carries the file-local helpers of the same translation unit (the
//! template is instantiated per curve kind, so the helpers live in this unit
//! exactly as the 3D instantiation keeps its own copies in
//! [`super::extrema_gen_ext_cc`]):
...
//! - `Extrema_GGenExtCC_PointsInspector` (hxx L204-246) — shared with the 3D
//!   unit ([`GGenCcPointsInspector`], dimension-agnostic gp_XY code).
//!
//! Template-parameter mapping: `TheCurve1` / `TheCurve2` (Adaptor2d_Curve2d)
//! and `TheCurveTool1` / `TheCurveTool2` (Extrema_Curve2dTool) map to the
//! [`ExtremaCurve2dTool`] trait object (see
//! [`super::extrema_curve2d_tool`]); `ThePOnC` maps to [`POnCurve2d`];
//! `ThePoint` maps to `glam::DVec2`; the objective `Extrema_GlobOptFuncCCC2`
//! runs its 2D adaptor arm (`GlobOptCurves::Curves2d`).
//!
//! GAP (the only untranslated dependency, see the module doc of
//! [`super::extrema_ext_cc2d`]): `TheExtPC = Extrema_ExtPC2d` (the
//! `Extrema_GGExtPC` 2D instantiation) has no kernel translation yet, so the
//! parallel-verification block `TheExtPC anExtPC1, anExtPC2; ...` +
//! `Extrema_GGenExtCC_ProjPOnC` (hxx L771-783, L248-263) cannot run; on that
//! path `Perform` abandons with `myDone == false` — the OCCT not-done
//! equivalent — instead of reporting an unverified solution set.
//!
//! rcad note (as in the 3D unit): OCCT lets `aFunc` (the objective) and
//! `aFinder` (the solver) coexist because `math_GlobOptMin` holds a raw
//! pointer; Rust forbids the aliasing, so the objective is re-created after
//! the solver is dropped.  The objective is stateless, so the call graph is
//! unchanged.
...
use crate::base::extrema_gen_ext_cc::GGenCcPointsInspector;
...
use crate::base::extrema_math_opt::CellFilter;
...
fn ggen_ext_cc2d_comp(the_a: &DVec2, the_b: &DVec2) -> std::cmp::Ordering {
    if the_a.x < the_b.x {
...
fn ggen_ext_cc2d_change_intervals(the_ints: &mut Vec<f64>, the_nb_ints: usize) {
    let a_nb_ints = the_ints.len() - 1;
    let a_nb_add = the_nb_ints as i64 - a_nb_ints as i64;
    if a_nb_add <= 0 {
        return;
    }
    let mut a_new_ints = the_ints.clone();
    let mut a_nb_last = a_nb_ints;
    for _ in 0..a_nb_add {
        let mut a_l_int_max = -1.0f64;
...
/// OCCT `Extrema_ECC2d` = `Extrema_GGenExtCC<...>` (Extrema_GGenExtCC.hxx
/// L55-126) — the 2D instantiation.
pub struct GenExtCC2d<'a> {
    ...
    my_points2: Vec<f64>,
    /// hxx L124: void* myC[2] — the two 2D curves.
    my_c1: &'a dyn ExtremaCurve2dTool,
    my_c2: &'a dyn ExtremaCurve2dTool,
...
impl<'a> GenExtCC2d<'a> {
    ...
    pub fn new(c1: &'a dyn ExtremaCurve2dTool, c2: &'a dyn ExtremaCurve2dTool) -> Self {
        let mut this = GenExtCC2d {
    ...
    pub fn with_params(
        ...
        let mut this = GenExtCC2d::new(c1, c2);
    ...
    pub fn perform(&mut self) {
        ...
        if indmax >= 0 {
            if indmax == 0 {
                ggen_ext_cc2d_change_intervals(&mut an_intervals1, a_nb_int_opt as usize);
                a_nb_inter[0] = an_intervals1.len() - 1;
            } else {
                ggen_ext_cc2d_change_intervals(&mut an_intervals2, a_nb_int_opt as usize);
                a_nb_inter[1] = an_intervals2.len() - 1;
            }
        }
        if c1.is_closed() && a_nb_inter[0] == 1 {
            ggen_ext_cc2d_change_intervals(&mut an_intervals1, 3);
            a_nb_inter[0] = an_intervals1.len() - 1;
        }
        if c2.is_closed() && a_nb_inter[1] == 1 {
            ggen_ext_cc2d_change_intervals(&mut an_intervals2, 3);
            a_nb_inter[1] = an_intervals2.len() - 1;
        }
        ...
        {
            // The 2D adaptor arm of Extrema_GlobOptFuncCCC2
            // (Extrema_GlobOptFuncCC.cxx L225-...): myType == 2.
            let mut a_func = GlobOptFuncCCC2::new_2d(c1, c2);
        ...
        a_pnts.sort_by(ggen_ext_cc2d_comp);
        ...
        if is_parallel {
            let _a_t1 = [self.my_low_border.get(1), self.my_upp_border.get(1)];
            let _a_t2 = [
                if b_dirs_coincide {
            ...
            // GAP: TheExtPC anExtPC1, anExtPC2; anExtPC1.Initialize(C1, ...);
            // anExtPC2.Initialize(C2, ...) (hxx L771-773) and
            // Extrema_GGenExtCC_ProjPOnC<TheCurve, Extrema_ExtPC2d, gp_Pnt2d>
            // (hxx L248-263, L777-780) — dependency not yet translated
            // (Extrema_ExtPC2d, the GGExtPC 2D instantiation, has no kernel
            // translation); OCCT failure path: the parallel state cannot be
            // verified, so Perform abandons with the OCCT not-done equivalent
            // instead of reporting an unverified solution set.
            self.my_done = false;
            return;
        }
    ...
    pub fn points(&self, the_n: usize, the_p1: &mut POnCurve2d, the_p2: &mut POnCurve2d) {
        if the_n < 1 || the_n > self.nb_ext() {
            panic!("Standard_OutOfRange");
        }
