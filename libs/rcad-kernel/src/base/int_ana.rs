...
//!
//! Tolerance constants: bare epsilon values tuned for unit-scale analytic geometry
//! (leaf predicates with no scale context), matching the algorithm-layer values in
//! rcad_algorithms::tolerance.

use glam::DVec3;
use crate::geom::{any_perpendicular, *};

// ── Tolerance constants (local, for analytic leaf predicates) ──────────────

/// General absolute tolerance (1e-7 ≈ OCCT Precision::Confusion).
const TOLERANCE_ABS: f64 = 1e-7;
/// Angular/cross-product tolerance (1e-9 rad for cross magnitude).
const TOLERANCE_ANG: f64 = 1e-9;
/// Float deduplication epsilon (1e-15).
const TOLERANCE_FLOAT_DEDUP: f64 = 1e-15;
/// Minimum length for normalising degenerate vectors (1e-12).
const TOLERANCE_LEN_MIN: f64 = 1e-12;
/// Loose float tolerance (1e-14).
const TOLERANCE_FLOAT_LOOSE: f64 = 1e-14;
...
#[inline]
fn is_zero_vec(v: DVec3) -> bool {
    v.length_squared() < TOLERANCE_ABS * TOLERANCE_ABS
}
...
mod tests {
    ...
    fn test_intersect_line_plane_basic() {
        ...
        assert!((intersection.param - 5.0).abs() < 1e-7);
    }
    ...
    fn test_intersect_line_cylinder() {
        ...
        assert_eq!(pts.len(), 2);
        assert!((pts[0].1 - (-1.0)).abs() < 1e-7);
        assert!((pts[1].1 - 1.0).abs() < 1e-7);
    }
    ...
    fn test_intersect_line_sphere() {
        ...
        let pts = intersect_line_sphere(&line, &sphere);
        assert_eq!(pts.len(), 2);
        assert!((pts[0].1 - (-1.0)).abs() < 1e-7);
        assert!((pts[1].1 - 1.0).abs() < 1e-7);
    }
    ...
    #[test]
    fn test_intersect_plane_plane_intersecting() {
        let p1 = Plane::new(DVec3::ZERO, DVec3::Z);
    ...
    #[test]
    fn test_intersect_plane_cylinder_circle() {
    ...
    #[test]
    fn test_intersect_plane_sphere_tangent() {
        let plane = Plane::new(DVec3::new(0.0, 0.0, 3.0), DVec3::Z);
        let sphere = SphericalSurface {
            center: DVec3::ZERO,
            axis: DVec3::Z,
            radius: 3.0,
        ...
        match intersect_plane_sphere_intana(&plane, &sphere) {
            PlnSphResult::TangentPoint(p) => {
                assert!((p - DVec3::new(0.0, 0.0, 3.0)).length() < 1e-7);
            }
    ...
    fn test_intersect_line_torus() {
        ...
        let pts = intersect_line_torus(&line, &torus);
        // A line through the torus center along X should hit it on both sides
        assert!(!pts.is_empty(), "expected at least 1 intersection");
    }

    #[test]
    fn test_solve_cubic_one_root() {
        // (x-2)(x+1)(x+3) = x^3 + 2x^2 - 5x - 6
        let roots = solve_cubic(1.0, 2.0, -5.0, -6.0);
        assert_eq!(roots.len(), 3);
        assert!((roots.iter().copied().fold(0.0_f64, |a, b| a + b) - (-2.0)).abs() < 1e-7);
    }
    ...
    fn test_solve_quadratic() {
        ...
        assert!((roots.iter().copied().sum::<f64>() - 3.0).abs() < 1e-7);
    }
