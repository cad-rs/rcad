...
impl NonManifoldSummary {
    pub fn is_clean(&self) -> bool {
        self.boundary_edges.is_empty()
            ...
            && self.multi_face_edges.is_empty()
    }
}
...
pub enum RepairHint {
    ...
    NonManifoldVertex {
        vertex_idx: usize,
        connected_multi_edges: Vec<usize>,
    },
}
...
impl ManifoldRepairHints {
    ...
    pub fn stitchable_pairs(&self) -> impl Iterator<Item = &RepairHint> {
        self.hints
            .iter()
            .filter(|h| matches!(h, RepairHint::StitchablePair { .. }))
    }
    ...
    pub fn orphan_edges(&self) -> impl Iterator<Item = &RepairHint> {
        self.hints
            .iter()
            .filter(|h| matches!(h, RepairHint::OrphanEdge { .. }))
    }
}
...
impl BRepGraph {
    ...
    pub fn from_brep(brep: &BRep) -> Self {
        ...
        // iterate faces from topods BRep
        for ts in &brep.tshapes {
            ...
            for sr in &wd.edges {
                let Some(ei) = ts_to_edge[sr.index] else {
                    continue;
                };
                if !edge_to_faces[ei].contains(&flat_fi) {
                    edge_to_faces[ei].push(flat_fi);
                }
                if !face_edge_set.contains(&ei) {
                    face_edge_set.push(ei);
                }
...
impl BRepGraphBuilder {
    ...
    /// Add an edge ace incidence entry.
    ...
    /// Add a face dge incidence entry.
...
impl<'a> BRepGraphTool<'a> {
    ...
    pub fn face_location(&self, face_idx: usize) -> Option<(usize, usize, usize)> {
        ...
        for (solid_idx, ts) in self.brep.tshapes.iter().enumerate() {
            ...
            for (shell_idx, shell_sr) in sd.shells.iter().enumerate() {
                let topods::TShape::Shell(shd) = &*self.brep.tshapes[shell_sr.index] else {
                    continue;
                };
    ...
    pub fn edge_points(&self, edge_idx: usize) -> Option<(DVec3, DVec3)> {
        ...
        Some((
            self.vertex_point(start_vertex)?,
            self.vertex_point(end_vertex)?,
        ))
    }
...
mod tests {
    ...
    fn non_manifold_tripod() -> BRep {
        ...
        let f2 = brep.add_tface(None, w2, vec![], None, None, vec![], true);
        let sh = brep.add_tshell(vec![f0, f1, f2]);
    ...
    fn box_every_edge_has_two_adjacent_faces() {
        ...
        for ei in 0..g.edge_count {
            ...
            assert_eq!(
                adj.len(),
                2,
                "edge {ei}: expected 2 adjacent faces, got {adj:?}"
            );
        }
    }
    ...
    fn box_every_vertex_has_three_adjacent_edges() {
        ...
        for vi in 0..g.vertex_count {
            ...
            assert_eq!(
                adj.len(),
                3,
                "vertex {vi}: expected 3 adjacent edges, got {adj:?}"
            );
        }
    }
    ...
    fn box_every_vertex_has_three_adjacent_faces() {
        ...
        for vi in 0..g.vertex_count {
            ...
            assert_eq!(
                adj.len(),
                3,
                "vertex {vi}: expected 3 adjacent faces, got {adj:?}"
            );
        }
    }
    ...
    fn edge_endpoints_match_brep_edges() {
        let brep = unit_box();
        let g = BRepGraph::from_brep(&brep);
        let fe = brep.flat_edges();
        for (ei, &(start, end)) in fe.iter().enumerate() {
            let ep = g.edge_endpoints(ei).expect("endpoints should exist");
            assert_eq!(ep.0, start);
            assert_eq!(ep.1, end);
        }
    }
    ...
    fn box_is_manifold_and_closed() {
        ...
        assert!(
            g.low_valence_vertices().is_empty(),
            "no low-valence vertices"
        );
    ...
    fn detects_multi_face_non_manifold_edge() {
        ...
        assert!(
            !g.is_closed(),
            "tripod has boundary edges on each side face"
        );
        ...
        assert_eq!(
            verts,
            vec![0, 1],
            "shared edge endpoints should be non-manifold vertices"
        );
    ...
    fn dfs_edges_from_vertex_visits_all_box_edges() {
        ...
        assert_eq!(
            visited.len(),
            12,
            "DFS from any vertex should reach all 12 edges"
        );
    ...
    fn edge_valence_manifold_box() {
        let brep = unit_box();
        let g = BRepGraph::from_brep(&brep);
        for ei in 0..brep.edge_count() {
            assert_eq!(
                g.edge_valence(ei),
                2,
                "every box edge should have valence 2"
            );
        }
    ...
    fn edge_valence_tripod_multi_face_edge() {
        ...
        assert_eq!(
            g.edge_valence(1),
            1,
            "side edge should have valence 1 (boundary)"
        );
    }
    ...
    fn vertex_degree_box() {
        ...
        for vi in 0..brep.vertex_count() {
            assert_eq!(
                g.vertex_degree(vi),
                3,
                "box vertex {vi} should have degree 3"
            );
        }
    }
    ...
    fn repair_hints_detect_multi_manifold_edge_in_tripod() {
        ...
        assert!(
            has_multi,
            "tripod must generate a MultiManifoldEdge hint for edge 0"
        );
    }
    ...
    fn repair_hints_detect_non_manifold_vertex_in_tripod() {
        ...
        let g = BRepGraph::from_brep(&brep);
        let hints = g.repair_hints(&brep);
        let has_nm_vert = hints
            .hints
            .iter()
            .any(|h| matches!(h, RepairHint::NonManifoldVertex { .. }));
    ...
    fn repair_hints_detect_unmatched_boundary_in_tripod() {
        ...
        let has_unmatched = hints
            .hints
            .iter()
            .any(|h| matches!(h, RepairHint::UnmatchedBoundaryEdge { .. }));
    ...
    fn restore_from_data_undoes_dirty_marks() {
        ...
        assert!(
            !g.has_modifications(),
            "all dirty bits should be cleared after restore"
        );
        assert!(g.modified_vertices().is_empty());
        assert!(g.modified_edges().is_empty());
        assert!(g.modified_faces().is_empty());
    ...
    fn validate_invariants_passes_for_unit_box() {
        ...
        assert!(
            errors.is_empty(),
            "unit box graph should have no invariant violations: {errors:?}"
        );
    }
    ...
    fn validate_invariants_passes_for_non_manifold_tripod() {
        ...
        assert!(
            errors.is_empty(),
            "tripod graph should have no index invariant violations: {errors:?}"
        );
    }
    ...
    fn builder_constructs_same_adjacency_as_box() {
        ...
        for edge_idx in 0..built.edge_count {
            assert_eq!(
                built.edge_adjacent_faces(edge_idx),
                graph_from_brep.edge_adjacent_faces(edge_idx)
            );
            assert_eq!(
                built.edge_endpoints(edge_idx),
                graph_from_brep.edge_endpoints(edge_idx)
            );
        }
        for face_idx in 0..built.face_count {
            assert_eq!(
                built.face_edges(face_idx),
                graph_from_brep.face_edges(face_idx)
            );
        }
        assert_eq!(
            built.vertex_adjacent_edges(0),
            graph_from_brep.vertex_adjacent_edges(0)
        );
    ...
    fn builder_reports_invalid_indices() {
        ...
        let errors = builder
            .build()
            .expect_err("out-of-range endpoint should fail validation");
        assert!(
            errors
                .iter()
                .any(|error| error.contains("end vertex 7 out of range")),
    ...
    fn tool_exposes_flat_face_and_geometry_access() {
        ...
        let fe = brep.flat_edges();
        let (start_vi, end_vi) = fe[0];
        let (start_point, end_point) = tool.edge_points(0).expect("edge 0 should have endpoints");
        assert_eq!(start_point, brep.vertex_point(start_vi).unwrap());
        assert_eq!(end_point, brep.vertex_point(end_vi).unwrap());

        // vertex access
        let v0 = tool.vertex(0).expect("vertex 0 should exist");
        assert_eq!(v0, brep.vertex_point(0).unwrap());

        // face() and face_normal() are not supported with topods::BRep (return None)
        assert!(tool.face(0).is_none());
        assert!(tool.face_normal(0).is_none());
        // cube faces have surface=None in build_unit_cube
        assert!(tool.face_surface(0).is_none());
        assert!(tool.face_surface_range(0).is_none());
    }
}
