...
fn refine_pair(mut pa: DVec3, mut pb: DVec3, a: &BRep, b: &BRep) -> (DVec3, DVec3, f64) {
    for _ in 0..30 {
        // Project pa onto B
        if let Some(r) = closest_on_brep(pa, b) {
            pb = r.point;
    ...
    match (fa.surface, fb.surface) {
        ...
        (Surface3::Plane(pa), Surface3::Sphere(sb)) => {
            analytic_plane_sphere_distance(pa.origin, pa.normal, &fa.polygon, sb.center, sb.radius)
                .map(
                    |(point_on_plane, point_on_sphere, distance)| ShapeDistance {
                        distance,
        ...
        (Surface3::Plane(pa), Surface3::Plane(pb)) => analytic_parallel_plane_distance(
            pa.origin,
            pa.normal,
            &fa.polygon,
            pb.origin,
...
fn single_face_info(brep: &topods::BRep) -> Option<SingleFaceInfo<'_>> {
    ...
    let solid_idx = brep.tshapes.iter().position(|ts| {
        if let topods::TShape::Solid(_) = &**ts {
            true
        } else {
            false
        }
    })?;
    let topods::TShape::Solid(sd) = &*brep.tshapes[solid_idx] else {
        return None;
    };
    let shell_sr = sd.shells.first()?;
    let topods::TShape::Shell(shd) = &*brep.tshapes[shell_sr.index] else {
        return None;
    };
    if shd.faces.len() != 1 {
        return None;
    ...
    let topods::TShape::Face(fd) = &*brep.tshapes[face_sr.index] else {
        return None;
    };
    ...
    let polygon: Vec<DVec3> = wd
        .edges
        .iter()
        .filter_map(|we| {
            let topods::TShape::Edge(ed) = &*brep.tshapes[we.index] else {
                return None;
            };
            let v_idx = if we.orientation.is_forward() {
                ed.first.index
            } else {
                ed.last.index
            };
            let topods::TShape::Vertex(vd) = &*brep.tshapes[v_idx] else {
                return None;
            };
            Some(vd.point)
        })
...
fn analytic_parallel_plane_distance(
    ...
    let poly_a_2d: Vec<DVec2> = polygon_a
        .iter()
        .map(|&p| to_plane_uv(p, origin_a, u, v))
        .collect();
...
fn point_in_planar_polygon(point: DVec3, origin: DVec3, normal: DVec3, polygon: &[DVec3]) -> bool {
    ...
    let poly_uv: Vec<DVec2> = polygon
        .iter()
        .map(|&p| to_plane_uv(p, origin, u, v))
        .collect();
...
mod tests {
    ...
    fn disjoint_spheres_exact_distance() {
        ...
        b.apply_transform(glam::DAffine3::from_translation(glam::DVec3::new(
            5.0, 0.0, 0.0,
        )));
        let d = min_distance(&a, &b);
        assert!(
