...
mod function_root_tests {
    ...
    fn newton_raphson_quadratic() {
        let root = crate::math::math_utils::newton_raphson(f_quadratic, df_quadratic, 3.0, 1e-12, 100);
        assert!(root.is_some());
    ...
    fn newton_raphson_sin() {
        let root = crate::math::math_utils::newton_raphson(f_sin, d_sin, 1.0, 1e-12, 100);
        assert!(root.is_some());
    ...
    fn bisection_quadratic() {
        let root = crate::math::math_utils::bisection(f_quadratic, 0.0, 5.0, 1e-12);
        assert!(root.is_some());
        assert!((root.unwrap() - 2.0).abs() < 1e-6);
    }
    ...
    fn bisection_no_root_in_interval() {
        ...
        let root = crate::math::math_utils::bisection(f_quadratic, 0.0, 1.0, 1e-12);
        assert!(root.is_none());
    }

    #[test]
    fn secant_quadratic() {
        let root = crate::math::math_utils::secant(f_quadratic, 1.0, 3.0, 1e-12);
        assert!(root.is_some());
        assert!((root.unwrap() - 2.0).abs() < 1e-6);
    }
    ...
    fn newton_raphson_no_convergence() {
        ...
        let root = crate::math::math_utils::newton_raphson(f_no_root, df_no_root, 0.0, 1e-12, 10);
        assert!(root.is_none());
    }
}
...
mod newton_set_root_tests {
    ...
    fn newton_2d_simple_intersection() {
        ...
        let sol = crate::math::math_utils::newton_2d(f, jacobian, DVec2::new(0.0, 0.0), 1e-10);
        assert!(sol.is_some());
        let s = sol.unwrap();
    ...
    fn newton_3d_linear_system() {
        ...
        let sol = crate::math::math_utils::newton_3d(f, jacobian, DVec3::ZERO, 1e-10);
        assert!(sol.is_some());
        let s = sol.unwrap();
...
mod polynomial_roots_tests {
    ...
    fn solve_linear_basic() {
        let root = crate::math::math_utils::solve_linear(2.0, -4.0);
        assert!((root.unwrap() - 2.0).abs() < TOL);
    }

    #[test]
    fn solve_linear_no_solution() {
        let root = crate::math::math_utils::solve_linear(0.0, 5.0);
        assert!(root.is_none());
    }

    #[test]
    fn solve_quadratic_two_roots() {
        let roots = crate::math::math_utils::solve_quadratic(1.0, 0.0, -4.0); // x^2 - 4 = 0
    ...
    fn solve_quadratic_no_real_roots() {
        let roots = crate::math::math_utils::solve_quadratic(1.0, 0.0, 1.0); // x^2 + 1 = 0
        assert!(roots.is_empty());
    }

    #[test]
    fn solve_quadratic_double_root() {
        let roots = crate::math::math_utils::solve_quadratic(1.0, -4.0, 4.0); // (x-2)^2 = 0
    ...
    fn solve_cubic_three_roots() {
        let roots = crate::math::math_utils::solve_cubic(1.0, -6.0, 11.0, -6.0); // (x-1)(x-2)(x-3)
    ...
    fn solve_cubic_single_root() {
        ...
        let roots = crate::math::math_utils::solve_cubic(1.0, 0.0, 0.0, -1.0);
        assert!(roots.len() >= 1);
        assert!((roots[0] - 1.0).abs() < TOL);
    }
    ...
    fn solve_quartic_four_roots() {
        ...
        let roots = crate::math::math_utils::solve_quartic(1.0, 0.0, -10.0, 0.0, 9.0);
...
mod integration_tests {
    ...
    fn simpson_x_squared() {
        // ∫₀¹ x² dx = 1/3
        let result = crate::math::math_utils::simpson_integrate(f_x_squared, 0.0, 1.0, 100);
    ...
    fn simpson_sin() {
        // ∫₀^π sin(x) dx = 2
        let result = crate::math::math_utils::simpson_integrate(f_sin, 0.0, std::f64::consts::PI, 100);
        assert!((result - 2.0).abs() < 1e-4);
    }

    #[test]
    fn gaussian_quadrature_x_squared() {
        let result = crate::math::math_utils::gaussian_quadrature(f_x_squared, 0.0, 1.0, 5);
        assert!((result - 1.0 / 3.0).abs() < 1e-4);
    }

    #[test]
    fn gaussian_quadrature_sin() {
        let result = crate::math::math_utils::gaussian_quadrature(f_sin, 0.0, std::f64::consts::PI, 5);
        assert!((result - 2.0).abs() < 1e-4);
    }
}
...
mod matrix_tests {
    ...
    fn determinant_identity() {
        assert!((crate::math::math_utils::determinant_3x3(DMat3::IDENTITY) - 1.0).abs() < TOL);
    }

    #[test]
    fn determinant_scale() {
        let m = DMat3::from_diagonal(DVec3::new(2.0, 3.0, 4.0));
        assert!((crate::math::math_utils::determinant_3x3(m) - 24.0).abs() < TOL);
    }

    #[test]
    fn inverse_identity() {
        let inv = crate::math::math_utils::inverse_3x3(DMat3::IDENTITY);
        assert!(inv.is_some());
    ...
    fn inverse_scale_then_roundtrip() {
        let m = DMat3::from_diagonal(DVec3::new(2.0, 3.0, 4.0));
        let inv = crate::math::math_utils::inverse_3x3(m).unwrap();
    ...
    fn eigenvalues_2x2_diagonal() {
        ...
        let (e1, e2) = crate::math::math_utils::eigenvalues_2x2(m);
    ...
    fn eigenvalues_3x3_diagonal() {
        ...
        let (e1, e2, e3) = crate::math::math_utils::eigenvalues_3x3(m);
...
mod optimization_tests {
    ...
    fn golden_section_min_quadratic() {
        ...
        let xmin =
            crate::math::math_utils::golden_section_min(|x| (x - 2.0) * (x - 2.0) + 1.0, 0.0, 5.0, 1e-8);
    ...
    fn golden_section_min_sin() {
        ...
        let xmin = crate::math::math_utils::golden_section_min(|x| x.sin(), 2.0, 5.0, 1e-8);
    ...
    fn golden_section_max_negative_quadratic() {
        ...
        let xmax =
            crate::math::math_utils::golden_section_max(|x| -(x - 3.0) * (x - 3.0) + 5.0, 0.0, 6.0, 1e-8);
...
mod gauss_tests {
    ...
    fn matrix_solve_3x3_identity() {
        ...
        let inv = crate::math::math_utils::inverse_3x3(a).unwrap();
...
mod svd_tests {
    ...
    fn svd_well_conditioned_3x3() {
        ...
        // Solve via SVD
        let x = crate::math::math_utils::svd_solve_3x3(a, b);
...
mod least_square_tests {
    ...
    fn least_squares_line_fit() {
        ...
        // Fit y = a + b*x
        let result = crate::math::math_utils::least_squares_linear(&x_pts, &y_pts);
...
mod bfgs_tests {
    ...
    fn bfgs_quadratic_bowl() {
        let x0 = vec![0.0, 0.0];
        let result = crate::math::math_utils::bfgs_minimize(&x0, quadratic_bowl_grad, 1e-10, 100);
    ...
    fn bfgs_rosenbrock() {
        ...
        let x0 = vec![-1.0, 1.0];
        let result = crate::math::math_utils::bfgs_minimize(&x0, rosenbrock_grad, 1e-8, 200);
...
mod newton_min_tests {
    ...
    fn newton_min_quadratic_bowl() {
        let x0 = vec![0.0, 0.0];
        let result = crate::math::math_utils::newton_minimize(&x0, quad_bowl_hessian, 1e-10, 50);
...
mod frpr_tests {
    ...
    fn frpr_quadratic_bowl() {
        let r = crate::math::math_utils::frpr_minimize(&[0.0, 0.0], quad_grad, 1e-10, 100);
        assert!(r.is_some());
        let x = r.unwrap();
    ...
    #[test]
    fn frpr_rosenbrock() {
        let r = crate::math::math_utils::frpr_minimize(&[-1.0, 1.0], rosenbrock_grad, 1e-8, 500);
        assert!(r.is_some());
        let x = r.unwrap();
...
mod householder_tests {
    ...
    fn householder_3x3() {
        ...
        let x = crate::math::math_utils::householder_solve(&a, &b, 3);
    ...
    fn householder_vs_crout() {
        ...
        let x_crout = crate::math::math_utils::crout_solve(&a, &b, 3);
        assert!(x_crout.is_some(), "Crout should solve the system");
        let x_hh = crate::math::math_utils::householder_solve(&a, &b, 3);
...
mod crout_tests {
    ...
    fn crout_3x3() {
        let a = vec![2.0, 1.0, 0.0, 1.0, 2.0, 1.0, 0.0, 1.0, 2.0];
        let b = vec![6.0, 9.0, 8.0];
        let x = crate::math::math_utils::crout_solve(&a, &b, 3);
        assert!(x.is_some());
        let x = x.unwrap();
...
mod biss_newton_tests {
    ...
    fn biss_newton_quadratic() {
        let r = crate::math::math_utils::biss_newton(|x| x * x - 4.0, |x| 2.0 * x, 0.0, 5.0, 1e-12);
        assert!(r.is_some());
    ...
    #[test]
    fn biss_newton_sin() {
        let r = crate::math::math_utils::biss_newton(|x| x.sin() - 0.5, |x| x.cos(), 0.0, 1.5, 1e-12);
        assert!(r.is_some());
...
mod trig_roots_tests {
    ...
    fn trig_sin_only() {
        ...
        let roots =
            crate::math::math_utils::trig_roots_sin_only(1.0, -0.5, 0.0, 2.0 * std::f64::consts::PI);
        assert!(roots.len() >= 1);
    ...
    fn trig_cos_sin() {
        // cos(x) + sin(x) - 1 = 0
        let roots =
            crate::math::math_utils::trig_roots_cos_sin(1.0, 1.0, -1.0, 0.0, 2.0 * std::f64::consts::PI);
        assert!(roots.len() >= 1);
    }
}
...
mod laguerre_tests {
    ...
    #[test]
    fn laguerre_linear() {
        let r = crate::math::math_utils::laguerre_roots(&[-4.0, 2.0]); // 2x - 4 = 0
        assert_eq!(r.len(), 1);
    ...
    fn laguerre_quadratic() {
        let r = crate::math::math_utils::laguerre_roots(&[0.0, -3.0, 1.0]); // x^2 - 3x = 0
        assert_eq!(r.len(), 2);
    ...
    fn laguerre_cubic() {
        ...
        let r = crate::math::math_utils::laguerre_roots(&[-6.0, 11.0, -6.0, 1.0]);
        assert_eq!(r.len(), 3);
...
mod brent_tests {
    ...
    fn brent_quadratic() {
        let xmin =
            crate::math::math_utils::brent_minimize(|x| (x - 2.0) * (x - 2.0) + 1.0, 0.0, 5.0, 1e-10);
        assert!((xmin - 2.0).abs() < 1e-6);
    }

    #[test]
    fn brent_sin() {
        let xmin = crate::math::math_utils::brent_minimize(|x| x.sin(), 2.0, 5.0, 1e-10);
        assert!((xmin - 3.0 * std::f64::consts::FRAC_PI_2).abs() < 1e-6);
    }
}
...
mod multi_root_tests {
    ...
    fn find_roots_quadratic() {
        let r = crate::math::math_utils::find_roots_in(|x| x * x - 4.0, -5.0, 5.0, 100);
        assert_eq!(r.len(), 2);
    ...
    #[test]
    fn find_roots_sin() {
        let r = crate::math::math_utils::find_roots_in(|x| x.sin(), 0.0, 4.0 * std::f64::consts::PI, 100);
        assert!(r.len() >= 2);
        for &root in &r {
    ...
    fn bracket_find_root() {
        let b = crate::math::math_utils::bracket_root(|x| x * x - 4.0, 0.0, 1.0, 10);
        assert!(b.is_some());
        let (a, b) = b.unwrap();
...
mod bracket_min_tests {
    ...
    fn bracket_min_via_scan() {
        let b = crate::math::math_utils::bracket_root(|x| (x - 3.0) * (x - 3.0), 0.0, 1.0, 10);
        assert!(b.is_some());
    }
}
...
mod glob_opt_tests {
    ...
    fn glob_opt_quadratic() {
        ...
        let x = crate::math::math_utils::glob_opt_min(
...
mod pso_tests {
    ...
    #[test]
    fn pso_quadratic() {
        let x = crate::math::math_utils::pso_minimize(
            |x| (x[0] - 1.0).powi(2) + (x[1] - 2.0).powi(2),
            &[-5.0, -5.0],
            &[5.0, 5.0],
            30,
...
mod lm_tests {
    ...
    #[test]
    fn lm_linear() {
        let sol = crate::math::math_utils::lm_solve(&[0.0, 0.0], linear_residual, 2, 50, 1e-10);
    ...
    #[test]
    fn lm_rosenbrock() {
        let sol = crate::math::math_utils::lm_solve(&[-1.0, 1.0], rosenbrock_residual, 2, 200, 1e-10);
...
mod geom_plate_tests {
    ...
    fn plate_from_four_points() {
        ...
        let tps = crate::math::math_utils::thin_plate_spline(&pts);
        assert!(tps.is_some(), "TPS solver should succeed for 4 points");
        let (w, a) = tps.unwrap();

        // Verify interpolation at each constraint
        for p in &pts {
            let f = crate::math::math_utils::evaluate_tps(p.x, p.y, &w, &a, &pts);
            assert!(
                (f - p.z).abs() < 1e-8,
    ...
    fn plate_surface_from_constraints() {
        ...
        let pts = make_quadrilateral_constraints();
        let surf = crate::math::math_utils::build_plate_surface(&pts, 5, 5);
    ...
    fn plate_curve_constraint() {
        ...
        let surf = crate::math::math_utils::build_plate_surface(&pts, 6, 6);
        assert!(
            surf.is_some(),
