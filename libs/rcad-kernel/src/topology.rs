...
///
/// `forward = true`  鈫?traverse edge from `edge.start` to `edge.end`.
/// `forward = false` 鈫?traverse edge from `edge.end`   to `edge.start`.
///
...
pub struct WireEdge {
    ...
    pub idx: usize,
    /// Traversal direction: `true` = forward (start鈫抏nd), `false` = reversed.
    pub forward: bool,
}
...
pub struct Face {
    ...
    /// Representative 3D point on this face, typically the centroid of its
    /// boundary polygon.  Populated by [`imprint_shape`] for downstream
    /// point-classification; `None` for geometry generated by other paths.
...
pub struct Compound {
    ...
    /// empty 鈥?it is used for assembly-tree bookkeeping and STEP name mapping.
