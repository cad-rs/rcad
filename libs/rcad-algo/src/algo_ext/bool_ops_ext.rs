...
/// Minimal boolean dispatch used by the partition helpers below.
///
/// Maps the legacy `boolean_op_generic` calls onto the current
/// `rcad_algo::bop::brep_algo_api::{common, cut}` entry points (BRep form).
fn boolean_op_generic(
    op: BooleanOpType,
    a: &rcad_kernel::BRep,
    b: &rcad_kernel::BRep,
) -> Result<rcad_kernel::BRep, BooleanError> {
    match op {
        BooleanOpType::Union => crate::bop::brep_algo_api::fuse(a, b),
        BooleanOpType::Intersection => common(a, b),
        BooleanOpType::Cut => cut(a, b),
        BooleanOpType::Cut21 => crate::bop::brep_algo_api::cut21(a, b),
...
fn partition_solid_object(
    ...
    for mask in 0..max_mask {
        ...
        for i in 0..n_tools {
            ...
            if inside {
                match boolean_op_generic(
                    BooleanOpType::Intersection, &cell, tool,
                ) {
                ...
                match boolean_op_generic(
                    BooleanOpType::Intersection, &cell, complement,
                ) {
                    Ok(r) => cell = compact_brep(&r),
                    Err(_) => { failed = true; break; }
                }
                ...
                if let (Some(tool_bbox), Some(cell_bbox)) = (bounding_box(tool), bounding_box(&cell)) {
                    ...
                    for (origin, u_dir, v_dir, w, h, d) in &comp_boxes {
                        ...
                        for cell_part in &cell_solids {
                            if let Ok(part) = boolean_op_generic(
                                BooleanOpType::Intersection, cell_part, &comp_box,
                            ) {
    ...
    cells.retain(|c| total_volume(c) > 1e-10);
    Ok(cells)
}
...
fn partition_face_object(
    ...
    for tool in &solid_tools {
        let inside = boolean_op_generic(
            BooleanOpType::Intersection, &remaining, tool,
        )?;
        cells.push(remaining.clone());
        remaining = boolean_op_generic(
            BooleanOpType::Cut, &remaining, tool,
        )?;
    }
