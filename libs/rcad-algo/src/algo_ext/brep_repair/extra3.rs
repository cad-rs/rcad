...
pub fn handle_degenerate_points(brep: &rcad_kernel::BRep, tolerance: f64) -> (rcad_kernel::BRep, usize) {
    ...
    // Iterate through all faces using topods API
    for (_fi, fd) in each_face(brep) {
        ...
        if let TShape::Wire(wd) = &*brep.tshapes[fd.outer_wire.index] {
            for er in &wd.edges {
                if let Some(edge) = ed_opt(brep, er.index) {
                    let vi = if er.orientation == Orientation::Forward { edge.first.index } else { edge.last.index };
                    if is_vertex_at_degenerate_point_idx(brep, vi, surface, &periodic_info, tolerance) {
    ...
    for vi in &degenerate_vertices {
        ...
        for (_ei, edge) in each_edge(&result) {
            if (edge.first.index == *vi || edge.last.index == *vi) && !edge.degenerated {
                // Edge is incident but not marked degenerated
...
/// Check if a vertex (by index) is at a degenerate point on a surface.
fn is_vertex_at_degenerate_point_idx(
    ...
    match surface {
        Surface3::Sphere(sphere) => {
            ...
            let dist_to_north = (pt - north_pole).length();
            let dist_to_south = (pt - south_pole).length();
        ...
        Surface3::Cone(cone) => {
            ...
            let dist_to_apex = (pt - apex).length();
...
pub fn merge_seam_edges(brep: &rcad_kernel::BRep, config: &PeriodicSeamConfig) -> (rcad_kernel::BRep, usize) {
    ...
    // 3. Have endpoints near the seam (one at U=0, one at U=2pi)
    ...
    for (_fi, fd) in each_face(brep) {
        ...
        if let TShape::Wire(wd) = &*brep.tshapes[fd.outer_wire.index] {
            for er in &wd.edges {
                if let Some(edge) = ed_opt(brep, er.index) {
                    // Look up pcurve by face (pcurves keyed by (ptr_id, location))
                    if let Some((curve2d, _, _)) = edge.pcurves.get(&brep.pcurve_key(_fi)) {
                        let uv_start = curve2d.point_at(if er.orientation == Orientation::Forward { 0.0 } else { 1.0 });
                        let uv_end = curve2d.point_at(if er.orientation == Orientation::Forward { 1.0 } else { 0.0 });
                        edge_uv_endpoints.push((er.index, uv_start, uv_end));
                    }
                }
            }
        }
...
/// Compute the flat face index for a given solid/shell/face tuple (topods API).
fn compute_flat_face_idx(brep: &rcad_kernel::BRep, solid_idx: usize, shell_idx: usize, face_idx: usize) -> usize {
    let mut idx = 0usize;
    for s in 0..solid_idx {
        if let Some(sd) = brep.tshapes.get(s).and_then(|ts| if let TShape::Solid(sd) = &**ts { Some(sd) } else { None }) {
            for sr in &sd.shells {
    ...
    if let Some(sd) = brep.tshapes.get(solid_idx).and_then(|ts| if let TShape::Solid(sd) = &**ts { Some(sd) } else { None }) {
        for sh in 0..shell_idx {
            if let Some(sr) = sd.shells.get(sh) {
                if let TShape::Shell(shd) = &*brep.tshapes[sr.index] {
                    idx += shd.faces.len();
                }
            }
...
/// Compute the approximate area of a face from TFaceData.
fn compute_face_area_from_tface(brep: &rcad_kernel::BRep, fd: &TFaceData) -> f64 {
    let mut pts: Vec<DVec3> = Vec::new();
    if let TShape::Wire(wd) = &*brep.tshapes[fd.outer_wire.index] {
        for er in &wd.edges {
            if let Some(edge) = ed_opt(brep, er.index) {
                let vi = if er.orientation == Orientation::Forward { edge.first.index } else { edge.last.index };
                pts.push(vpoint(brep, vi));
            }
        }
    }
...
pub fn merge_bspline_faces(
    ...
    // Get surfaces for both faces from TFaceData
    let fd1 = match &**brep.tshapes.get(face1_idx)? { TShape::Face(f) => f, _ => return None };
    let fd2 = match &**brep.tshapes.get(face2_idx)? { TShape::Face(f) => f, _ => return None };

    let surf1 = fd1.surface.as_ref()?;
    let surf2 = fd2.surface.as_ref()?;
    ...
    let wire1 = get_wire_edges(brep, face1_idx)?;
    ...
    let get_inner_wires = |brep: &rcad_kernel::BRep, fi: usize| -> Option<Vec<Vec<Shape>>> {
        ...
        for iwr in &fd.inner_wires {
            if let TShape::Wire(wd) = &*brep.tshapes[iwr.index] {
                inners.push(wd.edges.clone());
            }
        }
        Some(inners)
    };

    let inner2 = get_inner_wires(brep, face2_idx)?;

    let merged_wire = splice_wires_for_merge_topo(&wire1, &wire2, shared_edge)?;
    ...
    let merged_fd = TFaceData {
        my_shapes: Vec::new(),
        flags: tshape_flags::FREE | tshape_flags::MODIFIED | tshape_flags::ORIENTABLE,
        surface: fd1.surface.clone(),
        surface_location: fd1.surface_location,
        outer_wire: merged_wire_sr,
        inner_wires: all_inner_refs,
        sample_point: fd1.sample_point,
        uv_domain: fd1.uv_domain,
    ...
    let remove_flat = flat_face_index_global(&result, si, shi, remove_fi);

    // Replace the kept face TShape
    result.tshapes[kept_flat] = merged_tshape;
    ...
    if let TShape::Solid(sd) = &*result.tshapes[si] {
        if let Some(sr) = sd.shells.get(shi) {
            if let TShape::Shell(shd) = &*result.tshapes[sr.index] {
                ...
                new_faces.remove(remove_fi);
                let new_shd = TShellData {
                    my_shapes: Vec::new(),
                    flags: shd.flags,
                    faces: new_faces,
                };
                ...
                // Push new shell tshape
                result.tshapes.push(new_shd_arc);
                // Update solid
                result.tshapes[si] = Arc::new(TShape::Solid(new_sd));
            }
        }
    }

    Some((result, MergedFaceInfo {
        kept_face_idx: keep_fi,
        removed_face_idx: remove_fi,
        merged_edge_count,
        inner_wires_merged,
...
fn flat_face_index_global(brep: &rcad_kernel::BRep, si: usize, shi: usize, fi: usize) -> usize {
    ...
    if let Some(sd) = brep.tshapes.get(si).and_then(|ts| if let TShape::Solid(sd) = &**ts { Some(sd) } else { None }) {
        for sh in 0..shi {
            if let Some(sr) = sd.shells.get(sh) {
                if let TShape::Shell(shd) = &*brep.tshapes[sr.index] {
                    idx += shd.faces.len();
...
fn find_shared_edge(brep: &rcad_kernel::BRep, si: usize, shi: usize, fi1: usize, fi2: usize) -> Option<usize> {
    ...
    let get_face_edges = |brep: &rcad_kernel::BRep, si: usize, shi: usize, fi: usize| -> Option<HashSet<usize>> {
        let sd = match &*brep.tshapes[si] { TShape::Solid(sd) => sd, _ => return None };
        let sr = sd.shells.get(shi)?;
        let shd = match &*brep.tshapes[sr.index] { TShape::Shell(shd) => shd, _ => return None };
        let fr = shd.faces.get(fi)?;
        let fd = match &*brep.tshapes[fr.index] { TShape::Face(fd) => fd, _ => return None };
        let wd = match &*brep.tshapes[fd.outer_wire.index] { TShape::Wire(wd) => wd, _ => return None };
        Some(wd.edges.iter().map(|er| er.index).collect())
    };

    let edges1 = get_face_edges(brep, si, shi, fi1)?;
    let edges2 = get_face_edges(brep, si, shi, fi2)?;
...
/// Splice two wire edge lists together for merging (old WireEdge style, kept for compatibility).
...
impl ClosureReport {
    ...
    pub fn summary(&self) -> String {
        if self.is_closed {
            ...
            format!(
                "Open shell: {} open edges, V={}, E={}, F={}, chi={}",
    ...
    for face in &shell.faces {
        for we in &face.outer_wire.edges {
            if let Some(edge) = ed_opt(brep, we.idx) {
                let vi = if we.forward { edge.first.index } else { edge.last.index };
...
impl ShellValidationReport {
    ...
    pub fn summary(&self) -> String {
        ...
        format!("ShellValidation: {} | V={}, E={}, F={}, chi={}", status, self.vertex_count, self.edge_count, self.face_count, self.euler_characteristic)
    }
}
...
fn estimate_chain_area(chain: &[usize], brep: &rcad_kernel::BRep) -> f64 {
    ...
    for &ei in chain {
        if let Some(edge) = ed_opt(brep, ei) {
            let s = vpoint(brep, edge.first.index);
            let e = vpoint(brep, edge.last.index);
            if nodes.is_empty() { nodes.push(s); }
            nodes.push(e);
        }
    }
