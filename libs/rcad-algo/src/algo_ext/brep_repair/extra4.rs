...
pub fn fix_uv_gaps(
 ...
 let gap_report = crate::algo_ext::shape_analysis::detect_uv_gaps(solid_idx, shell_idx, face_idx, brep, config.closure_tolerance);
...
use crate::algo_ext::shape_analysis::{EndpointGap, PeriodicGap};
...
fn repair_single_gap(
 ...
 // Get the PCurve for this edge from TEdgeData.pcurves (keyed by (ptr_id, location))
 let Some((curve2d, t_min, t_max)) = (|| -> Option<(&rcad_kernel::Curve2d, f64, f64)> {
 let e = ed_opt(result, gap.edge_idx)?;
 e.pcurves.get(&result.pcurve_key(face_idx)).map(|(c, mn, mx)| (c, *mn, *mx))
 })() else {
 ...
 crate::algo_ext::shape_analysis::UvDirection::U => {
 if gap.at_max {
 ...
 crate::algo_ext::shape_analysis::UvDirection::V => {
 if gap.at_max {
