...
pub fn fix_edge_pcurve_uv_bounds(
 ...
 for (&fi_key, (curve2d, ta, tb)) in &edge_td.pcurves {
  // Check if this face has the matching surface
  let Some(fi) = result.index_by_ptr(fi_key.0) else { continue };
  ...
  let new_ts: TShape = match &**ts {
   ...
   TShape::Edge(ed) => {
    ...
    for (&(ptr, loc), (c, t1, t2)) in &ed.pcurves {
     ...
     let new_ptr = if new_fi < new_tshapes.len() {
      std::sync::Arc::as_ptr(&new_tshapes[new_fi]) as u64
     } else {
      ptr
     };
