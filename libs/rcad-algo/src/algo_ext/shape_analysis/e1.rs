...
pub fn check_face_uv_consistency(
    ...
    for (edge_idx, edge_forward) in all_edges {
        ...
        let Some((curve2d, u0, u1)) = ed.pcurves.get(&brep.pcurve_key(face_key)) else {
        ...
        // OCCT BRep_CurveOnClosedSurface: the second pcurve lives in the edge
        // representations, not in `pcurves` (which is keyed by (ptr_id, location)).
