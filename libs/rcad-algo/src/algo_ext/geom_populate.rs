...
use rcad_kernel::base::geom_api::project::{
    closest_point_on_surface, make_pcurve_on_surface,
};
use rcad_kernel::geom::*;
use rcad_kernel::math::fit::interpolate_points_2d;
use rcad_kernel::topods::{self, TShape};

/// Project a polyline of 3D points onto a surface and fit a 2D curve through
/// the UV samples (replaces legacy `pcurve_derive::polyline_pcurve_by_projection`).
fn polyline_pcurve_by_projection(polyline: &[DVec3], surface: &Surface3) -> Option<Curve2d> {
    let mut uv_pts: Vec<DVec2> = Vec::with_capacity(polyline.len());
    for &pt in polyline {
        let proj = closest_point_on_surface(surface, pt, 8);
...
pub fn populate_boolean_result_pcurves(brep: &mut rcad_kernel::BRep) {
    ...
    for (edge_idx, face_idx, surface) in face_edge_pairs {
        ...
        let already_has = {
            ...
            ed.pcurves.contains_key(&brep.pcurve_key(face_idx))
        };
        if already_has {
            continue;
        }
        ...
        let pcurve2d = if let (Some(curve), Some(t_range)) = (curve_opt, t_range_opt) {
            make_pcurve_on_surface(&curve, t_range, &surface, 17)
        } else {
            ...
            let polyline: Vec<_> = (0..17)
                ...
                .collect();
            match polyline_pcurve_by_projection(&polyline, &surface) {
                Some(c2d) => Some(c2d),
                None => continue,
            }
        };

        // Store in the edge's pcurves map, keyed by face.
        let fkey = brep.pcurve_key(face_idx);
        let Some(pcurve2d) = pcurve2d else { continue };
