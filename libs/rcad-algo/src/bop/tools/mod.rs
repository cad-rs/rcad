...
use rcad_kernel::geom::{Circle2d, Curve2d, Curve2dEval, Curve3, CurveEval, Line2d, Surface3};
use rcad_kernel::topods;

/// MakeSectEdge (BOPTools_AlgoTools).
...
/// BOPTools_AlgoTools::IsMicroEdge (BOPAlgo_Builder.cxx L2067-2104).
...
pub fn is_micro_edge(ei: usize, ds: &DS, check_splittable: bool) -> bool {
    ...
    let mut a_sr = crate::bop::int_tools::shrunk_range::ShrunkRange::new();
    ...
    if check_splittable && !a_sr.is_splittable() {
        return true;
    }
    false
}

/// ComputeState (BOPTools_AlgoTools).
...
/// GetNormalToFaceOnEdge (BOPTools_AlgoTools3D).
pub fn get_normal_to_face_on_edge(
    surface: &Surface3,
    face_normal: glam::DVec3,
    edge_mid: glam::DVec3,
) -> glam::DVec3 {
    match surface {
...
/// PointNearEdge (BOPTools_AlgoTools3D).
...
///  ?AdjustPCurveOnFace (BOPTools_AlgoTools2D.cxx L223-400).
...
pub fn adjust_pcurve_on_face(
    ...
    if (umax - umin).abs() < 1e-10 || (vmax - vmin).abs() < 1e-10 {
        return None;
    }
    ...
    if du != 0.0 || dv != 0.0 {
        ...
        let adjusted = match pcurve {
            ...
            rcad_kernel::geom::Curve2d::Bezier(bz) => {
                let mut bz = bz.clone();
                for p in &mut bz.control_points {
                    *p += shift;
                }
                rcad_kernel::geom::Curve2d::Bezier(bz)
            }
...
/// ComputeState point overload.
pub fn compute_state_point(
    pt: glam::DVec3,
    fi: &[usize],
    ds: &DS,
) -> crate::bop::ds::Classification {
    crate::classify::classify_point(pt, fi, ds)
}
/// IsHole (BOPTools_AlgoTools).
pub fn is_hole_wire(edges: &[crate::bop::ds::pave::PaveBlock]) -> bool {
    edges.len() == 1
}
/// Sense (BOPTools_AlgoTools).
pub fn sense_orientation(dot: f64) -> i8 {
    if dot > 1e-10 {
        1
    } else if dot < -1e-10 {
        -1
...
///  ?CorrectShapeTolerances (BOPTools_AlgoTools_1.cxx L389-423).
...
/// IsGrowthShell (BOPAlgo_BuilderSolid).
pub fn is_growth_shell(face_count: usize) -> bool {
    face_count > 0
}

/// IsGrowthWire (BOPAlgo_BuilderFace).
pub fn is_growth_wire(edge_count: usize) -> bool {
    edge_count >= 3
}

///  ?BOPAlgo_Tools::FillInternals (cxx L1751-1908).
///
...
pub fn fill_internals(
    ...
    let mut a_ms_solids_vertices: std::collections::HashSet<usize> =
    ...
    for si in 0..solids.len() {
        ...
        while i < a_l_parts.len() {
            ...
            if a_state == crate::bop::ds::Classification::In {
                if part.typ == 2 {
                    // FACE
...
///  ?ComputeStateByOnePoint (BOPTools_AlgoTools.cxx L623-656).
///
...
pub fn compute_state_by_one_point(
    ...
    if solid_faces.is_empty() {
        return crate::bop::ds::Classification::Out;
    }
    ...
    let pt = match shape_type {
        ...
        _ => return crate::bop::ds::Classification::Out,
    };
...
pub fn make_container(
    ...
    let shape: std::sync::Arc<TShape> = match shape_type {
        ...
        2 => std::sync::Arc::new(TShape::Solid(TSolidData {
            my_shapes: Vec::new(),
        ...
        3 => std::sync::Arc::new(TShape::Shell(TShellData {
            my_shapes: Vec::new(),
            flags: tshape_flags::FREE | tshape_flags::MODIFIED | tshape_flags::ORIENTABLE,
            faces: Vec::new(),
        })),
        4 => std::sync::Arc::new(TShape::Wire(TWireData {
            my_shapes: Vec::new(),
            flags: tshape_flags::FREE | tshape_flags::MODIFIED | tshape_flags::ORIENTABLE,
            edges: Vec::new(),
        })),
...
///  ?IsInvertedSolid (BOPTools_AlgoTools.cxx L2398-2408).
///
...
pub fn is_inverted_solid(solid_faces: &[usize], ds: &DS) -> bool {
    ...
    if size < TOLERANCE_LEN_SQ_DIV_SAFE {
        return false;
    }
    let far_point = aabb_max + glam::DVec3::splat(size * 100.0);
    let state = crate::classify::classify_point(far_point, solid_faces, ds);
    state == crate::bop::ds::Classification::In
}

///  ?IntermediatePoint (BOPTools_AlgoTools2D / IntTools_Tools).
pub fn intermediate_point(t1: f64, t2: f64) -> f64 {
    0.5 * (t1 + t2)
}

///  ?IntermediatePoint (IntTools_Tools.cxx L254-258).
...
///  ?IsDirsCoinside (IntTools_Tools.cxx L164-173).
...
///  ?IsDirsCoinside with custom threshold (IntTools_Tools.cxx L177-187).
pub fn is_dirs_coinside_with_tol(d1: glam::DVec3, d2: glam::DVec3, d_lim: f64) -> bool {
    let d = (d1 - d2).length();
    d < d_lim || (2.0 - d).abs() < d_lim
}

///  ?IsClosed (IntTools_Tools.cxx L78-102).
...
///  ?IsOnPave (IntTools_Tools.cxx L579-589).
...
///  ?IsInRange (IntTools_Tools.cxx L650-666).
...
///  ?ComputeIntRange (IntTools_Tools.cxx L783-804).
...
///  ?CurveTolerance (IntTools_Tools.cxx L430-464).
...
/// GetFaceOff (BOPTools_AlgoTools.cxx L994-1095).
///
...
pub fn get_face_off(ei: usize, fi: usize, candidates: &[(usize, usize)], ds: &DS) -> Option<usize> {
    ...
    for &(_, cfi) in candidates {
        ...
        if a_angle.abs() < an_angle_criteria || (a_angle - a_angle_min).abs() < an_angle_criteria {
            // Ambiguous =but still usable (OCCT sets bRet=false but continues)
        }

        // OCCT L1083-1086: normalize to [0, 2 ?
        if a_angle < 0.0 {
...
///  ?OrientFacesOnShell (BOPTools_AlgoTools.cxx L363-507).
///
/// OCCT algorithm:
/// 1. Build edge= ace map for the shell, deduplicate seam edges.
...
pub fn orient_faces_on_shell(
    ...
    // OCCT L370-377: TopExp::MapShapesAndAncestors =edge= ace map
    let mut a_ef_map: std::collections::BTreeMap<usize, Vec<usize>> =
    ...
    for &ei in &edge_keys {
        ...
        let Some(a_lf) = a_ef_map.get(&ei) else {
            continue;
        };
        if a_lf.len() != 2 {
            continue;
        ...
        if b_p1 && !b_p2 {
            if an_or_e1 == an_or_e2 {
                let e_closed = ds
                    .edges
                    .get(ei)
                    .map_or(false, |e| e.start_vertex == e.end_vertex);
                if !e_closed {
...
pub fn compute_tolerance_of_cb(cb: &crate::bop::ds::common_block::CommonBlock, ds: &DS) -> f64 {
    // OCCT L252-256: null check
    let a_pbr_pbs = cb.pave_blocks();
    if a_pbr_pbs.is_empty() {
        return 0.0;
    }
    ...
    if a_pbr_pbs.len() < 2 && cb.faces().is_empty() {
        return a_tol_max;
    }
    ...
    for &(a_pb_idx, _fi) in &a_pbr_pbs[1..] {
        ...
        for _ in 1..=a_nb_pnt {
            ...
            let proj = crate::bop::closest_point_on_curve(a_curve_other, a_p, 16);
            let a_dist = if proj.distance.is_finite() {
                proj.distance
            } else {
                0.0
            };
...
///  ?BOPTools_AlgoTools::TreatCompound (cxx:512-531).
...
///  ?BOPTools_AlgoTools::AreFacesSameDomain (cxx:1131-1197).
...
pub fn are_faces_same_domain(fi_a: usize, fi_b: usize, ds: &DS) -> bool {
    ...
    if fa.normal.dot(fb.normal).abs() < 0.99 {
        return false;
    }
    ...
    let max_dist = fa.boundary_verts[..n]
        .iter()
        .zip(&fb.boundary_verts[..n])
...
///  ?BOPTools_AlgoTools::CorrectRange (EE, cxx:284-360).
...
pub fn correct_range_ee(
    ...
    let res1 = match curve {
        Curve3::Line(_) => a_tol,
        _ => crate::bop::int_tools::curve_range::curve_resolution(curve, t1, a_tol),
    };
    let res2 = match curve {
        Curve3::Line(_) => a_tol,
        _ => crate::bop::int_tools::curve_range::curve_resolution(curve, t2, a_tol),
    };
    let ct1 = t1 + res1;
    let ct2 = t2 - res2;
...
///  ?BOPTools_AlgoTools::CorrectRange (EF, cxx:364-400).
pub fn correct_range_ef(tol_face: f64, t_range: [f64; 2], curve: &Curve3) -> [f64; 2] {
    let [t1, t2] = t_range;
    if matches!(curve, Curve3::Line(_)) {
        return t_range;
    }
    ...
    let res1 = crate::bop::int_tools::curve_range::curve_resolution(curve, t1, tol_face);
    let res2 = crate::bop::int_tools::curve_range::curve_resolution(curve, t2, tol_face);
    let ct1 = t1 + res1;
    let ct2 = t2 - res2;
    if ct2 - ct1 < d_t { t_range } else { [ct1, ct2] }
}

///  ?BOPTools_Set =set of shapes for same-domain dedup.
...
///  ?FindPlane from curve (BOPAlgo_Tools.cxx L910-970).
///
...
///  ?FindEdgeTangent (BOPAlgo_Tools.cxx L875-904).
///
...
///  ?WiresToFaces (BOPAlgo_Tools.cxx L665-799).
///
...
pub fn wires_to_faces(
    ...
    for (wi, &w_idx) in wire_indices.iter().enumerate() {
        ...
        for &sub in wire_edges_flat {
            ...
            let tangent = a_dm_edge_tgt
                .entry(ei)
                .or_insert_with(|| find_edge_tangent(&edge.curve).unwrap_or(glam::DVec3::Z));
    ...
    for i in 0..wire_indices.len() {
        ...
        let Some(ref pln_i) = a_dm_wire_pln[i] else {
            continue;
        };
        ...
        for j in (i + 1)..wire_indices.len() {
            ...
            let Some(ref pln_j) = a_dm_wire_pln[j] else {
                continue;
            };
    ...
    for group in &wire_groups {
        ...
        let plane_surface = rcad_kernel::geom::Surface3::Plane(rcad_kernel::geom::Plane::new(
            pln_info.point,
            pln_info.normal,
        ));
...
/// GetEdgeOnFace (BOPTools_AlgoTools.cxx L1809-1835).
...
pub fn get_edge_on_face(
    ...
    for &be in &face.boundary_edges {
        ...
        if edge.start_vertex == ds_edges[ei].start_vertex
            && edge.end_vertex == ds_edges[ei].end_vertex
            || edge.start_vertex == ds_edges[ei].end_vertex
                && edge.end_vertex == ds_edges[ei].start_vertex
        {
...
/// GetEdgeOff (BOPTools_AlgoTools.cxx L1099-1127).
...
/// IsHole (BOPTools_AlgoTools.cxx L1527-1596).
...
pub fn is_hole(
    ...
    for &(ei, fwd) in wire_edges {
        ...
        let (t_start, step) = if fwd { (range[0], dt) } else { (range[1], -dt) };
        let mut t = t_start;
...
/// Sense (BOPTools_AlgoTools.cxx L1201-1251).
...
/// IsBlockInOnFace (BOPTools_AlgoTools.cxx L1971-2063).
...
pub fn is_block_in_on_face(
    ...
    if !is_point_in_on_face(&p11, face_idx, ds) {
        return false;
    }

    // OCCT L2010-2028: test near-end point
    let p12 = edge.curve.point_at(l1);
    if !is_point_in_on_face(&p12, face_idx, ds) {
        return false;
    }
    ...
    let proj_dist = match &face.surface {
        ...
        _ => {
            ...
            let proj = rcad_kernel::projection::closest_point_on_surface(&face.surface, p_mid, 16);
...
pub(crate) fn is_point_in_on_face(
    ...
    match &face.surface {
        ...
        _ => {
            ...
            let proj = rcad_kernel::projection::closest_point_on_surface(&face.surface, *pt, 16);
            if proj.distance.is_finite() {
...
/// ComputeTolerance (BOPTools_AlgoTools_1.cxx L1093-1111).
...
pub fn compute_tolerance(
    ...
    for i in 0..n_samples {
        ...
        let dist = match &face.surface {
            rcad_kernel::geom::Surface3::Plane(pl) => (pt - pl.origin).dot(pl.normal).abs(),
            rcad_kernel::geom::Surface3::Sphere(s) => ((pt - s.center).length() - s.radius).abs(),
    ...
    if max_dist < TOLERANCE_LEN_SQ_DIV_SAFE {
        None
    } else {
...
/// MakeEdge (BOPTools_AlgoTools.cxx L1721-1738).
...
/// MakeSplitEdge (BOPTools_AlgoTools.hxx L155-160).
...
/// MakePCurve (BOPTools_AlgoTools.cxx L1649-1717).
