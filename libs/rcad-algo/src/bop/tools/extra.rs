...
pub fn do_split_seam_on_face(
 ...
 let edge = ds.edges.get(ei).unwrap();
 let face = ds.faces.get(face_idx).unwrap();
 let uv_s = crate::bop::algo::builder::world_to_uv(&face.surface, ds.vertex_point(edge.start_vertex));
 let uv_e = crate::bop::algo::builder::world_to_uv(&face.surface, ds.vertex_point(edge.end_vertex));
...
pub fn is_internal_face_against_solid(
 ...
 crate::bop::tools::get_edge_off(ei, &ds.edges, ds.faces.get(a_f1).unwrap())
 } else { None };
 ...
 Some(state == crate::bop::ds::Classification::In)
}
...
pub fn is_internal_face_core(
 ...
 crate::bop::tools::get_edge_off(the_edge, &ds.edges, ds.faces.get(the_face1).unwrap())
 } else { None };
 ...
 crate::bop::tools::get_edge_off(the_edge, &ds.edges, ds.faces.get(the_face2).unwrap())
 .unwrap_or(a_e1)
 } else { a_e1 };
 ...
 let a_f_off = crate::bop::tools::get_face_off(a_e1, the_face1, &lcs_off, ds);
...
/// - Find the unused edge, orient to connect (end tart).
/// - Move to next vertex.
///
...
pub fn point_in_face(
 ...
 let uv = crate::bop::algo::builder::world_to_uv(&face.surface, p3d)?;
 Some((p3d, uv))
}
...
pub fn compute_state_face_against_solid(
 ...
 None => crate::bop::ds::Classification::Out,
 }
}
