...
/// Process Solid 鈫?Shell 鈫?Face 鈫?Wire 鈫?Edge 鈫?Vertex, pushing each shape
/// to ds.shapes[] and its type-specific flat array in depth-first order.
/// Skips already-visited shapes via visited set, maps topods ti 鈫?DS index.
...
fn init_shape_topo(
    ...
    match &**ts {
        ...
        //   L342: myMapShapeIndex.Seek(aSubShape) 鈥?TopoDS identity check via
        ...
        //   L344: Append(aSubShape) 鈥?always creates new shape info entry.
        ...
        topods::TShape::Edge(ed) => {
            ...
            // ShapeInfo is now created by push_edge() 鈥?no duplicate needed.
        }
...
// 鈹€鈹€鈹€鈹€鈹€ OCCT-aligned per-type loading functions (prepareVertices/Edges/Faces/Solids) 鈹€鈹€鈹€鈹€鈹€
...
/// OCCT BOPDS_DS::prepareVertices 鈥?load all shapes from a topods::BRep into DS.
