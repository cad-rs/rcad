...
impl IntersectionCurve {
    ...
    pub fn init_pave_block1(ds: &mut DS) -> usize {
        ...
        ds.pave_blocks.push(crate::bop::ds::pave::SharedPB::new(
            PaveBlock::new_curve_block(),
        ));
        idx
    }
...
impl ShapeInfo {
    ...
    pub fn has_sub_shape(&self, idx: usize) -> bool {
        self.sub_shapes.contains(&idx)
    }
    ...
    pub fn box_is_out(&self, other: &Self) -> bool {
        let (Some(mn1), Some(mx1)) = (self.box_min, self.box_max) else {
            return false;
        };
        let (Some(mn2), Some(mx2)) = (other.box_min, other.box_max) else {
            return false;
        };
...
pub struct DS {
    ...
    // ----- Shape index arrays (flat shapes[] 鈫?per-type index mapping) -----
    ...
    pub pave_blocks_pool: Vec<Vec<crate::bop::ds::pave::SharedPB>>,
...
fn pair_iterator_ab_cross_group() {
    // A = [0,2), B = [2,5) 鈫?cross pairs: (0,2),(0,3),(0,4),(1,2),(1,3),(1,4) = 6
