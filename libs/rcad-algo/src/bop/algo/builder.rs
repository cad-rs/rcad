...
pub struct BooleanBuilder<'a> {
    // BOPAlgo_Builder.hxx L495 =myPaveFiller 鈥?NOT stored; rcad runs PaveFiller before Builder, passes DS only.
    ...
    // rcad: keyed by source solid index (usize) 鈫?OCCT keys by TopoDS_Shape; Rust adaptation for DS-indexed access.
...
impl<'a> BooleanBuilder<'a> {
    ...
    /// 鉁?OCCT-aligned: BuildDraftFace (Builder_2.cxx L1052-1189).
        ...
        for i in 0..face_boundary_edges.len() {
            ...
            if !has_images {
                ...
                for sp_sr in &imgs {
                    ...
                    let needs_rev =
                        crate::bop::algo::builder::edge_builders::is_split_to_reverse(ds, sp_ei, ei);
                    add_segment(
                        sp_ei,
                        forward != needs_rev,
                        Some(ei),
...
impl<'a> BooleanBuilder<'a> {
    ...
    fn check_data(&self) -> Result<(), BooleanError> {
        ...
        // OCCT: BOPAlgo_BuilderShape::CheckData() 鈥?base class checks.
        ...
        match self.my_operation {
            ...
            _ => self.my_report
                .borrow_mut()
                .add_alert(crate::bop::algo::Alert::BOPNotSet),
        }
    ...
    fn check_filler(&self) {
        ...
        if self.ds.vertex_count() == 0 {
            self.my_report.borrow_mut()
                .add_alert(crate::bop::algo::Alert::NoFiller);
        }
    }
    ...
    fn treat_empty_shape(
        ...
        if !has_a && !has_b {
            // OCCT L252-256: all empty 鈫?empty
            return Ok(Some(topods::BRep::new()));
        }
        // OCCT L258-317: one side empty 鈫?result depends on operation
        match self.my_operation {
            BooleanOpType::Union => {
            ...
            BooleanOpType::Difference => {
                if !has_a {
                    // OCCT L287-289: CUT with empty objects 鈫?empty
                    Ok(Some(topods::BRep::new()))
                } else {
                    // OCCT L281-289: CUT with empty tools 鈫?return objects
                    let brep =
            ...
            _ => {
                // Unknown operation 鈫?fall through to full pipeline
                Ok(None)
            }
        }
    }
    ...
    pub fn build_with_history_topods(
        ...
        // OCCT: BRep_Tool::Dimension(myArguments.First/Last) 鈫?VERTEX=0, EDGE=1, WIRE/FACE=2, SHELL/SOLID=3.
        ...
        // OCCT L569-573: 5. PrepareHistory 鈥?builds BRep TShapes + source shape history.
        let mut history = {
...
impl<'a> BooleanBuilder<'a> {
    ...
    fn brep_of_side_topods(&self, origin: ShapeOrigin, _na: usize, _nb: usize) -> topods::BRep {
        ...
        // Collect face refs and wrap in Shell 鈫?Solid
