...
use crate::bop::ds::{ConnexityBlock, DS};
use crate::bop::tools;
...
fn make_shell(faces: &[usize], ds: &DS) -> Vec<usize> {
    ...
    crate::bop::tools::orient_faces_on_shell(faces, ds, &mut reversed)
}
...
///
/// GetFaceOff angle selection via crate::bop::tools::get_face_off.
            ...
            for &ei in &all_edges {
                ...
                if !is_boundary || a_nb_ways_inside != 1 {
                    if a_nb_off == 1 {
                        ...
                        a_sel_f = crate::bop::tools::get_face_off(ei, a_f, &lcs_off, ds);
                    }
                }
