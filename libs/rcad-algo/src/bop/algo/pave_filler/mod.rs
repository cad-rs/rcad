...
use crate::bop::tools::bvh::BoxTree;
use crate::bop::tools::bvh::Bvh;
use crate::bop::int_tools;
use crate::bop::int_tools::context::Context as IntToolsContext;
use crate::bop::int_tools::fclass2d::{FClass2d, State};
use crate::tolerance::*;
use indexmap::IndexSet;
use crate::bop::closest_point_on_curve;
pub mod helpers;
use self::helpers::*;
    ...
    if b_cb {
        ...
        for &rp_idx in a_lpb {
            ...
            for &(old_pb_idx, old_face_idx) in &old_pbs {
                ...
                let mut pb2n =
                    crate::bop::ds::pave::PaveBlock::new(orig_e, rp_pave1.clone(), rp_pave2.clone());
                pb2n.new_edge = {
                    let rp_pb = ds.pave_blocks[rp_idx].0.read().unwrap();
                    rp_pb.new_edge
                };
                ...
                ds.pave_blocks.push(crate::bop::ds::pave::SharedPB::new(pb2n));
    ...
    if let Some(pb_faces) = the_pb_faces_map.get(&a_pbf) {
        ...
        for &fi in pb_faces {
            ...
            for &new_pb_idx in &new_pb_list {
                ...
                let b_coincide = {
                    crate::bop::int_tools::edge_face::is_coincident_edge_face(
                ...
                if b_coincide {
                    ...
                    if !found_cb {
                        ...
                        let mut new_cb = crate::bop::ds::common_block::CommonBlock::new();
...
pub struct PaveFiller<'a> {
    pub ds: &'a mut DS,
    /// =myIterator (BOPAlgo_PaveFiller.hxx) 閳?BOPDS_Iterator for pair enumeration.
    ...
    /// =myIncreasedSS (PaveFiller.hxx L651) 閳?sub-shapes with increased tolerance.
    ...
    if fi < ds.face_count() {
        for &ei in ds.face_boundary_edges(fi) {
            ...
            if ei < ds.edge_count() {
                ...
                aMI.insert(v_start);
                aMI.insert(v_end);
                // OCCT: resolve SD vertices 閳?if a boundary vertex has an SD
...
impl<'a> PaveFiller<'a> {
    ...
    fn prepare(&mut self) {
     ...
     // OCCT L873: IsBasedOnPlane(aF) 閳?checks Geom_Plane with TrimmedSurface unwrap
     ...
     && matches!(self.ds.locate_surface(n_f), Surface3::Plane(_))
        ...
        for i in 0..a_nb_f {
            ...
            // OCCT L893: aExp.Init(aF, aType[1]) 閳?explore EDGE sub-shapes
    ...
    fn init(&mut self, a: &topods::BRep, b: &topods::BRep, fuzzy_tol: f64) {
        ...
        // OCCT L196: Clear() 閳?reset report and state
        self.clear();
        ...
        // OCCT BOPDS_DS::Init: prepareVertices 閳?prepareEdges 閳?prepareFaces 閳?prepareSolids
        ...
        // OCCT L204: myContext = new IntTools_Context 閳?rcad: resize context
    ...
    /// OCCT BOPDS_DS::prepareVertices 閳?traverse both operands and load all shapes into DS.
    ...
    /// OCCT BOPDS_DS::prepareEdges 閳?structural no-op (loading done in prepareVertices).
    ...
    /// OCCT BOPDS_DS::prepareFaces 閳?structural no-op (loading done in prepareVertices).
    ...
    /// OCCT BOPDS_DS::prepareSolids 閳?structural no-op (loading done in prepareVertices).
    ...
    pub fn perform(&mut self, a: &topods::BRep, b: &topods::BRep) {
        ...
        // OCCT L251-366: pipeline body (Prepare 閳?ProcessDE)
        self.perform_body();
    }
    ...
    pub(crate) fn perform_body(&mut self) {
        ...
        // OCCT L265: myIterator->Intersect(/*...*/) 閳?BOPDS_Iterator with BVH
    ...
    pub(crate) fn update_vertex(&mut self, n_v: usize, a_tol_new: f64) -> usize {
        ...
        if self.ds.is_new_vertex(n_v) || sd_opt.is_some() || !self.non_destructive {
            ...
            if a_tol_v < a_tol_new {
                ...
                // OCCT L124: myIncreasedSS.Add(nV) 閳?adds the original vertex
                self.my_increased_ss.insert(n_v);
            }
            return n_v_new;
        }
        // OCCT L129-159: nV is old vertex 閳?create new SD vertex (non-destructive mode)
    ...
    pub(crate) fn remove_micro_section_edges(
        ...
        a_micro_pb: &mut Vec<crate::bop::ds::pave::PaveBlock>,
    ) {
        if a_mscpb.is_empty() {
            return;
        }
        ...
        for &edge_or_vertex in &keys {
            ...
            if edge_or_vertex < self.ds.edge_count() {
                // It's an edge 閳?check if it's a micro edge
    ...
    pub(crate) fn update_face_info_post(
        ...
        // OCCT L1715: anEdgeLPB 閳?map from edge index to list of PBs
            ...
            for &ci in &curve_idxs {
                ...
                for &pb_local in &curve_pbs {
                    ...
                    if let Some(replacements) = theDME.get(&pb_idx) {
                        ...
                        // OCCT L1761: aLPBC.Remove(aItPB) 閳?don't add to keep list
                        continue;
                    }

                    // OCCT L1765-1766: normal section PB 閳?add to both faces' pave_blocks_sc
        ...
        for &n_f1 in &a_mf {
            ...
            if b_edges {
                ...
                for &pb_idx in &on_copy {
                    if let Some(replacements) = theDME.get(&pb_idx) {
                        for &rp in replacements {
                            // OCCT L1959: RealPaveBlock(aPB1) 閳?in rcad the index IS the real block
    ...
    fn remove_micro_edges(&mut self) {
        ...
        for i in 0..a_nb_e {
            ...
            let a_lpb: Vec<crate::bop::ds::pave::SharedPB> = self.ds.edge_pave_blocks(i).to_vec();
            if a_lpb.is_empty() {
                continue;
            }
    ...
    pub(crate) fn make_split_edges(&mut self) {
        ...
        //
        // L386: aMCB 閳?dedup set for CommonBlocks (store CB indices)
        ...
        struct SplitEdgeTask {
            ...
            pb_shared: crate::bop::ds::pave::SharedPB,
            cb_idx: Option<usize>,
        }
        //
        ...
        for ei in 0..n_edges {
            ...
            //
            // aLPB = aPBP(i) 閳?the list of PBs for this edge (L402)
            let n_e = ei;
            //
            // L408-410: aSIE.HasFlag() 閳?skip degenerated edges
            ...
            let edge_pbs: Vec<crate::bop::ds::pave::SharedPB> =
            ...
            for spb in &edge_pbs {
                ...
                if b_cb && !a_mcb.insert(pb_cb_idx.unwrap()) {
                    // CB already processed 閳?skip (OCCT: aMCB.Add returns false)
                    continue;
                }
                //
                ...
                //
                if !b_v1 && !b_v2 {
                    // L430: no new vertices 閳?may avoid splitting
                    if !self.non_destructive || !b_cb {
                        if b_cb {
                            // L436-455: CB 閳?find the PB whose original edge has 1 PB
                            ...
                            //
                            if a_found_it {
                                // L447-456: edge has only 1 PB 閳?no split needed
                                b_to_split = false;
                            ...
                            // L457-461: single PB, no new vertices 閳?keep original edge
                            b_to_split = false;
                            // L460: aPB->SetEdge(nE) 閳?short-lived guard for write
                            let orig = {
                        ...
                        // L462-465: if !bToSplit 閳?skip (continue)
                        if !b_to_split {
                            continue;
                        }
                    }
                ...
                let (task_edge_idx, task_n_v1, task_n_v2, task_t1, task_t2, task_pb) = {
                    if b_cb {
                        ...
                        // L477: use current PB's data 閳?short-lived guard
                        let (t1, t2) = {
                            let p = spb.0.read().unwrap();
                            p.range()
                        };
        ...
        // (OCCT L449-454: CB edge with 1 PB 閳?no split, just tolerance)
        for update in &deferred_cb_updates {
            // L450: aCB->SetEdge(nE)
            self.ds.common_blocks[update.cb_idx].set_edge(update.edge_idx);
            // L453: ComputeToleranceOfCB
            let a_tol = crate::bop::ds::tools::compute_tolerance_of_cb(self.ds, update.cb_idx);
        ...
        for task in &tasks {
            ...
            if let Some(cb_idx) = task.cb_idx {
                ...
                let (_t1, cp1) = crate::bop::closest_point_on_curve(&orig_curve, v1_p);
                let (_t2, cp2) = crate::bop::closest_point_on_curve(&orig_curve, v2_p);
    ...
    pub(crate) fn check_self_interference(&self) -> Vec<crate::bop::algo::Alert> {
        if self.my_arguments.len() <= 1 {
            return Vec::new();
        }

        let mut a_alerts: Vec<crate::bop::algo::Alert> = Vec::new();
                    ...
                    if let Some(cb_idx) = pb.common_block_idx {
                        if a_cb_fence.insert(cb_idx) {
                            if let Some(cb) = self.ds.common_blocks.get(cb_idx) {
                                ...
                                if a_le.len() > 1 {
                                    a_alerts.push(crate::bop::algo::Alert::AcquiredSelfIntersection(
                                        a_le,
                                    ));
                                }
                            }
