...
/// 閴?OCCT-aligned: delegates to crate::bop::closest_point_on_curve (128 samples + analytic dispatch).
...
pub(crate) fn find_nearest_valid_point(
    ...
    while !is_out {
        ...
        if a_d1_mag > 0.0 {
            // OCCT L108-137: singularity handling 閳?double step through near-zero derivative regions
            ...
            loop {
                ...
                if a_d1.length_squared() < a_d1_mag {
                    ...
                    if (is_first && new_out > end_u) || (!is_first && new_out < end_u) {
                        // went out of range 閳?check if end point is outside
                        if curve.point_at(end_u).distance_squared(vert_pt) <= tol_sq {
                            return None;
                        }
                        u_out = end_u;
...
/// 閴?OCCT-aligned: delegates to crate::bop::curve_bounds (OCCT GeomBndLib per-type dispatch).
pub(crate) fn curve_bounding_box_simple(curve: &Curve3, tol: f64) -> Option<[DVec3; 2]> {
    let bbox = crate::bop::curve_bounds(curve, tol);
    if bbox.is_valid() {
...
pub(crate) fn intersect_edge_face_numeric(
    ...
    for i in 0..=N_SAMPLES {
        ...
        values.push(crate::bop::int_tools::marching::surface_implicit(surface, p));
        points.push(p);
    }
    ...
    for i in 0..N_SAMPLES {
        ...
        for _ in 0..MAX_BISECT {
            ...
            let fm = crate::bop::int_tools::marching::surface_implicit(surface, pm);
            if fm.abs() < zero_tol {
                hits.push((pm, tm));
                break;
            }
    ...
    for (point, t) in hits.iter_mut() {
        ...
        if let Some((refined_t, refined_uv)) = crate::bop::int_tools::curve_surface::newton_refine_curve_surface(
