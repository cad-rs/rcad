...
// Rust's Vec/HashMap handle memory automatically 閳?the structure (collection
...
// equals an index value from the other 閳?unlikely in practice.
...
impl<'a> super::PaveFiller<'a> {
        ...
        for ci in 0..self.ds.intersection_curves.len() {
            ...
            for &sei in &refs {
                if sei < self.ds.edge_count() {
                    ...
                    self.ds.edge_data_mut(sei).tolerance =
                        edge_tol.min(curve_tol).max(TOLERANCE_ABS);
                }
            }
        }
    }
    ...
    pub(super) fn get_stick_vertices_ff(
        ...
        a_mi.clear();
        let a_mi_1 = crate::bop::algo::pave_filler::build_face_shape_map(self.ds, n_f1);
        let a_mi_2 = crate::bop::algo::pave_filler::build_face_shape_map(self.ds, n_f2);
        for &v in &a_mi_1 {
            a_mi.insert(v);
        }
        for &v in &a_mi_2 {
        ...
        for inf in &self.ds.interf_vv {
            if inf.merged_vertex == usize::MAX {
                continue;
            }
    ...
    fn update_saved_tolerance(
        ...
        // OCCT L630-645: iterate SubShapes() 閳?all vertex sub-shapes of the edge
    ...
    fn is_existing_pb_via_lse(
        ...
        for &sei in a_lse {
            ...
            let (_t, a_proj) = if let Some(curve) = self.ds.edge_curve(sei) {
                crate::bop::closest_point_on_curve(curve, a_pm)
            } else {
                (0.0, a_pm)
            };
    ...
    fn is_existing_pb_via_bvh(
        ...
        for &pb_idx in &candidates {
                    ...
                    if is_vtgt1_valid {
                        ...
                        if !is_ic_line || !is_edge_line {
                            ...
                            let (_a_t, a_dist_m1m2) =
                                crate::bop::closest_point_on_curve(self.ds.edge_curve(edge_ei).unwrap(), a_pm);
                            ...
                            if a_vtgt2.length_squared() > f64::EPSILON {
                                ...
                                // OCCT L2162: if (abs(aCos) >= 0.9063) 閳?25-degree threshold
                                if a_cos.abs() >= 0.9063 {
            ...
            let (_t, proj) = crate::bop::closest_point_on_curve(self.ds.edge_curve(edge_ei).unwrap(), a_pm);
            ...
            if !i_flag1 {
                let (_t1, p1_proj) =
                    crate::bop::closest_point_on_curve(self.ds.edge_curve(edge_ei).unwrap(), a_p1);
                dist_p1 = (p1_proj - a_p1).length();
            }
            let mut dist_to_use = dist_to_sp;
            if n_v2 != n_v21 && n_v2 != n_v22 {
                let (_t2, p2_proj) =
                    crate::bop::closest_point_on_curve(self.ds.edge_curve(edge_ei).unwrap(), a_p2);
    ...
    pub(super) fn make_blocks(&mut self) {
        ...
        while loop_i < a_nb_ff {
            ...
            // L729-732: UserBreak check 閳?omitted (no progress range in rcad)

            // L735: aCurInd = i < aNbFFPrev ? i : aFFToRecheck[i - aNbFFPrev];
            let a_cur_ind = if i < a_nb_ff_prev {
                i
            } else {
                a_ff_to_recheck[i - a_nb_ff_prev]
            };
            ...
            for &ci in curves_of_ff {
                ...
                if self.ds.intersection_curves[ci].pave_blocks.is_empty() {
                    ...
                    pb.pave1 = Pave {
                        vertex_idx: self.ds.intersection_curves[ci].start_vertex,
                        param: self.ds.intersection_curves[ci].t_range[0],
                    };
                    pb.pave2 = Pave {
                        vertex_idx: self.ds.intersection_curves[ci].end_vertex,
            ...
            for (j, &ci) in curves_of_ff.iter().enumerate() {
                ...
                // OCCT L2928-2955: RemoveUsedVertices 閳?remove vertices already on any curve
                let a_mv_filtered: std::collections::HashSet<usize> = {
                    let mut mv = a_mv_stick.clone();
                    for &oci in curves_of_ff {
                        if oci >= self.ds.intersection_curves.len() {
                            continue;
                        }
                        let Some(aPB) = self.ds.intersection_curves[oci].pave_blocks.first() else {
                            continue;
                        };
                        let r = aPB.0.read().unwrap();
                        for pave in &r.ext_paves {
                            mv.remove(&pave.vertex_idx);
                        }
                ...
                // L848-863: aIC.HasBounds() 閳?PutBoundPaveOnCurve
                let has_bounds = {
            ...
            // L899: isToRecheck 閳?OCCT: (aNbC > 0) && (i < aNbFFPrev)
            ...
            for (j, &ci) in curves_of_ff.iter().enumerate() {
                ...
                for a_pb in &a_lpb {
                    ...
                    for (k, &fi) in [n_f1, n_f2].iter().enumerate() {
                        ...
                        let pc_opt = if k == 0 {
                            ic.pcurve_on_a.as_ref()
                        } else {
                            ic.pcurve_on_b.as_ref()
                        };
                    ...
                    if !b_valid_2d {
                        if std::env::var("RCAD_DBG_MB").is_ok() {
                            eprintln!(
                                "[DBG_MB]   sub-PB ({},{}) REJECTED by IsValidBlockForFaces",
                                n_v1, n_v2
                            );
                        }
                        continue;
                    ...
                    let b_exist_lse =
                        self.is_existing_pb_via_lse(&a_lse, a_pb, ci, &mut n_e_out, &mut a_tol_new);
                    if b_exist_lse {
                        if std::env::var("RCAD_DBG_MB").is_ok() {
                            eprintln!(
                                "[DBG_MB]   sub-PB ({},{}) EXISTING via LSE n_e={}",
                                n_v1, n_v2, n_e_out
                            );
                        }
                    ...
                    // OCCT L937-960: BRepLib::FindValidRange 閳?check if the pave block
                    ...
                    let has_valid_range = {
                        ...
                        crate::bop::algo::pave_filler::helpers::find_valid_range(
                    ...
                    if !has_valid_range {
                        if std::env::var("RCAD_DBG_MB").is_ok() {
                            eprintln!(
                                "[DBG_MB]   sub-PB ({},{}) REJECTED by FindValidRange",
                                n_v1, n_v2
                            );
                        }
                    ...
                    if b_exist_on_in {
                        if std::env::var("RCAD_DBG_MB").is_ok() {
                            eprintln!(
                                "[DBG_MB]   sub-PB ({},{}) EXISTING via BVH a_pb_out={}",
                                n_v1, n_v2, a_pb_out
                            );
                        }
                        if a_pb_out < self.ds.pave_blocks.len() {
                            ...
                            let b_in_f1 =
                                self.ds.face_info(n_f1).pave_blocks_on.contains(&a_pb_out)
                                    || self.ds.face_info(n_f1).pave_blocks_in.contains(&a_pb_out);
                            let b_in_f2 =
                                self.ds.face_info(n_f2).pave_blocks_on.contains(&a_pb_out)
                            ...
                            if !b_in_f1 || !b_in_f2 {
                                ...
                                if n_v1 != n_v_out1
                                    && n_v1 != n_v_out2
                                    && !a_mv_bounds.contains(&n_v1)
                                {
                                    a_verts_on_rejected_pb.insert(n_v1);
                                }
                                if n_v2 != n_v_out1
                                    && n_v2 != n_v_out2
                                    && !a_mv_bounds.contains(&n_v2)
                                {
                    ...
                    if std::env::var("RCAD_DBG_MB").is_ok() {
                        eprintln!("[DBG_MB]   sub-PB ({},{}) 閳?MAKE EDGE", n_v1, n_v2);
                    }
                    ...
                    let new_ei =
                        crate::bop::tools::make_edge(self.ds, ci, n_v1, n_v2, a_t1, a_t2, a_tol_r3d);
                    // L1058-1064: MakePCurve
                    crate::bop::tools::make_pcurve(
                        self.ds,
                        new_ei,
                        n_f1,
                        n_f2,
                ...
                // L1097: aLPBC.RemoveFirst() 閳?remove InitPaveBlock1
                if !self.ds.intersection_curves[ci].pave_blocks.is_empty() {
        ...
        // L1153-1156: if (HasErrors()) return; 閳?omitted (no HasErrors in rcad)

        // L1158: CorrectToleranceOfSE
        self.correct_tolerance_of_se();

        // L1161: UpdateFaceInfo
        self.update_face_info_post(&a_dm_ex_edges, &a_dm_new_sd, &a_pb_faces_map);
    ...
    fn build_pb_tree(ds: &DS, a_mpb_on_in: &HashSet<usize>) -> Option<BoxTree> {
        ...
        for &pb_idx in a_mpb_on_in {
            ...
            if r.new_edge.is_none() && r.original_edge == NO_EDGE {
                continue;
            }
            ...
            a_pb_aabbs.push(Aabb {
                min: mn - DVec3::splat(tol),
                max: mx + DVec3::splat(tol),
                gap: 0.0,
            });
        }
