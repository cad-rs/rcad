...
//!
//! rcad integration: uses crate::bop::int_tools::marching for the core walking step
...
use crate::extrema;
use crate::bop::int_tools::marching::{self, MarchingConfig};
...
impl PWalking {
    ...
    pub fn value(&self, index: usize) -> Option<&PntOn2S> {
        self.line.get(index - 1)
    }
    ...
    pub fn perform_first_point(&mut self, par_dep: &[f64; 4], first_point: &mut PntOn2S) -> bool {
        ...
        if dist > self.my_tol_tang.max(TOLERANCE_MESH_LEGACY) {
            return false;
        }
    ...
    /// rcad: uses crate::bop::int_tools::marching::march_intersection_with_config.
    pub fn perform_with_bounds(
        ...
        let step_size = self.pasuv[0]
            .min(self.pasuv[1])
            .min(self.pasuv[2])
            .min(self.pasuv[3]);
        let min_step = self.my_step_min[0]
            .min(self.my_step_min[1])
            .min(self.my_step_min[2])
            .min(self.my_step_min[3]);
        ...
        if !seed_p3d.is_finite() {
            return;
        }

        let result =
            marching::march_intersection_with_config(&self.s1, &self.s2, seed_p3d, &config, |p| {
        ...
        for p3d in &result.points {
            ...
            let Some((u1, v1)) = project_onto_surface(&self.s1, *p3d) else {
                continue;
            };
            let Some((u2, v2)) = project_onto_surface(&self.s2, *p3d) else {
                continue;
            };
    ...
    pub fn put_to_boundary(&mut self, s1: &Surface3, s2: &Surface3) -> bool {
        ...
        let mut snap_v2 = v2;
        if !is_v1_par {
            need = Self::snap_to_bnd(&mut snap_u1, u1b_first, u1b_last, a_tol_min, a_tol) || need;
        }
        if !is_v2_par {
            need = Self::snap_to_bnd(&mut snap_u2, u2b_first, u2b_last, a_tol_min, a_tol) || need;
        }
        if !is_u1_par {
            need = Self::snap_to_bnd(&mut snap_v1, v1b_first, v1b_last, a_tol_min, a_tol) || need;
        }
        if !is_u2_par {
            need = Self::snap_to_bnd(&mut snap_v2, v2b_first, v2b_last, a_tol_min, a_tol) || need;
        }
        if need {
    ...
    fn is_parallel(line: &[PntOn2S], check_surf1: bool, toler: f64) -> (bool, bool) {
        ...
        let mut a_u_min = f64::MAX;
        let mut a_u_max = f64::MIN;
        let mut a_v_min = f64::MAX;
        let mut a_v_max = f64::MIN;
        let mut a_n_point = 1.0_f64;
        for _ in 0..n_points {
            let idx = if (a_n_point as usize) > n {
                n - 1
            } else {
                a_n_point as usize - 1
            };
    ...
    pub fn distance_minimize_by_gradient(
        &self,
        s1: &Surface3,
        s2: &Surface3,
        init: &mut [f64; 4],
    ...
    pub fn distance_minimize_by_extrema(
        &self,
        surf: &Surface3,
        p0: DVec3,
        u0: &mut f64,
    ...
    fn handle_single_singular_point(
        &self,
        s1: &Surface3,
        s2: &Surface3,
        _a3d_tol: f64,
        ...
        for i in 0..4 {
            ...
            if let Some((nu1, nv1)) = project_onto_surface(s1, p3d) {
                pnt[0] = nu1;
                pnt[1] = nv1;
            }
            if let Some((nu2, nv2)) = project_onto_surface(s2, p3d) {
                pnt[2] = nu2;
                pnt[3] = nv2;
            }
            if at_low {
            ...
            for j in 0..4 {
                if (pnt[j] - low[j] + conf) * (pnt[j] - upp[j] - conf) > 0.0 {
                    in_domain = false;
                    break;
                }
            }
    ...
    pub fn seek_additional_points(
        ...
        if self.line.len() > min_nb_points {
            return true;
        }
        let (u1b_f, u1b_l, v1b_f, v1b_l) = uv_range(s1);
        let (u2b_f, u2b_l, v2b_f, v2b_l) = uv_range(s2);
    ...
    pub fn extend_line_in_common_zone(
        &mut self,
        s1: &Surface3,
        s2: &Surface3,
        the_choix_iso: ConstIsoparametric,
        the_direction_flag: bool,
    ) -> bool {
        let (u1b_f, u1b_l, v1b_f, v1b_l) = uv_range(s1);
        let (u2b_f, u2b_l, v2b_f, v2b_l) = uv_range(s2);
        ...
        if (param[0] - u1b_f).abs() < self.reso_u1 {
            return false;
        }
        if (param[1] - v1b_f).abs() < self.reso_v1 {
            return false;
        }
        if (param[2] - u2b_f).abs() < self.reso_u2 {
            return false;
        }
        if (param[3] - v2b_f).abs() < self.reso_v2 {
            return false;
        }
        if (param[0] - u1b_l).abs() < self.reso_u1 {
            return false;
        }
        if (param[1] - v1b_l).abs() < self.reso_v1 {
            return false;
        }
        if (param[2] - u2b_l).abs() < self.reso_u2 {
            return false;
        }
        if (param[3] - v2b_l).abs() < self.reso_v2 {
            return false;
        }
        ...
        while !b_stop && nb_iter_no_append < 20 && nb_equal < 20 {
            ...
            let f = 0.1_f64.max({
                match the_choix_iso {
                    ConstIsoparametric::U1
                    | ConstIsoparametric::V1
                    | ConstIsoparametric::U2
                    | ConstIsoparametric::V2 => 10.0,
                    _ => 10.0,
                }
            });
            ...
            match a_status {
                ...
                _ => {
                    ...
                    b_stop = self.test_arret(
                        the_direction_flag,
                        &mut param,
                        &mut ConstIsoparametric::None,
                    );
                    ...
                    if !b_stop {
                        ...
                        if let Some(ref pp) = self.previous_point {
                            if (np.u1 - pp.u1).abs() < self.reso_u1
                                && (np.v1 - pp.v1).abs() < self.reso_v1
                                || (np.u2 - pp.u2).abs() < self.reso_u2
                                    && (np.v2 - pp.v2).abs() < self.reso_v2
                            {
                                nb_equal += 1;
                            } else {
                                nb_equal = 0;
    ...
    pub fn test_deflection(
        ...
        let n = self.line.len();
        if n < 3 {
            return StatusDeflection::PasTropPetit;
        }
    ...
    pub fn test_arret(
        ...
        _deja_reparti: bool,
        param: &mut [f64; 4],
        choix_iso: &mut ConstIsoparametric,
    ) -> bool {
        ...
        let v2 = param[3];
        let at_u1_boundary =
            (u1 - self.um1).abs() < self.reso_u1 || (u1 - self.um1_max).abs() < self.reso_u1;
        let at_v1_boundary =
            (v1 - self.vm1).abs() < self.reso_v1 || (v1 - self.vm1_max).abs() < self.reso_v1;
        let at_u2_boundary =
            (u2 - self.um2).abs() < self.reso_u2 || (u2 - self.um2_max).abs() < self.reso_u2;
        let at_v2_boundary =
            (v2 - self.vm2).abs() < self.reso_v2 || (v2 - self.vm2_max).abs() < self.reso_v2;
...
fn project_onto_surface(surf: &Surface3, pt: DVec3) -> Option<(f64, f64)> {
    let (uv, _proj) = crate::bop::closest_point_on_surface(surf, pt);
