...
use crate::bop::algo::pave_filler::helpers::*;
use crate::bop::algo::{GlueEnum, fill_map, make_blocks};
use crate::bop::ds::{
    DS, DSVertex, InterferenceEE, InterferenceEF, InterferenceVE, InterferenceVF, InterferenceVV,
    ShapeOrigin,
};
use crate::bop::ds::pave::Pave;
use crate::bop::int_tools;
use crate::bop::int_tools::context::VfError;
use crate::bop::int_tools::edge_edge::compute_curve_aabb;
use crate::bop::int_tools::fclass2d::{FClass2d, State};
use crate::tolerance::*;
...
pub fn bounding_vertex(vi: &[usize], ds: &crate::bop::ds::DS) -> (glam::DVec3, f64) {
    let n = vi.len();
    if n == 0 {
...
// OCCT L55-162: BOPAlgo_EdgeFace 鈥?target for future alignment
struct EdgeFaceTask {
    myIE: usize,
    myIF: usize,
    myFlag: i32,
...
impl EdgeFaceTask {
    ...
    fn perform(&mut self, ctx: &mut crate::bop::int_tools::context::Context, ds: &DS) {
...
fn compute_ef_hits(
    ...
    let mut hits: Vec<EfHit> = match (edge_curve, face_surface) {
        (Curve3::Line(line), Surface3::Plane(plane)) => {
            crate::bop::int_tools::edge_face::intersect_line_plane_with_tol(
        ...
        (Curve3::Line(line), Surface3::Cylinder(cyl)) => {
            crate::bop::int_tools::curve_surface::intersect_line_cylinder_with_tol(
        ...
        (Curve3::Line(line), Surface3::Sphere(sph)) => {
            crate::bop::int_tools::curve_surface::intersect_line_sphere_with_tol(
        ...
        (Curve3::Line(line), Surface3::Cone(cone)) => {
            crate::bop::int_tools::curve_surface::intersect_line_cone_with_tol(
    ...
    if hits.is_empty() && ef_range[1] - ef_range[0] > ef_tol {
        ...
        use crate::bop::int_tools::bean_face_intersector::BeanFaceIntersector;
...
impl VertexFaceTask {
    ...
    fn perform(&mut self, ctx: &mut crate::bop::int_tools::context::Context, ds: &DS, fuzz: f64) {
...
impl<'a> super::PaveFiller<'a> {
    ...
    pub(crate) fn build_box_tree(
        ...
        for local_i in 0..n {
            ...
            let aabb = if is_edge {
                ...
                // Expand for edge tolerance 鈥?OCCT: extents + gap
    ...
    pub(crate) fn build_box_tree_combined(&self, is_edge: bool) -> crate::bop::tools::bvh::BoxTree {
        use crate::bop::tools::bvh::{Aabb, BoxTree};
        let n = if is_edge {
            self.ds.edge_count()
        } else {
    ...
    pub(crate) fn perform_ve(&mut self) {
        ...
        for _ in 0..i_size {
            ...
            // OCCT L165-168: aSIE.HasSubShape(nV) 鈥?vertex is subshape of edge?
    ...
    /// Returns false if the pave block's range is degenerate (length 鈮?TOLERANCE_LEN_MIN).
    /// On return, `the_box` contains the AABB of the PB's curve segment.
    pub(crate) fn get_pb_box(
        &self,
        the_e: usize,
        the_pb: &crate::bop::ds::pave::PaveBlock,
        ...
        *the_last = r2;
        ...
        let key = the_pb as *const crate::bop::ds::pave::PaveBlock as usize;
    ...
    pub(crate) fn perform_ee(&mut self) {
        ...
        struct EeTask {
            pb1: crate::bop::ds::pave::SharedPB,
            pb2: crate::bop::ds::pave::SharedPB,
            nE1: usize,
            nE2: usize,
            aT11: f64,
            aT12: f64,
        ...
        let mut a_mvcpb: std::collections::HashMap<usize, crate::bop::ds::pave::CoupleOfPaveBlocks> =
            std::collections::HashMap::new();
        ...
        for _ in 0..i_size {
            ...
            // OCCT L215-266: PB pair iteration 鈥?using get_pb_box for AABB + cache (OCCT L914-955)
        ...
        for k in 0..a_nb_edge_edge {
            ...
            let hits: Vec<(f64, f64, DVec3, [f64; 2], [f64; 2])> = match (&e1_curve, &e2_curve) {
                (Curve3::Line(l1), Curve3::Line(l2)) => intersect_line_line(l1, sr1, l2, sr2, tol)
                    ...
                    .collect(),
                _ => {
                    use crate::bop::int_tools::edge_edge::EdgeEdgeIntersector;
            ...
            for i in 0..a_nb_cprts {
                ...
                let b0 = crate::bop::tools::is_on_pave(a_t1, a_r11, a_tol_confusion)
                    || (a_r11[0] - a_t1).abs() <= a_tol_confusion;
                let b1 = crate::bop::tools::is_on_pave(a_t1, a_r12, a_tol_confusion)
                    || (a_r12[1] - a_t1).abs() <= a_tol_confusion;
                let b2 = crate::bop::tools::is_on_pave(a_t2, a_r21, a_tol_confusion)
                    || (a_r21[0] - a_t2).abs() <= a_tol_confusion;
                let b3 = crate::bop::tools::is_on_pave(a_t2, a_r22, a_tol_confusion)
                ...
                a_mvcpb.insert(
                    new_v,
                    crate::bop::ds::pave::CoupleOfPaveBlocks {
                        interf_idx: i_x,
                        vertex_index: new_v,
                        pb1: task.pb1.clone(),
                        pb2: task.pb2.clone(),
        ...
        if !a_mplpb.is_empty() {
            ...
            crate::bop::ds::tools::perform_common_blocks(&mut self.ds);
        }
    ...
    pub(crate) fn force_interf_ve(
        &mut self,
        n_v: usize,
        a_pb: &crate::bop::ds::pave::SharedPB,
        ...
        let a_pave = crate::bop::ds::pave::Pave {
            vertex_idx: n_vx,
            param: a_t,
        };
        {
        ...
        self.ds
            .interf_ve
            .push(crate::bop::ds::types::InterferenceVE {
                vertex: n_v,
                edge: n_e,
                param: a_t,
                index_new: n_vx,
    ...
    pub(crate) fn perform_vf(&mut self) {
        ...
        for _ in 0..i_size {
            ...
            //   (type-specific indices 鈥?matches rcad's interference matrix)
            ...
            //    never registered in OCCT either 鈥?effectively always false)
            //
    ...
    pub(crate) fn perform_ef(&mut self, pairs: &[(usize, usize)]) {
            ...
            if a_nb_cprts == 0 {
                ...
                if md < f64::MAX && md > a_tol_e + a_tol_f {
                    ...
                    self.distances.entry((nE, nF)).or_default().push(
                        crate::bop::algo::pave_filler::EdgeRangeDistance {
                            first: t_range[0],
                            last: t_range[1],
                            distance: md,
                        },
                ...
                match a_cpart {
                    EfHit::Vertex { point, param: a_t } => {
                            ...
                            || (b_line_plane && (b_is_on_pave[0] || b_is_on_pave[1]))
                        {
                            ...
                            if bv0 && bv1 {
                                // OCCT L427-437: EDGE-type treatment 鈥?set CommonPart
                        ...
                        // OCCT L442-444: splittable 鈥?if PB not splittable, skip
        ...
        self.update_vertices_of_cb();
        // OCCT L578+L612+L687: PerformNewVertices 鈫?TreatNewVertices 鈫?IntersectVE
    ...
    pub(crate) fn build_box_tree_face(&self, is_a: bool) -> crate::bop::tools::bvh::BoxTree {
        use crate::bop::tools::bvh::{Aabb, BoxTree};
    ...
    pub(crate) fn build_box_tree_face_all(&self) -> crate::bop::tools::bvh::BoxTree {
        use crate::bop::tools::bvh::{Aabb, BoxTree};
    ...
    pub(crate) fn perform_vv(&mut self) {
        ...
        // L100-101: 2. Make blocks 鈥?BOPAlgo_Tools::MakeBlocks(aMILI, aMBlocks, aAllocator)
        ...
        // L129-131: aMBlocks.Clear(); aMILI.Clear() 鈥?handled by Rust Drop
    }
    ...
    pub(super) fn make_sd_vertices_vv(&mut self, block: &[usize]) {
        ...
        // L162: MakeVertex(aLV, aVn) 鈥?compute centroid + bounding tolerance.
        ...
        // L163-179: 2. Determine nV 鈥?either update existing SD or append new
        let n_v: usize;
        ...
        for i in 0..block.len() {
            ...
            for j in (i + 1)..block.len() {
                ...
                if i_r1 == self.ds.rank(n2) {
                    self.my_report
                        .add_alert(crate::bop::algo::Alert::SelfInterferingShape(n1, n2));
                }
                // OCCT L223: if (myDS->AddInterf(n1, n2)) 鈥?fence check
                let key = if n1 < n2 { (n1, n2) } else { (n2, n1) };
    ...
    pub(crate) fn intersect_ee(
        ...
        let sr1 = match crate::bop::int_tools::curve_range::shrunk_range(&e1_curve, range1, tol, tol, tol)
        {
            Some(sr) => sr,
            None => return,
        };
        let sr2 = match crate::bop::int_tools::curve_range::shrunk_range(&e2_curve, range2, tol, tol, tol)
        {
            Some(sr) => sr,
            None => return,
        };
        ...
        // OCCT L529-551: EDGE-type common parts 鈥?coincident edges create EE interferences
        if hits.is_empty() {
    ...
    pub(crate) fn treat_new_vertices(
        &mut self,
        a_mvcpb: &std::collections::HashMap<usize, crate::bop::ds::pave::CoupleOfPaveBlocks>,
    ) -> Vec<usize> {
        ...
        // = =  Phase 2: IntersectVertices 鈥?group nearby vertices into chains = = =
    ...
    pub(crate) fn perform_new_vertices(
        ...
        // Steps 2-4: Build edge鈫抳ertices map and call IntersectVE (OCCT L655-687)
        // OCCT builds aMPBLI (PB 鈫?vertices) then calls IntersectVE(aMPBLI, ..., false).
        // rcad: build edge鈫抳ertices map and call self.intersect_ve().
        //
        ...
        // it keeps its old index 鈫?IntersectVE skips it. Handle this case manually.
    ...
    pub(crate) fn is_point_in_face(&mut self, point: DVec3, face_idx: usize, tol: f64) -> bool {
        ...
        match &face.surface {
            Surface3::Plane(plane) => {
                ...
                crate::bop::int_tools::edge_face::point_in_planar_face_with_tol(point, plane, &verts, tol)
            }
            _ => {
    ...
    pub(crate) fn intersect_ef(&mut self, edge_idx: usize, face_idx: usize, pb_range: &[f64; 2]) {
        ...
        let hits: Vec<(DVec3, f64)> = match (&edge_curve, &face_surface) {
            (Curve3::Line(line), Surface3::Plane(plane)) => {
                crate::bop::int_tools::edge_face::intersect_line_plane_with_tol(line, ef_range, plane, etf)
                    .into_iter()
                    .map(|h| (h.point, h.edge_param))
                    .collect()
            }
            (Curve3::Line(line), Surface3::Cylinder(cyl)) => {
                crate::bop::int_tools::curve_surface::intersect_line_cylinder_with_tol(line, ef_range, cyl, etf)
                    .into_iter()
                    .map(|h| (h.point, h.curve_param))
                    .collect()
            }
            (Curve3::Line(line), Surface3::Sphere(sph)) => {
                crate::bop::int_tools::curve_surface::intersect_line_sphere_with_tol(line, ef_range, sph, etf)
                    .into_iter()
                    .map(|h| (h.point, h.curve_param))
                    .collect()
            }
            (Curve3::Line(line), Surface3::Cone(cone)) => {
                crate::bop::int_tools::curve_surface::intersect_line_cone_with_tol(line, ef_range, cone, etf)
                    .into_iter()
                    .map(|h| (h.point, h.curve_param))
                    .collect()
            }
            (Curve3::Circle(circle), Surface3::Plane(plane)) => {
                ...
                crate::bop::int_tools::curve_surface::intersect_circle_plane_with_ref(
                    circle,
                    ef_range,
                    plane,
                    etf,
            ...
            (Curve3::Circle(circle), Surface3::Cylinder(cyl)) => {
                crate::bop::int_tools::curve_surface::intersect_circle_cylinder_with_tol(
            ...
            (Curve3::Circle(circle), Surface3::Sphere(sph)) => {
                crate::bop::int_tools::curve_surface::intersect_circle_sphere_with_tol(
            ...
            (Curve3::Circle(circle), Surface3::Cone(cone)) => {
                crate::bop::int_tools::curve_surface::intersect_circle_cone_with_tol(circle, ef_range, cone, etf)
                    .into_iter()
                    .map(|h| (h.point, h.curve_param))
                    .collect()
            }
            (Curve3::Ellipse(ellipse), Surface3::Plane(plane)) => {
                ...
                crate::bop::int_tools::ellipse_intersection::intersect_ellipse_plane_with_tol(
            ...
            (Curve3::Ellipse(ellipse), Surface3::Cylinder(cyl)) => {
                ...
                crate::bop::int_tools::ellipse_intersection::intersect_ellipse_cylinder_with_tol(
            ...
            (Curve3::Ellipse(ellipse), Surface3::Sphere(sph)) => {
                crate::bop::int_tools::ellipse_intersection::intersect_ellipse_sphere_with_tol(
            ...
            (Curve3::Ellipse(ellipse), Surface3::Cone(cone)) => {
                crate::bop::int_tools::ellipse_intersection::intersect_ellipse_cone_with_tol(
            ...
            (Curve3::Parabola(parabola), Surface3::Plane(plane)) => {
                ...
                crate::bop::int_tools::parabola_intersection::intersect_parabola_plane_with_tol(
            ...
            (Curve3::Parabola(parabola), Surface3::Cylinder(cyl)) => {
                ...
                crate::bop::int_tools::parabola_intersection::intersect_parabola_cylinder_with_tol(
            ...
            (Curve3::Parabola(parabola), Surface3::Sphere(sph)) => {
                crate::bop::int_tools::parabola_intersection::intersect_parabola_sphere_with_tol(
            ...
            (Curve3::Parabola(parabola), Surface3::Cone(cone)) => {
                crate::bop::int_tools::parabola_intersection::intersect_parabola_cone_with_tol(
            ...
            (Curve3::Hyperbola(hyperbola), Surface3::Plane(plane)) => {
                ...
                crate::bop::int_tools::hyperbola_intersection::intersect_hyperbola_plane_with_tol(
            ...
            (Curve3::Hyperbola(hyperbola), Surface3::Cylinder(cyl)) => {
                //  ?Partially aligned: numeric fallback
                crate::bop::int_tools::hyperbola_intersection::intersect_hyperbola_cylinder_with_tol(
            ...
            (Curve3::Hyperbola(hyperbola), Surface3::Sphere(sph)) => {
                crate::bop::int_tools::hyperbola_intersection::intersect_hyperbola_sphere_with_tol(
            ...
            (Curve3::Hyperbola(hyperbola), Surface3::Cone(cone)) => {
                crate::bop::int_tools::hyperbola_intersection::intersect_hyperbola_cone_with_tol(
