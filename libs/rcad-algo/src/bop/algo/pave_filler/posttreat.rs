...
impl<'a> PaveFiller<'a> {
    ...
    pub(super) fn post_treat_ff(
        ...
        for inf in &self.ds.interf_ff {
            ...
            let mut a_mi = crate::bop::algo::pave_filler::build_face_shape_map(self.ds, n_f1);
            let a_mi_b = crate::bop::algo::pave_filler::build_face_shape_map(self.ds, n_f2);
            for v in &a_mi_b {
                a_mi.insert(*v);
            }
            // Collect stick vertices
            for inf_ve in &self.ds.interf_ve {
                if !a_mi.contains(&inf_ve.vertex) {
                    continue;
                }
                a_mv.insert(inf_ve.vertex);
            }
            for inf_vf in &self.ds.interf_vf {
                if !a_mi.contains(&inf_vf.vertex) {
                    continue;
                }
                a_mv.insert(inf_vf.vertex);
            }
            for inf_ee in &self.ds.interf_ee {
                if inf_ee.new_vertex == usize::MAX {
                    continue;
                }
                if !a_mi.contains(&inf_ee.e1) || !a_mi.contains(&inf_ee.e2) {
                    continue;
                }
                let n_v = self
                    .ds
                    .has_shape_sd(inf_ee.new_vertex)
                    .unwrap_or(inf_ee.new_vertex);
                a_mv.insert(n_v);
            }
            for inf_vv in &self.ds.interf_vv {
                if inf_vv.merged_vertex == usize::MAX {
                    continue;
                }
                if !a_mi.contains(&inf_vv.v1) || !a_mi.contains(&inf_vv.v2) {
                    continue;
                }
                let n_v = self
                    .ds
                    .has_shape_sd(inf_vv.merged_vertex)
                    .unwrap_or(inf_vv.merged_vertex);
                a_mv.insert(n_v);
            }
            for inf_ef in &self.ds.interf_ef {
                if inf_ef.new_vertex == usize::MAX {
                    continue;
                }
                if !a_mi.contains(&inf_ef.edge) || !a_mi.contains(&inf_ef.face) {
                    continue;
                }
                let n_v = self
                    .ds
                    .has_shape_sd(inf_ef.new_vertex)
                    .unwrap_or(inf_ef.new_vertex);
                a_mv.insert(n_v);
                a_mv_ef.insert(n_v);
            }
            ...
            // OCCT: IndMap fence 鈥?vertices appearing once 鈫?VertsUnused, twice 鈫?removed
            for &vi in &a_mv {
        ...
        if a_nb_s == 1 && a_nb_me == 0 && a_nb_v_on_rpb == 0 && verts_unused.is_empty() {
            ...
            if key < self.ds.edge_count() {
                ...
                if cur_ind < self.ds.interf_ff.len() {
                    let mut ffp = crate::bop::ds::types::FFPoint::new(
                        vp,
                        glam::DVec2::ZERO,
                        glam::DVec2::ZERO,
                    );
        ...
        // ===== OCCT L1387-1398: Nested PaveFiller 鈫?aPF.Perform() =====
        ...
        let sd_pairs: Vec<(usize, usize)> = a_pds
            .shape_sd
            .sd_vertices_iter()
            .map(|&(a, b)| {
                let main_a = if a < a_map_nested_to_main.len() {
                    a_map_nested_to_main[a]
                } else {
                    a
                };
                let main_b = if b < a_map_nested_to_main.len() {
                    a_map_nested_to_main[b]
                } else {
                    b
                };
                (main_a, main_b)
        ...
        for &(shape_idx, st) in &a_ls {
            // OCCT L1411-1418: flatten COMPOUND 鈥?not needed in rcad (no compounds in a_ls)
            ...
            if st == 0 {
                ...
                // rcad: check nested VV pass result 鈥?was this vertex fused?
    ...
    pub(super) fn put_bound_pave_on_curve(
        &mut self,
        n_f1: usize,
        n_f2: usize,
        ci: usize,
        ...
        if let Some(pb) = self.ds.intersection_curves[ci].pave_blocks.first() {
            ...
            if !r.ext_paves.is_empty() {
                let min_pave = r
                    .ext_paves
                    .iter()
                    .min_by(|a, b| a.param.partial_cmp(&b.param).unwrap());
                let max_pave = r
                    .ext_paves
                    .iter()
                    .max_by(|a, b| a.param.partial_cmp(&b.param).unwrap());
                if let Some(p) = min_pave {
                    a_bnd_nv[0] = p.vertex_idx;
                }
                if let Some(p) = max_pave {
                    a_bnd_nv[1] = p.vertex_idx;
                }
            }
        }
        ...
        for j in 0..2 {
            if a_bnd_nv[j] != usize::MAX {
                ...
                if n_v < self.ds.vertex_count() {
                    ...
                    let i_flag =
                        crate::bop::tools::compute_vv_p(v, a_p[j], a_tol_r3d + TOLERANCE_ABS);
                    if i_flag != 0 {
                        // bound vertex NOT at this endpoint 鈫?treat as no bound, create new one
            ...
            let b_vf = self
                .context
                .is_valid_point_for_faces(self.ds, a_p[j], n_f1, n_f2, a_tol_r3d);
            if !b_vf {
                continue;
            }
            ...
            if let Some(pb) = self.ds.intersection_curves[ci].pave_blocks.first_mut() {
                pb.0.write().unwrap().append_ext_pave(Pave {
                    vertex_idx: n_vn,
                    param: a_t[j],
                });
            }
    ...
    pub(super) fn process_existing_pave_blocks(
        ...
        // L3100-3101: face infos 鈥?queried inline to avoid borrow conflicts
        ...
        for &pb_idx in &candidates {
            ...
            if b_in_f1 && b_in_f2 {
                // L3113-3119: both faces 鈥?add for post treatment
            ...
            if b_in_f1 && b_in_f2 {
                for ci in 0..self.ds.intersection_curves.len() {
                    self.ds.intersection_curves[ci]
                        .pave_blocks
                        .push(self.ds.pave_blocks[pb_idx].clone());
                }
