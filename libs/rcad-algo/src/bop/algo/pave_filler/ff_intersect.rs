...
impl<'a> super::PaveFiller<'a> {
    pub(crate) fn perform_ff(&mut self) {
        ...
        // ====================================================================
        // L323-324: aFFs.SetIncrement(iSize) 閳?pre-allocate interference array
        ...
        // DataMap<BOPDS_Pair, NCollection_List<int>>  閳? HashMap<(usize,usize), Vec<usize>>
        ...
        for &(n_f1, n_f2) in &pairs {
            ...
            if !self.use_glue() {
                ...
                // L380-391: CheckPlanes 閳?skip parallel planes without vertex overlap
                        ...
                        for &e2 in self.ds.face_boundary_edges(n_f2) {
                            ...
                            for &vi in a_vertex_indices {
                                ...
                                if shift_dist > vtx_tol {
                                    ...
                                    // OCCT: gp_Vec(aP1, aP2) 閳?rcad: a_p2 - a_p1
        ...
        for work in &a_v_face_face {
            ...
            // OCCT L545-553: if !IsDone || HasErrors 閳?empty FF + warning + continue
            ...
            if ff_entry.curves.is_empty() && ff_entry.points.is_empty() && !ff_entry.tangent_faces {
                // OCCT L546-553: intersection failed 閳?record warning, skip
                // AddIntersectionFailedWarning (OCCT BOPAlgo_PaveFiller_2.cxx L660)
                // Rcad: warning skipped 閳?does not affect topology
                continue;
            }
            ...
            let retained_curves: Vec<usize> = self.ds.interf_ff[ff_idx]
                ...
                .filter_map(|&ci| {
                    ...
                    let bb = crate::bop::curve_bounds_with_range(
                        &ic.curve,
                        ic.t_range[0],
                        ic.t_range[1],
            ...
            // OCCT L610-620: points already processed 閳?store BOPDS_Point
    ...
    /// OCCT BOPAlgo_PaveFiller_6.cxx L106-134: IsClosedFF 閳?checks if edge is a seam edge of a face.
    ///
    ...
    pub(crate) fn intersect_face_face(&mut self, work: &FFWork) {
        ...
        // OCCT L498-504: GetEFPnts 閳?SetList passes EF points to IntPatch's PutPointsOnLine.
    ...
    pub(crate) fn make_intersection_curve(
        ...
        // OCCT L748-751: IntPatch_Restriction 閳?skip LineConstructor
        ...
        // If !IsDone 閳?return empty. If NbParts <= 0 閳?return empty.
        ...
        for part in parts.iter() {
            ...
            if b_finite {
                ...
                let in1 = uv1.map_or(false, |(uv, _, _)| {
                    ...
                    fc.perform(uv, true) != crate::bop::int_tools::fclass2d::State::Out
                });
                if !in1 {
                    continue;
                }
    ...
    /// - rcad: with 0 vertices, TreatCircle returns fallback full [0, 2闂佹寧绮庨?interval.
    // OCCT L904-1095: MakeCurve for Circle/Ellipse
    fn make_analytic_periodic_curve(
        ...
        if std::env::var("RCAD_DBG_MB").is_ok() {
            eprintln!(
                ...
                geom_tol, orig_t_range[0], orig_t_range[1]
            );
        }
        use rcad_kernel::geom::Curve2dEval;
        ...
        for &[fprm, lprm] in &parts {
            // OCCT L953-956: if (|fprm|>eps || |lprm-2闂佹寧绮庨惃?eps)  -- not full-period
            let is_full_period =
            ...
            if !is_full_period && (lprm > fprm + 1e-12) {
                ...
                // Full-period, single part 閳?accept full circle
                ...
                for j in 0..=17 {
                    ...
                    let in1 = uv1.map_or(false, |uv| {
                        self.context.is_point_in_on_face(self.ds, f1, uv)
                    });
                    let in2 = uv2.map_or(false, |uv| {
                        self.context.is_point_in_on_face(self.ds, f2, uv)
                    });
                    if in1 && in2 {
                        let pca = self.compute_pcurve_on_surface(curve, f1);
                        let pcb = self.compute_pcurve_on_surface(curve, f2);
    ...
    fn line_constructor_parts(
        ...
        let mut sorted: Vec<f64> = vertices
            .iter()
        ...
        let t1 = if deduped.is_empty()
            || (orig_t_range[1] - *deduped.last().unwrap()).abs() > a_tol_pc
        {
            orig_t_range[1]
        } else {
            *deduped.last().unwrap()
        ...
        all_breaks.extend(
            deduped
                .iter()
                .filter(|&&p| p > t0 + a_tol_pc && p < t1 - a_tol_pc),
        );
    ...
    /// For Circle/Ellipse with vertices: sorts vertices by parameter in [0, 2闂?,
    ...
    fn treat_circle_parts(
        ...
        use crate::bop::int_tools::int_patch_line::IntPatchVertex;
        use std::f64::consts::TAU;
        if std::env::var("RCAD_DBG_FF").is_ok() {
            eprintln!(
        ...
        if typl == IntPatchIType::Circle || typl == IntPatchIType::Ellipse {
            ...
            let a_tol_3d = crate::tolerance::TOLERANCE_ABS;
        ...
        if aNbVtx == 0 {
            // OCCT L3007-3016: GLine without vertices 閳?place vertex at parameter 0.
            // Return full-period [0, 2*PI] so the circle is accepted as one part.
            return vec![[0.0, TAU]];
        }

        // Build vertex array with parameters projected to [0, 2闂? (OCCT L492-495).
        ...
        // OCCT L504: create last vertex at first.param + 2闂?
        ...
        // Add the last vertex (first.param + 2闂?, skip if duplicate of last deduped.
