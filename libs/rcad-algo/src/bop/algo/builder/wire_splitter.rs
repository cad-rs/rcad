...
pub(crate) fn edge_uv_tangent(
    ...
    match surface {
        Surface3::Sphere(s) => {
            ...
            (
                Some(na),
                Some((na + std::f64::consts::PI) % std::f64::consts::TAU),
            )
        }
        Surface3::Plane(p) => {
            let x_axis = p.u_dir;
...
///  ?micro-step Angle2D for a 3D curve mapped to face UV.
...
pub(crate) fn edge_angle_2d(
    ...
    if range < TOLERANCE_CLAMP_MIN {
        return None;
    }
    // OCCT L790-793: Precision::IsInfinite(aTV) 鈫?return 0
    if !t.is_finite() {
        return Some(0.0);
    }
    ...
    let a_tx = 0.05 * range;
    let a_tx = if a_tx < 5e-5_f64 {
        (5e-5_f64).min(range * 0.5)
    } else {
        a_tx
    };
    if dt > a_tx {
    ...
    if dir.length_squared() < TOLERANCE_UV_DIR_SQ_MIN {
        return None;
    }
...
pub(crate) fn is_edge_isoline(pcurve: &Curve2d, range: [f64; 2]) -> (bool, bool) {
    ...
    // ====================================================================
    // Build BVH tree (Bnd_Box2d per edge from pcurve UV bounds)
    // ====================================================================

    // ====================================================================
    // For each WireFace, classify avoided segments via IsInside
    // ====================================================================

    // ====================================================================
    // select edges inside this face via 2D ray casting
    // ====================================================================

    // ====================================================================
    // BVH box prefilter (skip if box doesn't overlap face)
    // ====================================================================

    // ====================================================================
    // MakeInternalWires (per-face BFS assembly)
    // ====================================================================
...
pub(crate) fn world_to_uv(surface: &Surface3, pt: DVec3) -> Option<DVec2> {
    match surface {
        ...
        // numerical projection for BSpline/Bezier surfaces
...
pub(crate) fn build_vi_to_canon(segments: &[WireSegment], ds: &DS) -> Vec<usize> {
    ...
    for seg in segments.iter() {
        ...
        for &vi in &[seg.start_vertex, seg.end_vertex] {
            if vi_to_canon[vi] != usize::MAX {
                continue;
            }
            let pt = ds.vertex_point(vi);
            let found = canon_vertices.iter().position(|c| {
                c.distance_squared(pt) < TOLERANCE_ABS * TOLERANCE_ABS * 100_000_000.0
            });
            let canon = found.unwrap_or_else(|| {
                canon_vertices.push(pt);
                canon_vertices.len() - 1
            });
...
///  physical-edge identity for a WireSegment.
...
///  WireSplitter / PerformLoops (BOPAlgo_WireSplitter).
...
pub(crate) fn build_closed_wires(
...
) -> (Vec<Vec<usize>>, Vec<Vec<usize>>, HashMap<usize, DVec3>) {
    ...
    // canonicalize vertex indices so that different DS vertex
    ...
    //  vi_to_canon built skipping deg edges (build_vi_to_canon).
 ...
 if ds.edge_pave_blocks(*ei).len() > 1)
    ...
    // edge TShape dedup (BOPTools_AlgoTools.cxx L199-211).
    ...
    for (si, seg) in segments.iter().enumerate() {
        if avoided.contains(&si) {
            continue;
        } // OCCT: avoided edges not in WireSplitter input
        //  degenerate self-loop seam edges (sphere pole) appear twice in
        ...
        if seg.is_closed_on_face && seg.start_vertex == seg.end_vertex {
            continue;
        }
    ...
    // Build vertex egments adjacency using CANONICAL vertex indices
    let mut vert_to_segs: HashMap<usize, Vec<usize>> = HashMap::new();
    for (si, seg) in segments.iter().enumerate() {
        if avoided.contains(&si) {
            continue;
        } // OCCT: avoided edges get no adjacency (not in aWES)
        let sv = vi_to_canon
            .get(seg.start_vertex)
            .copied()
            .unwrap_or(seg.start_vertex);
        let ev = deg_end_canon.get(&si).copied().unwrap_or_else(|| {
            vi_to_canon
                .get(seg.end_vertex)
                .copied()
                .unwrap_or(seg.end_vertex)
        });
        vert_to_segs.entry(sv).or_default().push(si);
        vert_to_segs.entry(ev).or_default().push(si);
    }
    //  DoSplitSEAMOnFace equivalent  ?reroute seam_rev
    ...
    let mut merged_blocks: Vec<Vec<usize>> = Vec::new();
    {
        ...
        // Build vertex lock index map using RAW vertex indices
        let mut v_to_b: std::collections::HashMap<usize, Vec<usize>> =
    ...
    if std::env::var("RCAD_DEBUG_IC").is_ok() {
        eprintln!(
            "[BLK_TRACE] fi={} n_merged_blocks={} n_total_segments={}",
            face_idx,
            merged_blocks.len(),
            segments.len()
        );
        for (bi, b) in merged_blocks.iter().enumerate() {
            let seg_desc: Vec<String> = b
                .iter()
                .map(|&si| {
                    let seg = &segments[si];
                    match &seg.source {
    ...
    for (bi, block) in merged_blocks.iter().enumerate() {
        ...
        //  aMS add/remove cycle (WireSplitter_1.cxx L149-152).
 ...
 if ds.edge_pave_blocks(*ei).len() > 1));
        ...
        //  Build SmartMap (WireSplitter_1.cxx L154-220).
 ...
 if ds.edge_pave_blocks(*ei).len() > 1));
        ...
        //  regularity check (L222-280) from SmartMap IN/OUT.
        ...
        if is_regular {
            ...
            if let Some(wire) =
                build_regular_wire(block, segments, &vert_to_segs, &vi_to_canon, &deg_end_canon)
            {
                wires.push(wire);
            }
        } else {
            ...
            if std::env::var("RCAD_DEBUG_IC").is_ok() {
                eprintln!(
                    "[BLK_WIRES] fi={} bi={} n_wires_in_block={}",
                    face_idx,
                    bi,
                    wires
                        .iter()
                        .filter(|w| w.iter().any(|&si| block.contains(&si)))
                        .count()
                );
            }
        }
...
///  ?BOPTools_AlgoTools::MakeConnexityBlocks (L105-154).
...
///  ?SplitBlock (BOPAlgo_WireSplitter.cxx L292-358).
...
///  ?Regular block (degree=2) wire build.
pub(crate) fn build_regular_wire(
    ...
    loop {
        ...
        let next = vert_to_segs
            .get(&arrived_vertex)
            .and_then(|neighbors| {
                neighbors
                    .iter()
                    .find(|&&ni| !visited[ni] && block_set.contains(&ni))
            })
            .copied();
        ...
        match next {
            Some(ni) => {
                let seg = &segments[ni];
                ci = ni;
                arrived_vertex = if cs(seg) == arrived_vertex {
                    ce(seg)
                } else {
                    cs(seg)
                };
...
/// EdgeInfo  (BOPAlgo_WireSplitter.lxx L22-69)
#[derive(Debug, Clone)]
...
/// source DS edge (or intersection curve).  TopoDS_Edge identity.
pub(crate) type Pid = (u8, usize, usize, usize);

/// Build PID egment and PID ndpoint maps from WireSegments.
...
pub(crate) fn build_pid_maps(
    ...
    for (si, seg) in segments.iter().enumerate() {
        ...
        pid_endpoints
            .entry(pid)
            .or_insert_with(|| (canon(seg.start_vertex), canon(seg.end_vertex)));
    }
...
/// AND the pid egment map for the caller to expand PIDs to segment indices.
pub(crate) fn perform_shapes_to_avoid(
    ...
    loop {
        ...
        for (&pid, &(a, b)) in &pid_endpoints {
            if avoided_pids.contains(&pid) {
                continue;
            }
            if a != usize::MAX {
                anc.entry(a).or_default().push(pid);
            }
            if b != usize::MAX {
                anc.entry(b).or_default().push(pid);
            }
        }
...
// ====================================================================
// Assemble internal wires from avoided segments
// (BOPAlgo_BuilderFace.cxx L327-382)
// ====================================================================
///  PerformInternalShapes (BOPAlgo_BuilderFace.cxx L327-382).
///  BuilderFace::PerformInternalShapes (L618-735).
...
pub(crate) fn assemble_internal_wires(
    ...
    let seg_uv_box: Vec<Option<[f64; 4]>> = avoided
        .iter()
        .map(|&si| {
            let seg = &segments[si];
            seg.first_pcurve.as_ref().map(|pc| {
                let [t0, t1] = seg.t_range;
                let n_pts = 8usize; // OCCT IntTools_FClass2d uses NbSamples ( ?)
    ...
    for (fi, wf) in wfs.iter().enumerate() {
        ...
        let outer_uv: Vec<DVec2> = wf
            ...
            .filter_map(|&si| {
                ...
                seg.first_pcurve
                    .as_ref()
                    .map(|pc| pc.point_at(seg.t_range[0]))
            })
            .collect();
        if outer_uv.len() < 3 {
            continue;
        ...
        let hole_uvs: Vec<Vec<DVec2>> = wf
            ...
            .filter(|poly: &Vec<DVec2>| poly.len() >= 3)
            .collect();
        ...
        for (ai, &si) in avoided.iter().enumerate() {
            let Some(box_e) = &seg_uv_box[ai] else {
                continue;
            };
            // Skip if edge box doesn't overlap face
            if outer_uv
                .iter()
                .all(|p| p.x < box_e[0] || p.x > box_e[1] || p.y < box_e[2] || p.y > box_e[3])
            {
                continue;
            }
            let uv_mid = DVec2::new(0.5 * (box_e[0] + box_e[1]), 0.5 * (box_e[2] + box_e[3]));
            if !point_in_polygon_2d(&outer_uv, uv_mid) {
                continue;
            }
            let in_hole = hole_uvs
                .iter()
                .any(|hole| point_in_polygon_2d(hole, uv_mid));
            if in_hole {
                continue;
            }
...
pub(crate) fn is_same_block_fwd_rev(a: &WireSegment, b: &WireSegment) -> bool {
    match (&a.source, &b.source) {
        (WireEdgeSource::DsEdge(ea), WireEdgeSource::DsEdge(eb)) => {
            ea == eb && a.start_vertex == b.end_vertex && a.end_vertex == b.start_vertex
        }
        //  IntersectionCurve FWD+REV share curve index
        ...
        //  SeamEdge FWD+REV (same seam, opposite directions).
...
pub(crate) fn mark_edge_passed_both_dirs(
    ...
    let Some(infos) = smart_map.get_mut(&vertex) else {
        return;
    };
...
pub(crate) fn physical_edge_id_topo_ds(
    seg: &super::types::WireSegmentTopoDS,
) -> (u8, usize, usize, usize) {
    let (tag, idx) = match &seg.source {
...
pub(crate) fn build_pid_maps_topo_ds(
    ...
    for (si, seg) in segments.iter().enumerate() {
        ...
        pid_endpoints
            .entry(pid)
            .or_insert_with(|| (seg.start_vertex.index, seg.end_vertex.index));
    }
    (pid_segs, pid_endpoints)
}

///  PerformShapesToAvoid for WireSegmentTopoDS.
...
///           if (aV1x.IsSame(aV2x)) continue;    // self-loop 鈫?skip
///           myShapesToAvoid.Add(aE1);
