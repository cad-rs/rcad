...
use crate::bop::algo::builder::wire_splitter::{EdgeInfo, build_closed_wires};
...
impl<'a> BooleanBuilder<'a> {
    /// 鉁?OCCT-aligned: BuildRC (BOPAlgo_BOP.cxx L597-881).
    pub(super) fn build_rc(&self, result: &mut ResultBuilder, t_brep: &mut topods::BRep) {
        ...
        //   rcad: OCCT re-iterates the compound by dimension (SOLID鈫扴HELL鈫扚ACE) with
        ...
        // Result edges are embedded in pre-assembled solids 鈥?no standalone DEs.

        result.tmp_solids = kept_solids;

        // Rebuild t_brep from DS faces in keep_set (OCCT L800-823: myRC).
        if !keep_set.is_empty() {
            let mut kept_refs: Vec<topods::Shape> = Vec::new();
    ...
    pub(super) fn detect_internal_voids(
        ...
        let hole_bvh = crate::bop::tools::bvh::BoxTree::build(hole_key, hole_aabbs);
        ...
        // Build reverse map: solid 鈫?list of holes
    ...
    /// 鉁?OCCT-aligned: FillInternalShapes (BOPAlgo_Builder_3.cxx L622-887).
    ...
    pub(super) fn fill_internal_shapes(&self, result: &mut ResultBuilder) {
        ...
        for &si in &a_lsd {
            ...
            while i < a_lsi.len() {
                ...
                // Shape is IN 鈥?add as INTERNAL to the solid
    ...
    /// 鉁?OCCT-aligned: FillImagesCompounds (BOPAlgo_Builder_1.cxx L197-216 + L280-342).
    ...
    ///     1. L290-293: Fence 鈥?theMFP.Add(theS) 鈫?skip if processed.
    ///     2. L295-312: Interference check 鈥?sub-shapes with myImages 鈫?bInterferred.
    ///        Sub-compounds 鈫?recurse.
    ...
    /// 鉁?OCCT-aligned: FillImagesCompounds (BOPAlgo_Builder_1.cxx L197-216) + FillImagesCompound (L280-342).
    pub(super) fn fill_images_compounds(&self, result: &mut ResultBuilder) {
        let mut t = self.my_shape.borrow_mut();
        // OCCT: NCollection_Map<TopoDS_Shape, TopTools_ShapeMapHasher> aMFP 鈥?fence
        ...
        for df in &self.ds.faces {
            if let Some(csi) = df.source_compsolid_idx {
                // OCCT L290-293: theMFP.Add(theS) 鈥?fence
                if a_mfp.insert(csi) {
        ...
        for &csi in &compound_groups {
            // OCCT L295-308: interference check 鈥?myImages.IsBound(aSx) for any sub-shape
            ...
            for &ssi in &sub_solid_indices {
                // OCCT: myImages.IsBound(aSx) 鈥?check if sub-shape has split images
                let side = self
                    .ds
                    .faces
                    .iter()
                    ...
                    .map(|f| match f.origin {
                        crate::bop::ds::ShapeOrigin::ShapeA => 0,
                        crate::bop::ds::ShapeOrigin::ShapeB => 1,
                    })
                    .unwrap_or(0);
            ...
            // OCCT L309-312: no interference 鈫?skip
            if !b_interferred {
                continue;
            }
            ...
            for &ssi in &sub_solid_indices {
                let side = self
                    ...
                    .unwrap_or(0);
                // OCCT L321: aOrX = aSX.Orientation() 鈥?get source orientation
            ...
            // OCCT L330: aBB.Add(aCIm, aSXIm) 鈥?add shapes to compound
            // OCCT L339-341: myImages.Bind(theS, aLSIm) 鈥?bind to myImages
            if !a_c_im.is_empty() {
                let solid_refs: Vec<topods::Shape> = a_c_im
                    .iter()
                    .filter_map(|&si| self.my_solids.borrow().get(si).cloned())
                    .collect();
                if !solid_refs.is_empty() {
                    let cmp_ref = t.add_tcompound(solid_refs);
                    // rcad: synthetic key (OCCT: original source shape via theS)
                    let ckey = topods::Shape::synthetic(usize::MAX - a_c_im.len(), topods::Orientation::Forward);
                    self.my_images
                        .borrow_mut()
                        .entry(ckey)
                        .or_default()
    ...
    ///   ON鈫扞N/OUT based on face orientation + solid side, not surface type.
    ...
    /// 鉁?OCCT-aligned: BOPAlgo_Builder::BuildResult (BOPAlgo_Builder_1.cxx L130-168).
    ///   For each argument (myArguments):
    ///     Path A 鈥?argument's ShapeType == theType: process directly.
    ///     Path B 鈥?TopExp_Explorer traverses sub-shapes of theType:
    ///       if myImages.IsBound(sub) 鈫?fence-add image shapes
    ///       else 鈫?fence-add the original sub-shape.
    ...
    pub(super) fn build_result(
        ...
        for (arg_idx, a_s) in args.iter().enumerate() {
            if a_s.shape_type() == the_type {
                ...
                for (fi, df) in self.ds.faces.iter().enumerate() {
                    ...
                    if !has_images {
                        // OCCT BuildResult: no images 鈫?add original shape as-is.
            ...
            // Path B for other types (e.g. Edge inside Solid) 鈥?no sub-shape
        ...
        if the_type == rcad_kernel::topods::ShapeType::Solid {
            ...
            for shell_faces in &solids {
                let mut wire_refs: Vec<topods::Shape> = Vec::new();
                for &fi in shell_faces {
                    if let Some(face_sr) = mfr.get(fi).cloned() {
                        if face_sr.is_null() {
                            continue;
                        }
                        let face_tshape = &t.tshapes[face_sr.index];
                        if let topods::TShape::Face(fd) = &**face_tshape {
                            wire_refs.push(fd.outer_wire.clone());
                        }
                    }
                }
            ...
            for solid_sr in self.my_solids.borrow().iter() {
                if a_m_fence.insert(solid_sr.index) {
                    self.add_to_result(solid_sr.clone(), the_type, result, &mut *t);
                }
            }
        }
    }

    /// add a Shape to result (equivalent to BRep_Builder.Add(myShape, aS)).
                ...
                if wi < self.ds.shape_info.iter().filter(|si| si.shape_type == rcad_kernel::topods::ShapeType::Wire && !si.is_new).count() {
                    ...
                    if let Some(imgs) = self.my_images.borrow().get(&w_ref) {
                        for img_sr in imgs {
                            ...
                            if nSpR < e_map.len()
                                && e_map[nSpR] != rcad_kernel::topods::Shape::null()
                            {
                                wire_edges.push(e_map[nSpR].clone());
                            }
                        }
                    } else {
                        for &sub in &self.ds.shape_info[wi].sub_shapes {
                            ...
                            if sub >= self.ds.a_vertex_count() {
                                ...
                                if ei < e_map.len() && e_map[ei] != rcad_kernel::topods::Shape::null()
                                {
                                    wire_edges.push(e_map[ei].clone());
                                }
                            }
                        }
                    }
                    ...
                    if !wire_edges.is_empty() {
                        ...
                        t.wire_mut(sr.clone()).flags |= rcad_kernel::topods::tshape_flags::CLOSED;
                        self.my_wire_refs.borrow_mut().push(sr);
                    } else {
                        self.my_wire_refs
                            .borrow_mut()
                            .push(rcad_kernel::topods::Shape::null());
                    }
                }
            }
            rcad_kernel::topods::ShapeType::Face => {
                // OCCT BuildResult: BRep_Builder().Add(myShape, aS) 鈥?adds original
    ...
    /// 鉁?OCCT-aligned: BuildBOP (BOPAlgo_BOP.cxx L885-926, state-based); ref: BOPAlgo_Builder.cxx L490-895.
    pub(super) fn build_bop(&self, result: &mut ResultBuilder, t_brep: &mut topods::BRep) {
        ...
        if !has_objects && !has_tools {
            self.my_report
                .borrow_mut()
                .add_alert(crate::bop::algo::Alert::TooFewArguments);
            return;
        }
        // State validation
        eprintln!(
        ...
        let mut faces_with_ori: [Vec<topods::Shape>; 2] = [Vec::new(), Vec::new()];
            ...
            for &src_si in &src_solids[side] {
                ...
                for &fi in &solid_faces {
                    let face_sr = my_face_refs
                        .get(fi)
                        .cloned()
                        .unwrap_or(topods::Shape::synthetic(f_base + fi, topods::Orientation::Forward));
                    if face_sr.is_null() {
                        continue;
                    }
                        ...
                        if let Some(imgs) = my_images.get(&face_sr) {
                            for img_sr in imgs {
                                ...
                                let need_reverse = if img_sr.index < t_brep.tshapes.len() {
                                    ...
                                    let split_normal = self.shape_ref_face_normal(img_sr.clone(), t_brep);
                                    split_normal.map_or(false, |sn| {
                                        crate::bop::tools::is_split_to_reverse(orig_normal, sn)
                                    })
                                } else {
                                    false
                                };
                                if need_reverse {
                                    let mut rev = img_sr.clone();
                                    rev.orientation = topods::Orientation::Reversed;
                                    faces_with_ori[side].push(rev);
                                } else {
                                    faces_with_ori[side].push(img_sr.clone());
                                }
                                faces_fence[side].insert(img_sr.ptr_id());
                            }
                        }
                    } else {
                        faces_with_ori[side].push(face_sr.clone());
                ...
                if let Some(in_face_indices) = my_in_parts.get(&src_si) {
                    for &in_fi in in_face_indices {
                        let in_sr = my_face_refs
                            .get(in_fi)
                            .cloned()
                            .unwrap_or(topods::Shape::synthetic(f_base + in_fi, topods::Orientation::Forward));
                        if !in_sr.is_null() {
        ...
        let mut a_m_res_faces_ori: Vec<topods::Shape> = Vec::new();
                    ...
                    if !faces_fence[opposite_side].contains(&f_id) {
                        ...
                        if is_same_ori_needed == is_same_ori {
                            if a_m_res_faces_fence.insert(f_id) {
                                a_m_res_faces_ori.push(f_sr.clone());
                            }
                        } else {
                            a_m_f_to_avoid.insert(f_id);
                        }
        ...
        let mut a_res_faces: Vec<topods::Shape> = Vec::new();
        for f_sr in &a_m_res_faces_ori {
            if !a_m_f_to_avoid.contains(&f_sr.ptr_id()) {
                a_res_faces.push(f_sr.clone());
            }
        }
        ...
        let mut a_bs = crate::bop::algo::builder_solid::BuilderSolid::new();
        a_bs.set_shapes(&bs_faces);
        a_bs.perform(&self.ds);
        // Validate each resulting solid has 鈮? face from objects/tools
        let mut a_res_solids: Vec<topods::Shape> = Vec::new();
        if !self.ds.solid_shape_indices().is_empty() || true {
            ...
            for area_faces in a_areas {
                ...
                let mut area_face_refs: Vec<topods::Shape> = Vec::new();
        ...
        // Collect unused faces 鈫?solids
        {
            ...
            let mut a_unused_faces: Vec<topods::Shape> = Vec::new();
            for f_sr in &a_res_faces {
                if !a_unused_fence.contains(&f_sr.ptr_id()) {
                    a_unused_faces.push(f_sr.clone());
                }
            }
        ...
        for sr in &a_res_solids {
            self.my_solids.borrow_mut().push(sr.clone());
        }
    }
    ...
    /// Lookup the normal of a Shape that refers to a Face TShape in the BRep.
    ...
    /// 鉁?OCCT-aligned: CheckArgsForOpenSolid (BOPAlgo_BOP.cxx L1396-1470+).
