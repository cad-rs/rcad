...
use crate::bop::algo::builder::types::*;
use crate::bop::algo::builder::wire_splitter::{EdgeInfo, build_closed_wires, world_to_uv};
...
use crate::bop::int_tools::edge_face::plane_local_basis;
use crate::tolerance::*;
...
impl<'a> BooleanBuilder<'a> {
    ...
    /// 閴?OCCT-aligned: FillImagesVertices (BOPAlgo_Builder_1.cxx L40-67).
    /// Maps each SD vertex pair as myImages[source]閳妭target], myShapesSD, myOrigins.
    pub(super) fn fill_images_vertices(&self) {
        for &(va, vb) in self.ds.shape_sd.sd_vertices_iter() {
            ...
            self.my_images
                .borrow_mut()
                .entry(self.brep_sr(src))
                .or_default()
                .push(self.brep_sr(sd));
            self.my_shapes_sd
                .borrow_mut()
                .insert(self.brep_sr(src), self.brep_sr(sd));
            self.my_origins
                .borrow_mut()
                .entry(self.brep_sr(sd))
                .or_default()
                .push(self.brep_sr(src));
        }
    }

    /// 閴?OCCT-aligned: FillImagesEdges (BOPAlgo_Builder_1.cxx L71-126).
    /// Maps source edges 閳?split images via pave-block new_edge.
    ...
    /// 閴?OCCT-aligned: FillImagesContainer(WIRE) (BOPAlgo_Builder_1.cxx L221-276).
    pub(super) fn fill_images_container_wire(&self, _result: &ResultBuilder) {
        ...
        for i_src in 0..nb_src {
            ...
            if si.shape_type != topods::ShapeType::Wire {
                continue;
            }
            ...
            // OCCT L235-240: no modified sub-shapes 閳?skip (original wire is used as-is)
            if !modified {
                continue;
            }
    ...
    /// 閴?OCCT-aligned: FillImagesFaces (BOPAlgo_Builder_2.cxx L215-229).
    ...
    pub(super) fn fill_images_faces(&self) {
        let mut result = crate::bop::algo::builder::result_builder::ResultBuilder::new();
    ...
    fn populate_source_shapes_in_t_brep(&self, t: &mut topods::BRep) {
        ...
        for fi in 0..self.ds.face_count() {
            ...
            t.ensure_face_at(
                flat_idx,
                Some(df.surface.clone()),
                outer_wire_sr,
                inner_wire_refs,
                sample_point,
                None,
                vec![],
                df.natural_restriction,
            );
        }
    }

    /// 閴?OCCT-aligned: PostTreat (BOPAlgo_Builder.cxx L456-486).
    pub(super) fn post_treat(&mut self) {
            ...
            for (ti, tol, st) in updates {
                if st == rcad_kernel::topods::ShapeType::Edge {
                    ...
                    if let rcad_kernel::topods::TShape::Vertex(vd) = &*t.tshapes[ti].clone() {
                        t.tshapes[ti] = std::sync::Arc::new(rcad_kernel::topods::TShape::Vertex(
        ...
        //   OCCT BOPTools_AlgoTools_1.cxx L389-423 + L1005-1055.
        {
            ...
            // Phase 1: Edge 閳?Vertex 閳?if vertex tolerance < edge tolerance, update vertex
            for (ti, ts) in t.tshapes.iter().enumerate() {
            ...
            // Phase 2: Face 閳?Edge 閳?if edge tolerance < face tolerance, update edge
            for (ti, ts) in t.tshapes.iter().enumerate() {
            ...
            if !updates.is_empty() {
                ...
                for (sr, tol, st) in updates {
                    ...
                    if st == rcad_kernel::topods::ShapeType::Edge {
                        ...
                        if let rcad_kernel::topods::TShape::Vertex(vd) = &*t.tshapes[ti].clone() {
                            t.tshapes[ti] =
                                std::sync::Arc::new(rcad_kernel::topods::TShape::Vertex(
                                    rcad_kernel::topods::TVertexData {
                                        tolerance: tol,
                                        ..vd.clone()
                                    },
    ...
    /// 閴?OCCT-aligned: BuildSplitFaces (BOPAlgo_Builder_2.cxx L233-555).
    ///   Iterates source faces 閳?splits each along intersection curves.
    ///   For faces with IN/SC PBs: full BuilderFace::Perform.
    ///   For ON-only faces: BuildDraftFace.
    ///   Faces with no interferences 閳?skipped (no images).
    pub(super) fn build_split_faces(&self, result: &mut ResultBuilder, t: &mut topods::BRep) {
        let a_nb_s = self.ds.nb_source_shapes;
        ...
        let mut a_vbf: Vec<crate::bop::algo::builder::BuilderFace> = Vec::new();
        ...
        for i in 0..a_nb_s {
            ...
            if si.shape_type != rcad_kernel::topods::ShapeType::Face {
                continue;
            }
            let fi = face_counter;
            face_counter += 1;
            ...
            let a_nb_av = {
                ...
                for &pb_idx in fi_info
                    .pave_blocks_in
                    .iter()
                    .chain(fi_info.pave_blocks_sc.iter())
                {
            ...
            if !has_pb_in && !has_pb_sc {
                ...
                if a_nb_av == 0 {
                    ...
                    // OCCT L330-333: no internals and no modified 閳?skip this face.
                ...
                if !has_internals {
                    if let Some(draft) = self.build_draft_face(fi) {
                        ...
                        continue; // OCCT L347: draft face stored 閳?skip BuilderFace.
                    }
                    // OCCT L349: BuildDraftFace returned null 閳?fall through to BuilderFace.
                }
                // OCCT L351 (implicit): has_internals 閳?fall through to BuilderFace.
            }
            ...
            // OCCT L353: aMFence 閳?fence for SEAM edge dedup.
                                ...
                                for e_sr in &wd.edges {
                                                ...
                                                .map_or(false, |rep| rep.pcurve2.is_some())
                                        {
                                            let (is_ui, is_vi) =
                                                self.ds.edge_on_face(ei, fi).map_or((false, false), |rep|
                                                    crate::bop::algo::builder::wire_splitter::is_edge_isoline(
                                    ...
                                    if let Some(imgs) = my_images.get(&e_sr) {
                                        for sp_sr in imgs {
                                            ...
                                            if b_is_closed {
                                                if a_m_fence_local.insert(a_sp.ptr_id()) {
                                                    ...
                                                    if sp_ei < self.ds.edge_count() {
                                                        ...
                                                        if !sp_is_closed {
                                                            crate::bop::tools::do_split_seam_on_face(
                                                                sp_ei, fi, self.ds,
                                                            );
                                                        }
                                                    }
                                            ...
                                            {
                                                let needs_rev = crate::bop::algo::builder::edge_builders::is_split_to_reverse(
                                                    self.ds, sp_ei, ei);
                                                if needs_rev {
            ...
            for &pb_idx in &self.ds.face_info(fi).pave_blocks_in {
                if pb_idx < self.ds.pave_blocks.len() {
                    let pb_ei = self.ds.pave_blocks[pb_idx]
                        .0
                        .read()
                        .unwrap()
                        .new_edge
            ...
            // OCCT L496-500: BuildPCurveForEdgesOnPlane 閳?speed up for planar faces.
                ...
                if matches!(
                    ...
                    for e_sr in &a_le {
                        ...
                        if let Some(pc) = crate::geom2d_api::project_curve_to_plane(
                            ...
                            // OCCT pattern: clone TEdgeData 閳?insert 閳?replace Arc.
                            if let topods::TShape::Edge(ed) = &*t.tshapes[e_sr.index].clone() {
                                let mut new_ed = ed.clone();
                                new_ed
                                    .pcurves
                                    .insert(face_sr.index, (pc, ed.range[0], ed.range[1]));
                                t.tshapes[e_sr.index] =
                                    std::sync::Arc::new(topods::TShape::Edge(new_ed));
                            }
                        }
                    }
                }
            }
            let mut bf = crate::bop::algo::builder::BuilderFace::new(
                self.ds,
                &brep_owned,
                &face_refs_owned,
                &ic_edge_map_owned,
        ...
        for (src_face_sr, a_lfr) in &a_faces_im {
            ...
            let an_ori_f = face_refs_owned
                .get(src_face_sr.index)
    ...
    /// 閴?OCCT-aligned: FillInternalVertices (BOPAlgo_Builder_2.cxx L929-1008).
    pub(super) fn fill_internal_vertices(&self, result: &mut ResultBuilder) {
        ...
        for (ds_fi, ds_face) in self.ds.faces.iter().enumerate() {
            ...
            let image_rfis: Vec<usize> = result
                .face_origins
                .iter()
                .enumerate()
            ...
            for &vi in &alone {
                ...
                for &rfi in &image_rfis {
                    ...
                    if let Some(uv) = world_to_uv(fs, v_pt) {
                        let fclass =
                            crate::bop::int_tools::fclass2d::FClass2d::new(self.ds, cfi, class_tol);
                        if fclass.perform(uv, true) == crate::bop::int_tools::fclass2d::State::In {
    ...
    /// 閴?OCCT-aligned: FillSameDomainFaces (BOPAlgo_Builder_2.cxx L580-925).
    pub(super) fn fill_same_domain_faces(&self, result: &mut ResultBuilder) {
        ...
        // is in the FF-interfering set 閳?matches OCCT's iteration scope)
        ...
        impl BOPToolsSet {
            fn from_result_face(result: &ResultBuilder, fi: usize) -> Self {
                ...
                // OCCT L149-159: TopAbs_INTERNAL 閳?add FORWARD + REVERSED
        ...
        for &fi in &result_fi_filtered {
            ...
            let dsfi = match &result.face_origins[fi] {
                FaceOrigin::FromA(sfi) => self
                    .ds
                    .faces
                    .iter()
                    .position(|f| f.origin == ShapeOrigin::ShapeA && f.source_face_idx == *sfi),
                FaceOrigin::FromB(sfi) => self
                    .ds
                    .faces
                    .iter()
                    .position(|f| f.origin == ShapeOrigin::ShapeB && f.source_face_idx == *sfi),
                _ => None,
            };
            if dsfi
                .and_then(|dsfi| self.ds.shape_info.get(dsfi))
        ...
        let faces_are_sd = |rfi: usize, rfj: usize| -> bool {
            ...
            let (Some(di), Some(dj)) = (ds_i, ds_j) else {
                return false;
            };
            let a_tol = face_tol_with_edges(di)
                + face_tol_with_edges(dj)
                + self.my_fuzzy_value.max(TOLERANCE_ABS);
            let (uv_j, proj_j) = crate::bop::closest_point_on_surface(surf_j, pt_i);
            let dist_j = proj_j.distance(pt_i);
            dist_j <= a_tol
                && self
                    .context
                    .borrow_mut()
                    .is_point_in_on_face(self.ds, dj, uv_j)
        };
    ...
    /// 閴?OCCT-aligned: FillImagesContainers (BOPAlgo_Builder_1.cxx L172-193).
    ...
    /// 閴?OCCT-aligned: FillImagesContainer(SHELL) (BOPAlgo_Builder_1.cxx L221-276).
    pub(super) fn fill_images_container_shell(
        &self,
        result: &mut ResultBuilder,
        t: &mut topods::BRep,
    ) {
        let mut pending: Vec<(topods::Shape, Vec<topods::Shape>)> = Vec::new();
        // Pre-build DS face 閳?result face Shape map for fast lookup
        ...
        for i_src in 0..nb_src {
            let si = &self.ds.shape_info[i_src];
            if si.shape_type != topods::ShapeType::Shell {
                continue;
            }
            ...
            // OCCT L235-240: no modification 閳?skip
            if !modified {
                continue;
            }
    ...
    /// 閴?OCCT-aligned: FillImagesContainer(COMPSOLID) (BOPAlgo_Builder_1.cxx L221-276).
    pub(super) fn fill_images_container_compsolid(
        &self,
        result: &mut ResultBuilder,
        t: &mut topods::BRep,
    ) {
                ...
                for &dsfi in ds_shell {
                    ...
                    for (rfi, fo) in result.face_origins.iter().enumerate() {
                        let matches = match (fo, self.ds.face_origin(dsfi)) {
                            (FaceOrigin::FromA(s), ShapeOrigin::ShapeA) => {
                                *s == self.ds.source_face_idx(dsfi)
                            }
                            (FaceOrigin::FromB(s), ShapeOrigin::ShapeB) => {
                                *s == self.ds.source_face_idx(dsfi)
                            }
                            _ => false,
                        };
                                        ...
                                        if let Some(ts) = t.tshapes.get(ssr.index) {
                                            if let topods::TShape::Solid(sd) = &**ts {
                                                if sd.shells.iter().any(|sh_sr| {
                                                    t.tshapes.get(sh_sr.index).map_or(
                                                        false,
                                                        |tsh| {
                                                            if let topods::TShape::Shell(shd) =
                                                                &**tsh
                                                            {
    ...
    /// 閴?OCCT-aligned: FillImagesSolids (BOPAlgo_Builder_3.cxx L60-93).
    ///   OCCT BOPAlgo_Builder_3.cxx L60-93: FillIn3DParts 閳?BuildSplitSolids 閳?FillInternalShapes.
    ...
    /// 閴?OCCT-aligned: BuildDraftSolid (BOPAlgo_Builder_3.cxx L267-368).
    /// Builds a draft solid from a source solid, replacing split faces with their images.
    pub(super) fn build_draft_solid(
        &self,
        ...
        for (_shi, ds_shell) in self.ds.collect_source_shells().iter() {
            let belongs = ds_shell.iter().any(|&dsfi| {
                self.ds
                    .faces
                    .get(dsfi)
                    .map_or(false, |f| f.origin == origin_side)
            });
            if !belongs {
                continue;
            }
    ...
    /// 閴?OCCT-aligned: FillIn3DParts (BOPAlgo_Builder_3.cxx L97-263).
    pub(super) fn fill_in_3d_parts(
        &self,
        result: &mut ResultBuilder,
        ...
        for dsfi in 0..self.ds.face_count() {
            ...
            if let Some(split_faces) = imgs.get(&src_key) {
                ...
                for &vi in &df.boundary_verts {
                    if let Some(v) = self.ds.vertices.get(vi) {
                        c += v.point;
                        n += 1;
                    }
                }
        ...
        let aabb_of_solid: Vec<Aabb> = a_l_solids
            .iter()
            .map(|shells| {
                let mut aabb = Aabb::empty();
                for sh in shells {
            ...
            .collect();
        let an_in_parts_list = crate::bop::algo::classify_faces(
            &a_l_faces,
            &face_samples,
            &a_l_solids,
            self.ds,
                    ...
                    if let Some(fe) = face_entries.get(ei) {
                        if fe.data != 0 {
                            ...
                            let same_solid = (dsi == 0
                                && *fo.unwrap_or(&ShapeOrigin::ShapeA) == ShapeOrigin::ShapeA)
                                || (dsi == 1
    ...
    /// 閴?OCCT-aligned: BuildSplitSolids (BOPAlgo_Builder_3.cxx L413-618).
    /// Build result solids from draft solids and IN faces.
    pub(super) fn build_split_solids(
        &self,
        result: &mut ResultBuilder,
        ...
        // Helper: result face index 閳?DS face index
        let result_to_ds = |rfi: usize, expected_origin: ShapeOrigin| -> Option<usize> {
            ...
            self.ds
                .faces
                .iter()
                .position(|f| f.origin == expected_origin && f.source_face_idx == *sfi)
        };
        ...
        let ds_to_result = |dfi: usize| -> Option<usize> {
            ...
            result
                .face_origins
                .iter()
                .position(|fo| match (dsf.origin, fo) {
        ...
        for &(si, side, _state) in assignments {
            ...
            if let Some((_shi, ds_shell)) = self.ds.collect_source_shells().get(si) {
                ...
                // OCCT L451-461: non-interfered solid 閳?build from source DS faces.
        ...
        // === Phase 1a: Interfered solids 閳?build result solid in T BRep ===
            ...
            if let Some((_shi, ds_shell)) = self.ds.collect_source_shells().get(si) {
                for &dsfi in ds_shell {
                    if !self
                        ...
                        .faces
                        .get(dsfi)
                        .map_or(false, |f| f.origin == origin)
                    {
                        continue;
                    }
                    let df = self.ds.faces.get(dsfi).unwrap();
        ...
        // OCCT L520-531: BuilderSolid parallel execution 閳?rcad bypasses, builds solids directly.
        // OCCT L539-577: collect BuilderSolid areas into aSolidsIm 閳?rcad Phase 1a stored in tmp_shells.
