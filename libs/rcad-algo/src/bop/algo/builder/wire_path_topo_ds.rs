...
// ---------------------------------------------------------------------------
// TopoDS-native EdgeInfo 鈥?no seg_idx, holds edge+face directly (OCCT aligned)
// ---------------------------------------------------------------------------
...
/// 鉁?OCCT-aligned: BOPAlgo_WireSplitter::Path (WireSplitter_1.cxx L358-626).
...
pub(crate) fn walk_path_extract_wires(
    ...
    loop {
        ...
        // OCCT L607-610: single inside way 鈫?forced
        if a_nb_ways_inside == 1 {
            p_edge_info = p_only_way_in;
        }

        // OCCT L612-616: no way out 鈫?return
...
fn refine_angles(
    ...
    for &v in &vertices {
        ...
        // IsInside: edge whose src_key is NOT in a_ms (appeared odd times) 鈫?interior edge
        {
        ...
        for ei in &infos {
            ...
            let b_refined = match pc {
                Some(pc) => {
                    ...
                    // Check if angle is near boundary 鈥?true refine via micro-step
                    let eps = 1e-12;
...
/// TopoDS-based SplitBlock 鈥?refine angles + path walk for irregular blocks.
pub(crate) fn split_block(
    ...
    // (rcad: skip bNothingToDo optimization 鈥?always run Path extraction)

    // src_key_of: maps segment 鈫?physical edge key (same as in build_smart_map)
...
/// 鉁?OCCT-aligned: BOPAlgo_BuilderFace::PerformLoops (BOPAlgo_BuilderFace.cxx L239-383).
/// TopoDS-based build_closed_wires 鈥?SmartMap + angle computation + wire walking.
///
...
/// 鉁?OCCT-aligned: BOPAlgo_WireSplitter::SplitBlock (WireSplitter_1.cxx L136-195, L298-318).
...
fn build_smart_map(
    ...
    // OCCT L134: NCollection_Map<TopoDS_Shape> aMS 鈥?edge parity tracking
    ...
    for &si in block {
        ...
        //   First vertex (i=0): FORWARD 鈫?bIsIN=false
        //   Last vertex (i=1): REVERSED 鈫?bIsIN=true
        // For REVERSED orientation of the edge in the face wire, the vertex
        // orientations are swapped by TopExp_Explorer:
        //   First vertex: REVERSED 鈫?bIsIN=true
        //   Last vertex: FORWARD 鈫?bIsIN=false
        ...
        // First vertex (i=0): orientation matches edge orientation in face
        {
            ...
            // OCCT TopExp: FORWARD edge 鈫?first vertex FORWARD(OUT), last REVERSED(IN)
            //              REVERSED edge 鈫?first vertex REVERSED(IN), last FORWARD(OUT)
            // rcad swaps start/end for REVERSED, so:
            //   REVERSED start = TShape last vertex 鈫?FORWARD 鈫?OUT
            //   REVERSED end = TShape first vertex 鈫?REVERSED 鈫?IN
            //   FORWARD start = TShape first vertex 鈫?FORWARD 鈫?OUT
            //   FORWARD end = TShape last vertex 鈫?REVERSED 鈫?IN
...
fn build_regular_wire(block: &[usize], segments: &[WireSegmentTopoDS]) -> Option<Vec<usize>> {
    ...
    if block.len() == 1 {
        // Single edge 鈫?self-loop
        return Some(vec![block[0]]);
    }

    // Build vertex 鈫?segment adjacency
    let mut vert_to_segs: HashMap<usize, Vec<usize>> = HashMap::new();
    for &si in block {
        let seg = &segments[si];
        vert_to_segs
    ...
    // Walk from the first segment's start vertex, following end鈫抯tart chain.
...
/// 鉁?OCCT-aligned: BOPTools_AlgoTools::MakeConnexityBlocks (BOPTools_AlgoTools.cxx L105-154).
...
/// TopoDS-based perform_areas 鈥?classifies wires as growth/hole.
///
/// 鉁?OCCT-aligned: BOPAlgo_BuilderFace::PerformAreas (BOPAlgo_BuilderFace.cxx L387-614).
...
/// OCCT uses BVH tree + IsInside for hole鈫抔rowth assignment; rcad uses UV box
/// prefilter + point_in_uv_polygon. Algorithm steps (classify 鈫?match 鈫?build)
...
pub(crate) fn perform_areas(
    ...
    for w in wires {
        ...
        let b_is_growth = if b_is_growth {
            ...
            area >= 0.0 // IsHole() = area < 0 鈫?growth = !IsHole = area >= 0
        };
    ...
    // OCCT: aHoleFaces.Extent() != aHoleFaceMap.Extent() 鈫?orphan holes exist
    // OCCT: aBoxF.IsOpen* 鈫?original face is unbounded (e.g. plane)
    // rcad: !natural_restriction 鈫?face is unbounded/open (e.g. plane, not sphere)
    ...
    //       unassigned 鈥?they were not enclosed by any growth.  Same as rcad.
    result
}
...
/// 鉁?OCCT-aligned: BOPAlgo_BuilderFace::PerformInternalShapes (BOPAlgo_BuilderFace.cxx L618-778).
...
pub(crate) fn perform_internal_shapes(
    ...
    // OCCT L670-671: aMEDone 鈥?fence to track classified internal edge groups.
...
/// 鉁?OCCT-aligned: BOPAlgo_BuilderFace::PerformLoops L327-382: group connected avoided edges into internal wires.
