use crate::bop::algo::builder::ds_as_brep::DSAsBRep;
use crate::bop::algo::builder::types::*;
use crate::bop::algo::builder::{FaceOrigin, WireEdgeSource};
use crate::bop::ds::DS;
...
pub(crate) struct BuilderFace<'a> {
    ...
    /// myAreas 鈥?resulting split-face TShape refs.
    my_areas: Vec<Shape>,
    /// Internal ResultBuilder for TShape creation during perform().
    my_result: crate::bop::algo::builder::result_builder::ResultBuilder,
}
...
impl<'a> BuilderFace<'a> {
    pub fn new(
        ...
        Self {
            ...
            shapes: None,
            my_areas: Vec::new(),
            my_result: crate::bop::algo::builder::result_builder::ResultBuilder::new(),
        }
    }
    ...
    pub(crate) fn perform(&mut self, t: &mut topods::BRep) {
        ...
        let segments_topo = crate::bop::algo::builder::builder_utils_topo_ds::segments_to_topo_ds(
            &segments,
            self.ds,
            self.face_idx,
            self.face_refs,
        ...
        let (avoided_pids, pid_segs) =
            crate::bop::algo::builder::wire_splitter::perform_shapes_to_avoid_topo(
                &segments_topo,
                tool,
            );
        let mut avoided =
            crate::bop::algo::builder::wire_splitter::expand_avoided_pids(&avoided_pids, &pid_segs);

        // OCCT L239-385: PerformLoops (via BOPAlgo_WireSplitter)
        let wires = crate::bop::algo::builder::wire_path_topo_ds::build_closed_wires(
            &segments_topo,
            &avoided,
            tool,
        );

        // OCCT L330-385: Post-treatment 鈥?build myLoopsInternal from myShapesToAvoid
        ...
        let internal_wire_groups = crate::bop::algo::builder::wire_path_topo_ds::build_internal_wires(
            &segments_topo,
            &avoided,
        );
        ...
        let wfs = if !wires.is_empty() {
            crate::bop::algo::builder::wire_path_topo_ds::perform_areas(
                &wires,
        ...
        crate::bop::algo::builder::wire_path_topo_ds::perform_internal_shapes(
            &mut wfs,
            &internal_wire_groups,
            &segments_topo,
            tool,
    ...
    /// 鉁?OCCT-aligned: Convert myShapes (Shape list from aLE) to WireSegments.
