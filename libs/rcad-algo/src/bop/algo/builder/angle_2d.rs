...
pub fn curve_geom_type(curve: &Curve2d) -> CurveGeomType {
    match curve {
        Curve2d::Line(_) => CurveGeomType::Line,
        Curve2d::Circle(_) => CurveGeomType::Circle,
        Curve2d::Ellipse(_) => CurveGeomType::Ellipse,
        Curve2d::Parabola(_) => CurveGeomType::Parabola,
        Curve2d::Hyperbola(_) => CurveGeomType::Hyperbola,
        _ => CurveGeomType::Other,
    }
}
...
pub fn angle_2d(
    ...
    if typ != CurveGeomType::Line {
        let d1 = curve2d_d1(curve, t);
        let d2 = curve2d_d2(curve, t);
        if d1.length_squared() > 1e-14 {
            let r = curve2d_lprop_curvature(d1, d2, 1e-14);
            if r > TOLERANCE_ABS {
                let r_curv = 1.0 / r;
                let cosphi = r_curv / (r_curv + a_tol_2d);
                if cosphi < 1.0 {
    ...
    let a_tx = if max_dt < 5e-5 {
        (5e-5_f64).min(range / 2.0)
    } else {
        max_dt
    };
    if dt > a_tx {
    ...
    let dir = if b_is_in {
        curve.point_at(t) - curve.point_at(t1)
    } else {
        curve.point_at(t1) - curve.point_at(t)
    };
...
pub fn clock_wise_angle(angle_in: f64, angle_out: f64) -> f64 {
    ...
    let ai = if angle_in >= TAU {
        angle_in - TAU
    } else {
        angle_in
    };
    let ao = if angle_out >= TAU {
        angle_out - TAU
    } else {
        angle_out
    };
