...
macro_rules! dbg_seg {
    ($label:expr, $face_idx:expr, $seg:expr) => {
        if $crate::bop::algo::builder::debug_utils::dbg_enabled_for($face_idx) {
            let __src = match &$seg.source {
                $crate::bop::algo::builder::WireEdgeSource::DsEdge(ei) => format!("DS{}", ei),
                $crate::bop::algo::builder::WireEdgeSource::IntersectionCurve(ci) => {
                    format!("IC{}", ci)
                }
                $crate::bop::algo::builder::WireEdgeSource::SeamEdge => "Seam".into(),
            };
            eprintln!(
                "[DBG] {} fi={} {} v{}->{} fwd={}",
...
macro_rules! dbg_result_verts {
    ($label:expr, $result:expr) => {{
        let __uniq: std::collections::BTreeSet<usize> =
            $result.edges.iter().flat_map(|e| [e.0, e.1]).collect();
        eprintln!(
            "[DBG] {} V={} E={}",
            $label,
            __uniq.len(),
    ...
    ($label:expr, $face_idx:expr, $wires:expr, $segments:expr) => {
        if $crate::bop::algo::builder::debug_utils::dbg_enabled_for($face_idx) {
            for (__wi, __w) in $wires.iter().enumerate() {
                let __srcs: Vec<String> = __w
                    .iter()
                    .map(|&__si| match &$segments[__si].source {
                        $crate::bop::algo::builder::WireEdgeSource::DsEdge(ei) => format!("DS{}", ei),
                        $crate::bop::algo::builder::WireEdgeSource::IntersectionCurve(ci) => {
                            format!("IC{}", ci)
                        }
