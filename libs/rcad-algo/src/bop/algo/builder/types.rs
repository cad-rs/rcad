...
pub enum BooleanError {
    ...
    InvalidOperation,
    /// AlertTooFewArguments 闂?fewer than 2 arguments.
    TooFewArguments,
    /// AlertNoFiller 闂?PaveFiller not initialized.
    NoFiller,
    /// AlertBOPNotAllowed 闂?non-licit operation for the arguments.
    BOPNotAllowed,
    /// AlertBOPNotSet 闂?operation type not set.
    BOPNotSet,
    /// AlertEmptyShape 闂?one argument is empty.
    EmptyShape,
    EmptyInput,
...
/// (FaceSampleData removed 闁?OCCT FClass2d + WireFace used instead)

/// DEPRECATED: OCCT = ?  split_face + emit  闂侀€炲苯鍘撮柛? ?
///  闂?=  FaceSampleData (classify)  ?WireFace (emit) ?
/// wire grouping result 闂?ordered segment chains forming a face boundary.
#[derive(Clone)]
pub struct WireFace {
...
/// 闂乧ollected sub-face result before classification.
...
pub(crate) struct WireSegment {
    ...
    /// 闂乿ertex parameters on the pcurve (BRep_Tool::Parameter,
...
/// 闂乮ntermediate result of the LOW-D phase (V+E+W creation)
