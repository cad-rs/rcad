...
fn make_restriction_curves(
    ...
    // OCCT GeomInt_IntSS::TreatRLine (GeomInt_IntSS_1.cxx L1139-1159): the 3D
    // curve is an approximation of the 2D curve ON ITS SURFACE —
    ...
    let Some(other_pcurve) = other_pcurve else {
        return out;
    };
    ...
    for an_ind in 0..params.len().saturating_sub(1) {
        ...
        out.push(IntersectionCurve {
            ...
            pcurve1,
            pcurve2,
            tolerance: tol_reached.max(CONFUSION),
            tang_tolerance: 0.0,
            pave_blocks: Vec::new(),
            bbox: None,
        });
