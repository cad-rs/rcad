...
//! OCCT dispatches Hyperbola 脳 {Cylinder, Cone, Sphere} through generic
//! numeric EF intersection (IntTools_EdgeFace), same as rcad.
//!
//! Hyperbola 脳 Plane: analytic solve via cosh/sinh 鈫?quadratic in e^t.
//! Hyperbola 脳 {Cylinder, Cone, Sphere}: numeric marching fallback,
...
/// The hyperbola is P(t) = center + a路cosh(t)路u + b路sinh(t)路v.
///
/// Substituting into n路(X - origin) = 0:
///
///   C + A路cosh(t) + B路sinh(t) = 0
///
/// where:
///   C = n路(center - origin)
///   A = a路(n路u)   (semi-major axis contribution)
///   B = b路(n路v)   (semi-minor axis contribution)
///
/// In exponential form (cosh(t) = (e^t+e^{-t})/2, sinh(t) = (e^t-e^{-t})/2):
///
///   2C + A路(e^t+e^{-t}) + B路(e^t-e^{-t}) = 0
///   2C + (A+B)路e^t + (A-B)路e^{-t} = 0
///
/// Multiply by e^t:
///   (A+B)路e^{2t} + 2C路e^t + (A-B) = 0
///
/// Let u = e^t > 0:
///   (A+B)路u虏 + 2C路u + (A-B) = 0
///
...
pub fn intersect_hyperbola_plane(
    ...
    intersect_hyperbola_plane_with_tol(hyperbola, t_range, plane, rcad_kernel::rcad_kernel::CONFUSION)
}
...
pub fn intersect_hyperbola_plane_with_tol(
    ...
    if coeff_a.abs() < rcad_kernel::rcad_kernel::CONFUSION && coeff_b.abs() < rcad_kernel::rcad_kernel::CONFUSION {
        // Hyperbola plane is parallel to the intersection plane
        if c_term.abs() < rcad_kernel::rcad_kernel::CONFUSION {
            // Hyperbola lies in the plane 鈥?infinite intersection, return nothing
    ...
    // Solve (A+B)路u虏 + 2C路u + (A-B) = 0, where u = e^t > 0
    ...
    if p_quad.abs() < rcad_kernel::rcad_kernel::CONFUSION * rcad_kernel::rcad_kernel::CONFUSION {
        // (A+B) 鈮?0: linear in u = e^t: 2C路u + (A-B) = 0
        if q_quad.abs() < rcad_kernel::rcad_kernel::CONFUSION * rcad_kernel::rcad_kernel::CONFUSION {
...
/// this is extremely rare in boolean operations 鈥?the numeric marching
...
pub fn intersect_hyperbola_cylinder(
    ...
    intersect_hyperbola_cylinder_with_tol(hyperbola, t_range, cyl, rcad_kernel::rcad_kernel::CONFUSION)
}
...
pub fn intersect_hyperbola_sphere(
    ...
    intersect_hyperbola_sphere_with_tol(hyperbola, t_range, sph, rcad_kernel::rcad_kernel::CONFUSION)
}
...
pub fn intersect_hyperbola_cone(
    ...
    intersect_hyperbola_cone_with_tol(hyperbola, t_range, cone, rcad_kernel::rcad_kernel::CONFUSION)
}
