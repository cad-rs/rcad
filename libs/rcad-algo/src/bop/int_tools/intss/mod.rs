...
//! are still empty. Seeds are tightened with [`project_onto_intersection_tol`](crate::bop::int_tools::marching::project_onto_intersection_tol).
//!
...
use rcad_kernel::geom::{
    BSplineCurve3, Circle3, ConicalSurface, Curve2d, Curve3, CurveEval, CylindricalSurface,
    Ellipse3, Hyperbola3, Line3, Parabola3, Plane, SphericalSurface, Surface3, SurfaceEval,
};
...
use crate::bop::int_tools::{
    ...
    cylinder_cylinder::{
        CylinderCylinderResult, intersect_cylinder_cylinder_with_tolerance,
        sample_perpendicular_offset_curves,
    },
...
pub fn intersect_surfaces_with_tolerance(
    ...
    match (s1, s2) {
        ...
        (Surface3::Cone(k1), Surface3::Cone(k2)) => cone_x_cone_with_tolerance(k1, k2, fuzzy_tol),
        (Surface3::Sphere(s), Surface3::Cylinder(c))
        | (Surface3::Cylinder(c), Surface3::Sphere(s)) => {
            sphere_x_cylinder_with_tolerance(s, c, fuzzy_tol)
        }
        (Surface3::Sphere(s), Surface3::Cone(k)) | (Surface3::Cone(k), Surface3::Sphere(s)) => {
            sphere_x_cone_with_tolerance(s, k, fuzzy_tol)
        }
        (Surface3::Torus(t), Surface3::Cone(k)) | (Surface3::Cone(k), Surface3::Torus(t)) => {
...
fn cylinder_x_cylinder(
    ...
    use crate::bop::int_tools::cylinder_cylinder::intersect_cylinder_cylinder;
        ...
        CylinderCylinderResult::TwoEllipses(e1, e2) => {
            for e in [e1, e2] {
                let pca = fallback_pcurve_by_projection(
                    &Curve3::Ellipse(e),
                    &[0.0, TAU],
                    &Surface3::Cylinder(*c1),
                );
                let pcb = fallback_pcurve_by_projection(
                    &Curve3::Ellipse(e),
                    &[0.0, TAU],
                    &Surface3::Cylinder(*c2),
                );
                out.curves.push(SurfaceIntersectionResult {
                    curve_3d: SurfaceCurve::Ellipse(e),
                    pcurve_on_a: Some(pca),
...
fn cylinder_x_cone(cyl: &CylindricalSurface, cone: &ConicalSurface) -> SurfaceSurfaceIntersection {
    use std::f64::consts::TAU;
    let mut out = SurfaceSurfaceIntersection::default();
...
/// General case  ?numerical marching.
fn cone_x_cone(k1: &ConicalSurface, k2: &ConicalSurface) -> SurfaceSurfaceIntersection {
...
fn torus_x_plane(
    ...
    numeric_intss(&Surface3::Torus(*torus), &Surface3::Plane(*plane))
}
...
fn torus_x_plane_perp(
    ...
    if r1 > TOLERANCE_ABS {
        ...
        let pcurve_b = crate::bop::int_tools::pcurve_derive::circle_pcurve_on_plane(
            &rcad_kernel::geom::Circle3::new(center_proj, circle_normal, r1),
            plane,
        );
        out.curves.push(SurfaceIntersectionResult {
            curve_3d: SurfaceCurve::Circle(rcad_kernel::geom::Circle3::new(
                center_proj,
                circle_normal,
                r1,
            )),
...
fn torus_x_sphere(
    ...
    let st_result = super::sphere_torus::intersect_skew_sphere_torus(sphere, torus);
...
fn torus_x_sphere_on_axis(
    ...
    // Find intersection of two circles in the ( , z) half-plane:
    // Torus tube circle: (  - R)  + (z - 0)  = r (tube center at (R, 0))
    // Sphere cross-section: + (z - z_s)  = R_s (sphere center at (0, z_s))
    //
    // We need to find ( , z) where both are satisfied, with > 0.
    // From sphere: = R_s  - (z - z_s)
    ...
    for i in 0..=n {
        ...
        if !prev_f.is_nan() && prev_f * f < 0.0 {
            ...
            for _ in 0..64 {
                ...
                let flo = ((R_s * R_s - (lo - z_s).powi(2)).max(0.0).sqrt() - R).powi(2) + lo * lo
                    - r * r;
                if fm * flo < 0.0 {
                    hi = mid;
                } else {
...
fn torus_x_cylinder_coaxial(
    ...
    // In the ( , z) plane: torus tube is circle of radius r at (R, 0).
    // Cylinder is vertical line at = r_cyl.
...
fn dedup_points_spatial(pts: Vec<DVec3>, tol: f64) -> Vec<DVec3> {
    ...
    for p in pts {
        if !kept.iter().any(|q| (*q - p).length_squared() <= tol_sq) {
            kept.push(p);
        }
    }
    kept
