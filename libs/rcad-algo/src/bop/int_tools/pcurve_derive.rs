...
use rcad_kernel::geom::{
    ...
    Ellipse2d, Ellipse3, Hyperbola3, Line2d, Line3, Parabola3, Plane, SphericalSurface, Surface3,
    SurfaceEval, any_perpendicular,
};
...
/// direction, and radii (unchanged 鈥?projection along a parallel normal
...
/// ProjLib_Sphere::Project(gp_Lin) 鈥?ProjLib_Sphere.cxx L181-184.
/// OCCT implementation is a stub: sets myType = GeomAbs_OtherCurve and relies on
/// the generic ProjLib_Projector::BuildResult (sampling 鈫?BSpline fit) for the
...
/// ProjLib_Sphere::Project(gp_Elips) 鈥?ProjLib_Sphere.cxx L186-189.
...
/// ProjLib_Sphere::Project(gp_Parab) 鈥?stub 鈫?generic.
...
/// ProjLib_Sphere::Project(gp_Hypr) 鈥?stub 鈫?generic.
...
/// ProjLib_Sphere::Project(gp_Circ) 鈥? pcurve.
///
/// OCCT ProJLib_Sphere_1.cxx L97-179 handles isoparametric circles analytically
/// (isIsoU/isIsoV 鈫?Line2d), wrapping the result in Geom2d_TrimmedCurve at the caller.
    ...
    match interpolate_points_2d(&pts) {
        Ok(mut bspline) => {
            if std::env::var("RCAD_DEBUG_PCURVE").is_ok() {
                eprintln!(
                    "[DBG_PCURVE] sphere pcurve: {} pts, BSpline, t_range=[{:.4},{:.4}]",
                    n_samp,
                    bspline.knots.first().unwrap_or(&0.0),
                    bspline.knots.last().unwrap_or(&0.0)
                );
            }
...
pub fn circle_pcurve_on_cylinder(circle: &Circle3, cyl: &CylindricalSurface) -> Curve2d {
    ...
    if (normal_dot - 1.0).abs() < TOLERANCE_MESH_LEGACY {
        ...
        let dir_sgn = if circle.normal.dot(axis) > 0.0 {
            1.0
        } else {
            -1.0
        };
...
/// Compute the azimuth 胃 of a line's origin on a cylindrical surface, mapped
/// to [0, 2蟺).  This is the u-coordinate the line's pcurve would have.
///
/// A line parallel to the cylinder axis at angular position 胃 returns 胃 in [0, 2蟺).
...
///
/// A line parallel to the cylinder axis at azimuth 胃 returns a vertical
/// [`Line2d`] at u = 胃 in (胃, h) space.
...
///
/// Samples the ellipse at 33 evenly-spaced parameter values over [0, 2蟺],
...
pub fn ellipse_pcurve_on_cylinder(ellipse: &Ellipse3, cyl: &CylindricalSurface) -> Curve2d {
    ...
    let mut pts: Vec<DVec2> = (0..n)
        .map(|i| {
            ...
            let radial = p3 - cyl.origin - axis * v;
            let mut u = radial.dot(v_axis).atan2(radial.dot(u_axis));
            // Map to [0, 2蟺] to match cylinder UV convention (same as
    ...
    // Unwrap seam discontinuities: samples near the 0/2蟺 seam may have a
...
fn sampled_curve_pcurve_on_cone(
    ...
    let mut bspline = match interpolate_points_2d(&pts) {
        ...
        Err(_) => {
            ...
            if span > 0.0 {
                return Curve2d::Line(Line2d {
                    origin: DVec2::new(a.x * span + t_range[0], a.y),
                    direction: DVec2::new(dir.x * span, dir.y),
                });
            }
            return line;
        }
...
pub fn circle_pcurve_on_cone(circle: &Circle3, cone: &ConicalSurface) -> Curve2d {
    ...
    sampled_curve_pcurve_on_cone(
        &rcad_kernel::geom::Curve3::Circle(*circle),
...
pub fn line_pcurve_on_cone(line: &Line3, cone: &ConicalSurface) -> Curve2d {
    ...
    sampled_curve_pcurve_on_cone(
        &rcad_kernel::geom::Curve3::Line(*line),
        &[-10.0, 10.0],
        cone,
    )
}
...
pub fn ellipse_pcurve_on_cone(ellipse: &Ellipse3, cone: &ConicalSurface) -> Curve2d {
    sampled_curve_pcurve_on_cone(
        &rcad_kernel::geom::Curve3::Ellipse(*ellipse),
        &[0.0, std::f64::consts::TAU],
        cone,
    )
}
...
pub fn fallback_pcurve_by_projection(
    ...
    // Unwrap seam discontinuities: u values are in [0, 2蟺] after the mapping,
    // but consecutive samples may still jump by ~2蟺 when the ellipse crosses
    ...
    for i in 1..pts.len() {
        let du = pts[i].x - pts[i - 1].x;
        if du > std::f64::consts::PI {
            // Jumped from near -蟺 back up to near +蟺: pull remaining down.
            for p in &mut pts[i..] {
                p.x -= std::f64::consts::TAU;
            }
        } else if du < -std::f64::consts::PI {
            // Jumped from near +蟺 down to near -蟺: push remaining up.
            for p in &mut pts[i..] {
                p.x += std::f64::consts::TAU;
            }
        }
    ...
    match interpolate_points_2d(&pts) {
        Ok(bspline) => {
            // OCCT : Geom2d_TrimmedCurve 鈥?wrap in TrimmedCurve2 to
...
pub fn polyline_pcurve_by_projection(polyline: &[DVec3], surface: &Surface3) -> Option<Curve2d> {
    ...
    // Phase 1: Standard seam unwrap using 蟺-threshold.
    if std::env::var("RCAD_DEBUG_PC").is_ok() && matches!(surface, Surface3::Cylinder(_)) {
        eprintln!(
            "[PC_DEBUG] Phase1 start: pts[0].x={:.6} pts[last].x={:.6} n={}",
            pts[0].x,
            pts[pts.len() - 1].x,
            pts.len()
        );
    ...
    if std::env::var("RCAD_DEBUG_PC").is_ok() && matches!(surface, Surface3::Cylinder(_)) {
        eprintln!(
            "[PC_DEBUG] Phase1 end: pts[0].x={:.6} pts[last].x={:.6}",
            pts[0].x,
            pts[pts.len() - 1].x
        );
        if pts.len() >= 5 {
            eprintln!(
                "[PC_DEBUG] Phase1 sample: [{:.6}, {:.6}, {:.6}, {:.6}, {:.6}]",
    ...
    // segments concatenated (e.g. PerpendicularOffsetCurves with a near-蟺 gap),
    // the standard 蟺-threshold unwrap may fold the sequence 鈥?u goes up then back
    // instead of continuing in one direction.
    //
    // Detection: 1 sign change in consecutive du values (e.g. all-negative 鈫?    // all-positive), with span < 2蟺 and near-zero net delta.
    //
    // NOTE: On Cylinder surfaces, this V-fold is geometrically correct 鈥?the
    // analytic atan2 formula gives identical values to Newton projection, and the
    // intersection curve genuinely wraps from u=0 to u=蟺 and back. The fix for
    ...
    if pts.len() >= 3 && matches!(surface, Surface3::Cylinder(_)) {
        ...
        if abs_span < std::f64::consts::TAU * 0.9 && net_delta.abs() < abs_span * 0.5 {
            ...
            // A V-shape fold has exactly one sign change (e.g. all-negative 鈫?all-positive).
            // A boundary jump between valid segments has two sign changes (e.g.
            // positive 鈫?single-negative-jump 鈫?positive).
            ...
            eprintln!(
                "[PCURVE_SIGNS] n_pts={} abs_span={:.6} net_delta={:.6} sign_changes={} fold_idx={}",
                pts.len(),
                abs_span,
                net_delta,
                sign_changes,
            ...
            if sign_changes == 1 && fold_idx >= 2 && fold_idx + 2 <= pts.len() {
                eprintln!(
                    "[PCURVE_FOLD] V-fold: span={:.6} net={:.6} fold_idx={} n={} u_first={:.4} u_fold={:.4} u_last={:.4}",
                    abs_span,
                    net_delta,
                    fold_idx,
                    pts.len(),
                ...
                if let Surface3::Cylinder(cyl) = surface {
                    // Analytic u recomputation is NOT effective here 鈥?the V-fold
                    ...
                    // concatenates two ~蟺 segments with a near-蟺 gap, and Phase 1
                    ...
                    let _ = cyl;
                    eprintln!("[PCURVE_FOLD] V-fold on Cylinder 鈥?diagnostic only, no fix applied");
                }
                // Note: for Cone/Torus surfaces we do not apply a fold fix
                // here.  Empirical testing shows the V-shape fold only occurs
                // on cylinder surfaces from Newton's 2蟺-periodic branch issue.
            }
        }
    }
...
///
/// `t_range` is the effective parameter range to sample 鈥?for most curves
...
// 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
// IsCurveValid 鈥?pcurve self-intersection check
// 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
// 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
// ComputeTolReached3d 鈥?deviation between 3D curve and pcurve
// 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
// 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
// PrepareLines3D 鈥?closed鈥慶urve splitting + redundant鈥憀ine filtering
// 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
/// properly trimmed as a single segment 鈥?OCCT splits it into complementary
...
pub fn prepare_lines_3d(curves: &mut Vec<crate::bop::ds::IntersectionCurve>, b_to_split: bool) {
    // OCCT L1936-1937: bToSplit parameter (OCCT default true).

    // 鈹€鈹€ Phase 1: split closed curves 鈹€鈹€
    // OCCT L1938-1963: for each curve, SplitCurve(aIC) 鈫?append result.
    let mut new_curves: Vec<crate::bop::ds::IntersectionCurve> = Vec::with_capacity(curves.len());
    for ic in curves.drain(..) {
        ...
        if b_to_split {
            if let Some([r0, r1]) = split_closed_curve(&ic.curve, &tr) {
                ...
                let first_half = crate::bop::ds::IntersectionCurve {
                    t_range: r0,
                    pcurve_on_a: ic
                        .pcurve_on_a
                        .as_ref()
                        .map(|pc| trim_curve2d(pc, r0[0], r0[1])),
                    pcurve_on_b: ic
                        .pcurve_on_b
                        .as_ref()
                        .map(|pc| trim_curve2d(pc, r0[0], r0[1])),
                ...
                let second_half = crate::bop::ds::IntersectionCurve {
                    t_range: r1,
                    pcurve_on_a: ic
                        .pcurve_on_a
                        .as_ref()
                        .map(|pc| trim_curve2d(pc, r1[0], r1[1])),
                    pcurve_on_b: ic
                        .pcurve_on_b
                        .as_ref()
                        .map(|pc| trim_curve2d(pc, r1[0], r1[1])),
    ...
    // 鈹€鈹€ Phase 2: Plane/Cone redundant-line rejection 鈹€鈹€
    ...
    // rcad: not yet implemented 鈥?this case only applies when Plane and Cone
...
fn trim_curve2d(curve: &Curve2d, lo: f64, hi: f64) -> Curve2d {
    match curve {
        // Analytical curve2ds evaluate as pure functions of t 鈥?no internal
...
mod tests {
    ...
    fn split_closed_line_none() {
        let l = Curve3::Line(Line3 {
            origin: DVec3::Z,
            direction: DVec3::X,
        });
        assert!(
            split_closed_curve(&l, &[-10.0, 10.0]).is_none(),
            "line not closed"
        );
    }
    ...
    fn split_closed_circle_midpoint() {
        ...
        let mid = result[0][1];
        assert!(
            (mid - std::f64::consts::PI).abs() < 1e-12,
            "mid should be pi"
        );
    }
