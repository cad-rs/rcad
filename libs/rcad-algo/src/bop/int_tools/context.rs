...
pub struct Context {
    ...
    tol_uv: f64,
    pub num_faces: usize,
    /// OCCT: ProjPS 閳?point-on-surface projector cache (latest result per face).
    proj_ps_latest: Vec<Option<SurfaceProjection>>,
    /// OCCT: ProjPC 閳?point-on-curve projector cache (latest result per edge).
    proj_pc_latest: Vec<Option<CurveProjection>>,
    /// OCCT: ProjPT 閳?single reusable point-on-curve projector for transient curves.
    proj_pt_latest: Option<CurveProjection>,
    /// OCCT: UVBounds cache 閳?precomputed UV bounds per face.
    uv_bounds_cache: Vec<Option<[f64; 4]>>,
    /// OCCT: SurfaceAdaptor 閳?precomputed surface references.
    surface_cache: Vec<Option<Surface3>>,
    /// OCCT: mySClassMap (IntTools_Context: solid i 閳?BRepClass3d_SolidClassifier*).
    /// rcad: unused 閳?classification uses separate classify_point function.
    #[allow(dead_code)]
    solid_classifier_map: std::collections::HashMap<usize, ()>,
    /// OCCT: myHatcherMap (IntTools_Context: face i 閳?Geom2dHatch_Hatcher*).
    /// rcad: unused 閳?hatch-based classification not implemented.
    #[allow(dead_code)]
    hatcher_map: std::collections::HashMap<usize, ()>,
    /// OCCT: myOBBMap (IntTools_Context: shape 閳?Bnd_OBB*).
    /// rcad: unused 閳?AABB-based filtering via Bvh instead of OBB.
    #[allow(dead_code)]
...
impl Context {
    ...
    /// OCCT: FClass2d(theFace) 閳?returns cached 2D point classifier for a face.
    ...
    /// OCCT: IsPointInOnFace(theFace, theUV) 閳?convenience wrapper.
    ...
    /// OCCT: IsPointInFace(theFace, theUV) 閳?convenience wrapper.
    ...
    /// OCCT: ProjPS(theFace) 閳?projects a 3D point onto the face surface.
    ...
    /// OCCT: ProjPC(theEdge) 閳?projects a 3D point onto the edge's curve.
    ...
    /// ProjPT(theP, theC) 閳?projects a 3D point onto a transient curve.
    ...
    /// OCCT: SurfaceAdaptor(theFace) 閳?returns cached surface reference for a face.
    ...
    /// OCCT: UVBounds(theFace) 閳?returns cached UV bounds [umin, umax, vmin, vmax].
    ...
    /// OCCT: StatePointFace(theFace, theP2D) 閳?returns the state (In/On/Out)
    ...
    /// OCCT: IsPointInFace(theP, theFace, theTol) 閳?3D point version.
    ...
    /// OCCT: IsValidPointForFaces(theP, theF1, theF2, theTol) 閳?returns true
    ...
    /// OCCT: IsInfiniteFace(theFace) 閳?returns true if the face has infinite bounds.
...
mod tests {
    ...
    fn is_point_in_face_3d_on_surface() {
        ...
        // Test runs without panic 閳?result correctness depends on build_face_reps.
    }
