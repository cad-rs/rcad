...
pub(crate) fn curve2d_nb_samples(curve: &Curve2d, t0: f64, t1: f64) -> usize {
    ...
    let nbs = match curve {
        Curve2d::Line(_) => 2,
        Curve2d::Circle(c) => {
            // OCCT: angle step = 2*acos(1 - eps) 鈮?0.283 rad for eps=0.01, R=1
...
/// OCCT GCPnts_QuasiUniformDeflection 鈥?adaptive curve sampling by chordal deflection.
...
impl CSLibClass2d {
    ...
    pub fn si_dans(&self, uv: DVec2) -> CSLibResult {
        ...
        // OCCT L173-183: 4-corner tolerance check 鈥?shift by 卤tol and re-classify
    ...
    fn internal_si_dans_ou_on(&self, px: f64, py: f64) -> CSLibResult {
        ...
        for a_next_idx in 1..=self.n {
            ...
            // OCCT L301-316: edge ON detection 鈥?interpolate Y at test point's X
...
pub struct FClass2d {
    ...
    ///   U/V periodicity for sphere (U=2蟺, V=蟺) and cylinder (U=2蟺).
    is_u_periodic: bool,
    is_v_periodic: bool,
    u_period: f64,
    v_period: f64,
...
impl FClass2d {
    ...
    ///   falls outside the cached [Umin,Umax]脳[Vmin,Vmax] bounds, shift the
    ...
    pub fn perform(&self, uv: DVec2, recadre_on_periodic: bool) -> State {
        ...
        loop {
            ...
            if u > self.u2 || !self.is_u_periodic {
                ...
                // OCCT L798-801: V exhausted 鈫?return last result
    ...
    fn perform_impl(&self, uv: DVec2) -> State {
        ...
        //   For each wire: if SiDans returns INSIDE and wire is FORWARD 鈫?IN.
        //   If SiDans returns INSIDE and wire is REVERSED 鈫?OUT.
        //   If SiDans returns UNCERTAIN 鈫?need fallback (BRepClass_FClassifier).
