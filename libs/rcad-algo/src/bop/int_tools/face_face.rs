...
/// IntTools_FaceFace 鈥?face-face intersection result.
///
...
fn surface_curve_to_curve3(sc: &SurfaceCurve) -> Option<(Curve3, [f64; 2])> {
    match sc {
        ...
        SurfaceCurve::Polyline(pts) => {
            let curve3 = crate::bop::int_tools::intss::polyline_to_bspline(pts, 1e-4)?;
...
pub fn intersect_faces(
    ...
    let intss =
        crate::bop::int_tools::intss::intersect_surfaces_with_tolerance(surf1, surf2, tol_f1 + tol_f2);
