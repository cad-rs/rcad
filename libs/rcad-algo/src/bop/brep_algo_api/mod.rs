...
use crate::bop::algo::builder::{BooleanError, BooleanOpType};
...
impl<'a> BooleanOp<'a> {
    ...
    pub fn build(&mut self) -> bool {
        ...
        self.err = None;
        self.history = None;
        // TODO: replace with direct builder pipeline
        let _ = (self.op_type, self.shape1, self.shape2);
        let result: Result<(rcad_kernel::BRep, crate::history::BooleanHistory), crate::bop::algo::builder::BooleanError> = Err(crate::bop::algo::builder::BooleanError::InvalidOperation);
        match result {
            Ok((t, h)) => {
                self.result = Some(t);
                self.history = Some(h);
