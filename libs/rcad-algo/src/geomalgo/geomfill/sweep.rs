...
//! - GAP carrier: [`Sweep::build_product`] — depends on GeomFill_LocFunction
//!   (not translated; confirmed absent from geomalgo/geomfill); the OCCT
//!   call site (Build L218) is commented out — the function is unreachable
//!   there.
...
impl Sweep {
    ...
    /// GeomFill_LocFunction (not translated; confirmed absent from
    /// geomalgo/geomfill); the OCCT call site (Build L218) is commented out
    /// — the function is unreachable there and keeps the OCCT failure path.
    fn build_product(&mut self, _continuity: GeomAbsShape, _degmax: i32, _segmax: i32) -> bool {
        panic!(
            "GAP: GeomFill_Sweep::BuildProduct (GeomFill_LocFunction) is not translated — see file header"
        )
    }
