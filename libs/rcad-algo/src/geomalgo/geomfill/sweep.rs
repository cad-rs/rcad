...
//! - [`ApproxSweepApproximation`] — 1:1 translation of
//!   Approx_SweepApproximation.cxx (TKGeomBase/Approx, whole file L31-872,
//!   plus the .lxx accessors) over AdvApprox_ApproxAFunction; the BuildAll
//!   result read-back is written literally against its typed accessors.
//!   Split into the [`approximation`] child module (file split only,
//!   re-exported here for the pipe.rs consumer).
//! - [`Sweep::build_product`] — the faithful translation of
//!   GeomFill_Sweep::BuildProduct (cxx L392-477) over the
//!   [`loc_function::LocFunction`] translation of GeomFill_LocFunction
//!   (cxx L25-147).  The OCCT call site (Build L218) is commented out
//!   upstream, so the function is unreachable there as well.
//! - `GeomFill_Sweep_Eval` (cxx L75-102) maps to the local [`SweepEval`]
//!   evaluator; the OCCT reference to the LocFunction maps to a mutable
//!   borrow for the duration of the ApproxAFunction construction.
...
// OCCT GeomFill_LocFunction (TKGeomAlgo/GeomFill) — translated in the
// loc_function.rs child module (file split).
#[path = "loc_function.rs"]
pub mod loc_function;

// OCCT Approx_SweepApproximation (TKGeomBase/Approx) — translated in the
// sweep_approximation.rs child module (file split; nothing changed
// semantically).
#[path = "sweep_approximation.rs"]
pub mod approximation;

pub use approximation::ApproxSweepApproximation;
...
/// OCCT class GeomFill_Sweep_Eval (GeomFill_Sweep.cxx L75-102) — the
/// evaluator for curve approximation, forwarding DN to GeomFill_LocFunction.
/// The OCCT reference to the LocFunction maps to a mutable borrow
/// (constructed only at the BuildProduct ApproxAFunction call).
// Only constructed by BuildProduct, whose OCCT call site (Build L218) is
// commented out upstream.
#[allow(dead_code)]
struct SweepEval<'a> {
    /// OCCT GeomFill_LocFunction& theAncore.
    the_ancore: &'a mut loc_function::LocFunction,
}

impl EvaluatorFunction for SweepEval<'_> {
    /// OCCT GeomFill_Sweep_Eval::Evaluate (cxx L90-102) — `result` plays
    /// the role of the double& Result of DN (Result[0] is the first of the
    /// 12 packed slots); the returned code is DN's Ier.
    fn evaluate(
        &mut self,
        start_end: &[f64; 2],
        parameter: f64,
...
impl Sweep {
    ...
    fn build_all(&mut self, continuity: GeomAbsShape, degmax: i32, segmax: i32) -> bool {
        ...
        let mut approx = ApproxSweepApproximation::with_owned(Box::new(func));
        ...
        if approx.is_done() {
            ...
            // La surface.
            let (mut u_degree, mut v_degree) = (0usize, 0usize);
            let (mut nb_u_poles, mut nb_v_poles) = (0usize, 0usize);
            let (mut nb_u_knots, mut nb_v_knots) = (0usize, 0usize);
            approx.surf_shape(
                &mut u_degree,
                &mut v_degree,
                &mut nb_u_poles,
            ...
            let mut poles = vec![Vec::new(); nb_u_poles];
            let mut weights = vec![Vec::new(); nb_u_poles];
            let mut u_knots = vec![0.0; nb_u_knots];
            let mut v_knots = vec![0.0; nb_v_knots];
            let mut u_mults = vec![0i32; nb_u_knots];
            let mut v_mults = vec![0i32; nb_v_knots];
    ...
    /// OCCT BuildProduct (L392-477) — the formal tensor-product path over
    /// the section BSplineSurface.  The OCCT call site in Build (L218) is
    /// commented out upstream ("// isProduct = BuildProduct(Continuity,
    /// Degmax, Segmax);") so isProduct stays false there; the rcad body is
    /// the faithful translation and is likewise unreachable from
    /// [`Sweep::build`].
    #[allow(dead_code)] // mirrors the commented-out OCCT call site (Build L218)
    fn build_product(&mut self, continuity: GeomAbsShape, degmax: i32, segmax: i32) -> bool {
        let mut ok = false;

        // OCCT: BSurf = down_cast<Geom_BSplineSurface>(
        //   mySec->BSplineSurface()->Copy()).
        let bsurf = match self
            .my_sec
            .as_ref()
            .expect("null mySec")
            .borrow()
        ...
        {
            Some(s) => s,
            None => return ok, // Ce mode de construction est impossible
        };

        // OCCT: GeomFill_LocFunction Func(myLoc).
        let mut func = loc_function::LocFunction::new(self.my_loc.clone());

        let nb_interval_c2 = self.my_loc.borrow().nb_intervals(GeomAbsShape::C2);
        let nb_interval_c3 = self.my_loc.borrow().nb_intervals(GeomAbsShape::C3);
        let mut param_de_decoupe_c2 = vec![0.0f64; nb_interval_c2 + 1];
        self.my_loc
            .borrow()
            .intervals(&mut param_de_decoupe_c2, GeomAbsShape::C2);
        let mut param_de_decoupe_c3 = vec![0.0f64; nb_interval_c3 + 1];
        self.my_loc
            .borrow()
            .intervals(&mut param_de_decoupe_c3, GeomAbsShape::C3);

        let preferentiel =
            PrefAndRec::with_default_weight(&param_de_decoupe_c2, &param_de_decoupe_c3);

        // OCCT: ThreeDTol = new NCollection_HArray1<double>(1, 4);
        // ThreeDTol->Init(Tol3d); // A Affiner...
        let three_d_tol = vec![self.tol3d; 4];

        // OCCT: GeomFill_Sweep_Eval eval(Func); AdvApprox_ApproxAFunction
        // Approx(0, 0, 4, ThreeDTol, ThreeDTol, ThreeDTol, First, Last,
        // Continuity, Degmax, Segmax, eval, Preferentiel).
        // Architecture note: the OCCT evaluator aliases Func by reference;
        // the Rust form borrows it mutably for the duration of the
        // ApproxAFunction construction (as in Approximation).
        let approx = {
            let mut ev = SweepEval {
                the_ancore: &mut func,
            };
        ...
        // OCCT #ifdef OCCT_DEBUG: Approx.Dump(std::cout) — debug-only build.

        ok = approx.has_result();
        if ok {
            // The tensor-product assembly block (cxx L435-470) is commented
            // out upstream; only the surface assignment survives.
            self.my_surface = Some(Surface3::BSpline(bsurf));
        }
        ok
    }
