//! TKBRep GTest translations — ported from rcad-algorithms.
...
fn shape_type_of(brep: &BRep, idx: usize) -> ShapeType {
...
mod topods_tshape_tests {
    ...
    fn shapetype_all_types() {
        ...
        assert_eq!(shape_type_of(&b, v.index), ShapeType::Vertex);

        let e = b.add_tedge(None, v, Shape::null(), [0.0, 1.0]);
        assert_eq!(shape_type_of(&b, e.index), ShapeType::Edge);

        let w = b.add_twire(vec![]);
        assert_eq!(shape_type_of(&b, w.index), ShapeType::Wire);

        let f = b.add_tface(None, w, vec![], None, None, vec![], true);
        assert_eq!(shape_type_of(&b, f.index), ShapeType::Face);

        let sh = b.add_tshell(vec![]);
        assert_eq!(shape_type_of(&b, sh.index), ShapeType::Shell);

        let s = b.add_tsolid(vec![]);
        assert_eq!(shape_type_of(&b, s.index), ShapeType::Solid);
    }
    ...
    fn flag_setters_getters() {
        ...
        assert!(b.has_flag(sr.clone(), tshape_flags::FREE));
        b.set_flag(sr.clone(), tshape_flags::FREE, false);
        assert!(!b.has_flag(sr.clone(), tshape_flags::FREE));
        b.set_flag(sr.clone(), tshape_flags::FREE, true);
        assert!(b.has_flag(sr.clone(), tshape_flags::FREE));
        assert!(b.has_flag(sr.clone(), tshape_flags::MODIFIED));
        b.set_flag(sr.clone(), tshape_flags::MODIFIED, false);
        assert!(!b.has_flag(sr.clone(), tshape_flags::MODIFIED));
    }
...
mod topexp_tests {
    ...
    fn vertex_tolerance_exists() {
        ...
        b.vertex_mut(v.clone()).tolerance = 1e-6;
    ...
    fn edge_has_curve_or_not() {
        ...
        if let TShape::Edge(ed) = te { assert!(ed.curve.is_none()); }
        else { panic!("expected Edge"); }
    }
    ...
    fn full_circle_edge_is_closed() {
        ...
        let circle = Some(Curve3::Circle(rcad_kernel::geom::Circle3::new(DVec3::ZERO, DVec3::Z, 1.0)));
        let e = b.add_tedge(circle, sv, ev, [0.0, std::f64::consts::TAU]);
        if let TShape::Edge(ed) = &*b.tshapes[e.index] { assert!(ed.first.index == ed.last.index); }
        else { panic!("expected Edge"); }
    }
    ...
    fn line_edge_not_closed() {
        ...
        let sv = b.add_tvertex(DVec3::ZERO);
        let ev = b.add_tvertex(DVec3::new(10.0, 0.0, 0.0));
        let line = Some(Curve3::Line(rcad_kernel::geom::Line3 { origin: DVec3::ZERO, direction: DVec3::X }));
