...
use rcad_kernel::{
    BRep, Curve3, CurveEval, Surface3, SurfaceEval, any_perpendicular, periodic_seam_edge_indices,
    salient_vertex_indices, vertex_adjacent_edges,
};
...
impl AxisGizmoLayout {
    pub fn contains_point(&self, point_px: [f32; 2]) -> bool {
        ...
        point_px[0] >= min_x && point_px[1] >= min_y && point_px[0] <= max_x && point_px[1] <= max_y
    }
}
...
impl SelectionState {
        ...
        match self.mode {
            ...
            SelectionMode::Edge => {
                let hit = pick_edge(brep, camera, aspect, viewport, cursor, edge_pick_radius_px)
                    .or_else(|| {
                        pick_vertex(brep, camera, aspect, viewport, cursor, edge_pick_radius_px)
                            .and_then(|vi| {
    ...
    pub fn hover_at(
        ...
        match self.mode {
            ...
            SelectionMode::Edge => {
                self.hovered_edge =
                    pick_edge(brep, camera, aspect, viewport, cursor, edge_pick_radius_px).or_else(
                        || {
                            pick_vertex(brep, camera, aspect, viewport, cursor, edge_pick_radius_px)
                                .and_then(|vi| {
                                    pick_edge_from_candidates(
...
pub fn pick_face(
    ...
    for ts in &brep.tshapes {
        let TShape::Solid(sd) = ts.as_ref() else {
            continue;
        };
                            ...
                            for esr in &wd.edges {
                                if let TShape::Edge(ed) = &*brep.tshapes[esr.index] {
                                    ...
                                    if let Some(p) = brep.vertex_point(ed.last.index) {
                                        ...
                                        if pts.last().map_or(true, |&last| {
                                            (glam::vec3(last[0], last[1], last[2]) - last_pt)
                                                .length_squared()
                                                > 1e-10
                                        }) {
                                            pts.push(last_pt);
    ...
    I: IntoIterator<Item = usize>,
{
    ...
    for idx in candidates {
        let TShape::Edge(ed) = &*brep.tshapes[idx] else {
            continue;
        };
        let Some(p0) = brep.vertex_point(ed.first.index).map(to_vec3) else {
            continue;
        };
        let Some(p1) = brep.vertex_point(ed.last.index).map(to_vec3) else {
            continue;
        };
...
pub fn pick_vertex(
    ...
    for vi in salient_vertex_indices(brep) {
        let Some(p) = brep.vertex_point(vi).map(to_vec3) else {
            continue;
        };
...
pub fn build_edges_highlight_mesh(brep: &BRep, edge_indices: &[usize]) -> Option<Mesh> {
    ...
    for &edge_index in edge_indices {
        let TShape::Edge(ed) = &*brep.tshapes[edge_index] else {
            continue;
        };
...
/// Line list (`Mesh::indices` pairs) tracing each selected face  outer and inner wires.
///
                        ...
                        if let TShape::Wire(wd) = &*brep.tshapes[fd.outer_wire.index] {
                            for esr in &wd.edges {
                                if seen_edge.insert(esr.index) {
                                    append_edge_as_line_list(
                                        brep,
                                        esr.index,
                                        esr.orientation.is_forward(),
                                        &mut nodes,
                                        &mut indices,
                                    );
                                }
...
pub fn build_vertex_dots_mesh(brep: &BRep) -> Option<Mesh> {
    ...
    for (i, &vi) in feature_indices.iter().enumerate() {
        ...
        indices.extend_from_slice(&[
            top, right, front, top, front, left, top, left, back, top, back, right, bottom, front,
            right, bottom, left, front, bottom, back, left, bottom, right, back,
        ]);
    }
...
/// Perspective aspect from a **snapped** pixel width/height (e.g. egui  rounded physical rect).
#[inline]
...
pub fn shader_widget_projection_aspect_and_depth_target(
    ...
    let intersect_f32 =
        |a: (f32, f32, f32, f32), b: (f32, f32, f32, f32)| -> Option<(f32, f32, f32, f32)> {
            let x = a.0.max(b.0);
            let y = a.1.max(b.1);
            let ax1 = a.0 + a.2;
            let ay1 = a.1 + a.3;
...
fn sample_edge_curve_points(brep: &BRep, edge_idx: usize) -> Option<Vec<[f32; 3]>> {
    ...
    // Some imported periodic edges carry a full [0, 2 ] range even when the
...
impl Tessellator {
                    ...
                    if fd.outer_wire.index < brep.tshapes.len() {
                        if let TShape::Wire(wd) = &*brep.tshapes[fd.outer_wire.index] {
                            ...
                            for esr in &wd.edges {
                                if let TShape::Edge(ed) = &*brep.tshapes[esr.index] {
                                    if let Some(di) =
                                        vert_tsi_to_dense.get(ed.first.index).and_then(|o| *o)
                                    {
                                    ...
                                    if let Some(di) =
                                        vert_tsi_to_dense.get(ed.last.index).and_then(|o| *o)
                                    {
                                        if dense_verts.last() != Some(&di) {
                                            dense_verts.push(di);
                                        }
                                ...
                                if dense_verts.len() >= 3 {
                                    for i in 1..(dense_verts.len() - 1) {
                                        ...
                                        let a = glam::DVec3::new(
                                            flat_verts[i0][0] as f64,
                                            flat_verts[i0][1] as f64,
                                            flat_verts[i0][2] as f64,
                                        );
                                        let b = glam::DVec3::new(
                                            flat_verts[i1][0] as f64,
                                            flat_verts[i1][1] as f64,
                                            flat_verts[i1][2] as f64,
                                        );
                                        let c = glam::DVec3::new(
                                            flat_verts[i2][0] as f64,
                                            flat_verts[i2][1] as f64,
                                            flat_verts[i2][2] as f64,
                                        );
                                        let e1 = b - a;
                                        let e2 = c - a;
                                        let fn_ = e1.cross(e2);
                                        ...
                                        for &di in &[i0, i1, i2] {
                                            if di < n_verts {
                                                let pt = glam::DVec3::new(
                                                    flat_verts[di][0] as f64,
                                                    flat_verts[di][1] as f64,
                                                    flat_verts[di][2] as f64,
                                                );
                                                let analytic = surface.and_then(|s| {
                                                    analytic_surface_normal_at_point(s, pt)
                                                });
                                                let contribution = analytic
                                                    .unwrap_or(fn_.normalize_or_zero())
                                                    * weight;
    ...
    pub fn tessellate_with_options(brep: &mut BRep, options: &TessellationOptions) -> Mesh {
        Self::tessellate_with_options_and_line_options(
            brep,
            options,
            &TessellateLineOptions::default(),
        )
    }
...
mod tests {
    ...
    fn test_tessellate_cylinder() {
        ...
        assert!(
            !mesh.nodes.is_empty(),
            "cylinder should generate mesh nodes"
        );
    ...
    fn tessellate_with_options_rebuilds_cached_sphere_torus_triangles() {
        ...
        let sphere_mesh =
            Tessellator::tessellate_with_options(&mut sphere, &TessellationParams::default());
        assert!(
        ...
        let torus_mesh =
            Tessellator::tessellate_with_options(&mut torus, &TessellationParams::default());
        assert!(
    ...
    fn tessellate_with_options_keeps_cached_planar_triangles() {
        ...
        let box_mesh =
            Tessellator::tessellate_with_options(&mut box_brep, &TessellationParams::default());
    ...
    fn test_torus_inner_equator_mesh() {
        ...
        assert!(
            has_inner_vertex,
            "torus should have mesh nodes on inner equator"
        );
    }
    ...
    fn test_cylinder_radial_distribution() {
        ...
        assert!(
            max_angle > 150.0,
            "mesh nodes should span most of 360 degrees"
        );
    }
    ...
    fn test_preview_quality_density() {
        ...
        let preview_mesh =
            Tessellator::tessellate_with_options(&mut brep, &TessellationParams::preview());
        ...
        let hq_mesh =
            Tessellator::tessellate_with_options(&mut brep, &TessellationParams::high_quality());
    ...
    fn test_empty_brep_mesh() {
        ...
        assert!(
            mesh.nodes.is_empty(),
            "empty brep should produce empty mesh"
        );
        assert!(
            mesh.indices.is_empty(),
            "empty brep should produce no indices"
        );
    }
}
...
fn adaptive_grid_minor_spacing(camera: &Camera, aspect: f32) -> f32 {
    let half_view_width =
        camera.distance.max(0.1) * (GRID_FOV_Y_RADIANS * 0.5).tan() * aspect.max(1.0);
...
impl WgpuRenderer {
    ...
    pub fn new(device: &wgpu::Device, surface_format: wgpu::TextureFormat) -> Self {
        ...
        for i in 0..3 {
            ...
            let verts_padded: Vec<[f32; 6]> = verts
                .iter()
    ...
    pub fn upload_mesh(&self, device: &wgpu::Device, mesh: &Mesh) {
        ...
        *self.vertex_buffer.lock().expect("render mutex poisoned") = Some(
            device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        ...
        *self.index_buffer.lock().expect("render mutex poisoned") = Some(
            device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        ...
        *self.index_count.lock().expect("render mutex poisoned") = mesh.indices.len() as u32;
        ...
        if mesh.line_indices.is_empty() {
            *self
                .line_index_buffer
                .lock()
                .expect("render mutex poisoned") = None;
            *self.line_index_count.lock().expect("render mutex poisoned") = 0;
        } else {
            *self
                .line_index_buffer
                .lock()
                .expect("render mutex poisoned") = Some(device.create_buffer_init(
            ...
            *self.line_index_count.lock().expect("render mutex poisoned") =
                mesh.line_indices.len() as u32;
        }

        if mesh.seam_line_indices.is_empty() {
            *self
                .seam_line_index_buffer
                .lock()
                .expect("render mutex poisoned") = None;
            *self
                .seam_line_index_count
                .lock()
                .expect("render mutex poisoned") = 0;
        } else {
            *self
                .seam_line_index_buffer
                .lock()
            ...
            *self
                .seam_line_index_count
                .lock()
                .expect("render mutex poisoned") = mesh.seam_line_indices.len() as u32;
        }
    }
    ...
    pub fn upload_highlights(
        ...
        if let Some(mesh) = face_mesh {
            ...
            *self
                .highlight_face_index_count
                .lock()
                .expect("render mutex poisoned") = mesh.indices.len() as u32;
        } else {
            *self
                .highlight_face_vertex_buffer
                .lock()
    ...
    pub fn update_camera(&self, queue: &wgpu::Queue, camera: &Camera, aspect: f32) {
        ...
        let padded_vertices: Vec<[f32; 6]> = grid_vertices
            ...
            .collect();
        queue.write_buffer(
            &self.grid.vertex_buffer,
            0,
            bytemuck::cast_slice(&padded_vertices),
        );
        queue.write_buffer(
        ...
        *self
            ...
            .major_index_count
            .lock()
            .expect("render mutex poisoned") = grid_major_indices.len() as u32;
        *self
            .grid
            .minor_index_count
            .lock()
            .expect("render mutex poisoned") = grid_minor_indices.len() as u32;
    ...
    pub fn draw_in_render_pass(
        ...
        {
            if draw_brep_lines && let Some(lib) = lib_guard.as_ref() {
            ...
            if draw_seam_lines && let Some(slib) = slib_guard.as_ref() {
                render_pass.set_pipeline(line_pipeline);
                render_pass.set_bind_group(0, &self.camera_bind_group, &[]);
    ...
    fn draw_grid_in_render_pass(
        ...
        let minor_index_count = *self
            .grid
            .minor_index_count
            .lock()
            .expect("render mutex poisoned");
        let major_index_count = *self
            .grid
            .major_index_count
            .lock()
            .expect("render mutex poisoned");
    ...
    pub fn screenshot(
        ...
        buffer_slice.map_async(wgpu::MapMode::Read, move |result| {
            sender
                .send(result)
                .expect("screenshot channel: receiver dropped before GPU callback");
        });
        device
            .poll(wgpu::PollType::wait_indefinitely())
            .expect("GPU device lost during screenshot");
        receiver
            .recv()
