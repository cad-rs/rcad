...
impl CreationController {
    ...
    pub fn set_tool(&mut self, tool: Tool, selection: &mut SelectionState) {
        ...
        match tool {
            ...
            Tool::Box | Tool::Sphere | Tool::Cylinder | Tool::Cone | Tool::Torus => {
...
fn offset_shape_refs(ts: &mut Arc<TShape>, base: usize) {
    ...
    match inner {
        ...
        TShape::Edge(ed) => {
            ed.first.index += base;
            ed.last.index += base;
            for sr in &mut ed.my_shapes {
                sr.index += base;
            }
        }
        TShape::Wire(wd) => {
            for sr in &mut wd.my_shapes {
                sr.index += base;
            }
            for sr in &mut wd.edges {
                sr.index += base;
            }
        }
        TShape::Face(fd) => {
            fd.outer_wire.index += base;
            for sr in &mut fd.inner_wires {
                sr.index += base;
            }
            for sr in &mut fd.my_shapes {
                sr.index += base;
            }
            for sr in &mut fd.internal_vertices {
                sr.index += base;
            }
        }
        TShape::Shell(sd) => {
            for sr in &mut sd.my_shapes {
                sr.index += base;
            }
            for sr in &mut sd.faces {
                sr.index += base;
            }
        }
        TShape::Solid(sd) => {
            ...
            for sr in &mut sd.shells {
                sr.index += base;
            }
            for sr in &mut sd.internal_vertices {
                sr.index += base;
            }
            for sr in &mut sd.internal_edges {
                sr.index += base;
            }
        }
        TShape::Compound(cd) => {
            for sr in cd.iter_mut() {
                sr.index += base;
            }
        }
        TShape::CompSolid(csd) => {
            for sr in csd.iter_mut() {
                sr.index += base;
            }
        }
    }
