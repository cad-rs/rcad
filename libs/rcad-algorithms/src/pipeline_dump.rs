...
impl DumpCtx {
    ...
    pub fn snapshot(&mut self, stage: &str, ds: &DS, brep: Option<&rcad_kernel::topods::BRep>) {
        ...
        let mod_part = if self.module.is_empty() {
            String::new()
        } else {
        ...
        let p = dir.join(format!(
            "rcad_{}_{}s{:02}_{}_ds.json",
            prefix, mod_part, seq, stage
        ));
...
fn serialize_ds(ds: &DS) -> serde_json::Value {
    ...
    let vertices: Vec<serde_json::Value> = ds
        .vertices
        .iter()
        .enumerate()
        .map(|(vi, v)| {
    ...
    let edges: Vec<serde_json::Value> = ds
        .edges
        .iter()
        .enumerate()
        .map(|(ei, e)| {
    ...
    let faces: Vec<serde_json::Value> = ds
        .faces
        .iter()
        .enumerate()
        .map(|(fi, f)| {
    ...
    let ics: Vec<serde_json::Value> = ds
        .intersection_curves
        .iter()
        .enumerate()
        .map(|(ci, ic)| {
    ...
    let pbs: Vec<serde_json::Value> = ds
        .pave_blocks
        .iter()
        .enumerate()
        .map(|(pi, pb)| {
    ...
    let cbs: Vec<serde_json::Value> = ds
        .common_blocks
        .iter()
        .enumerate()
        .map(|(cbi, cb)| {
    ...
    let interf_vv: Vec<serde_json::Value> = ds
        .interf_vv
        .iter()
        .map(|ivv| json!({"v1": ivv.v1, "v2": ivv.v2, "merged": ivv.merged_vertex}))
        .collect();

    // Edge interference details (EE): which edge pairs intersect
    let interf_ee: Vec<serde_json::Value> = ds
        .interf_ee
        .iter()
        .map(|iee| json!({"e1": iee.e1, "e2": iee.e2, "new_v": iee.new_vertex}))
        .collect();

    // Edge-face interference details (EF): which edge is on which face
    let interf_ef: Vec<serde_json::Value> = ds
        .interf_ef
        .iter()
        .map(|ief| json!({"edge": ief.edge, "face": ief.face, "new_v": ief.new_vertex}))
        .collect();

    // Face-face interference details (FF): which face pairs intersect
    let interf_ff: Vec<serde_json::Value> = ds
        .interf_ff
        .iter()
        .map(|iff| {
