//! OCCT BRepAlgoAPI_BuilderOperation equivalent 閳?boolean operation wrapper with history.
//!
...
//! - BRepAlgoAPI_BuilderOperation 閳?BRepAlgoAPI_BuilderShape 閳?BRepAlgoAPI_Algo
//!
//! # Example
//!
//! ```
//! use rcad_algorithms::crate::bop::brep_algo_api::builder_operation::BooleanOp;
...
/// `BuilderOperation` 鈥?a general boolean operation between two shapes with history tracking.
///
...
impl BuilderOperation {
    ...
    /// - `Union` 閳?BRepAlgoAPI_Fuse
    /// - `Intersection` 閳?BRepAlgoAPI_Common
    /// - `Difference` 閳?BRepAlgoAPI_Cut
    ...
    ///
    /// Pipeline:
    /// 1. 閴?Build BOPDS_DS from the two shapes
    /// 2. 閴?Run BOPAlgo_PaveFiller to compute all intersections
    /// 3. 閴?Build container images (FillImagesContainers)
    /// 4. 閴?Run BOPAlgo_Builder to build the result
    ///
    ...
    pub fn perform(&mut self) -> Result<&rcad_kernel::BRep, BooleanError> {
        ...
        // 閴?BOPAlgo_BOP::CheckInputData 閳?verify valid inputs
        ...
        // 閴?Build BOPDS_DS (data structure) from the two shapes
        // OCCT ref: BOPAlgo_PaveFiller::Perform 閳?BOPDS_DS::Alloc
        let mut ds = DS::new_from_topods(&a, &b, self.tolerance.max(TOLERANCE_ABS));

        // 閴?Build BVH acceleration (optional in OCCT)
        ...
        // FillImagesContainers 鈥?build wire/shell images
    ...
    // 閳光偓閳光偓 OCCT History Queries 閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓
    //
    ...
    // 閴?stubs return all known history.
    // 閳?Side-distinction (A vs B) not implemented 閳?section-style operations
    ...
    ///
    /// 閴?returns generated faces/edges/vertices.
    ...
    ///
    /// 閴?face deletion tracking via history.
