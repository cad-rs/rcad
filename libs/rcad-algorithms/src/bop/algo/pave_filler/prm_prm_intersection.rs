...
//!     - IntWalk_PWalking replaced by rcad marching (crate::bop::int_tools::marching)
...
impl T3Bits {
    ...
    pub fn add(&mut self, t: usize) {
        self.p[t >> 5] |= 1u32 << (t & 31);
    }

    /// OCCT L35: return (p[t >> 5] & (1 << (t & 31)))
    pub fn val(&self, t: usize) -> u32 {
        self.p[t >> 5] & (1u32 << (t & 31))
    }

    /// OCCT L37: p[t >> 5] &= ~(1 << (t & 31))
    pub fn raz(&mut self, t: usize) {
        self.p[t >> 5] &= !(1u32 << (t & 31));
    }
...
impl PrmPrmIntersection {
    ...
    pub fn dans_grille(&self, t: i32) -> bool {
        t >= 0 && t < BASE
    }
    ...
    ///   OCCT uses IntWalk_PWalking; rcad uses crate::bop::int_tools::marching for the walking step
    ...
    pub fn perform_with_seeds(
        ...
        for seed in seed_points {
            ...
            if dmini_point_ligne <= seuild_point_ligne {
                continue;
            }
            let walked = self.walk_line(
                s1, s2, seed, u_min1, v_min1, u_min2, v_min2, u_max1, v_max1, u_max2, v_max2,
                epsilon, deflection, increment,
            );
            ...
            let p3d_debut = walk_line
                .points
                .first()
                .map(|p| p.p3d)
                .unwrap_or(DVec3::ZERO);
            let p3d_fin = walk_line
                .points
                .last()
                .map(|p| p.p3d)
                .unwrap_or(DVec3::ZERO);
            ...
            for existing in &self.slin {
                ...
                let Some(ed) = existing.points.first() else {
                    continue;
                };
                let Some(ef) = existing.points.last() else {
                    continue;
                };
                if p3d_debut.distance(ed.p3d) <= tol_tangency
                    && p3d_fin.distance(ef.p3d) <= tol_tangency
                {
                    reject = true;
                    break;
                }
    ...
    fn walk_line(
        ...
        pw.perform_with_bounds(
            &par_dep, umin1, vmin1, umin2, vmin2, umax1, vmax1, umax2, vmax2,
        );
        ...
        Some(IntersectionLine {
            points,
            trans1: Transition::Undefined,
            trans2: Transition::Undefined,
        })
    }
    ...
    fn compute_transitions(
        ...
        let p_next = if mid + 1 < line.points.len() {
            &line.points[mid + 1]
        } else {
            &line.points[mid]
        };
        ...
        if norm1.length_squared() < TOLERANCE_LEN_SQ_DIV_SAFE
            || norm2.length_squared() < TOLERANCE_LEN_SQ_DIV_SAFE
        {
            return (Transition::Undefined, Transition::Undefined);
        }
    ...
    pub fn perform_main(
        ...
        let seeds: Vec<PntOn2S> = interference
            .seed_points()
            .iter()
            .map(|sp| PntOn2S {
                p3d: sp.p3d,
...
fn dublicate_of_lines_processing(
    ...
    if existing_len < current_len {
        return true;
    }
    if existing_len == current_len && existing_chord < current_chord {
        return true;
    }
    false
}
...
fn is_point_on_line(pnt: &PntOn2S, line: &IntersectionLine, _deflection: f64) -> bool {
    for lp in &line.points {
        if (pnt.u1 - lp.u1).abs() < TOLERANCE_MESH_LEGACY
            && (pnt.v1 - lp.v1).abs() < TOLERANCE_MESH_LEGACY
            && (pnt.u2 - lp.u2).abs() < TOLERANCE_MESH_LEGACY
            && (pnt.v2 - lp.v2).abs() < TOLERANCE_MESH_LEGACY
        {
            return true;
        }
    }
...
fn is_point_on_line_by_3d(p3d: &DVec3, line: &IntersectionLine, _deflection: f64) -> bool {
    for lp in &line.points {
        if p3d.distance(lp.p3d) < TOLERANCE_MESH_LEGACY {
            return true;
        }
    }
    false
