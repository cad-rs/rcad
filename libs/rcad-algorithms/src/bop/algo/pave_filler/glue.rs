...
use crate::bop::ds::ds::NearTangentType;
use crate::bop::ds::ds::{DSEdge, ShapeOrigin};
use crate::tolerance::*;
use glam::DVec3;
...
impl<'a> super::PaveFiller<'a> {
    ...
    pub(crate) fn detect_partial_glue_overlaps(&self) -> Vec<PartialOverlapInfo> {
        ...
        let mut pit = crate::bop::ds::ds::PairIterator::prepare_ab(a_fcount, self.ds.face_count());
        while pit.more() {
            let pk = pit.value();
            let f1_idx = pk.i1;
            let f2_idx = pk.i2;
    ...
    pub(crate) fn detect_edge_overlaps(&self) -> Vec<EdgeOverlapResult> {
        ...
        let mut eit = crate::bop::ds::ds::PairIterator::prepare_ab(a_ecount, self.ds.edge_count());
        while eit.more() {
            let pk = eit.value();
            let e1_idx = pk.i1;
            let e2_idx = pk.i2;
