...
impl<'a> PaveFiller<'a> {
    ...
    pub(crate) fn force_interf_ee(&mut self) {
        ...
        for (idx, entry) in a_v_edge_edge.iter().enumerate() {
            ...
            let mut ee_int = crate::bop::int_tools::edge_edge::EdgeEdgeIntersector::new();
        ...
        for (idx, opt_ee) in a_ee_results.iter().enumerate() {
            ...
            if self.ds.rank(entry.n_e1) == self.ds.rank(entry.n_e2) {
                self.my_report
                    .add_alert(crate::bop::algo::Alert::AcquiredSelfIntersection(vec![
            ...
            for &pb_ref in [pb1, pb2].iter() {
                if self.ds.is_common_block(pb_ref) {
                    ...
                    for &(_cb_pb_gi, _) in a_lpcb {
                        ...
                        crate::bop::algo::fill_map(&mut a_mpblpb, key1, key2);
                    }
                }
            }
        ...
        if a_mpblpb.len() > 0 {
            let a_m_blocks: Vec<Vec<(usize, usize)>> = crate::bop::algo::make_blocks(&a_mpblpb);
            for block in &a_m_blocks {
                ...
                let mut a_cb: Option<crate::bop::ds::common_block::CommonBlock> = None;
                ...
                let mut cb = a_cb.unwrap_or_else(crate::bop::ds::common_block::CommonBlock::new);
                cb.set_pave_blocks(block.clone());
                cb.set_faces(a_lfaces);
                let new_idx = match cb_idx {
                    Some(idx) => {
                        let tol = crate::bop::ds::tools::compute_tolerance_of_cb(&self.ds, idx);
    ...
    fn force_interf_ef_with(&mut self, the_mpb: &[(usize, usize)], the_add_interf: bool) {
        ...
        let mut a_pb_aabbs: Vec<crate::bop::tools::bvh::Aabb> = Vec::with_capacity(the_mpb.len());
        ...
        for (i_pb, &(ne, local_i)) in the_mpb.iter().enumerate() {
            ...
            let a_pb_box = crate::bop::tools::bvh::Aabb {
                min: a_p1.min(a_p2),
                max: a_p1.max(a_p2),
                gap: 0.0,
            };
        ...
        for n_f in 0..a_nb_s {
            ...
            let a_box_f = {
                ...
                let mut aabb = crate::bop::tools::bvh::Aabb::empty();
                if let (Some(mn), Some(mx)) = (si.box_min, si.box_max) {
                    aabb = crate::bop::tools::bvh::Aabb {
                        min: mn,
                        max: mx,
                        gap: si.box_gap,
                    };
            ...
            for &a_pb_idx in &a_overlapping {
                ...
                let a_t_mid = crate::bop::tools::intermediate_point(a_ts[0], a_ts[1]);
        ...
        for pair in &a_v_edge_face {
            ...
            if the_add_interf {
                ...
                let a_t_mid = crate::bop::tools::intermediate_point(pair.a_ts[0], pair.a_ts[1]);
            ...
            if the_add_interf {
                crate::bop::algo::fill_map(&mut a_mpbli, g_pb_idx, pair.fi);
            }
        }
