// Outer submodules (siblings of ds/)
pub mod common_block;
pub mod face_info;
pub mod pave;
pub mod tools;

// Inner submodules (formerly under ds/ds/, now flattened)
pub mod types;
pub use types::*;
pub mod new_ds;
...
use crate::bop::tools::bvh::{Aabb, BoxTree};
use crate::tolerance::*;
use glam::{DVec2, DVec3};
use rcad_kernel::PCurve;
use rcad_kernel::geom::{
    Curve2d, Curve2dEval, Curve3, Line2d, Line3, Plane, Surface3, any_perpendicular,
};
...
impl DS {
    ...
    pub fn vertex_data_mut(&mut self, vi: usize) -> &mut topods::TVertexData {
        assert!(
            vi < self.vertices.len(),
            "vertex_data_mut: {} >= vertices.len() {}",
            vi,
            self.vertices.len()
        );
        let si = if vi < self.vertex_shape_idx.len() {
            self.vertex_shape_idx[vi]
        } else {
            vi
        };
    ...
    pub fn edge_data_mut(&mut self, ei: usize) -> &mut topods::TEdgeData {
        let si = if ei < self.edge_shape_idx.len() {
            self.edge_shape_idx[ei]
        } else {
            self.vertices.len() + ei
        };
        match std::sync::Arc::make_mut(&mut self.shapes[si]) {
    ...
    pub fn face_data_mut(&mut self, fi: usize) -> &mut topods::TFaceData {
        let si = if fi < self.face_shape_idx.len() {
            self.face_shape_idx[fi]
        } else {
            fi
        };
        match std::sync::Arc::make_mut(&mut self.shapes[si]) {
    ...
    #[allow(dead_code)]
    pub fn edge_paves(&self, ei: usize) -> &[crate::bop::ds::pave::Pave] {
        self.edges.get(ei).map_or(&[], |e| &e.paves)
    }

    /// Mutable Edge paves.
    pub fn edge_paves_mut(&mut self, ei: usize) -> &mut Vec<crate::bop::ds::pave::Pave> {
        &mut self.edges[ei].paves
    }

    /// Edge pave_blocks.
    /// OCCT: myDS->ChangePaveBlocks(nE) 鈥?single path.
    /// rcad: prefer self.edges[ei].pave_blocks over parallel array.
    pub fn edge_pave_blocks(&self, ei: usize) -> &[crate::bop::ds::pave::SharedPB] {
        self.edges.get(ei).map_or(&[], |e| &e.pave_blocks)
    }

    /// OCCT: myDS->ChangePaveBlocks(nE) 鈥?lazy init via HasReference check.
    /// OCCT BOPDS_DS.cxx L425-431: if (!HasReference()) InitPaveBlocks(theIndex).
    pub fn edge_pave_blocks_mut(&mut self, ei: usize) -> &mut Vec<crate::bop::ds::pave::SharedPB> {
    ...
    /// Mutable FaceInfo 鈥?OCCT: ChangeFaceInfo 鈫?InitFaceInfo 鈫?UpdateFaceInfoOn.
    ...
    /// OCCT equivalent: BRepAdaptor_Surface(aFace) 鈥?transparently applies TopLoc_Location
    ...
    /// myDS->Append 闁?add a new TShape and return its index.
    ...
    /// BOPDS_DS::NbShapes 闁?total count of all shapes.
    ...
    pub fn is_vertex(&self, idx: usize) -> bool {
        self.shapes
            .get(idx)
            .map_or(false, |s| matches!(&**s, topods::TShape::Vertex(_)))
    }
    ...
    pub fn is_edge(&self, idx: usize) -> bool {
        self.shapes
            .get(idx)
            .map_or(false, |s| matches!(&**s, topods::TShape::Edge(_)))
    }
    ...
    pub fn is_face(&self, idx: usize) -> bool {
        self.shapes
            .get(idx)
            .map_or(false, |s| matches!(&**s, topods::TShape::Face(_)))
    }
    ...
    pub fn push_edge(
        ...
        match &curve {
            ...
            Curve3::Circle(circ) => {
                ...
                // Full circle: aAmp = sqrt(R虏*aXk虏 + R虏*aYk虏), box = center 卤 aAmp
                ...
                if t_range_span >= a_period {
                    // Full circle: OCCT L32-43 鈥?aBox.Update(aMin[0..2], aMax[0..2])
                    // Compute per-axis extrema, then set all 3 dims at once.
                    let axes = [DVec3::X, DVec3::Y, DVec3::Z];
                    let a_min = axes.map(|ax| {
                        aO.dot(ax)
                            - (aR * aR * aXd.dot(ax) * aXd.dot(ax)
                                + aR * aR * aYd.dot(ax) * aYd.dot(ax))
                            .sqrt()
                    });
                    let a_max = axes.map(|ax| {
                        aO.dot(ax)
                            + (aR * aR * aXd.dot(ax) * aXd.dot(ax)
                                + aR * aR * aYd.dot(ax) * aYd.dot(ax))
                            .sqrt()
                    ...
                    let tau = 2.0 * PI;
                    let a_period_inner = tau - 0.0; // ULast - UFirst = 2蟺
                    let preci = f64::EPSILON;
                    // OCCT L128-147: AdjustPeriodic
                    let adj_a_u1 = a_u1 - (a_u1 / a_period_inner).floor() * a_period_inner;
                    let adj_a_u1 = if tau - adj_a_u1 < preci {
                        adj_a_u1 - a_period_inner
                    } else {
                        adj_a_u1
                    };
                    let adj_a_u2 =
                        a_u2 - ((a_u2 - adj_a_u1) / a_period_inner).floor() * a_period_inner;
                    let adj_a_u2 = if adj_a_u2 - adj_a_u1 < preci {
                        adj_a_u2 + a_period_inner
                    } else {
                        adj_a_u2
                    };
                    // OCCT L95-111: ElCLib::InPeriod helper (theU 鈫?[base, base+period))
                    ...
                    // Add arc endpoints (OCCT L68-70) 鈥?already done above via start_pt/end_pt.
                    let axes = [DVec3::X, DVec3::Y, DVec3::Z];
                    for (k, axis) in axes.iter().enumerate() {
                        ...
                        if a_val_min > a_val_max {
                            // std::swap(aValMin, aValMax) 鈥?values implicitly swapped via usage
        ...
        // OCCT L1678-1685: anEdgeBoundBox.Add(aVertexInfo.Box()) 鈥?        // include endpoint vertex boxes (with tolerance) in edge AABB.
    ...
    /// Push a face 鈥?struct fields only, no parallel arrays.
    ...
    /// OCCT-aligned: push a Wire 鈥?no dedicated array, hierarchy via ShapeInfo.
    ...
    /// OCCT-aligned: push a Shell 鈥?no dedicated array, hierarchy via ShapeInfo.
    ...
    /// OCCT-aligned: push a Solid 鈥?no dedicated array, hierarchy via ShapeInfo.
    ...
    /// OCCT-aligned: push a CompSolid 鈥?no dedicated array, hierarchy via ShapeInfo.hapes[].
    ...
    pub fn edge_flag(&self, edge_idx: usize) -> i64 {
        let si_idx = if edge_idx < self.edge_shape_idx.len() {
            self.edge_shape_idx[edge_idx]
        } else {
            self.vertices.len() + edge_idx
        };
        ...
        if edge_idx < self.edges.len() {
            ...
            if e.start_vertex == e.end_vertex {
                ...
                return match &e.curve {
                    ...
                    | rcad_kernel::geom::Curve3::Ellipse(_) => false,
                    _ => true,
                };
            }
            return false;
    ...
    pub fn is_valid_shrunk_data(&self, pb: &crate::bop::ds::pave::PaveBlock) -> bool {
        ...
        for &(vi, ts) in &[(v1i, ts1), (v2i, ts2)] {
            ...
            if let Some(curve) = self.edges.get(pb.original_edge) {
                ...
                if a_tol - vp.distance(pp) > an_epsilon {
                    return false;
                }
            }
        }
    ...
    pub fn has_pave_blocks(&self, edge_idx: usize) -> bool {
        let si = if edge_idx < self.edge_shape_idx.len() {
            self.edge_shape_idx[edge_idx]
        } else {
            edge_idx
        };
    ...
    /// then calls update() to sort and create sub-PBs 鈥?one per vertex-to-vertex segment.
    pub fn init_pave_blocks_for_edge(&mut self, edge_idx: usize) {
        if edge_idx >= self.edges.len() {
            return;
        }
        // OCCT ChangePaveBlocks internal: if pool is empty, initialize
        if !self.edges[edge_idx].pave_blocks.is_empty() {
            return;
        }
        let (sv, ev, tr0, tr1) = {
            let e = &self.edges[edge_idx];
        ...
        // Don't set it here 鈥?let fill_shrunk_data compute the correct range.
        ...
        // OCCT L477-483: closed edges 鈥?add both endpoint paves as ext_paves
        if sv == ev {
        ...
        // OCCT L499: aPaveBlock->Update(...) 鈥?sort ext_paves and create sub-PBs
    ...
    pub fn is_sub_shape(&self, the_candidate: usize, the_parent: usize) -> bool {
        if the_parent < self.shape_info.len() {
            self.shape_info[the_parent]
                .sub_shapes
                .contains(&the_candidate)
        } else {
            false
        }
    }
    ...
    pub fn edge_has_vertex(&self, nV: usize, nE: usize) -> bool {
        self.edges
            .get(nE)
            .map_or(false, |e| e.start_vertex == nV || e.end_vertex == nV)
    }
    ...
    pub fn has_interf_ve(&self, vi: usize, ei: usize) -> bool {
        self.interf_ve
            .iter()
            .any(|inf| inf.vertex == vi && inf.edge == ei)
    }
    ...
    pub fn has_interf_vv(&self, v1: usize, v2: usize) -> bool {
        self.interf_vv
            .iter()
            .any(|inf| (inf.v1 == v1 && inf.v2 == v2) || (inf.v1 == v2 && inf.v2 == v1))
    }
    ...
    /// 闁?build vertex-to-edge map (BOPDS_DS::myMapVE).
    ...
    pub fn has_interf_vf(&self, vi: usize, fi: usize) -> bool {
        self.interf_vf
            .iter()
            .any(|inf| inf.vertex == vi && inf.face == fi)
    }
    ...
    pub fn has_interf_ef(&self, ei: usize, fi: usize) -> bool {
        self.interf_ef
            .iter()
            .any(|inf| inf.edge == ei && inf.face == fi)
    }
    ...
    pub fn has_interf_ff(&self, f1: usize, f2: usize) -> bool {
        ...
        self.interf_ff.iter().any(|ff| {
            let (fa, fb) = if ff.f1 < ff.f2 {
                (ff.f1, ff.f2)
            } else {
                (ff.f2, ff.f1)
            };
    ...
    /// 闁?BOPDS_DS::AddInterf (DS.cxx L410-420).
    ...
    pub fn dedup_ff_interferences(&mut self) {
        let mut merged: std::collections::HashMap<(usize, usize), (Vec<usize>, Vec<FFPoint>)> =
            std::collections::HashMap::new();
        for inf in &self.interf_ff {
            let key = if inf.f1 < inf.f2 {
                (inf.f1, inf.f2)
            } else {
                (inf.f2, inf.f1)
            };
            ...
            for p in &inf.points {
                if !entry
                    .1
                    .iter()
                    .any(|ep| ep.vertex_index != usize::MAX && ep.vertex_index == p.vertex_index)
                {
        ...
        for ((f1, f2), (curves, points)) in &merged {
            self.interf_ff.push(InterferenceFF {
                f1: *f1,
                f2: *f2,
                curves: curves.clone(),
                points: points.clone(),
    ...
    pub fn has_interf_shape_sub_shapes(
        &self,
        the_index1: usize,
        the_index2: usize,
        the_any_interference: bool,
    ) -> bool {
    ...
    pub fn compute_uv_boundaries(&mut self) {
        ...
        for fi in 0..self.faces.len() {
            ...
            let is_planar = matches!(&surface, Surface3::Plane(_))
                || (if let Surface3::BSpline(ref bsp) = surface {
                ...
                if n > 2 {
                    ...
                    for i in 0..n {
                        ...
                        if cross < TOLERANCE_MESH_LEGACY * len1.max(len2).max(f64::MIN_POSITIVE)
                            && len1 > f64::MIN_POSITIVE
                            && len2 > f64::MIN_POSITIVE
                        {
                            continue;
                        }
                        kept.push(curr);
    ...
    pub(crate) fn compute_edge_pcurve(
        curve: &Curve3,
        surface: &Surface3,
        pl_basis: Option<(DVec3, DVec3, DVec3)>,
    ) -> Option<(Curve2d, f64)> {
        match (surface, curve) {
            (Surface3::Plane(p), Curve3::Line(l)) => {
                let (u_axis, v_axis) = if let Some((u, v, _)) = pl_basis {
                    (u, v)
                } else {
                    let u = DVec3::X - p.normal * p.normal.dot(DVec3::X);
                    let u = if u.length_squared() < 1e-24 {
                        DVec3::Z - p.normal * p.normal.dot(DVec3::Z)
                    } else {
                        u
                    };
                    (u.normalize(), p.normal.cross(u).normalize())
                };
            ...
            (Surface3::Plane(p), Curve3::Circle(c)) => {
                ...
                if (normal_dot - 1.0).abs() < TOLERANCE_MESH_LEGACY {
                    ...
                    Some((
                        Curve2d::Circle(rcad_kernel::geom::Circle2d {
                            center: center_2d,
                            x_dir: DVec2::X,
                            y_dir: DVec2::Y,
                            radius: c.radius,
            ...
            _ => {
                let t_range = [0.0, 1.0];
                let pc =
                    rcad_kernel::projection::make_pcurve_on_surface(curve, t_range, surface, 16)?;
    ...
    pub fn build_face_reps(&mut self) {
        ...
        for fi in 0..self.faces.len() {
            ...
            let pl_basis: Option<(DVec3, DVec3, DVec3)> = match &surface {
                Surface3::Plane(p) => {
                    let u = any_perpendicular(p.normal).normalize();
                    Some((u, p.normal.cross(u).normalize(), p.origin))
                }
                _ => None,
            };
            ...
            for &ei in &all_ei {
                if self.edge_on_face(ei, fi).is_some() {
                    continue;
                }
                let Some(edge) = self.edges.get_mut(ei) else {
                    continue;
                };
                ...
                if let Some((ref u_axis, ref v_axis, ref pl_origin)) = pl_basis {
                    if let Curve3::Line(l) = &edge.curve {
                        ...
                        let len = dir.length();
                        if len > TOLERANCE_CLAMP_MIN
                            && (t_range[1] - t_range[0]).abs() > TOLERANCE_CLAMP_MIN
                        {
                            let scale = (t_range[1] - t_range[0]) / len;
                            edge.face_reps.push(DSCurveRepOnFace {
                                face_idx: fi,
                                pcurve: Curve2d::Line(Line2d {
                                    origin,
                                    direction: dir / len * scale,
                                }),
                                pcurve2: None,
                ...
                if let Some((mut pcurve, mut span)) =
                    Self::compute_edge_pcurve(&edge.curve, &surface, pl_basis)
                {
    ...
    pub fn find_vertex_near(&self, point: DVec3, tol: f64) -> Option<usize> {
        ...
        self.vertices
            .iter()
            .position(|v| (v.point - point).length_squared() <= tol2)
    }
    ...
    pub fn add_vertex_no_dedup(&mut self, point: DVec3) -> usize {
        ...
        let mut si =
            crate::bop::ds::types::ShapeInfo::new(rcad_kernel::topods::ShapeType::Vertex);
        si.is_new = true;
        si.rank = 0;
    ...
    pub fn common_block(&self, pb: &SharedPB) -> Option<&CommonBlock> {
        pb.0.read()
            .unwrap()
            .common_block_idx
            .and_then(|idx| self.common_blocks.get(idx))
    }
    ...
    pub fn common_block_mut(&mut self, pb: &SharedPB) -> Option<&mut CommonBlock> {
        pb.0.read()
            .unwrap()
            .common_block_idx
            .and_then(|idx| self.common_blocks.get_mut(idx))
    }
    ...
    pub fn detect_shared_topology(&mut self, tolerance: f64) -> &SharedTopologyInfo {
        ...
        for fi_a in 0..self.a_face_count() {
            for fi_b in self.a_face_count()..self.faces.len() {
                ...
                let partial_overlap = !full_overlap && self.faces_share_edges(fi_a, fi_b, face_tol);
    ...
    /// Phase 4: removed 鈥?wire/shell/solid images no longer stored on DS.
    ...
    pub fn release_pave_blocks(&mut self) {
        for ei in 0..self.edges.len() {
            ...
            let has_cb = self
                .edge_pave_blocks(ei)
                .iter()
                .any(|spb| spb.0.read().unwrap().common_block_idx.is_some());
            if has_cb {
                continue;
            }
            ...
            !on_set.iter().any(|&on_idx| {
                pave_blocks.get(on_idx).map_or(false, |on_pb| {
                    on_pb.0.read().unwrap().original_edge == pb.0.read().unwrap().original_edge
                        && on_pb.0.read().unwrap().pave1.vertex_idx
                            == pb.0.read().unwrap().pave1.vertex_idx
                        && on_pb.0.read().unwrap().pave2.vertex_idx
                            == pb.0.read().unwrap().pave2.vertex_idx
                })
            })
        });
    }
    ...
    /// 闁?Clear+recompute, GetSameDomainIndex on all vertices.
    pub fn update_face_info_in(&mut self, fi: usize) {
        ...
        let boundary_copy: Vec<usize> = self.faces[fi]
            .boundary_verts
            .iter()
            .map(|&vi| self.get_same_domain_index(vi))
            .collect();

        let vf_vertices: Vec<usize> = self
            .interf_vf
            .iter()
        ...
        let ef_new_vertices: Vec<usize> = self
            .interf_ef
            .iter()
            .filter(|inf| {
                inf.face == fi
        ...
        let ff_curve_endpoints: Vec<usize> = self
            .interf_ff
            .iter()
            ...
            .collect();
        let ff_curve_endpoints: Vec<usize> = ff_curve_endpoints
            .into_iter()
            .map(|vi| self.get_same_domain_index(vi))
            .collect();
    ...
    pub fn update_face_info_on(&mut self, fi: usize) {
        ...
        let boundary_verts: Vec<usize> = self.faces[fi]
            .boundary_verts
            .iter()
            .map(|&vi| self.get_same_domain_index(vi))
            .collect();
        ...
        for &vi in &edge_pb_vertices {
            info.vertices_on.insert(vi);
        }
    ...
    pub fn update_pave_blocks_with_sd_vertices(&mut self) {
        ...
        // For each entry (source 鈫?sd_vertex), replace source with sd_vertex
        // in all PaveBlocks.  There is never a reverse entry (sd_vertex 鈫?source).
        //
        ...
        // Then only insert source 鈫?target (never target 鈫?source).
        ...
        for &(a, b) in &sd_pairs {
            ...
            // Only insert source 鈫?target (the replacement direction)
    ...
    pub fn sub_shapes_on_in(
        ...
        let process_map =
            |pb_set: &indexmap::IndexSet<usize>,
        ...
        for &pb_idx in &a_face_info1.pave_blocks_on {
            if a_face_info2.pave_blocks_on.contains(&pb_idx)
                || a_face_info2.pave_blocks_in.contains(&pb_idx)
            {
    ...
    pub fn shared_edges(
        ...
        let mut a_first_face_edges: std::collections::HashSet<usize> =
...
/// Build a `BoxTree` from all DS faces of the given origin.
///
/// OCCT: `BOPDS_Iterator::Initialize` builds BVH over source shape bounding boxes.
pub fn build_face_bvh(ds: &DS, origin: ShapeOrigin) -> Option<BoxTree> {
    let faces: Vec<usize> = ds
        .faces
        .iter()
        .enumerate()
    ...
    Some(BoxTree::build(indices, aabbs))
}

/// Compute the AABB of a DS face from its boundary vertices and surface type.
///
/// OCCT: `BRepBndLib::Add(aF, aBox)` adds face boundary vertices and analytical
/// surface bounds for curved types.
pub fn face_aabb(ds: &DS, fi: usize) -> Aabb {
    ...
    // Surface bounds via BndLib_AddSurface per-type dispatch.
