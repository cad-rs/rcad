fn clip_one_coplanar_pair(
    ...
    // Skip curved (non-plane) faces 閳?a cylinder wall classified with an
    // axis-aligned face normal (e.g. after boolean On閳ユ吅ace dedup) must not
    // be treated as a planar face.  Clipping its 2D axis projection corrupts
    // face_surface_range and destroys the curved閳ユ唶urface sub閳ユ吅ace.
    for &f in &[fi, fj] {
    ...
    // 閳光偓閳光偓 Read phase (shared borrows only) 閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓
...

    {
        ...
        // Skip strict bbox subset 閳?already handled by remove_axis_coplanar_redundant_child_faces.
        let scale = (bi.1 - bi.0)
            .abs()
            .max(bi.3 - bi.2)
            .abs()
        ...
        if subset(bi, bj) && subset(bj, bi) {
            // Equal bboxes 閳?also handled by the subset pass.
            return None;
        }
    ...
    // 閳光偓閳光偓 Compute 2D polygon intersection 閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓
    // SH expects the clip polygon to be CCW (left-of-edge = inside).  Faces with negative
    // normals (e.g. 閳妬) project to CW in world-axis UV, so we normalise the clip polygon.
    ...
    // 閳光偓閳光偓 Write phase: create face from overlap polygon 閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓
