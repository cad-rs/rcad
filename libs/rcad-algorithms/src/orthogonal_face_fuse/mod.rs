...
/// as 閳ユ笅nner duplicate rectangle vs outer trimmed cap閳?(`bcommon_simple/C8`). Trimmed intersection
...
/// an untrimmed `1鑴?` top patch is kept alongside the true trimmed cap; they do not pass
/// [`rects_2d_bbox_positive_area_overlap`] for orthogonal fuse, but the smaller bbox lies inside
/// the larger).
///
/// Only considers faces with **no inner wires** and normals snapped to 鍗/鍗/鍗. Returns the
...
fn try_pick_redundant_axis_coplanar_face(
    ...
    // Both faces must be on planar surfaces 閳?a cylindrical wall whose normal happens
    // to snap to an axis (e.g. (-0,-1,0) 閳?-Y) would be falsely matched with a planar
    // face on the same axis, causing incorrect removal of valid geometry (H8 box閳兘yl).
...
///
/// Only considers faces whose normals snap to 鍗/鍗/鍗 and whose [`face_surface_area`] fills their
...
/// deterministic. Intended only for post-processing **plane閳ユ悕lane intersections** (callers gate).
pub fn remove_spurious_intersection_face_preserving_volume(
    ...
    for flat_rm in 0..n {
        ...
        // removing it would leave a hole 閳?it is structurally necessary even if the
        // divergence-theorem volume contribution happens to be zero (e.g. a face at x=0
        // with normal (-1,0,0) where r璺痭 = 0 for all points).
        // Only planar faces are counted 閳?curved faces (e.g. cylinder wall) whose normals
        // happen to snap to an axis are excluded to prevent false plane matches (H8 box閳兘yl).
...
fn fuse_orthogonal_in_shell(brep: &mut BRep, si: usize, shi: usize, tol: f64, total: &mut usize) {
    ...
    loop {
        ...
        for fi in 0..n {
            ...
            // 鈥?the orthogonal coplanar fuse projects faces onto a plane, which
        ...
        // components that overlap with *positive* area. A second 閳ユ竼dge only閳?pass was tried and
...
/// Corner-only (`wu`閳? and `wv`閳?) and separated rectangles are excluded.
...
/// True when the two faces share a full edge (閳? coincident vertex indices, or the same segment
/// within `geom_tol` 閳?booleans often duplicate vertex indices on seams).
...
fn try_fuse_one_axis_aligned_edge_adjacent_pair(brep: &mut BRep, si: usize, shi: usize, tol: f64) -> bool {
    ...
    for fi in 0..n {
        ...
        for fj in (fi + 1)..n {
            ...
            // Skip non-Plane faces (BSpline, etc.) 鈥?only merge axis-aligned
...
/// Physical face area divided by axis-aligned UV bbox area for 鍗xis planes (same projection as
/// [`face_axis_world_bbox`]). Near 1 閳?patch fills its bbox rectangle (typical redundant caps).
...
/// Non閳ユ彸xis-aligned planes keep a single bucket (previous behavior).
...
/// If `n` is within `tol_dir` of an axis, snap to exact 鍗/鍗/鍗.
/// When `n` is 鍗/鍗/鍗, return the two world axis indices that span the plane (e.g. Z铏?閳?(x,y)).
...
/// Map `n璺痻 = d` to a canonical `n` so that `(n, d)` and `(-n, -d)` (same
...
/// Union of axis-aligned rectangles 閳?outer ring first, then hole rings (UV coords).
...
fn replace_shell_faces_and_geom(
    ...
    // are pushed right rather than left-shifted and then padded at the end 閳?
    // the old resize path caused face_surface_range entries to drift by one
    // position (cylinder wall UV domain 閳?planar face, H8 box 閳?cylinder).
...
/// Collapse 180鎺?vertices on a closed UV ring from [`union_rects_to_rings_grid`].
...
/// This removes 180鎺?vertices until only corners remain. A true merged rectangle has 4; an L
...
// 閳光偓閳光偓 Coplanar overlap clipping for Intersection results 閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓閳光偓

/// Ensure a 2D polygon has counter閳ユ叾lockwise winding (SH expects CCW for the clip polygon).
/// Sutherland閳ユ厰odgman's `is_inside` uses left閳ユ唭f閳ユ吂dge as "inside", which requires the clip
/// polygon to be CCW.  Faces with negative normals (e.g. 閳妬) produce CW UV projections, so
...
///
/// This pass finds such pairs, computes the 2D polygon intersection via Sutherland閳ユ弻odgman,
...
pub fn clip_coplanar_overlap_for_intersection(brep: &BRep, a: &BRep, b: &BRep, tol: f64) -> (BRep, usize) {
    ...
    // Phase 1: pair-based clipping 閳?find coplanar face pairs in the result and
    ...
    // as "Out"), that pair is invisible to Phase 1 閳?the surviving face is too large.
    ...
    // Pre-scan is no longer needed 閳?the area check (sa_before / max_input_area < 0.97)
        ...
        for shi in 0..out.solids[si].shells.len() {
            ...
            while fi < out.solids[si].shells[shi].faces.len() {
                ...
                let (face, normal, _axes, point, pk, poly_uv, sa_before) = {
                    ...
                    // Skip curved (non-plane) faces 閳?the axis-aligned normal check
...
/// 3. Mutate `brep` 閳?no borrow conflicts because the "read" borrows are dropped.
