...
/// as = nner duplicate rectangle vs outer trimmed cap=(`bcommon_simple/C8`). Trimmed intersection
...
/// an untrimmed `1 ?` top patch is kept alongside the true trimmed cap; they do not pass
/// [`rects_2d_bbox_positive_area_overlap`] for orthogonal fuse, but the smaller bbox lies inside
/// the larger).
///
/// Only considers faces with **no inner wires** and normals snapped to  / / . Returns the
...
fn try_pick_redundant_axis_coplanar_face(
 ...
 // face on the same axis, causing incorrect removal of valid geometry (H8 box= yl).
...
///
/// Only considers faces whose normals snap to  / /  and whose [`face_surface_area`] fills their
...
/// deterministic. Intended only for post-processing **plane= lane intersections** (callers gate).
pub fn remove_spurious_intersection_face_preserving_volume(
 ...
 // with normal (-1,0,0) where r  = 0 for all points).
 // Only planar faces are counted =curved faces (e.g. cylinder wall) whose normals
 // happen to snap to an axis are excluded to prevent false plane matches (H8 box= yl).
...
fn fuse_orthogonal_in_shell(brep: &mut BRep, si: usize, shi: usize, tol: f64, total: &mut usize) {
 ...
 // components that overlap with *positive* area. A second = dge only=pass was tried and
...
/// Corner-only (`wu`= and `wv`=) and separated rectangles are excluded.
...
/// True when the two faces share a full edge (= coincident vertex indices, or the same segment
...
/// Physical face area divided by axis-aligned UV bbox area for  xis planes (same projection as
...
/// Non= xis-aligned planes keep a single bucket (previous behavior).
...
/// If `n` is within `tol_dir` of an axis, snap to exact  / / .
/// When `n` is  / / , return the two world axis indices that span the plane (e.g. Z ?=(x,y)).
...
/// Map `n  = d` to a canonical `n` so that `(n, d)` and `(-n, -d)` (same
...
/// Ensure a 2D polygon has counter= lockwise winding (SH expects CCW for the clip polygon).
/// Sutherland= odgman's `is_inside` uses left= f= dge as "inside", which requires the clip
/// polygon to be CCW.  Faces with negative normals (e.g. = ) produce CW UV projections, so
...
///
/// This pass finds such pairs, computes the 2D polygon intersection via Sutherland= odgman,
