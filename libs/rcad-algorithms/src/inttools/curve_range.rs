...
///
/// Compute a valid (shrunk) parameter range on a curve by offsetting from
/// vertex tolerance spheres, then clamping to the edge range.
///
/// Starting from each endpoint, find the nearest point on the curve that
/// lies *outside* the corresponding vertex tolerance sphere. The range
/// between those two outside points is the "valid range" where the curve
/// is free of vertex proximity influence.
///
/// # Arguments
/// * `curve` - The 3D curve (maps from `Adaptor3d_Curve`)
/// * `edge_tol` - Edge tolerance (`theTolE`)
/// * `par_v1` - First parameter of the edge range (`theParV1`)
/// * `pnt_v1` - First vertex position (`thePntV1`, maps from `gp_Pnt`)
/// * `tol_v1` - Tolerance at the first vertex (`theTolV1`)
/// * `par_v2` - Last parameter of the edge range (`theParV2`)
/// * `pnt_v2` - Second vertex position (`thePntV2`, maps from `gp_Pnt`)
/// * `tol_v2` - Tolerance at the second vertex (`theTolV2`)
///
/// # Returns
/// * `Some([valid_first, valid_last])` — the valid parameter range (first < last)
/// * `None` — no valid range (micro-edge or overlapping tolerance spheres)
pub fn find_valid_range(
    ...
    // OCCT L184-187: if theParV2 - theParV1 < Precision::PConfusion() → micro-edge
    if par_v2 - par_v1 < PCONFUSION {
        return None;
    }

    // OCCT L189: Precision::IsInfinite checks
    let is_inf_par_v1 = is_infinite_value(par_v1);
    let is_inf_par_v2 = is_infinite_value(par_v2);

    // OCCT L191-199: compute aMaxPar = max(|par_v1|, |par_v2|, 0)
    let a_max_par = {
        let mut m = 0.0;
        if !is_inf_par_v1 {
            m = par_v1.abs();
    ...
    // OCCT L201-202: anEps = max(curve.Resolution(theTolE) * 0.1, Epsilon(aMaxPar), PConfusion())
    let an_eps = {
        let res = curve_resolution_global(curve, par_v1, par_v2, edge_tol) * 0.1;
        let eps = parametric_epsilon(a_max_par);
        res.max(eps).max(PCONFUSION)
    };

    // OCCT L204-225: find theFirst (offset from first vertex)
    let valid_first = if is_inf_par_v1 {
        par_v1
    } else {
        match find_nearest_valid_point(curve, par_v1, par_v2, true, pnt_v1, tol_v1, an_eps) {
            Some(val) => {
                // OCCT L221-224: if theParV2 - theFirst < anEps → not enough room
                if par_v2 - val < an_eps {
                    return None;
                }
                val
            }
    ...
    // OCCT L227-248: find theLast (offset from second vertex)
    let valid_last = if is_inf_par_v2 {
        par_v2
    } else {
        match find_nearest_valid_point(curve, par_v1, par_v2, false, pnt_v2, tol_v2, an_eps) {
            Some(val) => {
                // OCCT L244-247: if theLast - theParV1 < anEps → not enough room
                if val - par_v1 < an_eps {
                    return None;
                }
                val
            }
    ...
    // OCCT L250-255: check found parameters — overlapping, not valid
    if valid_first > valid_last {
        return None;
    }

    Some([valid_first, valid_last])
}
