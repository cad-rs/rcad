...
mod tests {
 ...
 assert!(pts.len() >= 4, "polyline should have = points, got {}", pts.len());
 }
 }
 ...
 // Cone2: apex (0,0,0), axis Z, 30 ?(tan=1/=)
 // Coaxial =circle at h = =+1 =2.732 from cone1 apex
 ...
 // The torus tube is at ( ?5) ?+ z ?= 4; sphere is + z ?= 25.
 // Substituting: + 4 - ( ?5) ?= 25 =10 ?- 21 = 25 = ?= 4.6.
 // z ?= 25 - 4.6 ?= 3.84 =z =  ?.96 =two circles.
 ...
 assert!(pts.len() >= 4, "polyline should have = points");
 }
 other => {
 ...
 // Cone: apex=origin, axis=Z, 45 ?( ?z).
 // Substituting  ?z into ( ?5) ?z ?16:
 // 2z ?- 10z + 9 = 0 =z = (10 ら ?8)/4 ={3.82, 1.18} =two circles.
 let torus = Surface3::Torus(rcad_kernel::geom::ToroidalSurface {
 center: DVec3::ZERO,
 axis: DVec3::Z,
 major_radius: 5.0,
 ...
 // Coaxial, offset =circles where tube circles intersect in ( ?z) plane.
