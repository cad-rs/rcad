...
fn bnd_add_3d_curve(curve: &BRepAdaptorCurve, first: f64, last: f64, tol: f64) -> BndBox {
    ...
    crate::bnd_lib::add_curve_to_box(curve.curve(), first, last, &mut bbox, tol);
    bbox_to_bndbox(&bbox)
}

fn bnd_add_3d_curve_raw(curve: &Curve3, first: f64, last: f64, tol: f64) -> BndBox {
    let mut bbox = crate::brep_bnd::BoundingBox::new();
    crate::bnd_lib::add_curve_to_box(curve, first, last, &mut bbox, tol);
    bbox_to_bndbox(&bbox)
}
...
fn bbox_to_bndbox(bbox: &crate::brep_bnd::BoundingBox) -> BndBox {
...
mod tests {
    ...
    fn test_curve_resolution() {
        ...
        let res = crate::inttools::curve_range::curve_resolution(&curve, 0.0, 1e-7);
        assert!(res > 0.0);
    }
