...
use crate::tolerance::*;
use glam::DVec3;
use rcad_kernel::geom::{CurveEval, SurfaceEval, any_perpendicular};
    ...
    for &edge_idx in edge_indices {
        ...
        if let Some(curve) = &ed.curve {
            ...
            for i in 0..samples_per_edge {
                let t =
                    range[0] + (range[1] - range[0]) * (i as f64) / (samples_per_edge - 1) as f64;
...
pub fn find_surface_through_points(
    ...
    if let Some(plane) = fit_plane_to_points(points)
        && plane.rms_error < best_error
        && plane.rms_error < scale_tol
    {
    ...
    if points.len() >= 4
        ...
        && sphere.rms_error < best_error
        && sphere.rms_error < scale_tol
    {
    ...
    if points.len() >= 5
        ...
        && cylinder.rms_error < best_error
        && cylinder.rms_error < scale_tol
    {
...
fn fit_sphere_to_points(points: &[DVec3]) -> Option<FittedSphere> {
    ...
    let radius =
        (coeffs[0] * coeffs[0] + coeffs[1] * coeffs[1] + coeffs[2] * coeffs[2] + coeffs[3])
            .sqrt()
...
fn fit_circle_2d(points: &[(f64, f64)]) -> Option<(f64, f64, f64)> {
    ...
    let cx = coeffs[0];
    let cy = coeffs[1];
    let radius = (cx * cx + cy * cy + coeffs[2])
        .sqrt()
        .max(TOLERANCE_LINEAR_ULTRA_STRICT);
...
pub fn faces_share_surface(
    brep: &rcad_kernel::BRep,
    face1_idx: usize,
    face2_idx: usize,
) -> Result<bool, BRepLibError> {
    let faces: Vec<&Surface3> = brep
        .tshapes
        .iter()
        .filter_map(|ts| {
    ...
    match (s1, s2) {
        ...
        (Surface3::Cylinder(c1), Surface3::Cylinder(c2)) => {
            let axis_dot = c1
                ...
                .dot(c2.axis.normalize_or(DVec3::Z))
                .abs();
...
pub fn add_face_with_surface(
    ...
    if wires.is_empty() {
        return Err(BRepLibError::InvalidWire(
            "Face must have at least one wire".into(),
        ));
    }
...
pub fn compute_edge_bounds(
    brep: &rcad_kernel::BRep,
    edge_idx: usize,
) -> Result<[f64; 2], BRepLibError> {
    if edge_idx >= brep.edge_count() {
        return Err(BRepLibError::InvalidIndex {
            kind: "edge",
            index: edge_idx,
...
pub fn compute_face_bounds(
    brep: &rcad_kernel::BRep,
    face_idx: usize,
) -> Result<[f64; 4], BRepLibError> {
    // Collect face surfaces in flat order (iterate tshapes for TShape::Face)
    let face_surfaces: Vec<Option<[f64; 4]>> = brep
        .tshapes
        .iter()
        .filter_map(|ts| {
            if let TShape::Face(fd) = &**ts {
    ...
    let faces: Vec<Option<&Surface3>> = brep
        .tshapes
        .iter()
        .filter_map(|ts| {
            if let TShape::Face(fd) = &**ts {
                Some(fd.surface.as_ref())
            } else {
                None
            }
        })
