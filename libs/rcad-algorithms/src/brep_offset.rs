...
use rcad_kernel::{
 ...
 topods::{TShape, Orientation, ShapeRef},
...
impl<'a> MakeOffset<'a> {
 ...
 let first_ed = match &*brep.tshapes[wire.edges[0].idx] { TShape::Edge(ed) => ed, _ => unreachable!() };
 let last_ed = match &*brep.tshapes[wire.edges.last().unwrap().idx] { TShape::Edge(ed) => ed, _ => unreachable!() };
 ...
 let ed = match &*self.brep.tshapes[we.idx] { TShape::Edge(ed) => ed, _ => unreachable!() };
 ...
 // Get vertex position
 let vertex_idx = if !we.forward { ed.last.index } else { ed.first.index };
 ...
 let last_ed = match &*self.brep.tshapes[last_we.idx] { TShape::Edge(ed) => ed, _ => unreachable!() };
 let vertex_idx = if !last_we.forward { last_ed.first.index } else { last_ed.last.index };
 let vertex_pos = self.brep.vertex_point(vertex_idx).unwrap();

 // Compute offset for last vertex
 let prev_we = &self.wire.edges[self.wire.edges.len() - 1];
 let _prev_ed = match &*self.brep.tshapes[prev_we.idx] { TShape::Edge(ed) => ed, _ => unreachable!() };
 ...
 let ed = match &*self.brep.tshapes[edge_idx] { TShape::Edge(ed) => ed, _ => unreachable!() };
...
impl<'a> MakePipeShell<'a> {
 ...
 .map(|we| {
 let ed = match &*self.brep.tshapes[we.idx] { TShape::Edge(ed) => ed, _ => unreachable!() };
 let idx = if we.forward { ed.first.index } else { ed.last.index };
 ...
 // Create quad face
 if let Some(fr) = self.create_quad_face(
 &mut result_brep,
 v00, v01, v11, v10,
 ) {
 ...
 edge_srs.push(ShapeRef::synthetic_with_orientation(edge_idx, Orientation::Forward));
 }
 ...
 fn create_cap_face(&self, result_brep: &mut rcad_kernel::BRep, section_verts: &[usize]) -> Option<ShapeRef> {
 ...
 .iter()
 .fold(DVec3::ZERO, |acc, &v| acc + result_brep.vertex_point(v).unwrap())
...
fn add_face(brep: &mut rcad_kernel::BRep, surface: Surface3, outer: Wire, inner: Vec<Wire>) -> usize {
 ...
 let face_sr = brep.add_tface(Some(surface), outer_sr, inner_srs, None, None, Vec::new(), true);
 face_sr.index
}
...
fn wire_to_shape_ref(brep: &mut rcad_kernel::BRep, wire: &Wire) -> ShapeRef {
 let edge_srs: Vec<ShapeRef> = wire.edges.iter().map(|we| {
...
fn extract_wire(brep: &rcad_kernel::BRep, wire_sr: ShapeRef) -> Wire {
 let wd = match &*brep.tshapes[wire_sr.index] { TShape::Wire(wd) => wd, _ => unreachable!() };
 Wire {
 edges: wd.edges.iter().map(|&e| WireEdge::new(e.index, e.orientation == Orientation::Forward)).collect(),
 }
}
...
fn extract_face_from_sr(brep: &rcad_kernel::BRep, face_sr: ShapeRef) -> Face {
 let fd = match &*brep.tshapes[face_sr.index] { TShape::Face(fd) => fd, _ => unreachable!() };
 let outer_wire = extract_wire(brep, fd.outer_wire);
 let inner_wires: Vec<Wire> = fd.inner_wires.iter().map(|&iw| extract_wire(brep, iw)).collect();
...
fn extract_solid(brep: &rcad_kernel::BRep) -> Option<Solid> {
 ...
 if let TShape::Solid(sd) = ts.as_ref() { Some((i, sd)) } else { None }
 })?;

 let mut shells = Vec::new();
 for shell_sr in &sd.shells {
 let shd = match &*brep.tshapes[shell_sr.index] { TShape::Shell(shd) => shd, _ => continue };
...
fn first_shell_from_brep(brep: &rcad_kernel::BRep) -> Option<(Shell, usize)> {
 ...
 let shd = match &*brep.tshapes[shell_sr.index] { TShape::Shell(shd) => shd, _ => return None };
 let mut faces = Vec::new();
 for face_sr in &shd.faces {
 faces.push(extract_face_from_sr(brep, *face_sr));
 }
