    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::InvalidDraftAngle { angle_rad } => {
                write!(
                    f,
                    "draft angle {:.1} degrees is out of valid range (-89, 89)",
...
fn build_prism_from_sections(
    ...
    let n = bot.len();
    if n < 3 || top.len() != n {
        return Err(BRepFeatError::InvalidInput(
            "section vertex count mismatch or too few vertices".to_string(),
        ));
    }
    ...
    for i in 0..n {
        ...
        let face_normal = {
            ...
            if nv.length_squared() > TOLERANCE_VEC_SQ_MIN {
                ...
                -dir.cross(ab).normalize_or(DVec3::X)
            }
        };
...
fn build_polygon_face_brep(profile_verts: &[DVec3]) -> Result<topods::BRep, BRepFeatError> {
    if profile_verts.len() < 3 {
        return Err(BRepFeatError::InvalidInput(
            "profile needs >= 3 vertices".to_string(),
        ));
    }
    ...
    let normal = {
        ...
        if n_vec.length_squared() <= EPS {
            return Err(BRepFeatError::InvalidInput(
                "profile vertices are degenerate".to_string(),
            ));
        }
        n_vec.normalize()
    };
    ...
    let face_sr = brep.add_tface(
        Some(Surface3::Plane(Plane::new(
            profile_verts[0],
            profile_verts[0].normalize_or_zero(),
        ))),
        wire_sr,
        vec![],
        None,
...
pub fn make_rib(
    ...
    if profile_wire.len() < 3 {
        return Err(BRepFeatError::InvalidProfile(
            "profile needs >= 3 vertices".to_string(),
        ));
    }
    ...
    let profile_offset_neg: Vec<DVec3> = profile_wire
        .iter()
        .map(|&p| p - profile_normal * half_thickness)
        .collect();
    let profile_offset_pos: Vec<DVec3> = profile_wire
        .iter()
        .map(|&p| p + profile_normal * half_thickness)
        .collect();
...
pub fn make_prism_feature(
    ...
    let top: Vec<DVec3> = bottom
        .iter()
        .map(|&p| p + dir * max_h.abs().max(1.0))
        .collect();
...
pub fn make_pipe_feature(
    ...
    if spine.len() < 2 {
        return Err(BRepFeatError::InvalidInput(
            "spine needs >= 2 vertices".to_string(),
        ));
    }
...
fn build_pipe_solid(
    ...
    if spine.len() < 2 {
        return Err(BRepFeatError::InvalidInput(
            "spine needs at least 2 points".to_string(),
        ));
    }
    ...
    let sections: Vec<Vec<DVec3>> = frames
        .iter()
        .map(|(origin, right, up)| {
            profile
                .iter()
                .map(|&p| origin + right * p.x + up * p.y)
                .collect()
        })
        .collect();
...
pub(crate) fn build_loft_solid(
    sections: &[Vec<DVec3>],
) -> Result<rcad_kernel::BRep, BRepFeatError> {
    if sections.len() < 2 {
        return Err(BRepFeatError::InvalidInput(
            "need at least 2 sections for loft".to_string(),
        ));
    }
    ...
    if n < 3 {
        return Err(BRepFeatError::InvalidInput(
            "each section needs at least 3 vertices".to_string(),
        ));
    }
            ...
            .filter(|(_, face_sr)| {
                if let Some(wire_edges) = face_outer_wire_edges(target, face_sr) {
                    ...
                    if let Some(edge_idx) = wire_edges.first().map(|e| e.index) {
                        if let Some(TShape::Edge(ed)) =
                            target.tshapes.get(edge_idx).map(|ts| ts.as_ref())
                        {
                        ...
                        for &vi in &[vi_start, vi_end] {
                            if vi < vcount {
                                if let Some(v) = target.vertex_point(vi) {
                                    ...
                                    let radial_dir = (v - neutral_plane)
                                        .reject_from(pull_dir)
                        ...
                        for &vi in &[vi_start, vi_end] {
                            if vi < vcount {
                                if let Some(v0) = target.vertex_point(vi) {
                                    let disp = draft_vertex_displacement_occt(
                                        v0,
                                        neutral_point,
                                        n,
                                        pull,
...
pub fn make_drafted_prism(
    ...
    if draft_angle.abs() >= std::f64::consts::FRAC_PI_2 - TOLERANCE_MESH_LEGACY {
        return Err(BRepFeatError::InvalidDraftAngle {
            angle_rad: draft_angle,
        });
    }
...
pub fn make_loft_feature(
    ...
    if profiles.len() < 2 {
        return Err(BRepFeatError::InvalidInput(
            "need at least 2 profiles for loft".to_string(),
        ));
    }
