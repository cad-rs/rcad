    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::InsufficientVertices { minimum, actual } => {
                write!(
                    f,
                    "profile needs at least {} vertices, got {}",
                    minimum, actual
                )
            }
            ...
            Self::VertexCountMismatch { expected, actual } => {
                write!(
                    f,
                    "profile vertex count mismatch: expected {}, got {}",
                    expected, actual
                )
            }
...
pub fn linear_sweep_with_options(
    ...
    let start_vi: Vec<usize> = profile_pts
        ...
        .collect();
    let end_vi: Vec<usize> = end_pts.iter().map(|&p| add_vertex(&mut brep, p)).collect();
        ...
        wd.edges
            ...
            .filter_map(|edge_sr| {
                ...
                if let TShape::Edge(ed) = &**edge_ts {
                    let vi = if edge_sr.orientation == Orientation::Forward {
                        ed.first.index
                    } else {
                        ed.last.index
                    };
...
pub fn rotational_sweep_with_options(
    ...
    let end_vi: Vec<usize> = if full_revolution {
        ...
        rot_pts.iter().map(|&p| add_vertex(&mut brep, p)).collect()
    };
    ...
    if !full_revolution {
        ...
        // End cap
        {
            ...
            let rot_normal =
                rotate_point(profile_normal, axis_origin, axis_dir, angle) - axis_origin;
...
pub fn pipe_sweep_with_options(
    ...
    let cross_sections: Vec<Vec<DVec3>> = spine
        .iter()
        .enumerate()
        .map(|(i, &origin)| {
            let frame = &frames[i];
            profile_2d
                .iter()
                .map(|p2| origin + p2.x * frame.right + p2.y * frame.up)
                .collect()
        })
        .collect();
...
fn compute_spine_tangents(spine: &[DVec3]) -> Result<Vec<DVec3>, SweepError> {
    ...
    for i in 0..n {
        ...
        if tan.length_squared() < EPS {
            return Err(SweepError::DegenerateGeometry(
                "spine has zero-length segment",
            ));
        }
        tangents.push(tan);
    }
...
fn compute_frenet_frames(
    tangents: &[DVec3],
    is_frenet: bool,
) -> Result<Vec<FrenetFrame>, SweepError> {
    let n = tangents.len();
        ...
        let (right, up) = if is_frenet && i > 0 && prev_up.length_squared() > EPS {
            ...
            if right.length_squared() < EPS {
                ...
                if right_raw.length_squared() > EPS {
                    (
                        right_raw.normalize(),
                        right_raw.normalize().cross(tan).normalize_or_zero(),
                    )
                } else {
...
fn build_lofted_solid(
    ...
    // Add all vertices
    let vi: Vec<Vec<usize>> = sections
        .iter()
        .map(|sec| sec.iter().map(|&p| add_vertex(&mut brep, p)).collect())
        .collect();
...
impl Law for SineLaw {
    fn evaluate(&self, t: f64) -> f64 {
        self.offset
            + self.amplitude * (self.frequency * t * std::f64::consts::TAU + self.phase).sin()
    }
}
...
pub fn linear_law_sweep<L: Law>(
    ...
    let cross_sections: Vec<Vec<DVec3>> = spine
        ...
        .map(|(i, &origin)| {
            ...
            profile_2d
                .iter()
                .map(|p2| origin + scale * p2.x * frame.right + scale * p2.y * frame.up)
                .collect()
        })
        .collect();
