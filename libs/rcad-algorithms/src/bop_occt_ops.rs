    ...
    for e in &ds.edges {
        ...
        for pb in &e.pave_blocks {
            if pb.0.read().unwrap().original_edge != NO_EDGE
                && pb.0.read().unwrap().original_edge >= ne
            {
                return Err(BooleanError::InvalidResult(
                    "bop: DS pave_block original_edge out of range",
                ));
            }
            if pb.0.read().unwrap().pave1.vertex_idx >= nv
                || pb.0.read().unwrap().pave2.vertex_idx >= nv
            {
                return Err(BooleanError::InvalidResult(
...
fn optional_bvhs(a: &topods::BRep, b: &topods::BRep) -> (Option<bvh::Bvh>, Option<bvh::Bvh>) {
    let has_faces = |brep: &topods::BRep| -> bool {
        brep.tshapes
            .iter()
            .any(|ts| matches!(ts.as_ref(), topods::TShape::Face(_)))
    };
    (
        if has_faces(a) {
            Some(build_topods_bvh(a))
        } else {
            None
        },
        if has_faces(b) {
            Some(build_topods_bvh(b))
        } else {
            None
        },
    )
...
pub(crate) fn boolean_op_with_history_generic(
    ...
    let (mut ds, face_refs, ic_edge_map) = pave_fill(a, b, true, crate::tolerance::TOLERANCE_ABS);
...
pub(crate) fn fuse_with_bvh(
    a: &topods::BRep,
    b: &topods::BRep,
    use_bvh: bool,
) -> Result<topods::BRep, BooleanError> {
    let mut brep = rcad_kernel::topods::BRep::new();
    let (mut ds, face_refs, ic_edge_map) =
        pave_fill(a, b, use_bvh, crate::tolerance::TOLERANCE_ABS);
    let mut builder =
        builder::BooleanBuilder::with_brep(&ds, BooleanOpType::Union, brep, face_refs, ic_edge_map);
    let (result, _history) = builder.build_with_history_topods()?;
    Ok(result)
}
...
pub(crate) fn fuse_with_history(
    a: &topods::BRep,
    b: &topods::BRep,
) -> Result<(topods::BRep, BooleanHistory), BooleanError> {
...
pub(crate) fn fuse_with_history_par(
    a: &topods::BRep,
    b: &topods::BRep,
) -> Result<(topods::BRep, BooleanHistory), BooleanError> {
...
pub(crate) fn common_with_history_bvh(
    ...
    let (mut ds, face_refs, ic_edge_map) =
        pave_fill(a, b, use_bvh, crate::tolerance::TOLERANCE_ABS);
    validate_ds_invariants(&ds)?;
    let mut builder = builder::BooleanBuilder::with_brep(
        &ds,
...
pub(crate) fn common_with_history_par(
    a: &topods::BRep,
    b: &topods::BRep,
) -> Result<(topods::BRep, BooleanHistory), BooleanError> {
...
pub(crate) fn cut_with_history_par(
    a: &topods::BRep,
    b: &topods::BRep,
) -> Result<(topods::BRep, BooleanHistory), BooleanError> {
