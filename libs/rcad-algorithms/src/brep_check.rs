...
pub enum CheckIssue {
    ...
    ZeroNormal { solid: usize, shell: usize, face: usize },
    /// Face outer wire has fewer than 3 edges.
    DegenerateFace { solid: usize, shell: usize, face: usize },
...
pub fn check(brep: &BRep) -> CheckResult {
    ...
    for (eidx, edge) in brep.edges.iter().enumerate() {
        if edge.start >= n_verts {
            issues.push(CheckIssue::InvalidVertexIndex { edge: eidx, vertex_idx: edge.start });
        }
        if edge.end >= n_verts {
            issues.push(CheckIssue::InvalidVertexIndex { edge: eidx, vertex_idx: edge.end });
        }
    }
                ...
                for we in &wire.edges {
                    if we.idx >= n_edges {
                        issues.push(CheckIssue::InvalidEdgeIndex {
                            solid: si, shell: shi, face: fi, edge_idx: we.idx,
                        });
                        valid = false;
                    } else {
                    ...
                    if end_v != start_v {
                        ...
                        if (end_pt - start_pt).length() > 1e-6 {
                            issues.push(CheckIssue::OpenWire {
                                solid: si, shell: shi, face: fi, wire_pos: i,
                            });
                        }
                    }
                }
...
mod tests {
    ...
    fn unit_box_is_valid() {
        ...
        assert!(result.is_valid(), "unit box should pass all checks; issues: {:?}", result.issues);
    }
    ...
    fn open_wire_is_detected() {
        ...
        brep.vertices.push(Vertex { point: DVec3::new(0.0, 1.0, 0.0) }); // 3 (gap: wire goes 0→1→2 then 2→0 skips 3)
        ...
        assert!(result.issues.iter().any(|i| matches!(i, CheckIssue::OpenWire { .. })));
    }
    ...
    fn degenerate_face_is_detected() {
        ...
        let result = check(&brep);
        assert!(result.issues.iter().any(|i| matches!(i, CheckIssue::DegenerateFace { .. })));
    }
}
