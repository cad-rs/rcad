...
fn compute_plane_plane_fillet(
 ...
 // Rolling-ball centre offset from the edge: R = r / sin( /2)
 ...
 //
 // CylindricalSurface:
 // u = azimuth angle around the axis  [0, 2 ]
 // v = distance along the axis
 // The fillet occupies  u =  /2 (quarter-circle) and
 // v = edge length (along the axis).
...
fn compute_fillet_uv_domain(surface: &Surface3, edge_info: &EdgeInfo) -> [f64; 4] {
 match surface {
 Surface3::Cylinder(_) => {
 // Cylinder: u = azimuth around axis [0, 2 ], v = height along axis
 // For a plane-plane fillet the cross-section arc is  /2 (90 ).
...
fn apply_cylinder_fillet(
 ...
 let offset_dist = r / tan_half; // = r cot( /2)
 let r_centre = r / sin_half; // = r / sin( /2)
 ...
 // Take the shorter arc ( ? ).  Normalise so t_start < t_end.
 let mut t_start = t1;
 let mut t_end  = t2;
 ...
 //  € € Fillet face  € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € € €
 // Wire loop: contact_f1_fwd(V4' 5')  ?arc_v2_fwd(V5' 5'')
 // ?contact_f2_rev(V5'' 4'')  ?arc_v1_rev(V4'' 4')
