...
mod tests {
    ...
    // =====================================================================
    // Iso (AdvApp2Var_Iso_Test.cxx)
    // =====================================================================

    #[test]
    fn iso_constructor_with_orders() {
        let an_iso_u = Iso::new_with_orders(IsoType::IsoU, 3.0, 3, 1);
        assert_eq!(an_iso_u.iso_type(), IsoType::IsoU);
        assert_eq!(an_iso_u.u_order(), 3);
        assert_eq!(an_iso_u.v_order(), 1);
        assert!((an_iso_u.constante() - 3.0).abs() < 1e-12);
        assert!((an_iso_u.u0() - 0.0).abs() < 1e-12);
        assert!((an_iso_u.u1() - 1.0).abs() < 1e-12);
        assert!((an_iso_u.v0() - 0.0).abs() < 1e-12);
        assert!((an_iso_u.v1() - 1.0).abs() < 1e-12);
        assert!((an_iso_u.t0() - 0.0).abs() < 1e-12);
        assert!((an_iso_u.t1() - 1.0).abs() < 1e-12);
        assert_eq!(an_iso_u.position(), 0);

        let an_iso_v = Iso::new_with_orders(IsoType::IsoV, 3.0, 3, 1);
        assert_eq!(an_iso_v.iso_type(), IsoType::IsoV);
        assert_eq!(an_iso_v.u_order(), 3);
        assert_eq!(an_iso_v.v_order(), 1);
    }

    // =====================================================================
    // Node (AdvApp2Var_Node_Test.cxx)
    // =====================================================================

    #[test]
    fn node_constructor_initializes_values_to_zero() {
        let node = Node::new(DVec2::new(0.0, 0.0), 1, 2);
        for iu in 0..=1 {
            for iv in 0..=2 {
                ...
                assert!((node.error(iu, iv) - 0.0).abs() < 1e-12);
            }
        }
    }

    // =====================================================================
    // Network (AdvApp2Var_Network_Test.cxx)
    // =====================================================================

    #[test]
    fn network_update_in_v_preserves_patch_orders_per_column() {
        let patches = vec![
        ...
        assert!((net.v_parameter(2) - 0.5).abs() < 1e-12);

        let bl = net.patch(1, 1);
        let br = net.patch(2, 1);
        let tl = net.patch(1, 2);
        let tr = net.patch(2, 2);

        assert_eq!(bl.u_order(), 0); assert_eq!(bl.v_order(), 1);
        assert_eq!(br.u_order(), 2); assert_eq!(br.v_order(), 3);
        assert_eq!(tl.u_order(), 0); assert_eq!(tl.v_order(), 1);
        assert_eq!(tr.u_order(), 2); assert_eq!(tr.v_order(), 3);

        assert!((bl.v0() - 0.0).abs() < 1e-12);
        assert!((bl.v1() - 0.5).abs() < 1e-12);
        assert!((tl.v0() - 0.5).abs() < 1e-12);
        assert!((tl.v1() - 1.0).abs() < 1e-12);
    }

    #[test]
    fn network_update_in_u_preserves_patch_orders_per_row() {
        let patches = vec![
            Patch::new(0.0, 1.0, 0.0, 1.0, 0, 1),
            Patch::new(1.0, 2.0, 0.0, 1.0, 2, 3),
        ...
        assert_eq!(net.nb_patch_in_v(), 2);
        assert!((net.u_parameter(2) - 0.5).abs() < 1e-12);

        let bl  = net.patch(1, 1);
        let bm  = net.patch(2, 1);
        let br  = net.patch(3, 1);
        let tl  = net.patch(1, 2);
        let tm  = net.patch(2, 2);
        let tr  = net.patch(3, 2);

        assert_eq!(bl.u_order(), 0); assert_eq!(bl.v_order(), 1);
        assert_eq!(bm.u_order(), 0); assert_eq!(bm.v_order(), 1);
        assert_eq!(br.u_order(), 2); assert_eq!(br.v_order(), 3);

        assert_eq!(tl.u_order(), 4); assert_eq!(tl.v_order(), 5);
        assert_eq!(tm.u_order(), 4); assert_eq!(tm.v_order(), 5);
        assert_eq!(tr.u_order(), 6); assert_eq!(tr.v_order(), 7);

        assert!((bl.u0() - 0.0).abs() < 1e-12);
        assert!((bl.u1() - 0.5).abs() < 1e-12);
        assert!((bm.u0() - 0.5).abs() < 1e-12);
        assert!((bm.u1() - 1.0).abs() < 1e-12);
    }

    // =====================================================================
    // Framework (AdvApp2Var_Framework_Test.cxx)
    // =====================================================================

    fn make_ufrontier() -> Vec<Vec<Iso>> {
        let s1 = vec![
    ...
    #[test]
    fn framework_iso_lookup_returns_matching_iso() {
        let framework = Framework::new(make_node_grid(), make_ufrontier(), make_vfrontier());

        let an_iso_u = framework.iso_u(20.0, 0.0, 1.0);
        assert_eq!(an_iso_u.iso_type(), IsoType::IsoU);
        assert!((an_iso_u.constante() - 20.0).abs() < 1e-12);
        assert!((an_iso_u.t0() - 0.0).abs() < 1e-12);
        assert!((an_iso_u.t1() - 1.0).abs() < 1e-12);

        let an_iso_v = framework.iso_v(1.0, 2.0, 40.0);
        assert_eq!(an_iso_v.iso_type(), IsoType::IsoV);
        assert!((an_iso_v.constante() - 40.0).abs() < 1e-12);
        assert!((an_iso_v.t0() - 1.0).abs() < 1e-12);
        assert!((an_iso_v.t1() - 2.0).abs() < 1e-12);
    }

    // =====================================================================
    // Context (AdvApp2Var_Context_Test.cxx)
    // =====================================================================

    #[test]
    fn context_tolerances_are_aggregated() {
        let tol1d = vec![6.0];
        let tol2d = vec![12.0];
        let tol3d = vec![18.0];
        let tof1d = vec![vec![100.0; 4]; 1];
        let tof2d = vec![vec![100.0; 4]; 1];
        let tof3d = vec![vec![100.0; 4]; 1];

        let ctx = Context::new(0, 0, 0, 2, 2, 0, 1, 1, 1,
            &tol1d, &tol2d, &tol3d,
            &tof1d, &tof2d, &tof3d);
        ...
        let i_tol = ctx.i_toler();
        assert!((i_tol[0] - 3.0).abs() < 1e-12);
        assert!((i_tol[1] - 6.0).abs() < 1e-12);
        assert!((i_tol[2] - 9.0).abs() < 1e-12);
    }
}
