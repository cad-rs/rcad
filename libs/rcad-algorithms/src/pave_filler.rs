...
impl<'a> PaveFiller<'a> {
    ...
    pub fn perform(&mut self) {
        ...
        //    edge vertices share the same DS vertex.
        self.make_blocks();

        // ✅ OCCT-aligned: RefineFaceInfoIn — after MakeBlocks, remove On-overlapping In
        //    pave blocks (OCCT PaveFiller_6.cxx, called via D20/D21 in OCCT order).
        self.ds.refine_all_face_info();

        // ✅ OCCT-aligned: PutPavesOnCurve — inject block vertices onto IC curves
        //    (parameter sorting along curve + splitting).  After MakeBlocks + RefineFaceInfo.
