...
impl<'a> PaveFiller<'a> {
    ...
    pub fn perform(&mut self) {
        ...
        // ✅ OCCT-aligned: ForceInterfVE (PaveFiller_3.cxx, part of EE/EF force pass extended to VE)
        //    After EF force pass, vertices on a face with increased tolerance may now be
        //    within tolerance of boundary edges of that face. Check each face's vertices_in
        //    and vertices_on against all boundary edges.
        if !skip_ve && !skip_ef {
        ...
        //    After FF, vertices whose tolerance was increased during intersection may now
        //    be within tolerance of opposite-shape faces. Check all vertices against all
        //    opposite-shape faces.
        if !skip_vf {
            self.force_interf_vf();
        }
        ...
        // ✅ OCCT-aligned: split ICs at periodic boundary (seam) on sphere/cylinder faces.
        self.split_ics_at_periodic_boundary();

        // ✅ OCCT-aligned: MakeBlocks — inject EF/EE vertices onto FF curves (MakeBlocks L647+)
        //    OCCT PaveFiller_6.cxx L647-1200: place Pave vertices generated by EE/EF
        //    intersection onto FF intersection curves, so that IC endpoints and face boundary
        //    edge vertices share the same DS vertex, establishing full topology connectivity.
        //    rcad currently only does endpoint matching (full PutPavesOnCurve requires
        //    parameter sorting along curve + splitting).
        self.make_blocks();
        ...
        //    After MakeBlocks, validate that no self-interference exists within a
        //    single input shape. Self-interference means edges/faces from the same
        ...
        // ✅ OCCT-aligned: Clean up degenerate IC (start==end position coincide). OCCT IsValidBlockForFaces
        //    removes invalid blocks via mid-point projection, but rcad IC endpoints may coincide after EF replacement.
        //    ⚠️ Skip Circle curves: closed curve start==end is a natural geometric property (full circle,
        //    t_range=[0,TAU]), not degenerate. OCCT BOPTools_AlgoTools::MakeEdge creates different vertices
        //    at the two ends of closed curves, not treating them as degenerate. rcad's add_vertex dedup
        //    causes start and end vertex indices to be the same (same 3D position), but the curve itself is valid.
        ...
        //    OCCT L346: RemoveMicroEdges — after MakeBlocks, before MakePCurves,
        //    removes zero-length PaveBlocks (micro edges) where start==end.
        //    OCCT algorithm:
        //    1. L4239-4267: iterate all edges' PaveBlocks:
        //       a. L4242-4244: skip edges with <2 PaveBlocks (not split)
        //       b. L4246-4247: skip degenerate edges
        //       c. L4257-4265: get PaveBlock vertex indices; if nV1==nV2 and no valid
        //          ShrunkData → add to aMicroEdges
        ...
        //    split edges have been created.  The Builder's FillSameDomainFaces
        //    consumes these CommonBlocks to identify shared edges between coplanar
        //    faces.
