            ...
            for shell_sr in &sd.shells {
                if let rcad_kernel::topods::TShape::Shell(shd) = &*result.tshapes[shell_sr.index] {
                    for face_sr in &shd.faces {
                        if let rcad_kernel::topods::TShape::Face(fd) =
                            &*result.tshapes[face_sr.index]
                        {
                            // outer wire
                            if let rcad_kernel::topods::TShape::Wire(wd) =
                                &*result.tshapes[fd.outer_wire.index]
                            {
                                for e_sr in &wd.edges {
                                    if let rcad_kernel::topods::TShape::Edge(ed) =
                                        &*result.tshapes[e_sr.index]
                                    {
...
pub fn dump_result_topo(result: &rcad_kernel::BRep, label: &str) {
    ...
    eprintln!(
        "[TRACE] {}: verts_total={} verts_used={} edges={} faces={} shells={} solids={}",
...
pub fn dump_edge_stats(t: &rcad_kernel::topods::BRep, label: &str) {
    ...
    for ts in &t.tshapes {
        if matches!(ts.as_ref(), rcad_kernel::topods::TShape::Edge(_)) {
            n_tshape_edges += 1;
        }
    }
    let n_face_refs = count_edges_in_faces(t);
    eprintln!(
        "[TRACE] {}: tshape_edges={} edges_in_faces={}",
