...
impl<'a> PaveFiller<'a> {
    ...
    pub fn configure_glue_adaptive(&mut self, enable: bool, base_tolerance: f64, adaptive: bool) -> f64 {
        if !enable { self.glue = GlueEnum::GlueOff; return TOLERANCE_ABS; }
        self.glue = GlueEnum::GlueFull;
        if !adaptive { self.glue_tolerance = base_tolerance.max(TOLERANCE_ABS); return self.glue_tolerance; }
        let adaptive_tol = self.compute_adaptive_glue_tolerance(base_tolerance);
        self.glue_tolerance = adaptive_tol;
        adaptive_tol
    }

    pub fn configure_fuzzy(&mut self, fuzzy: f64) { self.fuzzy_tolerance = fuzzy.max(0.0); }
    pub fn set_run_parallel(&mut self, parallel: bool) { self.run_parallel = parallel; }
    pub fn set_non_destructive(&mut self, nd: bool) { self.non_destructive = nd; }
    pub fn set_non_destructive_auto(&mut self) { self.non_destructive = false; }
    pub fn set_use_obb(&mut self, use_obb: bool) { self.use_obb = use_obb; }
    pub fn set_section_attribute(&mut self, attr: SectionAttribute) { self.section_attribute = attr; }
    pub fn set_arguments(&mut self, args: Vec<rcad_kernel::topods::BRep>) { self.my_arguments = args; }
    pub fn arguments(&self) -> &[rcad_kernel::topods::BRep] { &self.my_arguments }
    pub fn set_avoid_build_pcurve(&mut self, flag: bool) { self.avoid_build_pcurve = flag; }
    pub fn is_avoid_build_pcurve(&self) -> bool { self.avoid_build_pcurve }
    pub fn effective_tolerance(&self, base: f64) -> f64 { base.max(self.fuzzy_tolerance) }
    ...
    fn compute_adaptive_glue_tolerance(&self, base_tolerance: f64) -> f64 {
        ...
        for edge in &self.ds.edges {
            ...
            if length > TOLERANCE_LINEAR_ULTRA_STRICT { min_edge_length = min_edge_length.min(length); }
        }
        for face in &self.ds.faces {
            ...
            if pts.len() >= 3 {
                let mut min_pt = pts[0]; let mut max_pt = pts[0];
                for p in &pts[1..] { min_pt = min_pt.min(*p); max_pt = max_pt.max(*p); }
                let diag = (max_pt - min_pt).length();
                if diag > TOLERANCE_LINEAR_ULTRA_STRICT { min_face_area = min_face_area.min(diag * diag); }
            }
        }
        let mut min_feature_size = f64::INFINITY;
        if min_edge_length.is_finite() { min_feature_size = min_feature_size.min(min_edge_length); }
        if min_face_area.is_finite() { min_feature_size = min_feature_size.min(min_face_area.sqrt()); }
        if min_feature_size.is_finite() && min_feature_size > 0.0 {
            base_tolerance.max(min_feature_size * 0.01).min(min_feature_size * 0.1)
        } else { base_tolerance }.max(TOLERANCE_ABS)
    }
}
