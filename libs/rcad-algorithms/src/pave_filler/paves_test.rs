...
impl<'a> super::PaveFiller<'a> {
 ...
 pub(crate) fn analyze_shrunk_data(&mut self, ei: usize, pb_idx: usize) {
     ...
     if si < self.ds.shape_info.len() {
         self.ds.shape_info[si].box_gap = self.ds.shape_info[si].box_gap.max(CONFUSION)
             + self.fuzzy_tolerance / 2.0;
     }
 }
 ...
 *aT = proj.param;
 ...
 if dot > 0.9999 {
 // Nearly parallel 鈥?check if coincident (OCCT: same plane within tolerance)
