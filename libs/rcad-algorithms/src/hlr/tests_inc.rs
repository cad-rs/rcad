...
mod tests {
 ...
 // At u= /2, v= /2: normal = u.cos * Z + u.sin * X = 0*Z + 1*X = X (perpendicular to Z view)
 let props = compute_surface_properties(&surface, std::f64::consts::FRAC_PI_2, std::f64::consts::FRAC_PI_2);
 let view_dir = DVec3::Z;

 assert!(props.is_near_silhouette(view_dir, 0.01), "equator at u= /2 should be near silhouette for Z view");
 assert!((props.normal_dot_view(view_dir)).abs() < 0.01);

 // At u=0, v= /2: normal = Z (parallel to view) - NOT a silhouette
