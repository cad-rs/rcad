...
pub(crate) fn curved_subface_boundary_3d(
 ...
 // spans > in u, edges near the seam wrap the "long way" around the
...
pub(crate) fn handle_degenerate_points(
 ...
 match surface {
 Surface3::Sphere(s) => {
 // Sphere has two poles at v=0 and v= ?
 let v_min = uv_poly.iter().map(|p| p.y).fold(f64::INFINITY, f64::min);
 let v_max = uv_poly.iter().map(|p| p.y).fold(f64::NEG_INFINITY, f64::max);
 ...
 // Check if polygon touches the south pole (v = ?
...
/// Split an edge at a periodic seam if it crosses the U=0/2 ?boundary.
///
...
/// - U period: 2 ?(around major circle)
/// - V period: 2 ?(around tube circle)
///
...
pub(crate) fn sphere_closed_trim_to_open_isolines(
 ...
 if diff > period * 0.5 {
 // The values straddle the seam (e.g.  ?and - ? =wrap to get the effective difference
