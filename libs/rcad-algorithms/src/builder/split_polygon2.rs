...
pub(crate) fn split_uv_polygon_by_trim(poly: &[DVec2], trim: &[DVec2]) -> Vec<Vec<DVec2>> {
 ...
 // => t*(dir b) = (a-origin) b  (2D cross: x.x*y.y - x.y*y.x)
 ...
 // inside the face).  Wrapped-closed trims (start and end differ by ~2 ?in u,
 ...
 // Don't split by closed trims =return the original polygon unchanged.
 ...
 // ✅ OCCT-aligned:    ?trim   ? ),  ㄧ ?
 // OCCT  ?BOPAlgo_BuilderFace  ?MakeBlocks  ?section edge
 // ( ) ㈢ € cad  ?split_uv_polygon_by_trim
 // = ?trim  ?  trim   ¤ (3 2 ?
 // 65 64 ?, ?OCCT  ?1 section edge /  ?
...
pub(crate) fn split_polygon_by_circle_2d(poly: &[DVec2], center: DVec2, radius: f64, op: Option<BooleanOpType>) -> (Vec<Vec<DVec2>>, Vec<Vec<DVec2>>) {
 ...
 // typical box/sphere trims) so segment= ircle intersections and arc sampling stay stable.
 ...
 // For Intersection A 〣, the inner_wire (hole) represents the
 ...
 //  Before fix: INSIDE N and ON NSIDE edges missed crossings because the
 ...
 // it (midpoint inside). Find BOTH crossings: entry (start id) and
 // exit (mid nd). This gives 2 crossings on the same edge.
 ...
 // if theta1 > theta2: wraps around =[theta1, 2 ? =[0, theta2]
...
/// Clip a subject polygon against a convex clip polygon using Sutherland odgman.
///
