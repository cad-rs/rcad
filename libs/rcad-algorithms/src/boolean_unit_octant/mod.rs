...
//! sphere, so the result was three untrimmed 1 ? squares. OCCT `bcommon_simple/A1`
//! expects the exact surface `5=4` and volume `=6` for the eighth ball.
//!
...
//! generic boolean path must handle sphere= =lique-plane trimming. Pave now
//! uses exact spherical UV in projection fallbacks (`SphericalSurface::world_to_uv`),
//! and plane= =here tangents are inflated to a micro-circle so every box face gets
...
pub fn try_containment(a: &BRep, b: &BRep, op: BooleanOpType) -> Option<BRep> {
 ...
 // circular edge at z=0 inflates the z-bbox to  ).
 ...
 // ✅ OCCT : inner unsplit outer( - )。
 // inner( / / / ),OCCT PaveFiller outer  ,
 // Some(outer.clone()) PaveFiller, ,  OCCT  。
...
fn build_extruded_brep(outline_cw: &[(f64, f64)], z_min: f64, z_max: f64) -> Option<BRep> {
 ...
 for i in 0..n {
 let _j = (i + 1) % n;
 // Reverse: i=  in the outline corresponds to j=  for the top face
...
/// planar faces with axis-aligned normals ( ,  ,  ), 8 vertices, and every
...
/// (a 3 ? ? grid, excluding the center cell).  Cells that fall entirely inside the
...
fn sew_slabs_into_solid(slabs: &[BRep], zero_tol: f64) -> BRep {
 ...
 // Use plane-equation distance (n entroid) for internal-face matching.
 ...
 // plane:  n entroid_a + (-n) entroid_b = d + (-d) = 0.
...
impl BoxInfo {
 ...
 let c = self.center;
 vec![
 (c - eu * u, -u), // u-min face: outward -u, interior u  =u_min
 (c + eu * u,  u), // u-max face: outward +u, interior u  =u_max
 (c - ev * v, -v), // v-min face
 (c + ev * v,  v), // v-max face
 (c - ew * w, -w), // w-min face
 (c + ew * w,  w), // w-max face
...
/// - All 8 vertices are at  xtent corners of the implied box (verified via
...
pub fn try_difference_box_general(a: &BRep, b: &BRep) -> Option<BRep> {
 ...
 // 1. u-min exterior: B ={u  =u_min_a}
 // plane: (u_min_a*u, +u)  =n=u, constraint u  =u_min_a
 if b_u_min < u_min_a - zero_tol {
 try_slab!(vec![(u * u_min_a, u)]);
 }

 // 2. u-max exterior: B ={u  =u_max_a}
 // plane: (u_max_a*u, -u)  =n=-u, constraint u  =u_max_a
 ...
 // B ={u_min_a =u  =u_max_a} ={v_min_a =v  =v_max_a} ={w  =w_min_a}
 ...
 // B ={u_min_a =u  =u_max_a} ={v_min_a =v  =v_max_a} ={w  =w_max_a}
