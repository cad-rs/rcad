...
///
/// Surface area: 2 r_c ?d  +  4 r_m R ? ? ? =r_m in( ?]
/// where d = =r_m ?=(r_c =R) ? and  ?= arccos(clamp((r_c= )/r_m, =, 1)).
fn build_cylinder_torus_intersection_brep(
 ...
 // phi_low = asin(sin_low) mapped to [ ?2, 3 ?2] range (where cos is = 0)
 ...
 // cos(phi) = beta always (since we're at r=r_c). So phi =  hi_0 + 2 k.
 ...
 // Lower circle (z = z_low): lower = 2 ?- phi_0 (or determined by z_low)
 // Upper circle (z = z_high):  upper = phi_0 (or determined by z_high)
 ...
 let e0 = make_edge(
 &mut brep,
 Curve3::Circle(Circle3::new(DVec3::new(0.0, 0.0, z_low), DVec3::Z, r_c,)),
 0.0, two_pi, v0, v0,
 ).ok()?;
 ...
 let e1 = make_edge(
 &mut brep,
 Curve3::Circle(Circle3::new(DVec3::new(0.0, 0.0, z_high), DVec3::Z, r_c,)),
 0.0, two_pi, v1, v1,
 ).ok()?;
 ...
 // Torus face pcurves
 // E0 (lower circle) at u= 0,2 ,  ?phi_lower
 let e0_on_tor = Curve2d::Line(Line2d {
 origin: glam::DVec2::new(0.0, phi_lower),
 direction: glam::DVec2::new(1.0, 0.0),
 });
 // E1 (upper circle) at u= 0,2 ,  ?phi_upper
