...
mod tests {
 ...
 // bcommon_simple_c1: 1 ? ? =1.5 ?.5 ?.5 =1 ?.5 ?.5 (SA=2.5, vol=0.25)
 ...
 // 2 ? ? box =0.5 ?.5 ?.5 inside =the inner 0.5^3 box
 ...
 // bcommon_simple A7: 1 ? ? =1 ?.5 ? =the contained 1 ? ? (SA=6, vol=1).
 ...
 // F1-like: A=1.5 ?.5 ?.5 at (-0.25,0,0), B=1 ? ? at origin.
 ...
 // bcut_simple_c1: 0.5 ?.5 ? at (0,-0.5,0) minus 1 ? ? at origin.
 // Excess only on y-lo [-0.5, 0]. Result: 0.5 ?.5 ? box, SA=2.5.
 ...
 // Expected SA = 2.5.
 let a = make_box_brep(DVec3::ZERO, DVec3::X, DVec3::Y, 1.0, 1.0, 1.0).unwrap();
 // box at (0.25, 0.25, 0) size 0.5 ?.5 ?1, pivot (.25,.25,0), rotate 30=Z
 let b = make_rotated_box(
 ...
 // U5 case: cylinder r=1, h=2, center (0,0,1), clip X=
 ...
 let expected = 3.0 * std::f64::consts::PI + 4.0; // 2=+ =2 + =2 + 4 = 3=4
