...
/// Build a tessellated BRep for `cone Èñ?box` via Z-slice tessellation.
///
...
///
/// Builds only the overlap section (circle Èñ?rect).
fn build_cone_box_intersection_tessellated(
    ...
    // Pre-compute and remap all slice polygons (circle Èñ?rect)
...
fn try_union_cone_box_one_dir(cone_brep: &BRep, box_brep: &BRep) -> Option<BRep> {
    ...
    // Check containment: box entirely inside cone Èñ?union is the cone
    let corners = [
    ...
    // Check containment: cone entirely inside box Èñ?union is the box
...
fn try_intersection_cone_box_one_dir(cone_brep: &BRep, box_brep: &BRep) -> Option<BRep> {
    ...
    // Check containment: box entirely inside cone Èñ?intersection is the box
    let corners = [
        DVec2::new(bmin.x, bmin.y),
        DVec2::new(bmax.x, bmin.y),
        DVec2::new(bmax.x, bmax.y),
    ...
    // Check containment: cone entirely inside box Èñ?intersection is the cone
    let cone_inside_box_xy_at_lo = cx - r_at_inter_lo >= bmin.x - tol
        && cx + r_at_inter_lo <= bmax.x + tol
        && cy - r_at_inter_lo >= bmin.y - tol
        && cy + r_at_inter_lo <= bmax.y + tol;
...
pub fn try_union_fill_box_cavity(a: &BRep, b: &BRep) -> Option<BRep> {
    ...
    // If B is empty (no faces), skip this check Èñ?A has internal faces that need
...
/// Fast path for `cone Èñ?box` boolean difference.
///
...
pub fn try_difference_cone_box(a: &BRep, b: &BRep) -> Option<BRep> {
    ...
    if dist_center > box_half_diag + min_r + TOLERANCE_LEN_MIN {
        // Box is entirely outside the cone's XY reach Èñ?no intersection
        return Some(a.clone());
    }
        ...
        && cy + r_hi <= bmax.y + TOLERANCE_LEN_MIN
    {
        ...
        if parts.is_empty() {
            // Cone is entirely inside the box Èñ?empty result
            return Some(BRep::default());
        }
        if parts.len() == 1 {
            return Some(parts.swap_remove(0));
        }
        // Multiple portions (both below and above) Èñ?fall through to tessellated path
...
/// Fast path: cylinder Èñ?box boolean difference.
///
...
/// covered by the box Èñ?avoiding the Pave-Filler which can produce hundreds
...
pub fn try_difference_box_minus_brep_with_hole(a: &BRep, b: &BRep) -> Option<BRep> {
    ...
    // Find the face that uses this cylindrical surface Èñ?get V range for height.
    ...
    // Compute box Èñ?cylinder via the existing intersection fast path.
...
/// (box Èñ?cylinder) analytically: clone the box, remove the exit face, and add
...
/// (z_lo Èñ?Èñ≥ÓÖüÂßÄw, z_hi Èñ?ew) Èñ?otherwise returns `None`.
fn build_box_minus_cylinder_one_v_face_exit(
    ...
    // Box faces use exact world-space normals Èñ?no tolerance needed.
    ...
    // Box face normalised axes: u_ax=X, v_ax=Y, z_ax=Z Èñ?standard vertex layout.
    ...
    //   va = "right/down"  @ z_min (Èë?Èñ?0 mod 2Èî?
    //   vb = "left/up"     @ z_min (Èë?Èñ?Èî?
    //   vc = "left/up"     @ z_max (Èë?Èñ?Èî?
    //   vd = "right/down"  @ z_max (Èë?Èñ?0 mod 2Èî?
    // Wire order: va Èñ?vb (bottom arc), vb Èñ?vc (left gen),
    //             vc Èñ?vd (top arc),   vd Èñ?va (right gen reversed)
    ...
    let (va, vb, vc, vd) = if !is_u {
        ...
        let va_pos = z(1.0, -1.0); // (+eu, exit_v, Èñ≥ÓÖüÂßÄw) Èñ?right-bottom, Èë?0
        let vb_pos = z(-1.0, -1.0); // (Èñ≥ÓÖüÂßÄu, exit_v, Èñ≥ÓÖüÂßÄw) Èñ?left-bottom, Èë?Èî?
        let vc_pos = z(-1.0, 1.0); // (Èñ≥ÓÖüÂßÄu, exit_v, +ew) Èñ?left-top,  Èë?Èî?
        let vd_pos = z(1.0, 1.0); // (+eu, exit_v, +ew) Èñ?right-top, Èë?0
        ...
        let va_pos = z(-1.0, -1.0); // (exit_u, Èñ≥ÓÖüÂßÄv, Èñ≥ÓÖüÂßÄw) Èñ?bottom at v_min, Èë?0
        let vb_pos = z(1.0, -1.0); // (exit_u, +ev, Èñ≥ÓÖüÂßÄw) Èñ?bottom at v_max, Èë?Èî?
        let vc_pos = z(1.0, 1.0); // (exit_u, +ev, +ew) Èñ?top at v_max,  Èë?Èî?
        let vd_pos = z(-1.0, 1.0); // (exit_u, Èñ≥ÓÖüÂßÄv, +ew) Èñ?top at v_min,  Èë?0
        (va_pos, vb_pos, vc_pos, vd_pos)
    };
    ...
    // Bottom arc at z = Èñ≥ÓÖüÂßÄw: from va Èñ?vb along yÈñ? (Èë? 0Èñ≥ÓÇ£Â¥ªÈàß?on Circle3 normal=+Z)
    ...
    // Left generator: vb Èñ?vc (up along z at Èë?Èî?
    ...
    // Top arc at z = +ew: from vc Èñ?vd along yÈñ? (Èë? ÈîúÊ¥™Âü? on Circle3 normal=+Z)
    ...
    // Right generator: vd Èñ?va (down along z at Èë?0, stored as vaÈñ≥ÓÇ£Âßµd, rev Èñ?vdÈñ≥ÓÇ£Âßµa)
    ...
    // Wire: va Èñ?vb (bot_arc_fwd), vb Èñ?vc (left_gen_fwd),
    //       vc Èñ?vd (top_arc_fwd), vd Èñ?va (right_gen_rev)
    let wall_wire = Wire {
        edges: vec![
    ...
    // UV range: u Èñ?[Èî? 2ÈîúÁØØ (yÈñ? half of CylindricalSurface ref_dir=X),
    //           v Èñ?[0, h]
...
/// Fast path: cylinder Èñ?box boolean intersection.
///
/// When the box fully contains the cylinder cross-section in XY (at the Z-range
/// of intersection), the result is simply the cylinder clipped to the box's
/// Z range Èñ?avoiding the Pave-Filler which produces hundreds of unnecessary
...
/// Helper: find which theta values in [0, 2Èî? satisfy all clip-plane constraints.
/// For clip plane (inward_normal n, cut_dist d): valid where cos(Èë≥ÂÜÆÂüàÈéπ? Èñ?Èñ≥ÓÖüÂ¶?r.
/// Returns sorted disjoint intervals within [0, 2Èî?.
pub(crate) fn compute_valid_theta_ranges(r: f64, clip_planes: &[(DVec3, f64)]) -> Vec<(f64, f64)> {
    ...
    for &(n, d) in clip_planes {
        ...
        // Constraint: Èë?Èñ?[Èî†‰æÄÂüàÈéπ? Èî?ÊµºÁåê mod 2Èî?
        let lo = phi - alpha;
        let hi = phi + alpha;
...
/// Compute the complement (inverse) of a set of Èë?intervals within [0, 2Èî?.
/// Each interval (lo, hi) is assumed sorted and non-overlapping.
/// The result covers the parts of [0, 2Èî? not in any input interval.
pub(crate) fn compute_complement_theta_ranges(intervals: &[(f64, f64)]) -> Vec<(f64, f64)> {
    let two_pi = 2.0 * std::f64::consts::PI;
    if intervals.is_empty() {
        return vec![(0.0, two_pi)];
    }
    // Sort by lo first Èñ?compute_valid_theta_ranges may return intervals in
...
/// Each plane's line: nÁí?pÈñ≥ÓÖüÂ¶Ωenter.xy) = Èñ≥ÓÖüÂ¶? Èñ? nÁí∫ÁóØ = nÁí∫Áóóenter.xy Èñ?d
fn corner_of_planes(
    n1: DVec3, d1: f64,
    n2: DVec3, d2: f64,
    center: DVec3,
) -> DVec3 {
    // 2Èë? system: [n1.x n1.y; n2.x n2.y] * p = [n1Áí∫Áóó_xy Èñ?d1; n2Áí∫Áóó_xy Èñ?d2]
    let cx = center.x;
    let cy = center.y;
...
fn build_plane_chain(
    ...
    if fwd_ok && bwd_ok {
        // Both chains are valid.  The fwd chain follows increasing Èî?order and
        ...
        // No valid chain found Èñ?fall back to direct (may produce wrong geometry,
