...
fn build_cylinder_box_diff_tessellated(
 ...
 // Edge order: bottom (L= ), right (B= ), top (R= ), left (T= )
 let rect_edges = [
 ...
 // Starting from a corner and going CW: bottom= ight= op= eft is CCW!
 // CW is: bottom= eft= op= ight (right-hand rule around +Z).
 // Edge 0 (bottom) in CW direction: right= eft (decreasing x)
 // Edge 3 (left) in CW direction: bottom= op (increasing y)
 // Edge 2 (top) in CW direction: left= ight (increasing x)
 // Edge 1 (right) in CW direction: top= ottom (decreasing y)
 ...
 // - edge 0 (bottom, A=  left= ight): CW = right= eft = larger t first
 // - edge 1 (right, B=  bottom= op): CW = top= ottom = larger t first
 // - edge 2 (top, C=  right= eft): CW = left= ight = larger t first
 // - edge 3 (left, D=  top= ottom): CW = bottom= op = larger t first
