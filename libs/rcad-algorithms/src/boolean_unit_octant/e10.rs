...
fn build_cylinder_box_difference_full_wall(
    ...
    let cyl_wire = Wire {
        edges: vec![
            WireEdge::rev(ba),     // bottom arc V0_loÈñ≥ÓÇ£Â£?_lo
            WireEdge::fwd(seam_gen), // up
            WireEdge::fwd(ta),     // top arc V0_loÈñ≥ÓÇ£Â£?_hi
...
pub(crate) fn build_cylinder_box_clipped_brep(
    ...
    // ---- 2. vertices Èñ?one pair (lo, hi) per unique Èë?----
    // Canonicalize: Èë?near 0 or 2Èî?Èñ?0
    let canon = |t: f64| if t >= two_pi - 1e-12 { 0.0 } else { t };
    ...
    for (i, &(s_raw, e_raw)) in intervals.iter().enumerate() {
        ...
        // Bottom circle (normal = Èñ≥ÓÖüÂ¶?
        // Circle3(normal=-Z): C(t) = center + r*(-sin(t), -cos(t), 0)
        // For vertex at standard CCW angle Èë? P = center + r*(cos(Èë?, sin(Èë?, 0)
        // Mapping: (-sin(t), -cos(t)) = (cos(Èë?, sin(Èë?) Èñ?t = -Èî?2 - Èë?
        let circle_bot = Curve3::Circle(Circle3::new(DVec3::new(center.x, -DVec3::Z, r));
        // Stored as V_e Èñ?V_s, used as rev in wires Èñ?effective V_s Èñ?V_e (CCW)
        let ba = next_curve(circle_bot, -pi / 2.0 - e_raw, -pi / 2.0 - s_raw, v_lo(ei), v_lo(si));

        // Right generator at Èë?= e
        ...
        // Top circle (normal = +Z)
        // Circle3(normal=+Z): C(t) = center + r*(-sin(t), cos(t), 0)
        // Mapping: (-sin(t), cos(t)) = (cos(Èë?, sin(Èë?) Èñ?t = Èë?- Èî?2
        let circle_top = Curve3::Circle(Circle3::new(DVec3::new(center.x, 0.0, 0.0), DVec3::Z, r);
        // Stored as V_s Èñ?V_e (fwd in cap wire = Èë≥ÂÉüsÈñ≥ÓÇ£Â¥πÁªîÂ°?CCW; rev in cyl wall = Èë≥ÂÉüeÈñ≥ÓÇ£Â¥πÁªîÂ°?
        let ta = next_curve(circle_top, s_raw - pi / 2.0, e_raw - pi / 2.0, v_hi(si), v_hi(ei));

        // Left generator at Èë?= s
        ...
        let cyl_wire = Wire {
            edges: vec![
                WireEdge::rev(ba),  // V_s_lo Èñ?V_e_lo
                WireEdge::fwd(rg),  // V_e_lo Èñ?V_e_hi
                WireEdge::rev(ta),  // V_e_hi Èñ?V_s_hi
                WireEdge::rev(lg),  // V_s_hi Èñ?V_s_lo
            ],
        };
        ...
        // UV range: u Èñ?[Èë≥ÂÉüs, Èë≥ÂÉüe], v Èñ?[0, h]
    ...
    // Each gap from interval[i].e Èñ?interval[i+1].s (mod n_int) may have 1+
    ...
    struct GapSeg {
        ...
        top_chord: usize,
        plane: usize,
        gen_from: usize, // V_from_lo Èñ?V_from_hi (stored direction)
        gen_to: usize,   // V_to_lo Èñ?V_to_hi (stored direction)
    }
    ...
    // Bottom cap: V_s_lo Èñ?V_e_lo (arc) Èñ?V_s_{i+1}_lo (chords) Èñ?...
    let mut bot_wire_edges: Vec<WireEdge> = Vec::new();
    let mut top_wire_edges: Vec<WireEdge> = Vec::new();
    for i in 0..n_int {
        // Arc from start-vertex to end-vertex (bottom: V_s_loÈñ≥ÓÇ£Â£ê_e_lo, top: V_s_hiÈñ≥ÓÇ£Â£ê_e_hi)
    ...
    // The face wire runs: bot_chord_fwd Èñ?gen_to_fwd Èñ?top_chord_rev Èñ?gen_from_rev
        ...
        for seg in segs {
            ...
            let (n, d) = {
                ...
                if on_p0 {
                    ...
                    if len < 1e-15 {
                        // Degenerate chord Èñ?fall back to original plane.
                        (n0, d0)
                    } else {
...
/// `(P - center)Áí∫Áóólip_n Èñ?cut_dist` (the outside-slab region). The arc is
/// centered on `clip_n` with half-angle `Êµ?= acos(cut_dist/r)`. The clip face
/// is on the plane `center + clip_nÁí∫Áóóut_dist` with outward normal `-clip_n`.
///
/// `clip_n`: horizontal unit normal from center toward the clip plane.
/// `cut_dist`: distance from center to clip plane (Èñ?, Èñ≥ÓáÖÊß?.
...
fn build_cylinder_arc_for_difference_skip(
    ...
    // (cos(Èî†‰Ω∏Â¥µÊµº?, sin(Èî†‰Ω∏Â¥µÊµº?)
    ...
    // F0: Cylindrical wall Èñ?wire V0Èñ≥ÓÇ£Â£?Èñ≥ÓÇ£Â£?Èñ≥ÓÇ£Â£?Èñ≥ÓÇ£Â£?
    let cyl_wire = Wire {
        edges: vec![
