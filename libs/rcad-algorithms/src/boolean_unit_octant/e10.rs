...
fn build_cylinder_box_difference_full_wall(
 ...
 let cyl_wire = Wire {
 edges: vec![
 WireEdge::rev(ba), // bottom arc V0_lo= 0_lo
 WireEdge::fwd(seam_gen), // up
 WireEdge::fwd(ta), // top arc V0_lo= 0_hi
...
pub(crate) fn build_cylinder_box_clipped_brep(
 ...
 // Mapping: (-sin(t), cos(t)) = (cos( ?, sin( ?) =t =  ?-  ?2
 let circle_top = Curve3::Circle(Circle3::new(DVec3::new(center.x, center.y, cz_hi), DVec3::Z, r,));
 // Stored as V_s =V_e (fwd in cap wire =  s=  CCW; rev in cyl wall =  e= )
 ...
 for i in 0..n_int {
 // Arc from start-vertex to end-vertex (bottom: V_s_lo= _e_lo, top: V_s_hi= _e_hi)
...
/// `(P - center) lip_n =cut_dist` (the outside-slab region). The arc is
/// centered on `clip_n` with half-angle ` ?= acos(cut_dist/r)`. The clip face
/// is on the plane `center + clip_n ut_dist` with outward normal `-clip_n`.
///
/// `clip_n`: horizontal unit normal from center toward the clip plane.
/// `cut_dist`: distance from center to clip plane (=, = ).
...
fn build_cylinder_arc_for_difference_skip(
 ...
 // E0: bottom arc (V1= 0), same convention as build_half_cylinder_intersection_brep
 let circle_bot = Curve3::Circle(Circle3::new(DVec3::new(center.x, center.y, cz_lo), -DVec3::Z, r,));
 ...
 // F0: Cylindrical wall =wire V0= 1= 2= 3= 0
 let cyl_wire = Wire {
 edges: vec![
