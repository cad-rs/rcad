...
/// its axis. Only the side where `(P - center)ç’ºç——lip_n é–?-cut_dist` is kept.
...
fn build_half_cylinder_intersection_brep(
    ...
    cut_dist: f64,   // distance from center to cut plane (é–?, into kept half)
) -> BRep {
    ...
    // Half-angle of the kept arc: æµ?= arccos(-cut_dist/r).
    // For cut_dist=0 (center cut): æµ?= é”?2 é–?é“»æ£ = é”?(half-cylinder).
    // For cut_dist=r (full cylinder): æµ?= arccos(-1) = é”?é–?é“»æ£ = 2é”?(full cylinder).
    let alpha = (-cut_dist / r).acos();

    // Vertices at the intersection of the cut plane with the cylinder surface.
    // V0/V3 at u = é”?- æµ?(left generator), V1/V2 at u = é”?+ æµ?(right generator).
    let (sa, ca) = alpha.sin_cos();
    let (sp, cp) = phi.sin_cos();

    // (cos(é” ä½¸å´µæµ¼?, sin(é” ä½¸å´µæµ¼?) = (cp*ca é–?sp*sa, sp*ca é?cp*sa)
    ...
    // Edge index helpers é–?push an edge and return its index.
    ...
    // E0: bottom arc (V1é–³î‚£å£? along kept side of bottom circle).
    // Circle3 with normal=-Z maps CylSurf u é–?Circle3 -u, so V1(CylSurf u=é”?æµ?
    // é–?Circle3 u=-(é”?æµ? and V0(CylSurf u=é”?æµ? é–?Circle3 u=-(é”?æµ?.
    ...
    // E2: top arc (V3é–³î‚£å£? along kept side of top circle).
    // Circle3 with normal=Z uses the same param as CylSurf, so V3(CylSurf u=é”?æµ?
    // é–?Circle3 u=é”?æµ?and V2(CylSurf u=é”?æµ? é–?Circle3 u=é”?æµ?
    ...
    // F0: Cylindrical face é–?wire V0é–³î‚£å£?é–³î‚£å£?é–³î‚£å£?é–³î‚£å£?
    // E0 stored as V1é–³î‚£å£? é–?E0_rev = V0é–³î‚£å£?
    // E1 stored as V1é–³î‚£å£? é–?E1_fwd = V1é–³î‚£å£?
    // E2 stored as V3é–³î‚£å£? é–?E2_rev = V2é–³î‚£å£?
    // E3 stored as V0é–³î‚£å£? é–?E3_rev = V3é–³î‚£å£?
    let cyl_wire = Wire {
        edges: vec![
            WireEdge::rev(e0),
            WireEdge::fwd(e1),
    ...
    // Set face_surface_range for analytic SA: u é–?[é”?æµ? é”?æµ¼çŒ, v é–?[0, h]
    ...
    // F1: Top half-disk (normal=+Z). Wire: V2é–³î‚£å£? (cut top) é–?V3é–³î‚£å£? (top arc)
    // E5_fwd: V2é–³î‚£å£?, E2_fwd: V3é–³î‚£å£?
    let top_wire = Wire {
        edges: vec![WireEdge::fwd(e5), WireEdge::fwd(e2)],
    };
    let _f1 = push_face(top_wire, si_top, DVec3::Z);

    // F2: Bottom half-disk (normal=-Z). Wire: V0é–³î‚£å£? (cut bottom) é–?V1é–³î‚£å£? (bottom arc)
    // E4_fwd: V0é–³î‚£å£?, E0_fwd: V1é–³î‚£å£?
    let bot_wire = Wire {
        edges: vec![WireEdge::fwd(e4), WireEdge::fwd(e0)],
    };
    let _f2 = push_face(bot_wire, si_bot, -DVec3::Z);

    // F3: Cut face (normal=-clip_n). Wire: V0é–³î‚£å£?é–³î‚£å£?é–³î‚£å£?é–³î‚£å£?
    // E4_fwd: V0é–³î‚£å£?, E1_fwd: V1é–³î‚£å£?, E5_fwd: V2é–³î‚£å£?, E3_rev: V3é–³î‚£å£?
    let cut_wire = Wire {
        edges: vec![
            WireEdge::fwd(e4),
            WireEdge::fwd(e1),
...
fn try_intersect_cylinder_box_one_dir(cyl_brep: &BRep, box_brep: &BRep) -> Option<BRep> {
    ...
    // Single clip plane é–?existing half-cylinder builder (backward compat, efficient).
    if clip_planes.len() == 1 {
        let (clip_dir, cut_dist) = clip_planes[0];
        // When cut_dist < -r the cylinder center is so far outside the box
        // that the cylinder doesn't reach the box face é–?empty intersection.
        // (The multi-plane builder below handles this via zero-range valid é‘?
        // intervals, but the half-cylinder builder asserts cut_dist é–?0.)
    ...
    // Multiple clip planes é–?general multi-plane builder.
    Some(build_cylinder_box_intersection_brep_with_splits(
        adj_center,
        cyl_r,
        h,
