...
pub fn fix_same_parameter(brep: &BRep, _tolerance: f64) -> (BRep, usize) {
    ...
    for edge_idx in 0..edge_count {
        ...
        // Linear reparameterization: [pc_t0, pc_t1] 閳?[range3d[0], range3d[1]].
...
pub fn remove_small_edges(brep: &BRep, min_length: f64) -> (BRep, usize) {
    ...
    loop {
        ...
        // Remap vertex references: drop_vi 閳?keep_vi, shift higher indices down.
...
pub enum ToleranceFlowDirection {
    /// Vertex 閳?edge 閳?face (bottom-up, for newly assembled results).
    BottomUp,
    /// Face 閳?edge 閳?vertex (top-down, for degraded imports).
    TopDown,
}
...
/// spreading in OCCT.
///
/// 鉁?OCCT 瀵归綈: BRepLib.cxx 鈥?UpdateTolerances (lines 125-195).
...
pub fn propagate_tolerances(
    ...
    match direction {
        ToleranceFlowDirection::BottomUp => {
            ...
            // Step 2: propagate vertex 閳?edge.
            for ei in 0..n_edges {
        ...
        ToleranceFlowDirection::TopDown => {
            ...
            // Step 3: propagate edge 閳?vertex.
            for ei in 0..n_edges {
...
///
/// 鉁?OCCT 瀵归綈: BRepLib.cxx 鈥?UpdateEdgeTolerance (lines 200-260).
...
///
/// 鉁?OCCT 瀵归綈: BRepLib.cxx 鈥?UpdateTolerances, face propagation step.
...
///
/// 鉁?OCCT 瀵归綈: BRepLib.cxx 鈥?SameRange (lines 75-120).
...
///
/// 鉁?OCCT 瀵归綈: BRepLib.cxx 鈥?EnsureNormalConsistency (lines 270-350).
...
///
/// 1. `ensure_all_same_range` 鈥?align PCurve ranges with 3D curve ranges.
/// 2. `update_all_edge_tolerances` 鈥?recompute edge tolerances from geometry.
/// 3. `update_all_face_tolerances` 鈥?propagate edge tolerances to faces.
/// 4. `ensure_normal_consistency` 鈥?orient face normals outward.
///
/// This is the aggregate equivalent of OCCT `BRepLib::UpdateTolerances` +
/// `BRepLib::SameRange` + `BRepLib::EnsureNormalConsistency`.
///
/// 鉁?OCCT 瀵归綈: BRepLib.cxx 鈥?combined update entry point.
...
pub struct PeriodicSurfaceInfo {
    /// U-period (e.g., 2锜?for cylinder, sphere, cone, torus).
    pub u_period: Option<f64>,
    /// V-period (e.g., 2锜?for torus, None for others).
    ...
    pub has_apex: bool,
    /// V value at the apex for cones (typically 0 or 锜?.
...
pub fn detect_periodic_surface_info(surface: &Surface3) -> PeriodicSurfaceInfo {
    match surface {
        ...
        Surface3::Sphere(_) => PeriodicSurfaceInfo {
            ...
            degenerate_at_v_max: true,  // V=锜?is south pole
            has_apex: false,
            apex_v: None,
        },
        Surface3::Cone(_) => PeriodicSurfaceInfo {
            ...
            degenerate_at_v_max: false,
            has_apex: true,
            apex_v: Some(0.0), // Apex is at V=0 (or can be at V=锜?depending on half_angle)
        },
