...
pub fn merge_close_vertices(brep: &BRep, tolerance: f64) -> (BRep, usize) {
    ...
    if n >= SPATIAL_HASH_THRESHOLD {
        ...
        // Brute-force O(n铏? 閳?fast enough for small models
        for i in 0..n {
            for j in (i + 1)..n {
                let d2 = (brep.vertices[i].point - brep.vertices[j].point).length_squared();
                if d2 <= tol2 {
    ...
    // Build a compact vertex list and a remap table old_idx 閳?new_idx
...
/// more than 1鎺?(indicating they were stale or flipped).
///
                ...
                .map(|shell| Shell {
                    faces: shell
                        ...
                        .map(|face| {
                            ...
                            // dot < cos(1鎺? 閳?0.9998 means the normal changed significantly
                            if dot < 0.9998 {
                                changed += 1;
                            }
...
///
/// For each open wire (end of edge i 閳?start of edge i+1 within `tolerance`),
