...
fn is_vertex_at_degenerate_point(
    ...
    match surface {
        Surface3::Sphere(sphere) => {
            ...
            // At south pole (V=锜?: vertex is at center - radius * axis
...
pub fn merge_seam_edges(brep: &BRep, config: &PeriodicSeamConfig) -> (BRep, usize) {
    ...
    // 3. Have endpoints near the seam (one at U閳?, one at U閳?锜?
...
impl ShellValidationReport {
    ...
    pub fn summary(&self) -> String {
        ...
        format!("ShellValidation: {} | V={}, E={}, F={}, 锠?{}", status, self.vertex_count, self.edge_count, self.face_count, self.euler_characteristic)
    }
}
