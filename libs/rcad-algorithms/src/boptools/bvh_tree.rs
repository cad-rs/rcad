...
impl Bvh {
    ...
    pub fn build(brep: &rcad_kernel::BRep) -> Self {
        ...
        let face_ts_indices: Vec<usize> = brep
            .tshapes
            .iter()
            .enumerate()
            ...
            if let Some(surf) = &fd.surface {
                match surf {
                    ...
                    Surface3::Cylinder(c) => {
                        ...
                        for &vh in &[v0, v1] {
                            for k in 0..8 {
                                ...
                                let p =
                                    c.origin + ax * vh + u_dir * r * a.cos() + v_dir * r * a.sin();
                                aabb.expand_point(p);
                            }
                        }
                    }
                    Surface3::Cone(c) => {
                        ...
                        let v_dir = ax.cross(u_dir).normalize_or_zero();
                        for &vh in &[v0, v1] {
                            let r_at = (c.radius + vh * c.half_angle_rad.tan()).abs()
                    ...
                    Surface3::Torus(t) => {
                        ...
                        for k in 0..8 {
                            let a = std::f64::consts::TAU * k as f64 / 8.0;
                            let c = t.center
                                + u_dir * t.major_radius * a.cos()
                                + v_dir * t.major_radius * a.sin();
