...
/// ✅ OCCT对齐: 为布尔结果中的球面添加 seam 边和极点退化边。
///
/// OCCT BOPAlgo_BuilderFace (BOPAlgo_BuilderFace.cxx L239-383 PerformLoops)
/// 通过 BOPAlgo_WireSplitter 为周期曲面（球面/柱面）自动添加 seam 边。
/// rcad 的 split_face → emit_face_with_origin 路径不创建 seam 边，
/// 因此在此后处理步骤补充 seam 边和退化边，使拓扑与 OCCT 一致。
///
/// 添加的内容（对应 OCCT BRepPrim_Sphere 的 seam meridian）:
/// - 南极点顶点（如不存在）
