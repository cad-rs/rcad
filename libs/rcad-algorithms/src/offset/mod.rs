...
use rcad_kernel::{
    ...
    geom::{
        ...
        OffsetSurface, Parabola3, Plane, SphericalSurface, Surface3, ToroidalSurface,
    },
...
pub enum OffsetError {
    ...
    RecoveryFailed { attempts: usize, last_error: String },
}
...
impl std::fmt::Display for OffsetError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::DegenerateSurface {
                face_index,
                distance,
            } => {
                write!(
                    f,
                    "offset distance {} would degenerate face {}",
                    distance, face_index
                )
            }
            ...
            Self::EdgeIntersectionFailed { edge_index } => {
                write!(
                    f,
                    "failed to compute offset edge intersection for edge {}",
                    edge_index
                )
            }
            ...
            Self::WallThicknessViolation {
                ...
                write!(
                    f,
                    "wall thickness {} below minimum {} at {}",
                    actual, minimum, location
                )
            }
            Self::JoinCreationFailed {
                join_type,
                edge_index,
                reason,
            } => {
                write!(
                    f,
                    "failed to create {:?} join at edge {}: {}",
                    join_type, edge_index, reason
                )
            }
            Self::InvalidVariableThickness {
                face_index,
                thickness,
                reason,
            } => {
                write!(
                    f,
                    "invalid thickness {} for face {}: {}",
                    thickness, face_index, reason
                )
            }
            Self::QualityCheckFailed {
                ...
                write!(
                    f,
                    "quality check '{}' failed: {} exceeds threshold {}",
...
impl VariableThickness {
    ...
    pub fn with_face(mut self, face_index: usize, thickness: f64) -> Self {
        self.face_thicknesses.push(FaceThickness {
            face_index,
            thickness,
        });
        self
...
pub fn offset_surface(surf: &Surface3, d: f64) -> Option<Surface3> {
    match surf {
        ...
        Surface3::Cone(c) => {
            ...
            // same half-angle but its true apex shifts by d/sin(伪) along the axis
            ...
            // Offset shifts the true apex by d/sin(伪) in the outward direction
        ...
        | Surface3::Helicoid(_)
        | Surface3::Pipe(_) => Some(Surface3::Offset(OffsetSurface {
        ...
        Surface3::Offset(o) => Some(Surface3::Offset(OffsetSurface {
            basis: o.basis.clone(),
...
pub fn intersect_offset_sphere_sphere(
    ...
    if (centers_dist - (r1 + r2)).abs() < TOLERANCE_ABS {
        ...
        return OffsetIntersectionCurve::Circle(Circle3::new(
            point,
            (sphere2.center - sphere1.center).normalize(),
            0.0,
        ));
    }
...
// 鈹€鈹€ Cone offset helper 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
fn offset_conical_surface(c: &ConicalSurface, d: f64) -> Option<ConicalSurface> {
    ...
    // apex by d/sin(伪) toward the smaller-radius end (opposite to the cone
...
// 鈹€鈹€ Phase 2: New offset handler functions 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
pub fn intersect_offset_surfaces(
    ...
    match (surf1, surf2) {
        (Surface3::Plane(p1), Surface3::Plane(p2)) => intersect_offset_plane_plane(p1, p2, d1, d2),
        ...
        (Surface3::Plane(p), Surface3::Sphere(s)) | (Surface3::Sphere(s), Surface3::Plane(p)) => {
        ...
        (Surface3::Plane(p), Surface3::Cone(c)) | (Surface3::Cone(c), Surface3::Plane(p)) => {
            intersect_offset_plane_cone(p, c, d1, d2)
        }
        (Surface3::Cylinder(c), Surface3::Cone(k)) | (Surface3::Cone(k), Surface3::Cylinder(c)) => {
            intersect_offset_cylinder_cone(c, k, d1, d2)
        }
        (Surface3::Sphere(s), Surface3::Cone(c)) | (Surface3::Cone(c), Surface3::Sphere(s)) => {
            intersect_offset_sphere_cone(s, c, d1, d2)
        }
        (Surface3::Cone(k1), Surface3::Cone(k2)) => intersect_offset_cone_cone(k1, k2, d1, d2),
        (Surface3::Plane(p), Surface3::Torus(t)) | (Surface3::Torus(t), Surface3::Plane(p)) => {
            intersect_offset_plane_torus(p, t, d1, d2)
        }
        (Surface3::Sphere(s), Surface3::Torus(t)) | (Surface3::Torus(t), Surface3::Sphere(s)) => {
        ...
        (Surface3::Torus(t), Surface3::Cone(c)) | (Surface3::Cone(c), Surface3::Torus(t)) => {
            intersect_offset_torus_cone(t, c, d1, d2)
        }
        (Surface3::Torus(t1), Surface3::Torus(t2)) => intersect_offset_torus_torus(t1, t2, d1, d2),
