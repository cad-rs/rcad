//! Boolean **Union** (fuse) aligned with Open CASCADE鈥檚 high-level phases.
//!
//! This module is the dedicated entry for [`crate::BooleanOpType::Union`]. The steps mirror
//! OCCT鈥檚 `BOPAlgo_Builder` / `BRepAlgoAPI_Fuse` control flow:
//!
//! 1. **Prepare arguments** 鈥?build the interference descriptor from both operands
//! ([`bopds::ds::DS::new`]), analogous to loading shapes into the BOP data structure.
//! 2. **Intersection and paving** 鈥?[`crate::pave_filler::PaveFiller::perform`], analogous to
//! `BOPAlgo_PaveFiller` (edge/face interferences, splits, pave sets).
//! 3. **Build fuse result** 鈥?[`crate::builder::BooleanBuilder`] with
//! [`crate::BooleanOpType::Union`], analogous to building the fused solid from classified
//! pieces.
//! 4. **Post-process** (serial `fuse` only) 鈥?[`crate::geom_populate::recompute_plane_surfaces`]
...
//! `UnifySameDomain` + same-domain analog; wire order can still leave analytic area ~5鈥?0% low
//! on some merged planes until the kernel鈥檚 planar integrator is tightened).
//!
...
/// DSU (Union-Find) for building connected SameDomain groups 鈥?equivalent to OCCT FillMap + MakeBlocks.
struct DSU {
 parent: Vec<usize>,
 rank: Vec<u32>,
}
...
/// shell here 鈥?downstream feature code may pass intermediate shapes that are not yet
/// `brep_check`鈥慶lean.
...
/// when BVH acceleration is toggled off (`false` 鈫?plain [`pave_filler::PaveFiller::new`]).
/// 鉁?OCCT-aligned: PaveFiller creation + configuration + Perform.
...
/// Union: DS 鈫?PaveFiller 鈫?BooleanBuilder(Union) 鈫?recompute plane surfaces.
///
...
/// 鉁?OCCT-aligned: DS 鈫?PaveFiller 鈫?BooleanBuilder(Union) 鈫?result.
