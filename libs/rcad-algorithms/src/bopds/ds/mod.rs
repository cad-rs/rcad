...
impl DS {
 ...
 self.shape_sd.add_sd_vertex(from, to);
  ...
  /// 鉁?OCCT-aligned: build vertex-to-edge map (BOPDS_DS::myMapVE).
 ...
 ///  ?OCCT-aligned: myDS->HasInterf(nF1, nF2) =checks FF interference exists.
  ...
  /// 鉁?OCCT-aligned: BOPDS_DS::AddInterf (DS.cxx L410-420).
...
/// Compute the characteristic scale of the model from all vertices.
 ...
 let a_f = self.a_face_count;
  ...
  // 鉁?OCCT-aligned: Bnd_Box::SetGap(Tol(V) + theAdditionalTolerance)
 ...
 // EDGE entries
  ...
  // 鉁?OCCT-aligned: BOPDS_ShapeInfo for an Edge stores its vertex sub-shapes.
  // OCCT stores sub-shape indices as flat shape indices into myLines.
  si.sub_shapes.push(nv + sv);
  if sv != ev {
  si.sub_shapes.push(nv + ev);
  }
  // 鉁?OCCT-aligned: Bnd_Box::SetGap with additional tolerance for edge.
  si.box_gap = self.edges[ei].geom_tol + self.fuzzy_tol * 0.5;
  // 鉁?OCCT-aligned: BOPDS_ShapeInfo::SetFlag for degenerated edges.
 ...
 mx = mx.max(p);
  ...
  // 鉁?OCCT-aligned: Bnd_Box::SetGap with additional tolerance for face.
 ...
 si.sub_shapes.push(nv + ne + nw + nf + nsh + soi);
  ...
  // 鉁?OCCT-aligned: build vertex-to-edge map (myMapVE) after all shapes registered.
  self.build_map_ve();
  }
 ...
 ///
 /// Uses DS internal data only 鈥?no external BRep needed.
 ...
 let n_vsd = vi; // OCCT: GetSameDomainIndex 鈥?skip SD lookup for boundary verts
