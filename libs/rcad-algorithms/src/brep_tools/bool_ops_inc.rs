...
fn partition_solid_object(obj: &rcad_kernel::BRep, tools: &[rcad_kernel::BRep]) -> Result<Vec<rcad_kernel::BRep>, crate::BooleanError> {
 ...
 if inside {
 match crate::bop_occt_ops::boolean_op_generic(crate::BooleanOpType::Intersection, &cell, tool) {
 ...
 match crate::bop_occt_ops::boolean_op_generic(crate::BooleanOpType::Intersection, &cell, complement) {
 Ok(r) => { cell = compact_brep(&r); },
 Err(_) => { failed = true; break; }
 }
 } else {
 ...
 for (origin, u_dir, v_dir, w, h, d) in comp_boxes.iter() {
  ...
  if let Ok(part) =
  crate::bop_occt_ops::boolean_op_generic(crate::BooleanOpType::Intersection, cell_part, &comp_box_old)
 {
 let p = part;
 if count_faces(&p) > 0 {
 parts.push(p);
 ...
 match crate::bop_occt_ops::boolean_op_generic(crate::BooleanOpType::Difference, &cell, tool) {
 Ok(r) => { cell = compact_brep(&r); }
 Err(_) => { failed = true; break; }
 }
 }
...
fn partition_face_object(obj: &rcad_kernel::BRep, tools: &[rcad_kernel::BRep]) -> Result<Vec<rcad_kernel::BRep>, crate::BooleanError> {
 ...
 let inside_t = crate::bop_occt_ops::boolean_op_generic(crate::BooleanOpType::Intersection, &remaining, tool)?;
 let inside = inside_t;
 let in_faces = collect_on_plane(&inside);
 if !in_faces.is_empty() {
 cells.push(extract_brep_subset(&inside, &in_faces));
 }
 let remaining_t = crate::bop_occt_ops::boolean_op_generic(crate::BooleanOpType::Difference, &remaining, tool)?;
