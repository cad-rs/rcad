...
pub fn analyze_periodic_surface_handling(
    ...
    for esr in &all_edge_refs {
        ...
        let pcurve1 = ed.pcurves.get(&face_key);
        let pcurve2 = ed.pcurves.get(&(face_key + nb_faces));

        if pcurve1.is_some() && pcurve2.is_some() {
            let pcurves = vec![pcurve1.unwrap(), pcurve2.unwrap()];
...
pub fn check_face_uv_consistency_by_idx(face_idx: usize, brep: &rcad_kernel::BRep) -> UvConsistencyReport {
    ...
    for (esr, edge_forward) in &all_edge_entries {
        ...
        if ed.pcurves.contains_key(&(face_key + nb_faces)) {
            let p1 = ed.pcurves.get(&face_key).unwrap();
            let p2 = ed.pcurves.get(&(face_key + nb_faces)).unwrap();
