...
use rcad_kernel::geom::{Curve3, CurveEval, Surface3, SurfaceEval, any_perpendicular};
...
impl TessellationParams {
    ...
    pub fn with_target_triangle_count(&self, target_count: usize) -> Self {
        let factor = (target_count as f64 / 1000.0)
            .powf(1.0 / 3.0)
            .max(0.1)
            .min(10.0);
        Self {
    ...
    for (index, point) in mesh.nodes.iter().enumerate() {
        ...
        if let Some(candidates) = buckets.get(&key) {
            for &candidate in candidates {
                if (welded_nodes[candidate] - *point).length_squared()
...
fn subdivide_quad(
    ...
    let should_subdivide = if depth < params.max_depth {
        ...
        if step_u < params.min_step * 2.0 && step_v < params.min_step * 2.0 {
            ...
            let chord_exceeded =
                check_chord_tolerance(surface, p00, p10, p11, p01, um, vm, params.chord_tolerance);

            // Normal variation across the quad
            let angle_exceeded = depth < params.max_depth / 2
                && check_angle_tolerance(surface, u0, u1, v0, v1, params.angle_tolerance);
    ...
    for we in &wire.edges {
            ...
            && !matches!(curve, Curve3::Line(_))
        {
            ...
            if matches!(curve, Curve3::Circle(_) | Curve3::Ellipse(_)) {
                ...
                let degree_likely_by_magnitude = max_abs > two_pi + TOLERANCE_COORD_SUB
                    && max_abs <= 360.0 + TOLERANCE_MESH_LEGACY;
                let tie = (err_deg - err_rad).abs()
                    <= TOLERANCE_LINEAR_RELAX_8 * (1.0 + err_rad.abs().max(err_deg.abs()));
            ...
            match curve {
                Curve3::Circle(c) => {
                    ...
                    let seam_tol = (TOLERANCE_ABS * c.radius.max(1.0))
                        .max(edge_tol * 5.0)
                    ...
                    if seam {
                        ...
                        if near_full_turn {
                            ...
                            if dt.abs() < TOLERANCE_MESH_LEGACY
                                && span_hint.is_finite()
                                && span_hint.abs() > TOLERANCE_ADAPTIVE_MAX
                            {
                                let sign = if span_hint < 0.0 { -1.0 } else { 1.0 };
                                dt = sign
                                    * span_hint.abs().clamp(
                                        TOLERANCE_ADAPTIVE_MAX,
                                        two_pi - TOLERANCE_MESH_LEGACY,
                                    );
                            }
                            ...
                            if chord > chord_tol && dt.abs() < TOLERANCE_ADAPTIVE_MAX {
                                let ratio = (chord / (2.0 * c.radius.max(TOLERANCE_LEN_MIN)))
                                    .clamp(0.0, 1.0);
                                let minor = (2.0 * ratio.asin())
                                    .clamp(TOLERANCE_MESH_LEGACY, std::f64::consts::PI);
                                let sign = if span_hint < 0.0 { -1.0 } else { 1.0 };
                                dt = sign * minor;
                            }
                            t0 = a0;
                ...
                Curve3::Ellipse(e) => {
                    ...
                    if seam {
                        ...
                        if near_full_turn {
                            ...
                            let a0 = wrap_2pi(
                                (v0.dot(y_ax) / e.minor_radius)
                                    .atan2(v0.dot(x_ax) / e.major_radius),
                            );
                            let a1 = wrap_2pi(
                                (v1.dot(y_ax) / e.minor_radius)
                                    .atan2(v1.dot(x_ax) / e.major_radius),
                            );
                            let reliable_hint = span_hint.signum() * TOLERANCE_ADAPTIVE_MAX;
                            let mut dt = choose_arc_delta(a0, a1, reliable_hint);
                            if dt.abs() < TOLERANCE_MESH_LEGACY
...
pub fn mesh_brep(brep: &mut rcad_kernel::BRep, params: &TessellationParams) {
...

    {
        for ts in &brep.tshapes {
            let topods::TShape::Solid(sd) = ts.as_ref() else {
                continue;
            };
            for sr in &sd.shells {
                let Some(shd) = (if sr.index < brep.tshapes.len() {
                    match &*brep.tshapes[sr.index] {
                        topods::TShape::Shell(sh) => Some(sh),
                        _ => None,
                    }
                } else {
                    None
                ...
                for _fr in &shd.faces {
                    ...
                    let Some(face_data) = (if tsi != usize::MAX {
                        match &*brep.tshapes[tsi] {
                            topods::TShape::Face(fd) => Some(fd),
                            _ => None,
                        }
                    } else {
                        None
                    ...
                    let surf_and_domain: Option<(Surface3, [f64; 4])> =
                        face_data.surface.clone().map(|surf| {
                            let domain =
                                face_data.uv_domain.unwrap_or_else(|| surf.default_domain());
                            (surf, domain)
                        });
                    ...
                    if let Some((surf, domain)) = surf_and_domain {
                        ...
                        let (u0, u1, v0, v1) =
                            clamp_domain_to_vertices(brep, face_flat_idx, &surf, u0, u1, v0, v1);
                        ...
                        let domain_ok =
                            (u1 - u0).abs() >= uv_domain_min && (v1 - v0).abs() >= uv_domain_min;
                    ...
                    if !filled {
                        ...
                        let outer_we: Vec<topology::WireEdge> = (if face_data.outer_wire.index
                            < brep.tshapes.len()
                        {
                            match &*brep.tshapes[face_data.outer_wire.index] {
                                topods::TShape::Wire(wd) => wd
                                    ...
                                    .iter()
                                    .map(|e| {
                                        topology::WireEdge::new(e.index, e.orientation.is_forward())
                                    })
                                    .collect(),
                                _ => Vec::new(),
                            }
                        ...
                        let inner_wires: Vec<topology::Wire> = face_data
                            .inner_wires
                            .iter()
                            .map(|iw_sr| {
                                let edges = (if iw_sr.index < brep.tshapes.len() {
                                    match &*brep.tshapes[iw_sr.index] {
                                        topods::TShape::Wire(wd) => wd
                                            ...
                                            .iter()
                                            .map(|e| {
                                                topology::WireEdge::new(
                                                    e.index,
                                                    e.orientation.is_forward(),
                                                )
                                            })
                                            .collect(),
                                        _ => Vec::new(),
                        ...
                        let hole_pts: Vec<Vec<DVec3>> = inner_wires
                            .iter()
...
fn clamp_domain_to_vertices(
    ...
    let u_want_hull = need_u || (du.is_finite() && span_too_small_for_axis(du, ax.u_period, None));
    let v_want_hull =
        need_v || (dv.is_finite() && span_too_small_for_axis(dv, ax.v_period, ax.v_natural));
    ...
    let outer_we: Vec<topology::WireEdge> = (if fd.outer_wire.index < brep.tshapes.len() {
        match &*brep.tshapes[fd.outer_wire.index] {
            topods::TShape::Wire(wd) => wd
                .edges
                .iter()
                .map(|e| topology::WireEdge::new(e.index, e.orientation.is_forward()))
                .collect(),
            _ => Vec::new(),
        }
    } else {
    ...
    pub fn is_good(&self, max_aspect_ratio: f64) -> bool {
        self.degenerate_count == 0
            ...
            && (self.triangle_count <= 10
                || self.poor_aspect_ratio_count < self.triangle_count / 10)
    }
...
impl AdaptiveSubdivider {
    ...
    fn subdivide_triangle(
        ...
        let m01 =
            self.get_or_create_midpoint(i0, i1, p0, p1, n0, n1, nodes, normals, edge_midpoints);
        let m12 =
            self.get_or_create_midpoint(i1, i2, p1, p2, n1, n2, nodes, normals, edge_midpoints);
        let m20 =
            self.get_or_create_midpoint(i2, i0, p2, p0, n2, n0, nodes, normals, edge_midpoints);
...
impl BoundarySensitiveTessellator {
    ...
    pub fn preserve_feature_edges(&self, mesh: &SurfaceMesh) -> SurfaceMesh {
        ...
        let feature_nodes: std::collections::HashSet<usize> = self
            .feature_edges
            .iter()
