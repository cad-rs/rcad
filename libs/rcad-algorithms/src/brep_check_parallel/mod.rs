    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ParallelCheckIssue::DuplicateVertex {
                ...
                write!(
                    f,
                    "DuplicateVertex: vertices {} and {} at same position (distance: {})",
            ...
            ParallelCheckIssue::IsolatedVertex { vertex_idx } => {
                write!(
                    f,
                    "IsolatedVertex: vertex {} not referenced by any edge",
                    vertex_idx
                )
            }
            ParallelCheckIssue::NonFiniteVertex { vertex_idx } => {
                write!(
                    f,
                    "NonFiniteVertex: vertex {} has NaN or infinite coordinates",
                    vertex_idx
                )
            }
        }
...
pub fn check_parallel_with_options(
    brep: &BRep,
...
fn compute_edge_face_counts_parallel(brep: &BRep, n_edges: usize) -> Vec<usize> {
    ...
    let counts: Vec<AtomicUsize> = (0..n_edges).map(|_| AtomicUsize::new(0)).collect();
...
fn check_faces_parallel_chunked(brep: &BRep, n_edges: usize, chunk_size: usize) -> Vec<CheckIssue> {
    ...
    face_items
        ...
        .flat_map(|chunk| {
            chunk
                .iter()
                .flat_map(|&(si, shi, fi)| check_single_face(brep, si, shi, fi, n_edges))
                .collect::<Vec<_>>()
        })
        .collect()
}
...
fn check_single_face(
    ...
    let vert_points: Vec<DVec3> = (0..brep.vertex_count())
        .map(|vi| vpoint(brep, vi))
        .collect();

    // C7: wire self-intersection
    check_wire_self_intersection_local(&wire_verts, &vert_points, si, shi, fi, 0, &mut issues);

    // C8: geometric self-intersection
    check_geometric_self_intersection_local(&wire_verts, &vert_points, si, shi, fi, &mut issues);
...
fn check_wire_self_intersection_local(
    ...
    for (&vidx, &count) in &vertex_count {
        if count > 2 && vidx < vert_points.len() {
...
fn find_duplicate_vertices_parallel(points: &[DVec3], tolerance: f64) -> Vec<ParallelCheckIssue> {
    use std::collections::HashMap;
...
pub fn check_many_parallel_with_options(
    breps: &[BRep],
    options: &ParallelCheckOptions,
) -> Vec<ParallelCheckResult> {
    breps
        .par_iter()
        .map(|brep| check_parallel_with_options(brep, options))
        .collect()
}
    ...
    pub fn summary(&self) -> String {
        if self.is_clean() {
            format!(
                ...
                self.shell_idx,
                self.face_idx,
                self.outer_wire_edge_count,
                self.inner_wire_count
            )
        } else {
            format!(
                "Face ({}/{}/{}): {} issue(s)",
...
impl EdgeCheckResult {
    ...
    pub fn summary(&self) -> String {
        if self.is_clean() {
            ...
            format!("Edge {}: {} issue(s)", self.edge_idx, self.issues.len())
        }
    }
}
    ...
    pub fn timing_summary(&self) -> String {
        let mut lines: Vec<String> = self
            ...
            .map(|t| {
                format!(
                    "  {}: {}ms ({} items)",
                    t.phase, t.duration_ms, t.items_processed
                )
            })
            .collect();
    ...
    if num_threads > 0 {
        ...
        pool.install(|| {
            face_items
                .par_iter()
                .map(|&(si, shi, fi)| {
                    check_single_face_detailed(brep, si, shi, fi, n_edges, tolerance)
                })
                .collect()
        })
    } else {
...
fn check_single_face_detailed(
    ...
    for esr in &outer_wire_edges {
        if esr.index >= n_edges {
            issues.push(FaceCheckIssue::InvalidEdgeIndex {
                edge_idx: esr.index,
            });
            valid = false;
        } else {
    ...
    if valid {
        ...
        for i in 0..n {
            ...
            if end_v != start_v {
                ...
                if gap > tolerance {
                    issues.push(FaceCheckIssue::OpenWire {
                        wire_pos: i,
                        gap_distance: gap,
                    });
                    outer_wire_closed = false;
        ...
        for (&vidx, &count) in &vertex_count {
            if count > 2 {
                issues.push(FaceCheckIssue::SelfIntersection {
                    vertex_idx: vidx,
                    wire_idx: 0,
                });
        ...
        if inner_valid {
            ...
            for i in 0..n_inner {
                ...
                if end_v != start_v {
                    ...
                    let gap = (end_pt - start_pt).length();
                    if gap > tolerance {
                        issues.push(FaceCheckIssue::InnerWireOpen {
                            wire_idx: wi + 1,
                            wire_pos: i,
                        });
                    }
...
fn check_geometric_self_intersection_face(
    ...
    for i in 0..n {
        for j in (i + 2)..n {
            ...
            if segments_intersect_2d(p1, p2, p3, p4) {
                issues.push(FaceCheckIssue::GeometricSelfIntersection {
                    edge_a: i,
                    edge_b: j,
                });
            }
...
fn check_single_edge(
    ...
    if !start_valid {
        issues.push(EdgeCheckIssue::InvalidVertexIndex {
    ...
    // Compute edge length
    let start_pt = if start_valid {
        vpoint(brep, edge.start)
    } else {
        DVec3::ZERO
    };
    let end_pt = if end_valid {
        vpoint(brep, edge.end)
    } else {
        DVec3::ZERO
    };
...
pub fn validate_shells_parallel(brep: &BRep) -> Vec<ShellValidationResult> {
    ...
    let shell_items: Vec<(usize, usize)> = brep
        ...
        .flat_map(|(si, sd)| sd.shells.iter().enumerate().map(move |(shi, _)| (si, shi)))
        .collect();
        ...
        for esr in &wd.edges {
            if esr.index < n_edges {
                edge_orientations
                    .entry(esr.index)
                    .or_default()
                    .push(esr.orientation == Orientation::Forward);
            }
        }
    }
