...
use rcad_kernel::{
    ...
    persistent_naming::{NamingEvent, OperationStats, PersistentId, PersistentNamingEngine},
};
    ...
    fn from(op: BooleanOperationType) -> Self {
        match op {
            BooleanOperationType::Union => {
                rcad_kernel::persistent_naming::OperationType::BooleanUnion
            }
            BooleanOperationType::Intersection => {
                rcad_kernel::persistent_naming::OperationType::BooleanIntersection
            }
            BooleanOperationType::Difference => {
                rcad_kernel::persistent_naming::OperationType::BooleanDifference
            }
        }
    }
}
...
impl HistoryTracker {
    ...
    pub fn record_face_modified_multi(
        ...
        for &result_idx in &result_indices {
            ...
            self.result_to_source
                .insert((EntityType::Face, result_idx), (source, source_index));
        }
    }
    ...
    pub fn record_edge_modified(
        ...
        for &result_idx in &result_indices {
            ...
            self.result_to_source
                .insert((EntityType::Edge, result_idx), (source, source_index));
        }
    }
    ...
    pub fn record_vertex_modified(
        ...
        for &result_idx in &result_indices {
            self.vertex_sources
                .insert(result_idx, (source, source_index));
            self.result_to_source
                .insert((EntityType::Vertex, result_idx), (source, source_index));
        }
    }
    ...
    pub fn record_face_generated_with_parents(
        ...
        self.face_sources
            .insert(face_index, (InputSource::Generated, face_index));
    }
    ...
    pub fn record_edge_generated_with_parents(
        ...
        self.edge_sources
            .insert(edge_index, (InputSource::Generated, edge_index));
    }
    ...
    pub fn record_vertex_generated_with_parents(
        ...
        self.vertex_sources
            .insert(vertex_index, (InputSource::Generated, vertex_index));
    }
    ...
    pub fn record_entity_deleted(
        ...
        self.deleted_lookup
            .insert((entity_type, entity_index), deletion_index);
    }
    ...
    pub fn is_face_generated(&self, result_index: usize) -> bool {
        self.generated_faces
            .iter()
            .any(|r| r.entity_index == result_index)
    }
    ...
    pub fn face_generation_record(&self, face_index: usize) -> Option<&GenerationRecord> {
        self.generated_faces
            .iter()
            .find(|r| r.entity_index == face_index)
    }
    ...
    pub fn edge_generation_record(&self, edge_index: usize) -> Option<&GenerationRecord> {
        self.generated_edges
            .iter()
            .find(|r| r.entity_index == edge_index)
    }
    ...
    pub fn vertex_generation_record(&self, vertex_index: usize) -> Option<&GenerationRecord> {
        self.generated_vertices
            .iter()
            .find(|r| r.entity_index == vertex_index)
    }
    ...
    pub fn is_deleted(&self, entity_index: usize, entity_type: EntityType) -> bool {
        self.deleted_lookup
            .contains_key(&(entity_type, entity_index))
    }
    ...
    pub fn deleted_faces(&self) -> impl Iterator<Item = &DeletionRecord> {
        self.deleted_entities
            .iter()
            .filter(|r| r.entity_type == EntityType::Face)
    }
    ...
    pub fn deleted_edges(&self) -> impl Iterator<Item = &DeletionRecord> {
        self.deleted_entities
            .iter()
            .filter(|r| r.entity_type == EntityType::Edge)
    }
    ...
    pub fn deleted_vertices(&self) -> impl Iterator<Item = &DeletionRecord> {
        self.deleted_entities
            .iter()
            .filter(|r| r.entity_type == EntityType::Vertex)
    }
    ...
    pub fn merge(&mut self, other: &HistoryTracker) {
        ...
        self.generated_vertices
            .extend(other.generated_vertices.clone());
...
impl BooleanHistory {
        ...
        for (idx, origin) in self.face_origins.iter().enumerate() {
            match origin {
                FaceOrigin::FromA(src_idx) if *src_idx == source_face_idx && from_a => {
                    return self.tracker.is_face_modified(idx)
                        || !self.tracker.modified_faces(*src_idx).is_empty();
                }
    ...
    pub fn populate_tracker(&mut self) {
        ...
        for (result_idx, origin) in self.face_origins.iter().enumerate() {
            match origin {
                FaceOrigin::FromA(src_idx) => {
                    self.tracker.record_face_modified_multi(
                        *src_idx,
                        vec![result_idx],
                        InputSource::A,
                    );
                ...
                FaceOrigin::Generated => {
                    self.tracker
                        .record_face_generated(result_idx, GenerationCause::Intersection);
                }
            }
        }
        for (result_idx, origin) in &self.co_face_origins {
        ...
        for (result_idx, origin) in self.edge_origins.iter().enumerate() {
            match origin {
                ...
                EdgeOrigin::Generated => {
                    self.tracker
                        .record_edge_generated(result_idx, GenerationCause::Intersection);
                }
            }
        }
        ...
        for (result_idx, origin) in self.vertex_origins.iter().enumerate() {
            match origin {
                VertexOrigin::FromA(src_idx) => {
                    self.tracker
                        .record_vertex_modified(*src_idx, vec![result_idx], InputSource::A);
                }
                VertexOrigin::FromB(src_idx) => {
                    self.tracker
                        .record_vertex_modified(*src_idx, vec![result_idx], InputSource::B);
                }
                VertexOrigin::Intersection => {
                    self.tracker
                        .record_vertex_generated(result_idx, GenerationCause::Intersection);
                }
            }
        }
        ...
        for &deleted_idx in &self.deleted_from_a {
            ...
            self.tracker
                .record_face_deleted_with_source(deleted_idx, reason, InputSource::A);
        }
        for &deleted_idx in &self.deleted_from_b {
            let reason = self
                ...
                .unwrap_or(DeletionReason::BooleanOperation);
            self.tracker
                .record_face_deleted_with_source(deleted_idx, reason, InputSource::B);
        }
    }
    ...
    pub fn to_naming_events(
        ...
        let mut split_sources: std::collections::HashMap<usize, Vec<usize>> =
        ...
        for (source_idx, target_indices) in split_sources {
            let source_entity_id = Self::edge_entity_id(source_idx);
            let target_entity_ids: Vec<u64> = target_indices
                .iter()
                .map(|&idx| Self::edge_entity_id(idx))
                .collect();
            // Create placeholder persistent IDs - engine will assign actual IDs.
            let target_persistent_ids: Vec<PersistentId> =
                target_indices.iter().map(|_| PersistentId::NULL).collect();
    ...
    pub fn apply_to_naming_engine(
        ...
        let events =
            self.to_naming_events(result_brep, entity_count_before_a, entity_count_before_b);
        ...
        let stats = OperationStats {
            ...
            entity_count_after: result_brep.vertices().len()
                + result_brep.edges().len()
                + result_brep
                    .solids()
                    .iter()
    ...
    fn matching_result_entities(
        &self,
        side: InputSide,
        target_ref: TopoEntityRef,
    ) -> Vec<TopoEntityRef> {
        match target_ref {
            ...
            TopoEntityRef::Solid(source_idx) => {
                ...
                self.solid_origins
                    ...
                    .enumerate()
                    .filter_map(|(result_idx, origin)| match (side, origin) {
                        (InputSide::A, SolidOrigin::FromA) => {
                            Some(TopoEntityRef::Solid(result_idx))
                        }
                        (InputSide::B, SolidOrigin::FromB) => {
                            Some(TopoEntityRef::Solid(result_idx))
                        }
                        _ => None,
                    })
...
impl HistoryChain {
    ...
    pub fn is_deleted_any(&self, entity_type: EntityType, entity_index: usize) -> bool {
        self.operations
            .iter()
            .any(|tracker| tracker.is_deleted(entity_index, entity_type))
    }

    /// Get the deletion reason for an entity.
    pub fn get_deletion_reason(
        &self,
        entity_type: EntityType,
        entity_index: usize,
