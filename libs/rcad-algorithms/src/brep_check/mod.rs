...
fn ns_wire_data<'a>(brep: &'a rcad_kernel::BRep, solid_idx: usize, shell_idx: usize, face_idx: usize, wire_idx: Option<usize>) -> Option<&'a topods::TWireData> {
 ...
 let wr = match wire_idx { None => fd.outer_wire, Some(i) => *fd.inner_wires.get(i)? };
...
impl std::fmt::Display for CheckIssue {
 fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
  match self {
   CheckIssue::OpenWire { solid, shell, face, wire_pos } => {
    write!(f, "OpenWire: solid={solid} shell={shell} face={face} at wire pos {wire_pos}")
   }
   ...
   CheckIssue::InvalidEdgeIndex { solid, shell, face, edge_idx } => {
    write!(f, "InvalidEdgeIndex: solid={solid} shell={shell} face={face} edge={edge_idx}")
   }
   ...
   CheckIssue::SelfIntersectingWire { solid, shell, face, wire_idx, vertex } => {
    write!(f, "SelfIntersectingWire: solid={solid} shell={shell} face={face} wire={wire_idx} vertex={vertex}")
   }
   CheckIssue::GeometricSelfIntersection { solid, shell, face, edge_a, edge_b } => {
    write!(f, "GeometricSelfIntersection: solid={solid} shell={shell} face={face} edges {edge_a} and {edge_b} cross")
   }
   CheckIssue::SurfaceContinuityViolation { solid, face_a, face_b, shared_edge, expected, actual, deviation } => {
    write!(f, "SurfaceContinuityViolation: solid={solid} faces {face_a}/{face_b} edge={shared_edge} expected C{expected} got C{actual} deviation={deviation:.6e}")
   }
...
pub fn brep_check_analyze(brep: &rcad_kernel::BRep) -> CheckResult {
 ...
 for (eidx, ts) in brep.tshapes.iter().enumerate() {
  ...
  if ed.first.index >= n_tshapes {
   issues.push(CheckIssue::InvalidVertexIndex { edge: eidx, vertex_idx: ed.first.index });
  }
  if ed.last.index >= n_tshapes {
   issues.push(CheckIssue::InvalidVertexIndex { edge: eidx, vertex_idx: ed.last.index });
  }
 }
  ...
  for shell_sr in &sd.shells {
   ...
   for face_sr in &shd.faces {
    ...
    for wesr in &owd.edges {
     if wesr.index < n_tshapes { edge_face_count[wesr.index] += 1; }
    }
 ...
 for (eidx, &count) in edge_face_count.iter().enumerate() {
  if count != 2 {
   issues.push(CheckIssue::NonManifoldEdge { edge_idx: eidx, face_count: count });
  }
 }
   ...
   for face_sr in &shd.faces {
    ...
    for wesr in &wd.edges {
     if wesr.index >= n_tshapes {
      issues.push(CheckIssue::InvalidEdgeIndex { solid: si, shell: shi, face: fi, edge_idx: wesr.index });
      has_invalid_edge = true;
     }
    }
    if has_invalid_edge || wire_verts.len() != wd.edges.len() { fi += 1; continue; }
    ...
    for i in 0..n {
     ...
     if end_v != start_v {
      ...
      if (end_pt - start_pt).length() > TOLERANCE_MESH_LEGACY {
       issues.push(CheckIssue::OpenWire { solid: si, shell: shi, face: fi, wire_pos: i });
      }
     }
    }
    ...
    for (wi, iw_sr) in fd.inner_wires.iter().enumerate() {
     ...
     check_wire_self_intersection(&inner_verts, brep, si, shi, fi, wi + 1, &mut issues);
    }
    fi += 1;
   }
   shi += 1;
 ...
 for (&vidx, &count) in &vertex_count {
  if count > 2 {
   issues.push(CheckIssue::SelfIntersectingWire {
    solid, shell, face, wire_idx, vertex: vidx,
   });
  }
 }
}
...
fn check_geometric_self_intersection(
 ...
 let segs: Vec<(DVec3, DVec3)> = wire_verts.iter().map(|&(sv, ev)| {
 ...
 let (origin, axis_u, axis_v) = {
  ...
  match found { Some(b) => b, None => return }
 };
  ...
  for j in (i + 2)..n {
   ...
   if segments_2d_properly_intersect(seg2d[i].0, seg2d[i].1, seg2d[j].0, seg2d[j].1) {
    issues.push(CheckIssue::GeometricSelfIntersection {
     solid, shell, face, edge_a: i, edge_b: j,
    });
    return;
   }
  }
...
fn segments_2d_properly_intersect(
 ...
 if cross.abs() < TOLERANCE_LEN_MIN { return false; }
 let dx = p3[0] - p1[0];
 let dy = p3[1] - p1[1];
...
fn count_geometric_self_intersections(
 ...
 let (origin, axis_u, axis_v) = {
  ...
  match found { Some(b) => b, None => return 0 }
 };
 let project = |p: DVec3| -> [f64; 2] { let d = p - origin; [d.dot(axis_u), d.dot(axis_v)] };
 let seg2d: Vec<([f64; 2], [f64; 2])> =
  segs.iter().map(|&(p0, p1)| (project(p0), project(p1))).collect();
 let mut count = 0usize;
 for i in 0..n {
...
pub fn check_wire_closed(brep: &rcad_kernel::BRep, face_idx: usize) -> bool {
 ...
 if !check_single_wire_closed(brep, fd.outer_wire) { return false; }
 for iw_sr in &fd.inner_wires {
  if !check_single_wire_closed(brep, *iw_sr) { return false; }
 }
 true
}
...
fn check_single_wire_closed(brep: &rcad_kernel::BRep, wire_sr: ShapeRef) -> bool {
 let TShape::Wire(wd) = &*brep.tshapes[wire_sr.index] else { return true };
 ...
 for i in 0..n {
  ...
  if ev != sv {
   ...
   if (end_pt - start_pt).length() > TOLERANCE_MESH_LEGACY { return false; }
  }
 }
 true
}
...
pub fn check_wire_self_intersection_pairs(
 ...
 let (_, _, _, fd) = match find_face_by_flat_idx(brep, face_idx) {
  Some(f) => f, None => return Vec::new(),
 };
 let mut result = Vec::new();
 let TShape::Wire(owd) = &*brep.tshapes[fd.outer_wire.index] else { return result };
...
fn check_single_wire_self_intersection_pairs(
 ...
 for (i, wesr) in wd.edges.iter().enumerate() {
  if let Some(ed) = e_edge_data(brep, wesr.index) {
   let (sv, ev) = if wesr.orientation.is_forward() { (ed.first.index, ed.last.index) } else { (ed.last.index, ed.first.index) };
...
pub fn check_face_wire_on_surface(
 ...
 let Some(surface) = fd.surface.as_ref() else { return true };
 for wesr in {
  let TShape::Wire(owd) = &*brep.tshapes[fd.outer_wire.index] else { return true };
  &owd.edges
 } {
  if !check_edge_on_surface(brep, wesr.index, surface, tolerance) { return false; }
 }
 for iw_sr in &fd.inner_wires {
  let TShape::Wire(iwd) = &*brep.tshapes[iw_sr.index] else { continue; };
  for wesr in &iwd.edges {
...
fn check_edge_on_surface(
 ...
 let Some(ed) = e_edge_data(brep, edge_idx) else { return true };
 let Some(curve) = ed.curve.as_ref() else { return true };
 let range = ed.range;
 ...
 for (_pc_face_idx, (curve2d, _t1, _t2)) in &ed.pcurves {
  ...
  for &t_frac in &sample_ts {
   ...
   if (p3d - p_surf).length() > tolerance { return false; }
  }
 }
 true
}
...
impl SameParameterDiagnosis {
 pub fn is_clean(&self) -> bool { self.suspect_edges.is_empty() }
}
...
pub fn diagnose_face_surface_consistency(
 ...
 for (edge_idx, ts) in brep.tshapes.iter().enumerate() {
  let TShape::Edge(ed) = &**ts else { continue };
  let Some(curve3) = ed.curve.as_ref() else { continue };
  let range3 = ed.range;
  ...
  for shell_sr in &sd.shells {
   ...
   for face_sr in &shd.faces {
    ...
    for (wire_idx, wire_sr) in all_wires {
     ...
     for wesr in &wd.edges {
      if wesr.index >= tshapes.len() { valid = false; break; }
      let topods::TShape::Edge(ed) = &*tshapes[wesr.index] else { valid = false; break };
      let (sv, ev) = if wesr.orientation.is_forward() {
       (ed.first.index, ed.last.index)
      } else {
       (ed.last.index, ed.first.index)
      };
      if sv >= v_points.len() || ev >= v_points.len() { valid = false; break; }
     ...
     if n > 1 {
      for i in 0..n {
       ...
       if end_v != start_v {
        ...
        if (end_pt - start_pt).length() > tolerance { open_gaps += 1; }
       }
      }
     }
     ...
     let topological_self_intersections = vertex_count.values().filter(|&&c| c > 2).count();
     let geometric_self_intersections = count_geometric_self_intersections(&wire_verts, &v_points);

     if open_gaps > 0 || topological_self_intersections > 0 || geometric_self_intersections > 0 {
      report.wires.push(WireIssueReport {
       solid: si, shell: shi, face: fi, wire_idx,
...
pub fn diagnose_same_parameter(brep: &rcad_kernel::BRep, tolerance: f64) -> SameParameterDiagnosis {
 ...
 for (edge_idx, ts) in brep.tshapes.iter().enumerate() {
  ...
  let p_start = match brep.vertex_point(ed.first.index) { Some(p) => p, None => continue };
  let p_end = match brep.vertex_point(ed.last.index) { Some(p) => p, None => continue };
  ...
  if start_gap > tolerance || end_gap > tolerance {
   suspects.push(SuspectEdge { edge_idx, start_gap, end_gap });
  }
 }
...
pub fn analyze_shell_topology(brep: &rcad_kernel::BRep) -> ShellTopologyReport {
 ...
 for ts in &brep.tshapes {
  ...
  for shell_sr in &sd.shells {
   ...
   for face_sr in &shd.faces {
    ...
    let TShape::Wire(owd) = &*brep.tshapes[fd.outer_wire.index] else { continue };
    for wesr in &owd.edges {
     if wesr.index < edge_face_count.len() { edge_face_count[wesr.index] += 1; }
    }
    for iw_sr in &fd.inner_wires {
     let TShape::Wire(iwd) = &*brep.tshapes[iw_sr.index] else { continue };
     for wesr in &iwd.edges {
 ...
 for (ei, ts) in brep.tshapes.iter().enumerate() {
  let TShape::Edge(ed) = &**ts else { continue };
  if ed.first.index < vertex_used.len() { vertex_used[ed.first.index] = true; }
  if ed.last.index < vertex_used.len() { vertex_used[ed.last.index] = true; }
 }
...
pub fn euler_analysis(brep: &rcad_kernel::BRep) -> Vec<EulerAnalysis> {
 ...
 for ts in &brep.tshapes {
  ...
  let genus = if is_closed {
   ...
   if (2 - euler_number) % 2 == 0 && g >= 0 { Some(g) } else { None }
  } else {
   None
  };

  results.push(EulerAnalysis {
   solid_idx: si, vertices: v, edges: e, faces: f,
   euler_number, is_closed, genus,
  });
  si += 1;
 }
...
pub fn check_orientation_consistency(brep: &rcad_kernel::BRep) -> OrientationReport {
 ...
 for ts in &brep.tshapes {
  ...
  let solid_centroid: DVec3 = {
   let sum: DVec3 = solid_verts.iter()
  ...
  for shell_sr in &sd.shells {
   ...
   for face_sr in &shd.faces {
    ...
    let TShape::Wire(owd) = &*brep.tshapes[fd.outer_wire.index] else { flat_face_idx += 1; continue; };
    ...
    if n == 0 { flat_face_idx += 1; continue; }
...
pub fn richer_validity_analysis(brep: &rcad_kernel::BRep) -> RicherValidityReport {
 ...
 RicherValidityReport { check_result, shell_topology, euler, orientation, is_fully_valid }
}
...
impl SurfaceAnalysisReport {
 pub fn is_clean(&self) -> bool { self.total_issues == 0 }
 pub fn summary(&self) -> String {
  if self.is_clean() {
   format!("{} faces analyzed, no UV issues", self.faces_analyzed)
  } else {
   format!("{} faces analyzed, {} UV bounds violations", self.faces_analyzed, self.faces_with_uv_bounds_violation.len())
  }
 }
}
