    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::OpenWire {
                ...
                write!(
                    f,
                    "wire is not closed: gap={} between {:?} and {:?}",
                    gap, first_vertex, last_vertex
                )
            }
            Self::SelfIntersectingWire {
                edge1_idx,
                edge2_idx,
            } => {
                write!(
                    f,
                    "wire self-intersects at edges {} and {}",
                    edge1_idx, edge2_idx
                )
            }
            Self::OpenShell {
                ...
                write!(
                    f,
                    "shell is not closed: {} boundary edges",
                    boundary_edge_count
                )
            }
            Self::InconsistentOrientation {
                ...
                write!(
                    f,
                    "shell has inconsistent orientation between faces {} and {}",
                    face1_idx, face2_idx
                )
            }
            ...
            Self::InvalidEdgeIndex {
                edge_idx,
                edge_count,
            } => {
                write!(
                    f,
                    "edge index {} out of bounds (0..{})",
                    edge_idx, edge_count
                )
            }
            ...
            Self::InvalidParameter { param, valid_range } => {
                write!(
                    ...
                    param, valid_range.0, valid_range.1
                )
            }
...
impl BuildWire {
    ...
    pub fn build_wire_from_edges(
        ...
        if check_closed && !validate_wire_closed_internal(&wire, edges, vertices, tol) {
    ...
    pub fn build_planar_face_from_points(
        ...
        if points.len() < 3 {
            return Err(BuildError::EmptyInput(
                "points (need at least 3 for a face)",
            ));
        }
        ...
        if normal.length_squared() < 0.5 {
            return Err(BuildError::DegenerateGeometry(
                "points are collinear, cannot define a face",
            ));
        }
...
/// * `tol` - Tolerance for vertex coincidence.
pub fn validate_wire_closed(wire: &Wire, edges: &[Edge], vertices: &[Vertex], tol: f64) -> bool {
...
impl Rebuild {
    ...
    pub fn rebuild_face(face: &Face, edges: &[Edge], vertices: &[Vertex], tol: f64) -> Face {
        // Rebuild outer wire
    ...
    pub fn rebuild_wire(wire: &Wire, edges: &[Edge], vertices: &[Vertex], tol: f64) -> Wire {
        if wire.edges.is_empty() {
            return wire.clone();
        }
