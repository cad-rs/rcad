...
pub enum BlendError {
    ...
    SurfacesTooFarApart { distance: f64, max_distance: f64 },
    ...
    SpineComputationFailed { reason: String },
    /// Guide curve computation failed.
    GuideCurveComputationFailed { index: usize, reason: String },
    ...
    /// Vertex blend failed.
    VertexBlendFailed { vertex_index: usize, reason: String },
    /// Numerical failure during computation.
    NumericalFailure { description: String },
    /// Self-intersection detected in blend surface.
    SelfIntersection { location: String },
}
...
impl std::fmt::Display for BlendError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::SurfacesTooFarApart {
                distance,
                max_distance,
            } => {
                write!(
                    f,
                    "surfaces {} apart exceed maximum {} for blend",
                    distance, max_distance
                )
            }
            Self::BoundaryComputationFailed {
                surface_index,
                reason,
            } => {
                write!(
                    f,
                    "boundary computation failed on surface {}: {}",
                    surface_index, reason
                )
            }
            ...
            Self::ContinuityViolation {
                ...
                write!(
                    f,
                    "continuity violation at {}: expected {:?}, got {:?}",
                    location, expected, actual
                )
            }
            Self::EdgeFaceBlendFailed {
                edge_index,
                face_index,
                reason,
            } => {
                write!(
                    f,
                    "edge {} to face {} blend failed: {}",
                    edge_index, face_index, reason
                )
            }
            Self::VertexBlendFailed {
                vertex_index,
                reason,
            } => {
...
impl BlendContinuity {
    ...
    pub fn requires_tangent_continuity(&self) -> bool {
        matches!(
            self,
            BlendContinuity::C1 | BlendContinuity::G1 | BlendContinuity::G2
        )
    }
...
pub enum RadiusLaw {
    ...
    Linear { start_radius: f64, end_radius: f64 },
    /// Smooth radius variation defined by control points.
    Smooth { params: Vec<f64>, radii: Vec<f64> },
            ...
            RadiusLaw::Smooth { params, radii } => {
                ...
                for i in 0..params.len() - 1 {
                    if t_clamped >= params[i] && t_clamped <= params[i + 1] {
                        let alpha = (t_clamped - params[i])
                            / (params[i + 1] - params[i]).max(TOLERANCE_LINEAR_ULTRA_STRICT);
...
pub fn blend_two_surfaces(
    ...
    match params.mode {
        ...
        BlendMode::IsoParametric => {
            compute_iso_parametric_blend(surf1, surf2, curve1, curve2, params)
        }
    }
}
...
pub fn compute_rolling_ball_blend(
    ...
    let t1_min = if domain1[0].is_finite() {
        domain1[0]
    } else {
        0.0
    };
    let t1_max = if domain1[1].is_finite() {
        domain1[1]
    } else {
        1.0
    };
    let t2_min = if domain2[0].is_finite() {
        domain2[0]
    } else {
        0.0
    };
    let t2_max = if domain2[1].is_finite() {
        domain2[1]
    } else {
        1.0
    };
...
pub fn compute_spine_curve(
    ...
    let ed = match brep.tshapes.get(edge_idx) {
        Some(ts) => match ts.as_ref() {
            TShape::Edge(e) => e,
            _ => {
                return Err(BlendError::SpineComputationFailed {
    ...
    let p0 =
        brep.vertex_point(ed.first.index)
            .ok_or_else(|| BlendError::SpineComputationFailed {
...
pub fn compute_guide_curves(
    ...
    for g in 0..num_guides {
        ...
        for i in 0..n_samples {
            ...
            let guide_point =
                spine_point + radius * (angle.cos() * normal + angle.sin() * binormal);
...
pub fn blend_edge_to_face(
    ...
    let ed = match brep.tshapes.get(edge_idx) {
        Some(ts) => match ts.as_ref() {
            TShape::Edge(e) => e,
            _ => {
                return Err(BlendError::EdgeFaceBlendFailed {
                    edge_index: edge_idx,
                    face_index: face_idx,
    ...
    let p0 = brep
        .vertex_point(ed.first.index)
        .ok_or_else(|| BlendError::EdgeFaceBlendFailed {
            edge_index: edge_idx,
            face_index: face_idx,
            reason: "vertex index out of range".to_string(),
        })?;
    let p1 = brep
        .vertex_point(ed.last.index)
        .ok_or_else(|| BlendError::EdgeFaceBlendFailed {
            edge_index: edge_idx,
            face_index: face_idx,
            reason: "vertex index out of range".to_string(),
    ...
    let face_surface = face_surface.unwrap_or({ Surface3::Plane(Plane::new(p0, DVec3::Z)) });
...
pub fn blend_vertex(
    ...
    let center = brep
        .vertex_point(vertex_idx)
        .ok_or_else(|| BlendError::VertexBlendFailed {
            vertex_index: vertex_idx,
            reason: "vertex index out of range".to_string(),
        })?;

    // Find all edges connected to this vertex (via tshape iteration)
    let connected_edges: Vec<usize> = brep
        .tshapes
        .iter()
        .enumerate()
        .filter(|(_, ts)| {
...
fn eval_bspline(spline: &BSplineCurve3, t: f64) -> DVec3 {
    ...
    let alpha = (t - spline.knots[0])
        / (spline.knots[spline.knots.len() - 1] - spline.knots[0])
...
fn create_pipe_surface(
    ...
    let is_line = tangents
        .windows(2)
        .all(|t| (t[0] - t[1]).length() < tol || (t[0] + t[1]).length() < tol);
...
fn compute_blend_quality(surface: &Surface3, target_radius: f64, tol: f64) -> BlendQuality {
    ...
    let estimated_radius = if avg_curvature > 0.0 {
        1.0 / avg_curvature
    } else {
        target_radius
    };
    ...
    BlendQuality {
        ...
        max_curvature,
        min_curvature: if min_curvature == f64::MAX {
            0.0
        } else {
            min_curvature
        },
        ...
        has_self_intersection: false,
        smoothness: if deviation < target_radius * 0.1 {
            1.0
        } else {
            0.5
        },
