...
fn b7_detail() {
    ...
    let bb = make_box_brep(DVec3::new(0.0, -0.5, 0.0), DVec3::X, DVec3::Y, 0.5, 0.5, 1.0).unwrap();
    ...
    for (fi, face) in r.solids.iter().flat_map(|s|&s.shells).flat_map(|sh|&sh.faces).enumerate() {
        ...
        println!("[B7] face[{}]: {} nv={} x=[{:.2},{:.2}] y=[{:.2},{:.2}] z=[{:.2},{:.2}]",
            fi, sname, nv, xmin, xmax, ymin, ymax, zmin, zmax);
    }
    println!("[B7] TOTAL: {}f {}PLANE+{}BS", nf, np, nb);
}
