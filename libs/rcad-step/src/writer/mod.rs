...
use crate::{
    ...
    RunoutToleranceType, StepDatumReferenceFrame, StepDatumTarget, StepDimensionalTolerance,
    StepFormTolerance, StepOrientationTolerance, StepPositionTolerance, StepProfileTolerance,
    StepRunoutTolerance, StepToleranceValue, StepToleranceZoneDefinitionEnhanced,
};
use crate::{
...
use crate::{
    StepAnnotationOccurrence, StepAnnotationPlane, StepCameraModelD3, StepDatumFeatureCallout,
    StepDimensionCurve, StepNote, StepTerminatorSymbol, StepView, StepViewVolume, TerminatorType,
    ViewVolumeType,
};
...
impl Part21Writer {
    ...
    fn finish(self) -> String {
        let schema_token = match self.protocol {
            StepProtocol::Ap214 => "AUTOMOTIVE_DESIGN { 1 0 10303 214 1 1 1 1 }",
            StepProtocol::Ap242 => {
        ...
        let implementation_level = self.header.implementation_level.as_deref().unwrap_or("2;1");
        out.push_str(&format!(
            "FILE_DESCRIPTION(('{}'),'{}');\n",
        ...
        let preprocessor = self
            ...
            .as_deref()
            .unwrap_or("RCAD");
        let originating = self.header.originating_system.as_deref().unwrap_or("RCAD");
    ...
    /// Entry point for topods::BRep — routes to topods-native write_brep.
    ...
    fn write_brep(
        ...
        let mut selected_edge_set: BTreeSet<usize> =
            selection.selected_edges.iter().copied().collect();
                        ...
                        for face_sr in &shd.faces {
                            if selected_face_set.contains(&face_index) {
                                if let topods::TShape::Face(fd) = &*tbrep.tshapes[face_sr.index] {
                                    if let topods::TShape::Wire(w_outer) =
                                        &*tbrep.tshapes[fd.outer_wire.index]
                                    {
                                        selected_edge_set
                                            .extend(w_outer.edges.iter().map(|sr| sr.index));
                                    }
                                    for inner_sr in &fd.inner_wires {
                                        if let topods::TShape::Wire(w_inner) =
                                            &*tbrep.tshapes[inner_sr.index]
                                        {
                                            selected_edge_set
                                                .extend(w_inner.edges.iter().map(|sr| sr.index));
                                        }
                                    }
                                }
                            }
        ...
        let tbrep = self.tbrep();
        let is_compound = tbrep
            .tshapes
            .iter()
            .any(|ts| matches!(&**ts, topods::TShape::Compound(_)));
        let is_compsolid = tbrep
            .tshapes
            .iter()
            .any(|ts| matches!(&**ts, topods::TShape::CompSolid(_)));
                ...
                for shell_sr in &sd.shells {
                    if let topods::TShape::Shell(shd) = &*tbrep.tshapes[shell_sr.index] {
                        let face_indices: Vec<usize> =
                            shd.faces.iter().map(|sr| sr.index).collect();
                        shells.push(TopodsShellData {
                            sr_index: shell_sr.index,
                            face_indices,
                        });
                    }
                }
        ...
        for solid_data in &topods_solids {
            ...
            for shell_data in &solid_data.shells {
                ...
                for &face_tshape_idx in &shell_data.face_indices {
                    if export_all || selected_face_set.contains(&face_index) {
                        let export = self.write_face_topods(face_tshape_idx);
            ...
            let num_exported_solids = if export_all {
                ...
                if closed.len() == 1 {
                    ...
                    let solid_id = self
                        .manifold_solid_brep(&format!("solid_{}", solid_data.solid_idx), shell_id);
                            ...
                            for face_sr in &shd.faces {
                                if let topods::TShape::Face(fd) = &*tbrep.tshapes[face_sr.index] {
                                    ...
                                    for inner_sr in &fd.inner_wires {
                                        ...
                                        {
                                            face_edge_set
                                                .extend(wd.edges.iter().map(|sr| sr.index));
                                        }
                                    }
                                }
                            }
        ...
        let collect_standalone =
            export_all && (self.export_standalone_wire_overlay || !selected_edge_set.is_empty());
        unsafe {
            let tbrep = &*self.topods_brep;
                ...
                for (ti, ts) in tbrep.tshapes.iter().enumerate() {
                    if let topods::TShape::Vertex(v) = &**ts {
                        if !used_vertices.contains(&ti) {
                            standalone_point_items
                                .push(self.cartesian_point("", dvec3_to_array(v.point)));
                        }
                    }
                }
        ...
        let (ctx_name, proto_name, proto_year) = match self.protocol {
            StepProtocol::Ap214 => ("automotive_design", "automotive_design", 2000_i32),
            StepProtocol::Ap242 => (
        ...
        if export_all {
            ...
            for (i, solid_id) in solid_items.iter().enumerate() {
                ...
                let child_axis =
                    self.axis2_placement_3d("", child_origin, child_axis_dir, child_ref_dir);
                let child_uncertainty =
                    self.uncertainty_measure_with_unit_value(length_unit, write_tol);
            ...
            for (i, edge_item) in edge_items.iter().enumerate() {
                ...
                let child_axis =
                    self.axis2_placement_3d("", child_origin, child_axis_dir, child_ref_dir);
                let curve_set =
                    self.geometric_curve_set("wireframe", std::slice::from_ref(edge_item));
                let child_uncertainty =
                    self.uncertainty_measure_with_unit_value(length_unit, write_tol);
                let child_context = self.geometric_representation_context(
                    3,
            ...
            for (i, (child_rep, child_axis)) in child_reps.iter().enumerate() {
                ...
                let _child_shape_def =
                    self.shape_definition_representation(child_shape, *child_rep);
                ...
                let rep_rel = self.representation_relationship_with_transformation(
                    "", "", *child_rep, root_rep, tr,
                );
                let _cdsr = self.context_dependent_shape_representation(rep_rel, placement_shape);
                let _child_cat =
                    self.product_related_product_category("part", None, &[child_product]);
            }
        } else {
            let mut brep_rep = None;
            if export_all && !compound_items.is_empty() {
                brep_rep = Some(self.advanced_brep_shape_representation(
                    "rcad_export",
                    &compound_items,
                    context,
                ));
            ...
            if use_root_bridge {
                ...
                let root_axis =
                    self.axis2_placement_3d("", root_origin, root_axis_dir, root_ref_dir);
                ...
                let primary_rep = primary_rep
                    .unwrap_or_else(|| self.shape_representation("rcad_export", &[], context));
    ...
    fn write_face_topods(&mut self, face_tshape_idx: usize) -> FaceExportResult {
        ...
        let (
            ...
            let inner_wires_data: Vec<Vec<OrientedEdgeExport>> = fd
                .inner_wires
                .iter()
                .filter_map(|iw_sr| {
                    let topods::TShape::Wire(iwd) = &*tbrep.tshapes[iw_sr.index] else {
                        return None;
                    };
                    let oriented: Vec<OrientedEdgeExport> = iwd
                        .edges
                        .iter()
                        .filter_map(|sr| {
            ...
            && edge_entries.len() == 4
        {
            ...
            if seam_idx != usize::MAX {
                let seam_curve = edge_entries.iter().find_map(|(idx, _, _, curve, _)| {
                    if *idx == seam_idx { Some(*curve) } else { None }
                });
                ...
                if let (Some(seam_curve), 2) = (seam_curve, circle_entries.len()) {
                    let axis = face_surface
                        ...
                        .and_then(|surf| match surf {
                            Surface3::Cylinder(cyl) => {
                                Some(canonicalize_axis_sign(cyl.axis.normalize_or_zero()))
                            }
                    ...
                    let top_circle = if z_of(tbrep_local, c0.1) >= z_of(tbrep_local, c1.1) {
                        c0
                    } else {
                        c1
                    };
                    let bottom_circle = if z_of(tbrep_local, c0.1) < z_of(tbrep_local, c1.1) {
                        c0
                    } else {
                        c1
                    };
    ...
    fn write_seam_edge_curve_topods(
        ...
        let final_curve = if !pcurves.is_empty() || synthetic_curve2d.is_some() {
            ...
            for (pc_i, pce) in pcurve_entries.iter().enumerate() {
                ...
                if seam_is_cylinder && let Some(Curve2d::Line(mut l)) = curve2d {
                    let eps = 1e-9;
    ...
    fn write_surface(
        ...
        match face_surface {
            ...
            Some(Surface3::Cone(cone)) => {
                ...
                self.conical_surface("face_cone", placement, cone.radius, cone.half_angle_rad)
            }
            Some(Surface3::Torus(torus)) => {
 ...
 name, degree_u, degree_v, cp_str, mu_str, mv_str, ku_str, kv_str
        ...
        let curve_id = if ed_opt.is_none() {
            ...
            if !pcurves.is_empty() {
                ...
                for (pc_i, pc) in pcurves.iter().enumerate() {
                    ...
                    if is_cylinder && let Some(Curve2d::Line(mut l)) = curve2d {
                        let eps = 1e-9;
                    ...
                    if pcurves.len() == 1 {
                        first_plane =
                            get_surface(self.tbrep(), pc.surface_idx).and_then(|s| match s {
                ...
                if pcurve_ids.len() == 1 && first_is_line {
                    ...
                    if let Some(base_plane) = first_plane
                        && let Some((peer_surface_idx, extra_plane)) =
                            find_peer_plane_surface_for_line_edge_topods(
                                self.tbrep(),
                                edge_idx,
                                &base_plane,
                            )
                    ...
                    if extra_surface.is_none()
                        && let Some((surface_idx, plane)) =
                            find_plane_surface_for_edge_topods(self.tbrep(), edge_idx)
                    {
                    ...
                    if extra_surface.is_none()
                        && let Some(plane) =
                            find_topological_plane_for_edge_topods(self.tbrep(), edge_idx)
                    {
                    ...
                    if extra_surface.is_none()
                        ...
                        && count_plane_face_occurrences_for_line_edge_topods(self.tbrep(), edge_idx)
                            >= 2
                    {
                    ...
                    if let Some((surface_idx, extra_plane)) = extra_surface
                        && let Some(extra_curve2d) = synthesize_plane_pcurve_for_edge_topods(
                            self.tbrep(),
                            edge_idx,
                            &extra_plane,
                        )
                ...
                if let Some((face_tshape_idx, plane)) =
                    find_plane_surface_for_edge_topods(self.tbrep(), edge_idx)
                {
                    if let Some(curve2d) =
                        synthesize_plane_pcurve_for_edge_topods(self.tbrep(), edge_idx, &plane)
                    {
                        let promote_plane_line =
                            should_promote_plane_line_pcurve_topods(self.tbrep(), edge_idx);
                        ...
                        if let Some((peer_surface_idx, peer_plane)) =
                            ...
                            && let Some(peer_curve2d) = synthesize_plane_pcurve_for_edge_topods(
                                self.tbrep(),
                                edge_idx,
                                &peer_plane,
                            )
                        ...
                        } else if count_plane_face_occurrences_for_line_edge_topods(
                            self.tbrep(),
                            edge_idx,
                        ) >= 2
                            && let Some(peer_curve2d) = synthesize_plane_pcurve_for_edge_topods(
                        ...
                        find_topological_plane_for_edge_topods(self.tbrep(), edge_idx)
                    {
                            ...
                            synthesize_plane_pcurve_for_edge_topods(self.tbrep(), edge_idx, &plane)
                        {
                            ...
                            let surface_id =
                                self.write_surface(Some(Surface3::Plane(plane)), Some(placement));
                            let param_curve_id = self.write_curve2d(Some(curve2d));
                            let def_rep = self.definitional_representation(param_curve_id);
                            let pcurve_id = self.pcurve(surface_id, def_rep);
    ...
    fn write_curve2d(&mut self, curve2d: Option<Curve2d>) -> u64 {
        match curve2d {
            ...
            Some(Curve2d::Parabola(_))
            | Some(Curve2d::Hyperbola(_))
            | Some(Curve2d::Offset(_))
            | Some(Curve2d::AHTBezier(_))
            | Some(Curve2d::TBezier(_)) => {
                let p = self.cartesian_point_2d("pc_origin", [0.0, 0.0]);
                let dir = self.direction_2d("pc_dir", [1.0, 0.0]);
                let vec = self.vector("pc_vec", dir, 1e-9);
                self.line("pcurve_line", p, vec)
    ...
    fn write_curve3_entity(
        ...
        match curve {
            ...
            Curve3::Trimmed(tc) => {
                self.write_curve3_entity(tc.basis_curve(), start_point, end_point)
            }
        }
    }
 ...
 name, description, rep_1, rep_2, transformation
    ...
    fn next_assembly_usage_occurrence(
        ...
        self.push(format!(
            ...
            id, name, description, relating_product_definition, related_product_definition
        ))
    }
    ...
    fn write_form_tolerance(&mut self, tol: &StepFormTolerance) {
        ...
        self.push(format!(
            "{}({},{},{},{})",
            entity_name, name, desc, val, shape
        ));
    }
    ...
    fn write_tolerance_zone_definition_enhanced(
        &mut self,
        def: &StepToleranceZoneDefinitionEnhanced,
    ) {
    ...
    fn write_view_volume(&mut self, volume: &StepViewVolume) {
        ...
        let view_center = volume
            .view_center
            .map(|v| format!("({},{},{})", v[0], v[1], v[2]))
            .unwrap_or_else(|| "$".to_string());
        let view_plane_dist = volume
            .view_plane_distance
            .map(|v| format!("{}", v))
            .unwrap_or_else(|| "$".to_string());
        let up_dir = volume
    ...
    fn write_terminator_symbol(&mut self, symbol: &StepTerminatorSymbol) {
        ...
        self.push(format!("{}({},{})", term_type, name, curve));
    }
        ...
        if bytes[i] == b'\\' && i + 2 < len {
            let is_directive = match bytes[i + 1] {
                ...
                b'P' => {
                    i + 2 < len
                        && bytes[i + 2] != b'\\'
                        && find_closing_backslash(s, i + 2).is_some()
                }
                b'X' => {
                    if bytes[i + 2] == b'\\' {
                        // \X\ (single char after)
