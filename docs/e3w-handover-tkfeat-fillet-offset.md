...
> **一句话（追加 49 收尾态，2026-09-15 —— 最新，读这一行即可开工）**：**工作口径仍是"先译后调"**。本轮为**写者迁移第二步 + reshape 池外感知 + MakeOffset 盘点轮（2 后台代理 + 主代理）**，落在追加 48 之上：**① pcurves 写者迁移第二步——路由完成**：剩余 map-only 写站全库双写化（~22 站：kernel `same_parameter` 平面 pcurve、builder.rs `materialize`/`bridge` 两 or_insert 站（**push 仅在 Vacant 时执行**——回归教训：无条件 push 会复活被表示遮蔽的死写，曾翻 3 例 builder_stage + 2 例 HLR，同步把 `materialize` pass-A 守卫与 `same_parameter` 守卫改双存储感知）、evolved_d、sweep_b/sweep_builder 四 helper **补 UpdateCurves 移除臂**（W1 保真缺口）、face_restrictor 池臂、generator bind_pcurve/bind_seam_pcurves（seam 补 map 镜像）、fillet 4 站、thru_sections 4 站 + 本地 `bind_representation` 助手、geom_populate、extrude_profile 5 站（**闭合缝分支不推单 pcurve 表示**——OCCT 只存 CurveOnClosedSurface）、shhealing 2 站（(0,0)/(0,loc) 键控表示约定）、repair extra4/extra5（**extra5 remap 补 representations 键重映射**——双存储一致性缺口）+ primapi 全家直插（box/revol 9/planar 2/face/cylinder 6/cone 4/sphere 2，新共享 `primapi/pcurve_bind.rs` 助手）+ rcad-step 导入站）· **② PaveFiller prepare migrate 位点：决策退役**（写者全双写后 map-only 条目不可能再产生；prepare 加 migrate 是 OCCT 没有的自创步骤；**读者普查代理**：~102 个 READ 站分六簇（规范层/BOP 主链/brep_fill 系/feat 克隆/hlr-topalgo/自创工具区），serde 安全证实（pavefiller/builder 基线 JSON 零 pcurves，`skip_serializing_if` 下删字段兼容旧 JSON），顺带发现 offset_wire.rs:499 双写下重复计数现行 bug → **删除终局 = 下一轮 4 批机械扫描 + 3 项架构决策**）· **③ reshape.rs 池外感知**（W2 移交项收口）：本域哨兵签名判别 `occt_shape_is_null`（null 哨兵 = index==MAX + 零容差 Vertex；真实顶点 tolerance=CONFUSION 永不碰撞），replace/is_recorded/value/status/value_leaf/apply 全部 15 处 IsNull 门换判别式——池外形状从此可记录替换、value 对未记录池外形状返回自身；**W2 引擎侧绕开退役**（update_tolerances 的 Value 归一化删除——根缺陷修后它会遮蔽合法 Remove）；4 判别测试 + **扰动实证**（旧 conflating 形下 3 新测试 FAILED）；**kernel is_null 语义变更仍记档为上位决策** · **④ BRepOffset_MakeOffset 盘点（代理）**：**追加 48 的"剩余最大翻译目标"定性过期**——rcad offset/ 52,085 行 vs OCCT ~46,245 **已近乎全量**；真缺口 = **A 组 ~13 处陈旧 GAP 载体**（真身在库只差接线：VolumeProperties→gprop/props.rs:1592、BuildCurve3d→geom_lib.rs:645、BRepCheck_Analyzer→brep_check_analyzer.rs:478（签名差）、WireExplorer→brep_tools_wire_explorer.rs、ProjLib_ProjectedCurve→proj_lib/proj_lib_projected_curve{,_b}、ShapeCustom_Curve2d→shape_custom_curve2d.rs:414、GeomProjLib::Curve2d→geom_proj_lib/mod.rs:27、Geom2dConvert_ApproxCurve→geom2d_convert（44 号轮）、GeomFill_Generator→thru_sections.rs、GC_MakeCylindricalSurface→gc/surfaces.rs:98；真缺仅 gce_MakeDir/gce_MakeCone ~382 行）+ BRepAdaptor_Curve(E,F) 3D 回退闭合（Adaptor3d_CurveOnSurface）；**代理 B 清单第一名" BRepLib_MakeFace 缺失 924 行"被推翻**（在 topalgo/brep_lib/make_face.rs:57）；8 体积断言全走 perform_by_join→MakeOffsetShape L837-1081→BuildOffsetByInter 链，垃圾巨值 = Deboucle3D/SelectShells 回绕链未收边界而非体积积分器。
> **门槛（当前基线）**：六门槛 **507/0/0 · 803/0 · 36/36 · 26/26 · 76/76 · 1/1**（algo 503→507 = reshape 池外判别 4 测试）；tkgeom_algo_gtests **135/0**；八网格 **8/8**（375/378/379/373/12/102/83/110，最终 exe 重编后实测）；16 域网格**逐格同追加 42–48 基线**（12/10 · 50/48 · 46/44 · 12/12 · 8/8 · 1/1 · 2/2 · 15/15 · 6/6 · 5/5 · 26/26 · 4/0 · 32/0 · 10/0 · 2/0 · 19/19——"零可见翻转"第十八轮）。
> **★ 本轮发现**：① **or_insert 类写站的双写 push 必须 Vacant 条件化**——map-only 时代的"死写"（被 representations 主读遮蔽）在加表示臂时会复活并替换真 pcurve（cylinder_x_cylinder_rotated 系 + HLR bug25813_1 当场翻车，门槛抓到）；② **普查窗口式 grep 会被"helper 调用式双写"骗过**（`bind_representation(ed,...)` 调用行不含 representations 字样）——单行窗口普查只能当初筛，逐站读上下文才是终审（本轮 primapi 漏网 14 站靠第二遍精细普查抓回）；③ **代理盘点报告的文件名/行号/缺失声明都会漂移**——A 组接线前必须逐站回源（B 清单第一名 BRepLib_MakeFace 即为反例）；④ **根目录 `libs/rcad-algo/`（仅 brep_fill 一文件）是游离残留物**，根 workspace 的 path-dep 实际指向 `rcad/libs/rcad-algo`——八网格验证链有效，但根仓库那目录该人工清理。
> （历史：追加 21–48 收尾态见下方存档块；追加 48 = 基建 + 池模型轮：pcurves 写者第一步 + UpdateTolerances 池外写 + geom 拆分 + offset failure-map。）
...
## 0. 新 session 一句话提示词（直接粘贴 —— 追加 49 收尾态，2026-09-15）

> 读 `rcad/docs/e3w-handover-tkfeat-fillet-offset.md`（本交接：门槛实测值 / 提交链 / 三域队列 / 配方 / 坑清单；
> **先读顶部"追加 49 收尾态"一行 + §0.0y**）与 `rcad/docs/tkfeat-fillet-offset-port-plan.md` §E3-W 追加 11–49
> （权威脉络，**追加 49 是当前状态**）；
> 先 `cd rcad` 跑 6 条门槛确认 **507/0/0 · 803/0 · 36/36 · 26/26 · 76/76 · 1/1**，
...
### 0.0y 追加 49 收尾态（2026-09-15；**最新** —— 写者迁移第二步完成 + reshape 池外感知 + MakeOffset 盘点：双写收官轮）
