...
> **一句话（追加 42 收尾态，2026-09-15 —— 最新，读这一行即可开工）**：**工作口径仍是"先译后调"**。本轮为**引擎落地轮（并行 6 代理）**：**① AppBlend_AppSurf.gxx 全量新译**（1748 行三文件，`TheSectionGenerator` trait 泛型使 GeomFill_AppSurf/GeomFill_AppSweep 双实例共享——Both Perform 重载 + 全部 lxx 存取器；**pipe.rs `approx_surf` 与 thru_sections_b `total_surf` 双消费点激活真引擎**；仅 InternalPerform 的 UseSmoothing 子分支（AppDef_Variational，gxx L474-541）留失败路径载体）· **② BRepAdaptor_CompCurve 全量新译**（~880 行：Initialize 双形态/Intervals 双解析方向/Prepare+InvPrepare/链式缩放 D0-D3/DN；PipeShell 的 D1 消费层变真身；剩一层 = geomfill `section_placement.rs` Perform 需改接 `Arc<dyn Adaptor3dCurve>`——记队）· **③ C1 拼接家族全量入核**（kernel math/hermit/ 双文件 + c1_concat.rs：Hermit::Solution Geom2d 臂 + ConcatC1 + statics 全套 + FunctionReparameterise + Geom2dBSplineCurve D0/D1/SetPeriodic + **C0BSplineToC1BSplineCurve 落地**；16/16 独立闭式测试——**SameParameter 前置链就此全通，下轮只剩引擎本体 ~705 行**；3D Hermit 臂因无 OCCT 成员式 Geom_BSplineCurve 结构体记档顺延）· **④ GeomFill_LocFunction**（按该 OCCT 版本实际形态：ctor+D0/D1/D2/DN；**build_product 变真体** + OCCT 调用点注释态不可达注记）· **⑤ Convert 椭圆/双曲/抛物精确转换**（BuildCosAndSin 周期重载 L626-785 + 三 conic + SetPeriodic；trimmed/full 椭圆、双曲、抛物 staged 臂退役；9 闭式测试；**2D full-ellipse 臂仍 staged**：BSplineCurve2 无周期标志）· **⑥ GCPnts_TangentialDeflection 内核迁移**（逐字节移动 + CurveToolAsGCPnts adapter 桥 + DeflCurvIntervals 调用点补全 OCCT L83-91；**endless_loop_prevention gtest 首次转绿**——另修 sngrl_func 的 d4/d5 预存错路：OCCT EvalD2/D3 的 aD4/aD5 是基曲线 DN 而非 SnglrFunc::dn）· **⑦ sweep.rs 拆分** 2684→1662 + sweep_approximation.rs（提取逐字节 diff 验证）。
> **门槛（当前基线）**：六门槛 **454/0/0 · 765/0 · 36/36 · 26/26 · 76/76 · 1/1**（algo 452 → **454**、kernel 742 → **765** = 本轮新引擎的 23 个独立闭式测试；**endless_loop_prevention gtest 首次转绿**：tkgeom_algo_gtests 135/0——GCPnts 内核迁移 + sngrl_func d4/d5 错路修复）；八网格 **8/8**（375/378/379/373/12/102/83/110，exe 重编）；16 域网格**逐格同追加 41 基线**（12/10 · 50/48 · 46/44 · 12/12 · 8/8 · 1/1 · 2/2 · 15/15 · 6/6 · 5/5 · 26/26 · 4/0 · 32/0 · 10/0 · 2/0 · 19/19——thru_sections 的真引擎路径在域网格断言层之下，移动未达）。
> **★ 本轮发现**：① **B5 预存偏差记档**：`bspline_curve_builder_2d` 左手系 conic 框架镜像错误（gp_Trsf2d.cxx L48-70 的 mirror+transform 应塌缩为 `Loc + x·XDir + y·YDir`，rcad 翻转 y 偏移）——右手系（全部现有 circle 用例）精确，左手系待修；② **B4 前提修正**：本 OCCT 版本的 GeomFill_LocFunction **没有** Init/InitDS/SectionUpdater（早前记档描述不符），按实际源码形态译出——"记档必须回源"再 +1；③ 8 轮并行代理累计**零文件冲突**：mod.rs 注册权单点化 + #[path] 子模块声明是可复制的切分法。
> （历史：追加 21–41 收尾态见下方存档块；追加 41 = AppDef_Compute + Approx_SweepApproximation + 面级 BSpline + GProps + CompCurve 双类 + pipe law 链 + en_large_face 补全 + math_Vector 内核缺陷修复。）
...
> **先读顶部"追加 42 收尾态"一行 + §0.0r**）与 `rcad/docs/tkfeat-fillet-offset-port-plan.md` §E3-W 追加 11–42
> （权威脉络，**追加 42 是当前状态**）；
...
### 0.0r 追加 42 收尾态（2026-09-15；**最新** —— 引擎落地轮：AppBlend_AppSurf + BRepAdaptor_CompCurve + C1 拼接家族 + LocFunction + Convert conics + GCPnts 内核迁移）
