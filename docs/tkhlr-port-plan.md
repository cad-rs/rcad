...
> **交接快照（2026-09-07 session 19 终 —— 取代下方 session 18 终快照，两份都要读）**
> - **提交链（本子模块）**：`<本次提交>`（apply_transform regularity 修复 + wd1/wd2 哑槽真对齐 + acceptance VCompute 树组装 + 9 文件探针删净）；根仓库指针同步。
