...
> - **session 11 进行中（P/Q/R/S 四代理 + 主线程）**：
>   1. **Q（intersector 对齐）**：hider 双锚转绿——根因不在 intersector 层，而是 hider 测试 fixture 的对象寿命（栈上 Data 调 update() 植入 &myProj 后被 Arc::new 移动，指针悬空）；修复 = 先 Arc/Box 稳定地址再 update（生产 InternalAlgo 同形）。**ON 锚的"失败"经安装版 OCCT 8.0.0 二进制求证是手工对拍错误**：`HLRAlgo_EdgeStatus::Hide` 的 `if (!OnFace)` 短路门（OCCT 全历史存在）使共面 ON 情形整体可见——rcad 行为与 OCCT 一致，测试期望改为 OCCT 真值。全量 **354/0/0 零 ignore**。
>   2. **R（fclass2d 序 + MakeEdge2d）**：fclass2d.rs 的 wire-walk vmap 从 HashMap 改 IndexedVertexMap（OCCT NCollection_IndexedMap 插入序语义，BRepTools_WireExplorer.cxx L224/L315-323 依据；双链路同修，3 锚点含 3 实例一致性）；kernel 新增 `topo/brep_lib/`（BRepLib_MakeEdge2d.cxx L17-669 全文 1:1 + 全 ErrorStatus 臂 + ElCLib AdjustPeriodic/LineParameter/CircleParameter 依据；14 锚点）；kernel lib **663**。
...
>   1. **第一波（F/G 并行）**：`geomalgo/hatch/hatcher.rs`（1995 行 = Hatcher.hxx 209 + cxx 1919 + lxx 257 全 1:1：add_element 双载 / rem_element / add_hatching / trim 三重载 / global_transition L855-1121 / compute_domains L1128-1806 / domain / lxx 访问器组；**cxx L934 Param 无条件覆盖死代码（finding #25）与 L545 无条件 cout 照抄**；锚点 5 个）+ `topalgo/brep_top_adaptor/tool.rs`（BRepTopAdaptor_Tool 165 行：my_topol_tool 值所有权 + my_h_surface = Option\<SurfaceHandle\>）+ `hlr/topo_brep/` 数据层（Data 148hxx+303cxx+lxx、FaceData、VData；**map 键 = Shape::ptr_id()（TopTools_ShapeMapHasher 语义）、Clean() no-op 照抄、InsertBefore 迭代器不移动语义**）。
>   2. **第二波（H/I/J 契约化并行）**：`hlr/topo_brep/ds_filler.rs`（1422 行 = cxx 758 全 1:1：Insert 的 MST 缓存路径、InsertFace 的 Walking 三分支（<1 失败 / <5 度1 BSpline / else BRepApprox_Approx 链）、IntL 重合顶点合并循环、**L447-450 `Maxz>Maxx → Maxx=Maxy` OCCT 笔误照抄**、`Approx.Value(1)/Value(2)` 参数被忽略的核对；kernel topods.rs 补 3 个 BRepBuilder 方法（add_to_edge/remove_from_edge/update_vertex_on_edge，各带 OCCT 行号）；锚点 8 个）+ `face_iso_liner.rs`（~990 行：Perform 的 UVBounds 无穷 clamp / Hatcher 五入口消费 / MakeVertex / MakeIsoLine；**MakeEdge 重置 INTERNAL 方向为 FORWARD 怪癖测试锚定**；kernel 边界以私有 stand-in 翻译（各带 OCCT 行号）；锚点 5 个）+ `out_liner.rs`（~1050 行：Fill 的 Perspective 分支 / ProcessFace 的 SplE 替换 + SameEdge 判据（0.34/0.66 参数点、1e-14）/ BuildShape 三轮 avoid 循环；**cxx L106-123 注释掉的 splitted 预扫描照抄为无条件 EmptyCopied**；TopAbs 复杂度排序与 rcad ShapeType 相反的换算；锚点 3 个）。
>   3. **集成**：H↔I 契约签名（`perform(fi, f, ds, nb_isos)`）一次对接成功；J 适配 H 落地的 `insert(brep, ...)` 签名（rcad 数据模型：builder arena 显式出入参）；4 个 topalgo 类型补 `#[derive(Clone)]`（OCCT DataMap::Bind 值拷贝语义）。
> - **HLR 消费链现状（3d 全链就绪）**：OutLiner::Fill(Projector, MST, nbIso) → DSFiller::Insert(Contap_Contour + BRepApprox_Approx + FaceIsoLiner/Hatcher) → HLRTopoBRep_Data（SplE/IntL/OutL/IsoL/OldS）。`hlr/brep/` 十三件 + 3e 干涉八件；BRepApprox_Approx 链路全通。
> - **下一 session 入口 = Stage 3f**：HLRBRep_Data（2683 行拆 `brep/data/` 子模块，持 Arc\<BRep\> 所有权；落 3e 的 EdgeInterferenceTool `Data` trait；EdgeData::Set 的 TopoDS→CurveView 桥接）→ 3g（Hider 852 → ShapeBounds → InternalAlgo 1020 → Algo → HLRToShape + 烟囱测试）→ Stage 4 验收闭环。
> - **已知遗留（非 3f 验收项）**：Contap quadric-exact 死分支仍待 Adaptor3d_CurveOnSurface（3f+）；landed approx_int 引擎的非 Bezier ComputeLine 分支未落（buildCurve L655+ else 臂）；DSFiller `insert` 签名带 rcad `&mut BRep` arena 参数（OCCT 全局 TShape 图显式化，备案）；FaceIsoLiner pcurve key 用 `(face.ptr_id(), face.location)` Shape-only 约定（单位 location 输入不受影响）。
> - **session 8 完成清单（3c-2 三步全关 + 并行加做）**：
>   1. **引擎接缝接通**：`brep_approx/` 目录化（mod.rs 数据层 1912 行 + `seams.rs` 接缝 425 行）——`impl ApproxIntWLine for ApproxLine`（NbPnts/Point(i).Value/ParametersOnS1/S2）+ `impl ApproxIntMultiLine for TheMultiLineOfApprox`（value/tangency/what_status/make_ml_* 转发 + sub_line 按 ApproxInt_Approx.gxx L548-564/L638-654 构造，**字面怪癖 P2DOnFirst=myData.ApproxU1V1 照抄**）。锚点 4 个（WLine 缝读数、转发+null-SvSurfaces 失败路径、sub_line 字面、引擎端到端抛物线）。
>   2. **函数族**（并行子代理 A/B 产物）：`brep_approx_prm_prm.rs`（2000 行：ZerParFunc=IntImp_ZerParFunc.gxx 全文+lxx、Int2S=IntImp_Int2S.gxx/.lxx 含边界溢出链与 carreau 递归、PrmPrmSvSurfaces=ApproxInt_PrmPrmSvSurfaces.gxx 实现 SvSurfaces；连带 IntImp 包级 `choix_ref`/`compute_tangence`/`ConstIsoparametric`；锚点 6 个）+ `brep_approx_imp_prm.rs`（1786 行：ZerImpFunc=IntImp_ZerImpFunc.gxx/.lxx、ImpPrmSvSurfaces=ApproxInt_ImpPrmSvSurfaces.gxx 含 IsSingular/SingularProcessing 4 case/NonSingularProcessing/FillInitialVectorOfSolution 周期平移、solver 双路径；**gxx L611 `SetCoord(tu2, tu2)` 笔误照抄**；锚点 3 个）。集成改造：`TheMultiLineOfApprox<'a>` 加生命周期（OCCT `void* PtrOnmySvSurfaces` 的借用语义，sv 槽 = `Option<Arc<dyn SvSurfaces + 'a>>`；BRepAdaptorSurface 本体零改动）。
>   3. **BRepApprox_Approx 本体**（并行子代理 C 产物）：`brep_approx_approx.rs`（747 行：`BRepApproxApprox { apx: WLineApprox }` 壳——perform_wline=gxx L184-218、perform_prm_prm=L226-343 quadric switch（GetType 双查 + switch(typeS1) 四臂 + default 嵌套 switch(typeS2) + SecondIsImplicit）、perform_implicit=L350-395（isTheQuadFirst 选 MyImplicitFirst 态 + SetUseSolver(false)）、SetParameters/accessors 转发；hxx L113 无参 Perform() OCCT 无定义体不落）。引擎唯一重构（零行为变更，236 基线验证）：`WLineApprox::perform` 拆出 `perform_impl(..., cut)`——gxx Perform(WLine) Init(true) vs PrmPrm/ImpPrm 尾段 Init(cut=myBezierApprox) 的字面差异。锚点 4 个（双平面 PrmPrm 端到端、平面 Quadric×斜平面端到端、quadric switch 调度等价证明 + 平面×圆柱臂、WLine 无 sv 路径）。
>   4. **并行加做（D/E 代理）**：① **Geom2dHatch 叶子层**（`geomalgo/hatch.rs` → `hatch/` 目录 9 文件 2545 行：HatchGen 六类（IntersectionType/ErrorStatus/IntersectionPoint/PointOnElement/PointOnHatching/Domain）+ HatchElement/HatchElements（CheckPoint/Reject/Segment/OtherSegment 逐行 + **OCC12627 magic 拷贝构造**手写 Clone）+ HatchIntersector（base=geom2d_int::GInter）+ FClass2dOfClassifier/Classifier（TopClass 两个 .pxx 模板体内联）+ Hatching；锚点 8 个，tkgeom_algo 既有 geom2d_hatch 契约 6/6 零回归）；② **Stage 3e 干涉八件**（`hlr/brep/`：bi_point/bi_pnt_2d/area_limit/edge_ilist/vertex_list/face_iterator/edge_interference_tool/edge_builder，~2400 行；EdgeBuilder 19 方法全 1:1 含 Drop=析构、Arc::ptr_eq=句柄相等（null==null true 语义）、**HasArea 的 right==myLimits 约定**与 Orientation() 末尾 return EXTERNAL（WNT 警告怪）照抄；EdgeInterferenceTool 经 `Data` trait 镜像 4 个 HLRBRep_Data 成员（3f Data 落地时实现即通）；锚点 11 个，hlr::brep 33/33）。
> - **3d 勘察结论（session 8 实测，下一 session 直接开工）**：HLRTopoBRep 本体 ~1940 行 .cxx（DSFiller 758 + Data 303+148hxx + FaceIsoLiner 488 + OutLiner 344+77hxx+lxx + FaceData/VData 小件）。依赖矩阵：DSFiller 依赖 BRepApprox_Approx（✅ session 8）+ AppDef_BSplineCompute（✅）+ BRepTopAdaptor（✅）+ Contap（✅）+ BRepExtrema_ExtPC/LocateExtPC（✅ topalgo/brep_extrema + kernel extrema）；FaceIsoLiner 依赖 **Geom2dHatch_Hatcher 主引擎（1919 cxx+209 hxx+257 lxx，唯一缺口——叶子层 session 8 全落，Hatcher 只需 AddElement/AddHatching/Trim/ComputeDomains/Domain 五入口）**；OutLiner 依赖全就绪（HLRAlgo_Projector/Contap_Contour/BRepAdaptor_Curve）。**3d = Geom2dHatch_Hatcher 主引擎 + HLRTopoBRep 包，预计一个 session。**
> - **并行子代理模式（实证有效，沿用）**：单 session 吞吐 ≈3 倍。做法：主代理先做文件切分（每代理独占 1-2 个文件，mod.rs 注册由主代理完成）+ 发自包含 brief（OCCT 绝对路径 + 目标文件 + landed 机制清单 + 1:1 纪律 + 锚点要求 + 验证命令 `cargo test -p rcad-algo --lib <module>` 全绿才返回）；代理并行期间主代理不得触碰其文件，全部交卷后统一集成编译 → 全量回归 → 集成提交。**session 8 实证补充**：(a) 集成前主代理预创建骨架文件 + 预注册 mod.rs，避免多代理并发注册冲突；(b) 有编译依赖的下游代理（C 依赖 A/B 类型）等上游交卷后再发，主代理在 brief 里定死跨代理类型契约（类型名/构造签名/生命周期形状）；(c) 等待期间主代理做集成审查（git diff 审计子代理改动面 + 怪癖照抄抽查）与下一 Stage 勘察。
> - **HLR 消费链现状**：`hlr/brep/` 十三件 + 3e 干涉八件（session 8）；Contap 可经 BRepTopolTool 的 ContapDomain 直接消费真实面域；CInter/Intersector 可对投影边跑 2D 相交（conic 全臂闭式激活）、Intersector 可跑线/面 CS；BRepApprox_Approx 链路全通（PrmPrm/ImpPrm/quadric switch）。
> - **已知遗留（非 3d 验收项）**：Contap quadric-exact 死分支仍待 Adaptor3d_CurveOnSurface（3d+）；EdgeData::Set 的 TopoDS→CurveView 桥接在 3f Data 落地；landed approx_int 引擎的非 Bezier ComputeLine 分支未落（buildCurve L655+ else 臂，OCCT Init 双计算线——anchor 不触发，3d DSFiller 消费前评估）。
> - 已确立的翻译模式（沿例勿改）：OCCT gxx 模板参数 → Rust trait；具体实例化 = 类型别名（GInter/CInter 先例）+ marker 类型（`PhantomData<&'a ()>` 生命周期弹性）；工具静态组绑进 landed 泛型引擎（as_any = DownCast 先例）；gxx/lxx 内联翻译；每函数 `// OCCT <文件> L<起>-<止>` 标注；每个叶子翻译配 OCCT 解析锚点单测。**HLR 方法论特例**：HLRBRep_Surface 持 `my_proj: *const Projector` 裸指针、Intersector 持 `my_surface: Option<*const Surface>`（OCCT 同形，Data 拥有点对象）；`HLRBRep_Curve::d3_2d`/`dn` 是 OCCT 空函数体的照抄；**ApproxInt_SvSurfaces 的 void* 用 `Arc<dyn SvSurfaces + 'a>` 生命周期槽**（TheMultiLineOfApprox<'a>）。
> - 踩坑累积：**(6)** `&Arc<dyn Trait>` → `&dyn Trait` 不自动 deref 强转——调用点必须 `.as_ref()`；**(7)** heredoc 大文件追加第 7 次截断——一律 Write 落盘 + `cat >>` 拼接，截断后 `sed -i 'N,Md'` 清尾再重拼；**(8)** `git checkout <file>` 恢复前先确认目标状态；**(9)** 1:1 审查点：OCCT 字面 no-op（如 Lin-Circ 的 `if (Cinf >= Csup)` 同值自赋）与未初始化 out 参数（Lin-Lin 的 Pos1a/2a、ImpPrmSvSurfaces 的 `SetCoord(tu2, tu2)`）必须照抄/以中性默认初始化，不许"顺手修正"；**(10)** 具体类型的方法解析优先命中固有方法——经 trait 缝调用必须显式限定 `<T as Trait>::method(...)`（seams.rs 测试踩过）；**(11)** 引擎泛型缝的 value 访问器分路径（Bezier 读 my_bez_to_bspl、非 Bezier 读 my_value）——测试一律走 `apx.value()` 访问器不摸字段。
