...
> **交接快照（2026-09-06 session 18 终 —— 取代下方 session 17 快照，两份都要读）**
...
> - **新增回归测试（rcad-algo lib，366+1 通过）**：`bug25813_1_hidden_compound_three_arcs`（全管线：隐藏 3 弧 + mass 62.3360975 + visible mass 204.1903269）、`bug25813_1_intl_wedges_present_and_reoriented_after_update`（IntL wedge 注册+OrientOutLine 重写断言）、`bug25813_1_interference_loop_over_outlined_faces`（诊断：RJ 真值复现 HIT In@1.4289/3.2835 + CLASSIFY 门判打印）。注意：直接 `ds.init_edge()` 的诊断绕过 `init_bound_sort` 会使 MoreEdge phase-2（全局排序边表）为空——复现必须走 `algo.hide_shape(1)` 或补 `init_bound_sort`。
> - **缺口③（下一 session 首任务，已定位到语句级）**：rcad 把两条缝边（大缝 24.4949@x'=7.071、小缝 17.9629@x'=5.657）放进了 **RgNLineVCompound**（42.458 全部为多余——OCCT 结果 15 边 = 11 弧 + 4 轮廓线，mass 204.19 不含缝；缝边在 OCCT 的 reg1=regn=1(rg=CN) 属实 X22，但结果里没有）。待查：`HLRBRep_HLRToShape.cxx` 的 UpdateEdgeStatus/DrawEdge 绘制门（typ==4: `edf.RgNLine() && !Itf.OutLine()`，Itf 为面 w_edge 旗标）与 per-edge 分支（L246-254 `ed.RgNLine()`）、`added`/`Used` 簿记、以及 DS 侧缝边 regn 旗标的 rcad 载入（EdgesToFaces.Extent()==2 + Continuity(CN) 路径）。harness 口径的 Iso 双算（+17.96 → 生成测试现值 222.153251）与 RgN 42.458 是两个独立的缝线记账问题，一并收。
...
> - **验证命令（不变）**：生成测试 `cargo test -p occt-generated-tests --test generated_occt_boolean_hlr_exact_hlr`（现值 222.153251，缺口③后应到 204.19）；acceptance `cargo test -p rcad-algo --lib acceptance -- --ignored --nocapture`；回归 lib 366+1+2i / builder 76 / pavefiller 26 / tktopalgo 36 全绿。OCCT 插桩纪律：同命令 build+cp+grep+run，用后还原。
