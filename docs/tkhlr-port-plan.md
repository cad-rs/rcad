...
> - **下一步 = 2a-3④：IntCurveSurface_HInter 组装（合计 ~4k 行）**——HInter.cxx（581，薄壳：全部委托模板）+ `IntCurveSurface_Inter.pxx`（1006，InterImpl::Perform/PerformBounds/PerformPolygon/PerformPolyhedron/PerformPolygonPolyhedron/InternalPerform/InternalPerformBSB）+ `IntCurveSurface_InterUtils.pxx`（1637，DoSurface/DoNewBounds/InternalUtils 网格化与相切分类）+ `TheHCurveTool`（Adaptor3d_Curve 的工具）+ `TheQuadCurvExactHInter`+`TheQuadCurvFuncOfTheQuadCurvExactHInter`（二次曲面×曲线精确求交，消费 IntAna_IntConicQuad）。
>   已就绪的积木：TheInterferenceOfHInter（=2a-3① alias）、ThePolygonOfHInter/ThePolyhedronOfHInter 及其工具 marker、TheCSFunctionOfHInter（=IntImp_ZerCSParFunc 实例，int_imp/zer_cs_par_func.rs 已有泛型体，HLRBRep 实例化是 3a 纯 impl）、TheExactHInter（=IntImp_IntCS 实例，int_imp/int_cs.rs）、Bnd_BoundSortBox、IntAna。
>   rcad 接法建议：HInter 引擎结构体 + `HSurfaceTool`/`HCurveTool` trait（沿 PSurfaceTool/CurveTool3d 先例），Inter.pxx/InterUtils.pxx 译为 int_curv_surf/h_inter/ 子模块；测试锚点用平面 HSurfaceTool×直线曲线（直线穿越平面 → IntCS 精确点）+ 圆柱面×母线圆（QuadCurvExact 路径）。之后 2a-4 TopolTool → 2b Contap 主体。
