...
> - 提交链（rcad 子模块分支 `sd-hash-wip`）：`7011f938` 计划落盘 → `89fdd3ec` Stage 0 → `78789bbf` Stage 1 → `6f0f12b8` 2a 审计+Bnd_Range → `34dfaa5b` 2a-1 批次（Intf 数据类/BoundSortBox/PGProps+PEquation）→ `da383601` 2a-1 完成（IntCurve 三泛型）→ `5ef9ae00` 2a-2 IntImp 部分（ZerCSParFunc + IntCS）→ `57ab4ff5` 2a-2 完成（IntImpParGen_Intersector 泛型引擎 + IntConicCurve 双泛型）→ `5cde756c` 2a-3①（Intf_InterferencePolygonPolyhedron 1368 行 1:1）→ `f528da46` 2a-3②（Intf_InterferencePolygon2d 819 行 1:1 + Polygon2dGen 实现 IntfPolygon2d trait）→ `3466868b` 2a-3③（IntCurve_IntPolyPolyGen 1797 行 1:1，IntCurveCurveGen 两个 unimplemented 臂接通）→ 2a-3④ 勘察完成（HInter 组装，写码下一 session）。
> - 回归基线（全部全绿）：algo lib 135/135（130+5 新增）、kernel lib 645/645、tkhelix_gtests 16/16、pavefiller_stage_tests 26/26、tkgeom_algo_gtests 134+1。
> - **下一步 = 2a-3④：IntCurveSurface_HInter 组装（勘察完成 2026-09-04，未写码）。**结构：HInter.cxx（581）是薄壳，全部委托 `IntCurveSurface_InterImpl::*`（Inter.pxx 1006）与 `IntCurveSurface_InterUtils::*`（InterUtils.pxx 1637）。
>   Inter.pxx 函数清单（全部须 1:1）：Perform（C2 区间分解→PerformBounds 循环）、PerformBounds（曲线类型分发：conic→PerformConicSurfLine/Circle/Ellipse/Parabola/Hyperbola；非二次面→NbIntervals(C2)+SamplePars 分段→polygon→InternalPerform；二次面→InternalPerformCurveQuadric；bug26525 的 ClampUVParameters 防溢出）、PerformPolygon/PerformPolyhedron/PerformPolygonPolyhedron/PerformPolygonPolyhedronBSB（nbsu/nbsv 钳 40）、InternalPerform/InternalPerformBSB（Interference→CSFunction→ExactInter(math_FunctionSetRoot)→CollectInterferencePoints→SortStartPoints→ProcessSortedPoints→Append）、InternalPerformCurveQuadric、InternalPerformPolygonBounds（BSplineSurface 分支用 Adaptor3d_TopolTool::SamplePnts 采样 → Polyhedron(Upars,Vpars)——**依赖 2a-4**）、PerformConicSurfLine（Plane/Cylinder/Sphere 直连 IntAna_IntConicQuad；Torus→ProcessLinTorus；Cone 半角校正 1e5·Angular；其余→EstLim* 无限面估界→Intf_Tool::LinBox 分段→polygon）、PerformConicSurf{Circle,Ellipse,Parabola,Hyperbola}（NBSAMPLESONCIRCLE=32/ELLIPSE=32/PARAB=16/HYPR=32）、AppendIntAna（IsParallel 传播）。
>   InterUtils.pxx 清单：UVBounds/DecomposeSurfaceIntervals（C2 UV 区间分解）、ClampUVParameters、CollectInterferencePoints（SectionPoint→候选点：Intf_EDGE/VERTEX→Polyhedron 参数恢复；Intf_FACE→直接用）、SortStartPoints/ProcessSortedPoints（按 W 排序去重→ExactInter::Perform 起点/方向分类→TransitionOnCurve 判定）、ProcessIntAna（IntAna_IntConicQuad 结果→IntersectionPoint：InQuadric 无限交→Tangent；NbPoints→逐点 QuadParameters/ComputeAppendPoint 判据）、PerformCurveQuadric（QuadCurvExactHInter 路径）、ProcessLinTorus、ComputeAppendPoint（Dist<=Tol 判据）、DoSurface/DoNewBounds（InterUtils 网格采样，HInter::DoSurface 委托）、EstLimForInfSurf/Extr/Revl/Offs + ProjectIntersectAndEstLim（无限 Extrusion/Revolution/Offset 面估界——依赖 ProjLib_Plane/IntAna2d/Extrema_ExtElC；rcad ProjLib/IntAna2d 状态需先清点）。
>   依赖状态：已就绪 = TheInterferenceOfHInter（alias）、ThePolygon/Polyhedron+工具、TheCSFunctionOfHInter（ZerCSParFunc）、TheExactHInter（IntCS）、Bnd_BoundSortBox、Intf_Tool、IntAna_IntConicQuad（kernel int_ana.rs，需核对 IntConicQuad API 面）；**缺** = IntCurveSurface_IntersectionPoint/TransitionOnCurve 数据类、TheHCurveTool（3D 曲线工具）、HSurfaceTool trait（GetType/Plane/Cylinder/Sphere/Cone/Torus/NbSamplesU/V/UTrim/VTrim/BasisSurface/BasisCurve/Direction/AxeOfRevolution/OffsetValue——UTrim/VTrim/TopolTool 分支依赖 2a-4，可先做 documented unimplemented）、TheQuadCurvExactHInter+函数（ElSLib/IntAna 已有）。
>   建议切分：④-a 数据类+工具 trait+InterUtils 核心（Collect/Sort/Process/ProcessIntAna/ComputeAppendPoint/DoSurface/DoNewBounds/ClampUV/Decompose）→ ④-b Inter.pxx + HInter 壳（Plane/Cylinder/Sphere/Cone 解析路径全通，Torus/EstLim*/BSplineSurface-TopolTool 分支 documented unimplemented）→ ④-c 补 EstLim*/ProcessLinTorus/QuadCurvExact。锚点：直线×平面（IntAna 直连路径）、直线×圆柱（IntAna）、Bezier×平面（IntCS 精化路径）。
