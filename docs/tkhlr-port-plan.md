...
>   - **⑤a 事故与恢复记录（2026-09-07，给 fillet session）**：本 session 在布尔网格归因时执行了 `git checkout <commit> -- libs/`，覆盖了三个冻结脏文件。**已全部恢复**：`fillet/topopebrepbuild.rs`（冻结 WIP，blob ab97efd2，82237 字节，hash 验证字节精确——含 Line3::new 迁移，因其暂存发生在该迁移提交时点）、`tests/tkgeom_algo_gtests.rs`（blob d7b15fa2，133289 字节——含 Ellipse2d minor_dir 字段补丁）、`algo_ext/mod.rs` 的 fillet 接线摘除（按 TEMP-EXCLUDED 语义重建，注释措辞可能与原版略异——fillet session 接手时请过目）。恢复后验证：algo lib 369 / tkgeom 134+1 全绿。教训已记：对含未提交 WIP 的文件严禁 `checkout <commit> -- <path>`，恢复必须先 `git stash` 或逐文件确认。
>   - **⑤b bcommon J1/J2 失败归因（非本 session 回归，待专门处理）**：`bcommon_simple J1/J2`（两同径圆柱平移 50 的 common，checkprops -s 8796.46 vs rcad 15079.64）在本 session 起点 91aed34e 与中途 a68a438d 上**逐位同样失败**——是 2026-09-01 基线之后的既有缺陷（布尔网格自 09-01 起未重生成复测；期间 HLR session 的共享代码改动或生成 harness 的 -s 断言新增均可能，需另行定位）。本 session 的 Line3/Ellipse2d/FunctionSetRoot 修改已被证明与该失败无关。
>   - **⑤ 剩余（非本线，按 runway 原序）**：poly 线路 Stage 5；ElSLib/Contap 层的 ulp 对齐（glam recip-multiply 全局换逐分量除法 + Ax2 框架链两项需 OCCT 侧逐位对拍支持，当前被 Debug 构建崩溃阻塞）。**在册 ulp 清单三项已修（追加 6）**：
>   - **追加 6（Line3 gp_Dir 不变量收口——ulp 清单第 3 项「line_parameter 重复归一化」）**：`Line3` 的方向字段按 OCCT gp_Dir 不变量应恒为单位向量，但 166 处字面量构造绕过 `Line3::new` 的归一化，迫使 `elclib::line_parameter/line_value` 每次调用重归一化（OCCT 是「构造时归一化一次 + 点积直接用存储方向」，ElCLib.cxx L1192-1197）。修复 = 166 处字面量全量迁移为 `Line3::new(origin, direction)`（构造时归一化一次）+ elclib 去掉逐调用 `normalize_or_zero`（按 OCCT 原文直接点积存储方向）。行为修正两处：coons 夹具的非单位方向 (0,1,1) 按单位方向 (0,3/5,4/5) 重写（闭合补丁 + 断言点重算）；`cont_ana_cone_dir` 测试期望按 OCCT gp_Dir::SetXYZ 的归一化语义修正（解析方向 (1,0,√3) 归一化后 z=√3/2，gp_Dir.hxx SetCoord 快慢双路径实证）。ptorus 验收 mass 逐位不变（302.6854487286546）。其余 ulp 清单两项（glam recip-multiply、Ax2 框架链）待 OCCT 侧逐位对拍可用后再动。Elips2d::Reverse 误译（Ellipse2d 补 minor_dir 字段 = OCCT Ax22d 双轴形式；reverse 翻 Y 保持 X = P'(t)=P(-t)；HLRBRep_Curve::ellipse 双轴投影 + D1.Z<0 翻 Y；IConicTool 用存储 Y；30 构造点全量迁移 + kernel Reverse 锚点）与 Parameter3d 阈值（1e-15 → DBL_MIN）。**顺带修复 rcad-step occt_brep 2D circle/ellipse 读取器 token 错位**（OCCT 流存 center+XAxis+YAxis+半径，原读取器把 Y 轴当半径消费）。OCCT 侧事实备忘：干净树 Debug 构建 bug25813_3 在 DSFiller 前段崩溃（早于 IWalking，box/bug25813_1 正常）——重启 OCCT 侧插桩前先解决或改用 Release 自建。
...
>   - **附带对齐修复**：`function_set_root.rs` 非 border 分支两处 **MinimizeDirection 调用对调**（OCCT cxx L1019-1031：`DescenteIter==0` → 2点+导数变体（SolSave,Delta,OldF,F2,DHSave,GH,Tol,F）；else-if → 3点变体（PreviousSolution,SolSave,Sol,OldF,Delta,Tol,F）；rcad 原本写反）。本工况该分支未触发（输出逐位不变），纯形式对齐。
>   - **附带守卫清理**：删 `test_arret_cadre` 两处自创 `k/j >= u_mult.len()` break 守卫（OCCT 无此检查；不变量 sum(nbMultiplicities)==Umult.Length() 保证在界内）。
> - **追加 4 附（IntWalk gxx 个人亲审完毕——追加 3 的亲审清单全部完成）**：TestArretAjout（L965-1031，含 PntOn2S 的 false=S2 约定与 IWLine::value 1-based 验证）、FillPntsInHoles（L1033-1181，含 `i--` 重试语义、BadSolutions Map 语义）、TestArretCadre（L1183-1397）、TestDeflection（L2628-2899，五个减半分支逐一核对）、IsPointOnLine 两个重载（L3029-3048 / L3059-3152，PP1/PP2 取负点积不变）、ComputeOpenLine（L1414-1928，四个 NbPoints()==1 取消分支 + lbr 循环 + movementdirectioninfo 全部 1:1）、ComputeCloseLine（L2007-2624，OpenLine/IsValidEndPoint/RemoveTwoEndPoints/退化边框检查全 1:1）、MakeWalkingPoint/AddPointInCurrentLine/OpenLine（L2908-2998）。IntWalk 层文本层面无残留分歧。
> - **剩余缺口（按序，下轮首任务）**：
>   - **① mass 逐位（Δ1.7e-3）**：走线 dep2/dep3 采样密度差（rcad 2680/1584 点 vs OCCT 81/215；净长一致 88.472/149.725 vs 88.469/149.723——同一曲线、采样不同）。诊断实证（探针已删，结论在案）：TestDeflection 五个减半分支全零触发、零 badroot；步长被压小的源头 = **math_FunctionSetRoot done=false 事件**（迭代 100 次耗尽——froot 已 1e-16 仍不触发停止测试，约每 5-6 步一次，PasC 减半后靠 1.5x/步恢复，再次失败）。下轮首查 `SearchDirection` 的 math_SVD 语义：OCCT Ninc>Neq 走 `math_SVD(DF)` 且检查 `IsDone()`（失败→ChangeDirection=true 切梯度），Solve 有 `wmin=Eps*max(Diag)` 截断（默认 Eps=1e-10）；rcad 的解析坍缩 `-FF·DF/|DF|²` 永不失败——数学等价但失败/截断分支不同。
>   - **② 隐藏门 86.94 需重新推导（重要修正）**：诊断实证 rcad 的 4 条提取边端点=各线 departure 对、投影长 26.26/80.83/88.51/107.09 合计 302.687=全轮廓投影——环面轮廓远近半环的投影互相重合（旋转体轮廓的投影对称性）。故「隐藏 ≈86.94」= 3D 走线总长 389.62 − 投影可见 302.685 的**混合量纲推导值**，非 OCCT 实测（runner exact 在该 case 崩溃=OCCT 自身 bug、viewer path 不提取隐藏）。rcad Hider 当前对 4 条轮廓边零隐藏（DS 状态 all_visible）——真实 OCCT 隐藏行为需在③之后重查；acceptance 的 `assert_rel(m2, 86.94)` 在重新推导前不具裁决力。
>   - **③ OCCT 侧真相获取不可用的环境事实**：今天的干净 OCCT 树重编 TKGeomAlgo（含/不含插桩）+TKHLR 后，runner Debug 构建 bug25813_3 在 IWalking 之前的 DSFiller 前段段错误（box/bug25813_1 正常）；302.685 来自官方 DRAWEXE viewer path（Release）。session-16 的 81/215 点走线真值在当时（Sep 6）可用——外部进程还原过 OCCT 工作树（session-17 已记录的 gremlin），当前树的崩溃点已变。重启 OCCT 侧插桩前先解决该崩溃（或改用 Release 自建）。
> - **回归基线（本追加后，全绿）**：algo lib **368+1i** / builder 76+1 / pavefiller 26 / tktopalgo 36 / tkbo 40 / tkgeom_algo 134+1 / kernel 663；生成测试 exact_hlr **4/4**、helix_standard 56、bfuse_simple_a1 1（blend_simple 编译失败=他 session fillet 半成品 `make_fillet_edge` 未导出，与本任务无关）；acceptance_ptorus 保持 ignore（理由已更新为当前诚实状态）。
> - **验证命令（不变）**：`cargo test -p occt-generated-tests --test generated_occt_boolean_hlr_exact_hlr`（根仓库）；`cargo test -p rcad-algo --lib acceptance_ptorus -- --ignored --nocapture`；回归清单见上。
> - **纪律提醒（沿袭）**：4 个他 session 遗留脏文件勿 add（algo_ext/mod.rs、fillet/topopebrepbuild.rs、tests/tkgeom_algo_gtests.rs——纯 HEAD 缺脏版 mod.rs 不可编译）；tkfeat-fillet-offset-port-plan.md 保持未跟踪；本追加全部 rcad/OCCT 侧探针已删净（OCCT gxx 已 git checkout 还原 + 干净 TKGeomAlgo.dll 已重建放回 runner Debug 目录；temp/add_occt_probes.py 已删）；方法论 = 静态逐行审查 + OCCT 侧插桩取真相，rcad 侧不加探针（测试侧诊断 harness 允许，用后删）。

> **交接快照（2026-09-07 session 19 终（追加 2/3）——被上方追加 4 部分取代，仍要读）**
> - **追加 3（个人逐行纪律审查进度——下轮继续，勿再依赖代理结论）**：已亲自逐行核实 1:1：`TestArretPassage` open（gxx L593-775：wd2 IFV 分支/两遍 etat 扫描/候选 min-sqdist/多重性窗口/sp.Values 副作用——数组对齐后全部一致）、`SearchDirection`（gxx math_FunctionSetRoot L439-533：Ninc>Neq 的 math_SVD 对 1×2 系统与 rcad 坍缩式数学等价，排除）、`Cadrage`、`Contap_SurfProps`、`Contap_SurfFunction`。**下轮按序亲审（对照 gxx 行号）：TestArretAjout（L842 起）、TestArretCadre（L1203-1396）、TestDeflection（L2628-2899）、IsPointOnLine（L3054-3150）、ComputeCloseLine（L2112-2626）、IsTangentExtCheck（L52-88）、FillPntsInHoles（L1100-1200）。**症状定位：rcad 2 开放 + 1 闭合 vs OCCT 3 开放 + 1 闭合——某个开放行走线未在另一 departure 处停下或某个 walk 被取消路径吞掉；重点查 ComputeOpenLine 内三个 `NbPoints()==1` 取消分支的到达条件。**
> - **追加 2（同窗口第三次提交）：`math_FunctionSetRoot` 补全整个停车测试段（cxx L1292-1390）——碎片化主因确认。**rcad 特化版此前缺失：Save(Kount)=F2 历史、Verif/IsSolutionReached 提前收敛退出（hxx L69-79：末步 |Delta(i)|<=Tol(i)）、进度分析（aTol_Func 应为 Standard_Real **Epsilon(F2)**，曾误写 f.tolerance()；0.95*Save(Kount-5) 历史、Gnr1>0.9*Oldgr 的 Newton/梯度切换）、回归处理（Cas de regression）。旧版收敛只能靠梯度退化副作用出口，跑满 Itermax 后 Done=false → 走线侧 bad-root → PasC 减半 → 碎断成 39 碎片。**补全后：3 条整线/276.4/顶点 5（2 开放 + 1 闭合；OCCT 4 条/389.62/顶点 7 = 3 开放 + 1 闭合）**——缺 1 条开放线（内容 ≈113.19）。
> - **下轮首任务（一次一个）**：① `SearchDirection` 语义对照（OCCT Ninc>Neq 走 math_SVD 最小范数解 vs rcad 的 −FF·Grad/|Grad|² 解析坍缩——数学等价但 SVD 截断阈值可产生不同合法方向；确认 rcad 坍缩式与 OCCT SVD 的数值一致性）；② departure 前置检查 `IsPointOnLine` 是否把某 departure 误标 processed（8 departures → 只走出 3 线）；③ TestArretCadre/ExcludeAllSol 分支。验收线不变：4 整线/302.685 逐位/隐藏 86.94。
> - **追加 1：`ToroidalSurface` 补解析 D2**（此前走 rcad 自创有限差分 trait 默认；OCCT ElSLib 解析）——Contap walker 对环面经 NormAndDn default 分支消费 D2。
> - **主提交（session 19）**：① bug25813_1 acceptance 全绿 un-ignore——OCCT `nbshapes_hidden`（36/18/6）实为整棵 resulth 树计数（空 aCompHid 也非 null 被计入）；acceptance 重写为 VComputeHLR 树组装，run1=30/15/5、run2=36/18/6 全等，mass rel≤3.5e-8。② 修复 apply_transform 漏变换 CurveOn2Surfaces 曲面（BRepTools_Modifier L191-195 锚）——缝边 CN 降级 C0 漏进 V 的先在回归（stash 实证先于本 session）。③ `MakeEdge2d` 顶点走 `add_tvertex_unique`（BRepLib_MakeVertex 1:1 每次新建；位置量化注册表为 rcad 自创，OCCT 无对应物）。④ IntWalk wd 布局反转：Clear()（gxx L110-137）本就注 etat=-10 哑槽 → 1-based 循环对齐在界内；session-16/17 的「0-based 越界复刻」是误读，已恢复真对齐。⑤ 9 文件探针删净（grep 零残留）。回归：lib 368+1i / builder 76+1 / pavefiller 26 / tktopalgo 36 / tkbo 40 / tkgeom_algo 134+1 / kernel 663 全绿；生成测试 3/4（ptorus 走向收敛中）。
> - **验收线（沿袭）**：AGENTS.md 有效位标准（逐位）；1e-2 只是中间里程碑。
> - **纪律提醒（沿袭）**：4 个他 session 遗留脏文件勿 add（纯 HEAD 缺脏版 mod.rs 不可编译）；tkfeat-fillet-offset-port-plan.md 保持未跟踪；方法论 = 静态逐行审查 + OCCT 侧插桩取真相，rcad 侧不加探针（测试侧诊断 harness 允许，用后删）。
> - **验证命令**：`cargo test -p occt-generated-tests --test generated_occt_boolean_hlr_exact_hlr`（根仓库）；`cargo test -p rcad-algo --lib acceptance_ptorus -- --ignored --nocapture`；回归清单见上。
