...
>   - **剩余唯一断点（已钉死）**：rcad 的 `classify(E=2, iFace=4, lf=1, 2.35619)` 在 CS 之前被 Z 门拒绝（无 CS trace），OCCT 同门通过（8 字比较全过 → CS nbpts=2 → IN）。OCCT 侧实测字值：w7=[fMax=32474, vMin=12849, vMax=12849, fMin=13432]、w0=[17206,10641,10641,128]、w1=[17383,10704,10704,130]、w2=[17884,10875,10875,134]、w3=[18628,11152,11152,144]、w4=[19503,11545,11545,160]、w5=[21357,12114,12114,189]、w6=[24123,13020,13020,242]。注意字内半字排列：按 EncodeMinMax（HLRAlgo.cxx L101-157），MM.Min[7] lo=Min.Min[7] hi=Min.Min[6]、MM.Max[7] lo=Max.Max[7] hi=Max.Max[6]——**dim14=字7 hi(dim=Max.Min[6]/Max.Max[6] 的 max 侧)、dim15=字7 lo**，diff 时勿再搞错半字。
>   - **下一动作（收口动作，精确到函数）**：在 rcad `classify.rs` 的 lf=true 分支 encode 之后（L~1895 门比较前）加与 OCCT `[DBG-G]` 完全同格的 8 字 dump（iFaceMinMax 7 字 + MinMaxVert 7 字的 lo/hi 半字），跑 bug25813_1 对拍 → 找出第一个不同的字 → 回溯其输入（vert 侧 = reject1 的对应维；face 侧 = 已证相同）→ 修复对应构造。修复预期：16.93 隐藏 → harness visible 239.09−16.93−(seam 17.97 ③) → 204.19。
