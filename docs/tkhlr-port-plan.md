...
> - 提交链（rcad 子模块分支 `sd-hash-wip`）：`7011f938` 计划落盘 → `89fdd3ec` Stage 0 → `78789bbf` Stage 1 → `6f0f12b8` 2a 审计+Bnd_Range → `34dfaa5b` 2a-1 批次（Intf 数据类/BoundSortBox/PGProps+PEquation）→ `da383601` 2a-1 完成（IntCurve 三泛型）→ `5ef9ae00` 2a-2 IntImp 部分（ZerCSParFunc + IntCS）→ `57ab4ff5` 2a-2 完成（IntImpParGen_Intersector 泛型引擎 + IntConicCurve 双泛型）→ `5cde756c` 2a-3①（Intf_InterferencePolygonPolyhedron 1368 行 1:1）。
> - 回归基线（全部全绿）：algo lib 125/125（120+5 新增）、kernel lib 645/645、tkhelix_gtests 16/16、pavefiller_stage_tests 26/26、tkgeom_algo_gtests 134+1。
> - **下一步 = 2a-3②：`Intf_InterferencePolygon2d.cxx`（819 行 + hxx 78，`$OCCT_SRC/src/ModelingAlgorithms/TKGeomAlgo/Intf/`）**——2D 多边形自/双干涉引擎，是 `IntCurve_IntPolyPolyGen.gxx`（1797，2a-3③）的直接前置（IntPolyPolyGen 消费它的 NbSectionPoints/PntValue/NbTangentZones/ZoneValue+GetPoint）。要点：自干涉 ctor 用 `Intf_Interference(true)` + `|iObje1-iObje2|<=1` 跳过相邻段；`Clean()` 把单段相切带降级为 SectionPoint 并删除被切相带包含的点（注意 `myTZones.Remove(ltz-decal)` 的 decal 回卷与宏 PI1/PI2）；`Intersect` 用 8 槽 parO/parT 数组 + `Epsilon(lgO+lgT)` floatgap + `sinTeta=Tolerance/rayIntf` 相切带。 rcad 侧 `IntfPolygon2d` trait（intf_interference.rs L18）需扩展 `Segment(i)->(p1,p2)`/`NbSegments`/`DeflectionOverEstimation`/`Bounding`/`Closed`（OCCT Intf_Polygon2d 面）；消费方 Polygon2dGen 已实现其中大部分（int_curve_generics.rs，注意其 `segment(i)` 返回元组 1-based）。之后 2a-3④ = IntCurveSurface_HInter 组装（581，消费 TheInterferenceOfHInter 即 2a-3① 的 alias）。
...
| Intf_InterferencePolygon2d.cxx（819） | IntPolyPolyGen 的前置 | ❌ 2a-3②（下一项） |
