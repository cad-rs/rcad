...
> - 提交链（rcad 子模块分支 `sd-hash-wip`）：`7011f938` 计划落盘 → `89fdd3ec` Stage 0 → `78789bbf` Stage 1 → `6f0f12b8` 2a 审计+Bnd_Range → `34dfaa5b` 2a-1 批次（Intf 数据类/BoundSortBox/PGProps+PEquation）→ `da383601` 2a-1 完成（IntCurve 三泛型）→ `5ef9ae00` 2a-2 IntImp 部分（ZerCSParFunc + IntCS）→ `57ab4ff5` 2a-2 完成（IntImpParGen_Intersector 泛型引擎 + IntConicCurve 双泛型）→ `5cde756c` 2a-3①（Intf_InterferencePolygonPolyhedron 1368 行 1:1）→ `f528da46` 2a-3②（Intf_InterferencePolygon2d 819 行 1:1 + Polygon2dGen 实现 IntfPolygon2d trait）→ `3466868b` 2a-3③（IntCurve_IntPolyPolyGen 1797 行 1:1）→ **2a-3④ 完成（IntCurveSurface_HInter 组装，见 §8 新翻译模式）**。
> - 回归基线（全部全绿）：algo lib **139**/139（135+4 新增）、kernel lib **645**/645、tkhelix_gtests 16/16、pavefiller_stage_tests 26/26、tkgeom_algo_gtests 134+1。
> - **下一步 = 2a-4：TopolTool 真实版**——Adaptor3d_TopolTool/BRepTopAdaptor_TopolTool/HVertex/BRepAdaptor_Curve2d + so_on_bounds 的 BRep 边分支保真（Contap_TheSearch 依赖；也是 HInter 的 BSplineSurface-TopolTool 分支依赖——`HSurfaceTool::u_trim/v_trim` 已是 documented unimplemented，接通点在 `inter_impl::internal_perform_polygon_bounds` 的 BSplineSurface 分支）。
...
1. **2a-4**：Adaptor3d_TopolTool/BRepTopAdaptor_TopolTool/HVertex/BRepAdaptor_Curve2d（Contap_TheSearch 依赖；也是 HInter 的 BSplineSurface-TopolTool 分支依赖——接通点 `inter_impl::internal_perform_polygon_bounds` 的 BSplineSurface 臂 + `HSurfaceTool::u_trim/v_trim` 的 documented unimplemented）。
