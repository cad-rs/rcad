...
>   - **② 机制已定位（最终，OCCT 侧实测 + 静态对读，取代下行 7-9 的中间推断）**：16.93 的隐藏通道 = **Hider 的 ILOn 通道无条件隐藏**（HLRBRep_Hider.cxx L753-794：`EB.Builds(TopAbs_IN)` 于 ILOn（ON 态干涉表），区间**不经 classify 直接** `ES.Hide(p1,tol1,p2,tol2,true,false)`"on the Face"；L796-837 ON 段同理 `...,true,true`）。链路：大顶缘 E=2 与小圆柱两条轮廓线（face4 的 internal w_edges）的投影交点被 OCCT 判 **ON 态** → 进 ILOn → 中段 [1.4289,3.2835]（2D 域 [0.6435,2.4981]，Parameter2d 偏移 ox=0.7854）被无条件按"面上"隐藏。Z 门与该通道无关。
>   - **OCCT 侧实测（DBG-G2 全字）**：E=2 的两次 EB classify（ILHidden-IN 通道 + ON 通道，均 iFace=4 lf=1）都被 Z 门拒绝（t2=0x80008000：中段点深度 zsta=9.1555 在 face4 深度域外——几何正确，Z 门无罪）；同一区间仍被隐藏 = ILOn 无条件通道所致。
>   - **rcad 现状**：同一区间被 rcad 判 **IN 态** → 进 ILHidden → classify Z 门拒绝 → 丢弃。rcad 的 ILOn 通道存在且结构 1:1（hider.rs L862-949），但 ILOn 里没有这个区间。**偏差 = 跨越点 State 的 ON/IN 判定**，位置 = 干涉组装函数（OCCT Data.cxx L~2400-2608 vs rcad classify.rs L~2250-2500 同款 `st` 计算段；OCCT L2583 `if (myFEInternal) decal = 2;` 提示 internal w_edge 跨越与 ON/decal 的关联——`st` 的赋值段就在函数头到 L2583 之间）。
>   - **下一动作（收口，静态可决）**：逐行对照 `st` 判定段，专查"被跨越的 FE 是 hiding face 的 internal/outline w_edge"时 st 的赋值路径（预期 OCCT 判 ON → myIntf 进 ILOn → 无条件隐藏；rcad 误判 IN → 进 ILHidden → 被 Z 门丢弃）。修后预期：16.93 隐藏 → bug25813_1 harness 239.09−16.93−(③ seam 17.96) → 204.19。注意：trace 证据链（DBG-G2 t2 拒绝 vs EB-IN state=0 的先后归属）在 session 17 内部有过反复，以上结论以 DBG-G2 全字（t2=0x80008000）+ ILOn 无条件隐藏代码为准；若下 session 复测发现矛盾，优先重验"两条轮廓线交点的 State 是 ON 还是 IN"。
...
> - **OCCT 侧插桩已全部还原**（git checkout 已执行，TKHLR debug DLL 需重编才能去除 print——不影响 rcad 工作）。runner 的本地 build/Debug/TKHLR.dll 副本会遮蔽 PATH，重编 OCCT 后必须重新 cp（本 session 踩过）。
> - **纪律提醒（沿袭）**：4 个他 session 遗留脏文件勿 add；tkfeat-fillet-offset-port-plan.md 保持未跟踪；rcad 探针收口时删净（清单不变）；方法论 = 静态逐行审查 + OCCT 侧调试工作流取真相，rcad 侧不加新探针。
