...
- **下 session 队列（按序）**：
  1. **x100 侧 transition 值审计（p9 终结前置；E3-Q 主代理两轮已定论，修正此前全部假设）**：**OCCT 侧真值 = a1 从不调用 `ChFiDS_CommonPoint::SetArc`**（唯一汇聚点冻结探针零输出），且 `ChFi3d_FilCommonPoint` 弧臂、SpKP `FillSD`/`CompCommonPoint` 三个独立探针同样零输出；而 **rcad 侧 set_arc 被调 6 次**（param 10.0，4 次来自 `chfi3d_builder_spkp::fill_sd`、2 次来自 `chfi3d_builder_c1::update_on_face`）→ rcad 的 CP 落在弧上 → `ChFi3d_FilPointInDS` 发射 CurvePoint 干涉（Builder_0 L2824 的 `V1.IsOnArc()` 门前置）→ 多余边分割 → 相对参考 +1 边/+2 点/1 开壳（7F/16E/12V vs 7/15/10）。**结论：分歧在组合层上游——rcad 进入了 SpKP/fill_sd 路径并把 CP 标成 on-arc，OCCT 两者皆无**（此前"死 FilCommonPoint 臂""KPart 路径 1:1 即无虞"两判断均已被证伪：组合函数本身 1:1，但根本不该在此处被调用）。下一轮切入点：(a) **OCCT 侧 a1 确为 KPart（plane/plane）**——`ChFiKPart_ComputeData::Compute` L94-106 分派 `ChFiKPart_MakeFillet`→FilPlnPln（该文件无 SetArc ⇒ OCCT 的 CP 不带弧），随后 SplitKPart 的 **FillSD 未达**；(b) rcad 的调用序列与 OCCT **逐语句一致**（`chfi3d.rs` L3158 k_particular 门 → L3169 `compute_data_compute` → L3182 `split_k_part`），故分歧在 **SplitKPart 的分支选择判据**：OCCT SpKP.cxx L856-915 三分支（`C1.IsNull()&&C2.IsNull()→return false`；`C1.IsNull()||(Nb1==1&&!H1.Domain(iH1,1).HasFirstPoint())→退化边支(带 FillSD)`；其余通用支），a1 落在哪支取决于 `C1/C2` 与 `Tri(H2,iH2,...)` 的 hatch 域状态——rcad 落进了带 fill_sd 的那支而 OCCT 没有，故下一步 = 逐语句核对这三个判据（hatch 域 HasFirstPoint/Tri 结果）在两侧的取值。**复现资产**：`tools/occt-bool-runner/blend_probe.cpp` 新增 `a1` 模式（box 100³ + 棱 idx5 = (100,0,0)-(100,0,100) r=10，真值面积 59527.876，与 DRAW 链一致）。**排障副产物**：OCCT debug 增量构建存在**陈旧对象**陷阱——前 session 回退源码但 .obj 更新，导致旧 `[BLENDPROBE] AddPoint` 探针残留在 DLL（已强制重编 TKFillet 清除）；判断探针是否生效前须核对 DLL 时间戳。之后 p9 载体切换（cncrn.rs perform 硬编码 done=false → 委托 geomplate::BuildPlateSurface 真身；类型桥 = cncrn 本地 Adaptor3dCurveOnSurface(Geom2dAdaptorCurve,GeomAdaptorSurface 均 pub 字段) → kernel CurveOnSurface(Curve2dHandle=Arc<dyn Adaptor2dCurve2d>, SurfaceHandle) → geomplate CurveConstraint::new(CurveBoundary::OnSurface(..))) → 跑 p9/q7。
