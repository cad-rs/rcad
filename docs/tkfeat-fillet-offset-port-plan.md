...
- **下 session 队列（按序）**：
  1. **x100 侧 transition 值审计（p9 终结前置）**：a1 探针实证 x100 侧两条切边 CurvePoint transition=REVERSED（y0 侧 FORWARD 正确选片）——REVERSED 在 rcad 边存储方向下选废片。溯源：OCCT FilCommonPoint 的 SetArc 组合 `Tr=Reverse(Compose(TrsfTrans(TransLine),Or))`（Builder_0.cxx L2281-2292）在 rcad **死路径**（builder_6.rs `point_on_rst_edge` pending boundary 恒 None——BRepBlendPointOnRst 不携带 TopoDS_Edge）；现值来自其它填充点。修复 = 让限制曲线携带所属边并激活 FilCommonPoint 弧臂 1:1。之后 p9 载体切换（cncrn.rs perform 硬编码 done=false → 委托 geomplate::BuildPlateSurface 真身；类型桥 = cncrn 本地 Adaptor3dCurveOnSurface(Geom2dAdaptorCurve,GeomAdaptorSurface 均 pub 字段) → kernel CurveOnSurface(Curve2dHandle=Arc<dyn Adaptor2dCurve2d>, SurfaceHandle) → geomplate CurveConstraint::new(CurveBoundary::OnSurface(..))) → 跑 p9/q7。
