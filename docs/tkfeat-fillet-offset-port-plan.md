...
- **下轮队列（按优先级）**：
  1. **g6 解析回转体**：生成器改为对 `profile … C r angle` 输出真圆弧（勿折线近似），rcad 侧按 `BRepSweep_Revol` 补含圆弧导线的回转（复用 `libs/rcad-algo/src/brep_sweep/{revol,rotation,num_linear_regular_sweep}.rs`），让 rev2 得到 2 圆柱 + 2 回转面；随后 g6 拓扑应可逐项对上 10F/27E/18V。**注意 g6 现在只要 0.65s**，调试成本已可接受（`RCAD_STEP_DIR=… RCAD_STEP_ONLY=1 <exe> --exact <case>` 先导 STEP 再 `step-topo-diff`）。
