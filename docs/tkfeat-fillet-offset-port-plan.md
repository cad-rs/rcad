...
- **下 session 队列（按序）**：
  1. **x100 侧 transition 值审计（p9 终结前置；E3-Q 主代理首轮已收束路径，修正此前假设）**：a1 走 **KPart（SpKP）路径**——set_arc backtrace 实证 4 个 CP 弧值全部来自 `chfi3d_builder_spkp.rs fill_sd`（SpKP.cxx L627-742）→ comp_common_point/comp_tra，**逐语句核对已 1:1**；此前"死 FilCommonPoint SetArc 臂"假设对 a1 不成立（该死臂只影响非 KPart 路径，`builder_6.rs point_on_rst_edge` 恒 None 仍待激活）。剩余分歧输入 = fill_sd 上游：HatchGen 域的 `interference(ons).transition()` 初值与 M 图中边朝向（SpKP hatch 机制）——下一探针轮从这里切入；a1 面积虽过 1% 带但拓扑仍 7F/16E/12V vs 参考 7/15/10（1 开壳），废片/缺片在此机制内。之后 p9 载体切换（cncrn.rs perform 硬编码 done=false → 委托 geomplate::BuildPlateSurface 真身；类型桥 = cncrn 本地 Adaptor3dCurveOnSurface(Geom2dAdaptorCurve,GeomAdaptorSurface 均 pub 字段) → kernel CurveOnSurface(Curve2dHandle=Arc<dyn Adaptor2dCurve2d>, SurfaceHandle) → geomplate CurveConstraint::new(CurveBoundary::OnSurface(..))) → 跑 p9/q7。
  2. **offset IsDone 余前沿**（40 例矩阵，BuildCurves3d 已解锁尾断言）；③ 报告：brep_offset_tool_b.rs 两调用点未切（无 &mut BRep 可达，超"仅切换所需行"边界——已记档）。
