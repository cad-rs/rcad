...
### E3-P. 三批并行续推记录（2026-09-10 续——I/J/K 收官，全量回归基线保持）

- **基线复验**：lib 411/0/0、kernel 672/0、stage 76+smoke 1、pavefiller 26、五网格 744/4 · 751/4 · 741/4 · 201/0 除 g6 · 16/2 —— 全绿。
- **I 近似栈引擎核心批（提交 `f6da9b87`）**：ApproxF2var **全量 33=33**（mma1cdi/cnt/fdi/fer/jak/noc/nop + mma2can/cd1/cd2/cd3/cdi/ce1/ce2/cfu/cfv/ds1/ds2/er1/er2/fnc + mmjacpt）+ MathBase 闭包 21 叶（累计 25）+ Mmjcobi/Mmcmcnp 块数据表 + EvaluatorFunc2Var trait；mma2ce1_ 未初始化 XMAXJU quirk 保真注记。余 GAP = AdvApp2Var 类层（~4,170 行：Node/Iso/Network/Framework/Context/Patch/Criterion/ApproxAFunc2Var）+ PLib::EvalPolynomial + Convert 12 参 ctor（kernel，主代理随类层补）。
- **J offset 深层分歧批（提交 `c1974d70`，10 项锚定修复）**：真 `BRep_Builder::UpdateFace`（原为 no-op GAP——偏移面从未获得偏移曲面！）+ 单池架构修正（BRepOffsetOffset 私有 per-face arena 解散，&mut BRep 贯穿 11 文件）+ ElSLib GAP 全家（ConeParameters/VIso/UIso/normalizeAngle/gp_Ax2 旋转）+ Cone Init 真顶点 + IndexedDataMap 1 基缺陷 + WireExplorer 耗尽返 null + Precision::Infinite(2e100) 约定 + MakeSplitEdge 1:1 + offset_shape_is_null 免池谓词。**进展：13/40 例跑至最终断言（此前 0 例过 Intersection3d 早期崩溃）；除新暴露的 BuildCurve3d GAP 外管线级 panic 清零**。
- **K W3 tranche 2（提交 `747f955d`）**：ShapeFix_Wire 全类 **105=105**（含 Wire_1 FixGaps + lxx 内联 + 10 静态，~7,500 行，子模块拆分）；ShapeFixWireGap 载体退役；ShapeFixFaceGap/ShapeGap 嵌真 ShapeFixWire；UnifySameDomain 的 set_fix_wire_modes 驱动真 mode 标志。余：ShapeFix_Face/Shell（下批）。
- **主代理 ①filds 审计三轮收束（提交 `f9152c8a` 记档）**：CommonPoint 数据与 DS 登记均正确（两弧均入 bopds）——**真因 = 8 字 WES 块**（两个 3 度顶点在分割点）被整环输出为 1 个 7 边面；`detect_unclosed_wire` 判 CLOSEDW 放行。**下一钻点 = FaceAreaBuilder 环拆分/面积分类机制全文 1:1**。
- **下 session 队列（按序）**：① FaceAreaBuilder 环拆分批（blend a1/b5 终结）；② AdvApp2Var 类层 + PLib::EvalPolynomial + Convert 12 参 ctor（p9 载体切换的最后前置）+ cncrn.rs 载体切换；③ BRepLib::BuildCurve3d/MakeCurve3d 翻译批（解锁 offset_shape_type_i 10 例）；④ offset 余前沿（相切对 Inter3d / MakeThickSolid ya_result / i_c 14 例的 algo_ext 退化棱柱面——owner=occt-test-gen）；⑤ W3 tranche 3（ShapeFix_Face/Shell）；⑥ kernel 全域 Precision::Infinite 约定统一（~82 处 is_infinite 消费者，需协调批）。
- **在役协议**：验证禁经管道；回归口径 lib+kernel+stage+pavefiller+五网格；探针即用即清；并行代理文件不相交 + CARGO_TARGET_DIR 隔离 + 每 Edit 后整树 check；`git add` 显式清单；blend 不在门槛内但状态如实记档。
