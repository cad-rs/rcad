...
### E3-W. 新 session 入口（输入 pcurve 架构批落地 + g6 打通全链路，2026-09-11——唯一权威交接点）

- **开场三步**：① 通读本档 §0 → §9 E3-S/E3-U/E3-V/E3-W → AGENTS.md（**铁律：非 TKBool 模块的代码与修复一律严格 1:1 翻译对齐——逐行语句对照 + 函数计数等式 + OCCT 行号锚点 + 禁载体/禁等价替换/禁运行时凑结果；架构差异必须先消灭再对齐；`algo_ext/` 下凡与 OCCT toolkit 有明确对应的代码必须随手迁往其 OCCT 目录**）→ ShHealing 两份；② `cd rcad && cargo test -p rcad-algo --lib` 确认基线 **412/0/0**（kernel **677/0**、builder_stage 76 + smoke 1、pavefiller 26、**boolean 八网格**：bopfuse 371/4 · bopcommon 374/4 · bopcut 379/0 · boptuc 369/4 · splitter 10/2（失败集 ze7-ze9/zf1 + a2/b2）+ bfuse_simple 102/102 · bcommon_simple 83/83 · bcut_simple 109/109 **（必须 `-Exclude "g6"`）**）；③ 从下方队列取项开工。
...
- **提交链尾与推送状态**：rcad **`main` 已推**：`39ceac52`（**本轮核心**：E3-W pcurve 架构批 + g6 全链路 + Extrema_CurveTool 差异消灭 + make_revol 迁移）+ 其上的 docs 提交（本节提交链尾记录）← `36e043c7`（docs：E3-V 入口，上一轮的 G6 根因记录 + 任务书）← `e92af2a3`（IntTools_Context FClass2d 按面缓存，1:1 缺口修复）。**根仓库 `b4c9858`（rcad 指针 sync）已提交但按用户要求未推送**；根 `main` ahead 3。**rcad 工作树干净（仅 `temp/` 未跟踪）**。
- **下轮队列（按优先级）**：
  ...
  4. **清理**：`rcad-kernel/src/math/gprop/` 死目录；`face_classifier.rs` 的 `[FC]` eprintln；`CurveToolHandle::for_curve3` resolution=1.0 占位；`normal_projection.rs` 的 BRepLibMakeWire 载体切 A2 真身；`rcad/temp/regress-e3167`（894MB）。
- **本轮固化/强化的坑与协议**：
...
- **资产位置**：FClass2d 缓存 = `bop/int_tools/context.rs`（`fclass2d_cache`/`fclass2d`）；UV 域替代 = `bop/ds/mod.rs::face_actual_uv_bounds`（含它自述的 GAP 注释）；pcurve 构建机制 = `fillet/topopebrep_tool_curve_tool.rs:342`；DS 面/边 pcurve 键 = `ds/mod.rs::pcurve_face_key` + `topods.rs::TEdgeData::pcurves`；八网格 = `tests/occt`（test 名 `generated_occt_boolean_{bopfuse,bopcommon,bopcut,boptuc,bfuse,bcommon,bcut}_simple` + `splitter`）；OCCT 源 = `C:/Users/lilu/works/OCCT/src/`。
