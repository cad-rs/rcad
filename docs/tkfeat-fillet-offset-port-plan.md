...
- **下轮队列（按优先级）**：
  1. **g6 解析回转体**：生成器改为对 `profile … C r angle` 输出真圆弧（勿折线近似），rcad 侧按 `BRepSweep_Revol` 补含圆弧导线的回转，让 rev2 得到 2 圆柱 + 2 回转面；随后 g6 拓扑应可逐项对上 10F/27E/18V。**注意 g6 现在只要 0.65s**，调试成本已可接受（`RCAD_STEP_DIR=… RCAD_STEP_ONLY=1 <exe> --exact <case>` 先导 STEP 再 `step-topo-diff`）。**关键资产（本次已勘明，勿重复摸索）**：rcad **已有**全套 1:1 `BRepSweep_*` 翻译——`libs/rcad-algo/src/brep_sweep/`（`revol.rs` 157 + `rotation.rs` 1008 + `num_linear_regular_sweep.rs` 782 + `trsf.rs` 452 + `tool_rehost.rs` 1191 + `translation.rs` 553 + `brep_sweep_builder.rs` 588 ≈ 5.5k 行，含 `BRepSweepRevol::with_angle/with_full_circle`）。但 **`BRepSweep_*` 属 TKPrim**（`$OCCT_SRC/src/ModelingAlgorithms/TKPrim/BRepSweep/`），按 module-map 该 crate 是 `rcad-modeling`，而它现在住在 `rcad-algo` ⇒ 与刚迁入 `rcad-modeling/prim/primapi/make_revol.rs` 的 `BRepPrimAPI_MakeRevol` 被依赖方向割开（`rcad-algo` → `rcad-modeling` 单向，`rcad-modeling` 无法反向调用）。**故本项含一次目录/归属迁移：把 `brep_sweep/` 整体迁入 `rcad-modeling/prim/brepsweep/`**（对齐 module-map §1 的 rcad-modeling ↔ TKPrim 行），再让 `make_revol.rs` 用真 `BRepSweep_Revol` 建含圆弧导线的回转；生成器侧把含 `ProfileSegOut::Arc` 的轮廓改发新 API（`ProfileSegOut`/`ProfileSegment` 已能表达圆弧）。迁移需全局改 `crate::brep_sweep::…` 引用点并逐包 `cargo check`。**这是个大件（跨 crate 迁移 + 新 API + 生成器改动），建议单开一批做。**
