...
- **下 session 队列（按序）**：
  ...
  3. **ProjLib 非解析分支**（ComputeApproxOnPolarSurface/Approx_CurveOnSurface）；④ 的 Geom2dIntGInter 全臂。
  4. **kernel Precision::Infinite 约定统一**（~82 处，J 已示范）。
  5. shape_analysis/wire_checks.rs check_loop 四出图扩展（④ 为 FixLoopWire 所加，W2 owner 知悉）。
