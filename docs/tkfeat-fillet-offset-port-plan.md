...
- **下轮队列（按优先级）**：
  1. **g6 拓扑对齐**：rcad V=52 vs OCCT 18（F/E 未及打印）。用 `step-topo-diff` 对比 `tests/occt/step_output/occt_boolean_bcut_simple_g6.step` 与 `ref/bcut_simple_g6.step`，从多出的 34 个顶点回溯到产生它的阶段（先怀疑 EF/FF 的 section 边拆分与 `FaceMakeCurve` 的 PLine 合并）。注意 g6 单跑 ~282s，调试成本高，建议先做一次 `RCAD_DUMP_PIPELINE=1` 阶段快照定位。
