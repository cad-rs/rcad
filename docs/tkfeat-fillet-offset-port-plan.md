...
- **下 session 队列（按序）**：
  ...
  2. **offset IsDone 余前沿**（40 例矩阵，BuildCurves3d 已解锁尾断言）——**E3-Q 首轮子代理因基础设施错误中断，未产出修复**（其遗留的 5 个 offset 文件仅含 `[OFFDBG]` eprintln 探针，已按探针即用即清回退；tree 现干净）。重派时建议先从 `brep_offset_make_offset_c.rs` L681-856 的 6 个 UserBreak 闸门与 `check_single_point` 的 BadNormalsOnGeometry 零导数分支切入（探针位点已由该代理勘明）；③ 报告：brep_offset_tool_b.rs 两调用点未切（无 &mut BRep 可达，超"仅切换所需行"边界——已记档）。
