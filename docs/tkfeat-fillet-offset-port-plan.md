...
### E3-W 追加 19 补记 3（2026-09-13：**逐例失败地图是不稳定的** —— 两处"待归因"实测为**测量假象**，含 5 次复跑的 3:2 分裂证据）

> **本补记撤销补记 2 里的"两处待归因"框架**：两处都已用"换树复测 + 同树复跑"定性，结论是**测量方法的问题，不是代码回归**。
> **方法（可复用，两个都是廉价的决定性手段）**：① **换树复测** = `cd rcad && git checkout -b tmp <上一轮 tip>` → 在根仓库 `cargo test --no-run` 重编 → 跑同一张地图 → `git checkout main` + 删临时分支；
> ② **同树复跑** = 同一份已编译的 exe **连跑 5 次**同一条用例。

- **① `draft_angle` 的 25 vs 28 —— 记档漂移，非回归**。在 `33ad8514`（追加 18 收尾树）上实测：**库内 panic = 28**，
  构成 **17 × `draft_modification_1_b.rs` + 6 × `draft_modification_1_c.rs` + 2 × `make_revol.rs` + 2 × `approx_int.rs` + 1 × `brep_offset_api_draft_angle.rs`**（其余 21 例为测试断言），
  **与本 session 当前树逐项相同**。⇒ 交接档里追加 18 记的"25"是**过期的数字**（本档坑 7 早就警告过"文档基线会漂移，开场必须实跑"），
  **本 session 五个批次对 `draft_angle` 的失败层零影响**。补记 2 与追加 19 正文里的"25 → 20"两步都是错的，**以本补记的数字为准**。
- **② `offset_shape_type_a` a4 的 `image.rs:159` ↔ `brep_offset_make_offset_c.rs:52` —— 同树同 exe 的**运行间**分裂**。
  **证据**：在同一棵已验证的树（`fe297616`）上、用**同一份已编译的 exe** 连跑 5 次，得到
  **`brep_algo/image.rs:159` ×3 与 `brep_offset_make_offset_c.rs:52` ×2** 两种结果。
  ⇒ 补记 2 记的"批次 3 推到 image.rs:159 / 批次 4-5 又退回 GAP"**不成立**——那只是两次各跑一次时的**抛硬币结果**；
  批次 4 tip 单独实测也同时在两种结果间摇摆（同一次测量给出 `brep_offset_make_offset_c.rs:52`，而更早那次给出 `image.rs:159`）。
- **根因（高度可信，未逐点证实）**：rcad 代码里大量遍历 `std::collections::HashMap` / `HashSet`，其**默认 hasher 是每进程随机种子**
  ⇒ **同一棵树的两次进程，遍历顺序不同** ⇒ 第一个触发的失败点在"有多个独立失败点"的用例上就会变。
  本用例正属此类（offset 的 `encode_regularity` 用 `HashMap` 形状集合 + `image.rs` 的 image 判据）。
  **⚠ 这不只是"报数难看"**：**任何"取第一个元素"的算法都会随进程漂移**（不是堆栈随机化，是**哈希序**）——
  凡涉及"多解/多候选取一"的路径，rcad 与 OCCT 的确定性假设**不成立**，需要按 OCCT 的容器序（`NCollection_Sequence`/`IndexedDataMap` 的插入序）逐点核对。
  这条**可能不止影响报数**，是**潜在的行为差异来源**，值得单独立卡（见下文"待立卡"）。
- **⇒ 方法纪律（并入坑 33）**：① **失败地图必须连跑 ≥3 次**，报"某例在 X 处失败"时若出现过多个 X，要写成**集合**并注明分裂比例；
  ② **归因（"谁改的"）必须换树复测**，不能拿两次单跑的差异当因果；③ 同树复跑成本≈0（exe 已编译），**没有任何理由省这一步**。
- **⇒ 待立卡（新，重要）**：**定位并消除 offset/几何管线里的 `HashMap` 遍历序依赖**（至少：`encode_regularity` 的形状集合、
...
### E3-W 追加 23（2026-09-13：**给真 TKBO 体补 OCCT 公共 facade**（两域并行）—— brep_fill 的 #7 偏差消灭；`BRep_Tool::Tolerance` 收敛出**一份带下限的真身**（另发现**尚存 24 份重复**））

- **组织方式**：主代理做关键路径 + 两个**文件域不相交**的子代理并行（`bop/**`+`brep_algo/**` / 容差六文件及其 import 行），各自 `CARGO_TARGET_DIR`、只 check/门槛/八网格/该域网格、不提交、不改 docs。
- **批次 1（rcad `82383294`，`bop/**`）：补 OCCT 公共 facade**（上一轮 brep_fill 接线缺的正是这些，故当时只能近似）：
  - **`BOPAlgo_Builder`**：`add_argument` / `set_arguments_with_fence`（`BOPAlgo_Builder.cxx` **L103-125**）、`set_to_fill_history` / `has_history`（`BOPAlgo_BuilderShape.hxx` **L114/L117**）、`clear`（cxx **L90-99**）、`perform_with_filler`（**L198-208**）、`perform_internal`（**L212-227**）、`perform_internal1`（**L310-446**，= 把 BuildShape 从 GF 趟里拆出去的形态）；`BOPAlgo_BuilderShape` 的 **history 读面**（hxx **L52-110**：`modified`/`generated`/`is_deleted`/`has_*`/`history`，含 `myFillHistory` 门控与 `History()` 的惰性构造）与 perform 后访问器；`build_bop` / `build_bop_states`（状态表 hxx **L214-249**，体 cxx **L479-885**）与 `FillInternals`（`BOPAlgo_Tools.cxx` **L1751-1908**）。
    ⚠ **没有复制第二份管线**：新增 `my_is_builder_only` 标志（与既有 `my_is_splitter` 并排）使 BuildShape 步成为条件步 —— 这正是 OCCT `BOPAlgo_Builder::PerformInternal1` 与 `BOPAlgo_BOP::PerformInternal1` 的关系。
  - **`BRepAlgoAPI_BuilderAlgo`**：`my_is_intersection_needed` / `my_fill_history` / `my_history` + `new_with_filler()`（cxx **L38-47**，flag=false）、`build_with_filler()`（cxx **L81-101 + L107-134 + L138-164**）、`modified`/`generated`/`is_deleted`；`SectionOp::from_shapes_with_filler` / `build_with_filler` / `run_build_section_brep_with_filler`（`ByteOperation.cxx` **L174-200**：跳过求交，`BOPAlgo_Section` 直接跑 `myDSFiller->Arguments()`）。
  - **`BRepTools_History`**：以 `HistoryAlgo` trait 承载 C++ 类模板 —— `from_algorithm(&[Shape], &dyn HistoryAlgo)`（hxx **L99-132** 逐字：`TopExp::MapShapes` 去重前序 → 仅受支持类型 → `Remove`/`AddModified`/`AddGenerated`）、`merge_algorithm`（hxx **L212-217**）、`map_shapes`（`TopExp.cxx` **L49-61**）。
  - **`BOPAlgo_MakerVolume`**：结构体改为持有继承来的 `my_images`/`my_origins`/`my_shapes_sd`/`my_in_parts`/`my_history`（OCCT 基类成员）并暴露同一读面 ⇒ MakerVolume 结果**不再是无历史的**（OCCT `maker_volume.cxx` **L131-149** 填 `myImages`）；`prepare_history` 改调真 `Builder::prepare_history`；`my_fill_history` 默认修正为 true（hxx **L122**）。
  - **顺手修掉两处翻译 bug（都会让后续消费者接线静默出错）**：① `TopExp_Explorer(S, SOLID)` 在 `S` 本身是 solid 时**必须产出 `S`**（首版对 solid 参数产出空 ⇒ `BuildBOP` 结果为空）；② 组合进 MakerVolume 的 `Builder` 未继承 `myFillHistory` ⇒ history 为 null。
  - **已记档的偏差**：`myIsIntersectionNeeded` 无法与**借用的** filler 同存于 Rust 值类型 ⇒ 构造期 flag 与 Build 步拆成 `new_with_filler` + `build_with_filler`（语句全在）；`PerformInternal` 的 try/catch（`Standard_Failure` ⇒ `AlertBuilderFailed`）只有成功路径（Rust 不捕获 panic）；rcad 无 OBB ⇒ `my_use_obb` 取 OCCT 默认；**rcad 的 DS 深拷贝参数** ⇒ 持有"filler 前"形状的调用方**必须经 `ds.argument_remap`** 才能命中 image/history 键（既有 `builder_modified` 就是这么做的）。
  - **已知体积问题**：`bop/algo/builder.rs` 现 **8012 行**（改动前 7020，基线即已超 2000 行规范）⇒ 已立卡，**不**在本批拆分（拆核心 builder 需单独一批 + 全套复测）。
- **批次 2（rcad `8ad5f981`）：`BRep_Tool::Tolerance` 收敛出一份带下限的真身**。OCCT 对**三类形状都做 `Precision::Confusion` 下限**（Vertex `BRep_Tool.cxx` **L1316-1327**、Edge **L881-893**、Face **L139-148**），而 rcad 的六份 re-host 里**五份没有下限**。
  `brep_algo/tool.rs::brep_tool_tolerance` 现为**三臂带下限**的唯一样本（逐臂标 OCCT 锚 + `P_MIN` 对应 `constexpr double pMin`）；删除另外五份（`feat/loc_ope_wires_on_shape_b.rs`、`feat/brep_feat_rib_slot.rs`、`feat/brep_feat_make_d_prism.rs`、`brep_fill/brep_fill_evolved.rs`、`brep_fill/offset_wire_b.rs` —— 最后一份因调用方在 `brep_fill/offset_wire.rs`（本批范围外）而保留**带注释的薄委托壳**，壳内无逻辑），并改其 import 行。
  ⚠ 记档：OCCT **只有三个重载**（`BRep_Tool.hxx` **L87/L256/L345**）、**没有**通用的 `Tolerance(TopoDS_Shape)` 分发 ⇒ rcad 的 match-over-kind 即 C++ 重载决议的替身；`MaxTolerance` 是另一个函数，未动；旧注释里的错锚 `L886-898` 已删。
  **★ 新发现（比原估计大得多）**：库里**另有 23 份** `brep_tool_tolerance` re-host（`feat/loc_ope_{build_shape,pipe,d_prism,find_edges,generator_b,spliter,split_drafts}.rs`、`fillet/chfi3d_perform_elspine.rs`、`hlr/topo_brep/face_iso_liner.rs`、`shhealing/shape_analysis/*` 六份、`shhealing/shape_fix/*` 两份、`topalgo/brep_lib_find_surface.rs`、`topalgo/brep_lib/build_curves3d.rs`、`topalgo/brep_check/brep_check_result.rs` 三份、`rcad-kernel/src/base/proj_lib/brep_adaptor.rs`）—— 其中**只有两份对 Edge 做了下限，没有一份对 Vertex/Face 做** ⇒ **同一处分歧在 24 个定义上重复**。已立卡（见队列 2）。
  另记：`brep_check_result.rs:578` 引的 `BRep_Tool.cxx L896-910` 是**过期锚点**；`shhealing/shape_fix/edge.rs:1260` 的 `CONFUSION` 守卫是 `ShapeFix_Edge.cxx:850` 的**局部变量**逐字翻译，**不是** re-host。
- **批次 3（rcad `90375e64`，主代理）：#7 偏差**消灭** —— 新 facade 的第一个消费者**。`BRepFill_Draft::Fuse` 的 `BRepAlgoAPI_Section aSec(Sol1, Sol2, aPF)`（cxx **L599**）不再用"同参重跑 + 显式传 attribute"顶替，改用 `SectionOp::from_shapes_with_filler` + `build_with_filler`（= OCCT 带 filler 构造的语义：Build 绑定既有 filler、跳过 `IntersectShapes`、section attribute 继承 filler 默认，正是下方 `BRep_Tool::CurveOnSurface(aSEMin, …, 2)` 需要的）。文件头与调用点的 #7 注释已换成字面映射。
  **#8/#9 本轮**未**消灭，障碍已精确记档（不硬凑）**：#8 —— facade 入口已在，但 `perform_with_filler` 会**从 filler 复制** `my_glue`，而 `PaveFiller::set_glue(enable, tolerance)` 只能表达 `GlueFull`/`GlueOff`，**表达不了 cxx L749-753 要的 `BOPAlgo_GlueShift`** ⇒ 正解是给 filler 加 OCCT `BOPAlgo_Options::SetGlue(BOPAlgo_GlueEnum)`（或让 glue 走 `perform_with_filler` 不覆盖的 Builder 侧通路），**不是**把 glue 挪到趟后；#9 —— `merge_algorithm` 体已在，但 **rcad 的 DS 深拷贝参数** ⇒ 调用方的原始形状对不上 builder 的 image 键，直接 merge 会**静默为空** ⇒ 消费者须先经 `ds.argument_remap` 映射。
- **验收（全部在树实测）**：六门槛 **415/0/0 · 689/0 · 36/36 · 26/26 · 76/76 · 1/1**；八网格 **8/8**（375·378·379·373·12·102·83·110，重编 exe 后）；
  **十个域网格逐例失败地图与追加 22 基线逐字节相同**（`draft_angle` 仍 **50/48**、`b3` 未回归）⇒ 三批**零回退**；探针 = 0。
- **⇒ 下一批队列（更新后，前三条为本轮新增且已证据化）**：
  1. **★ #8/#9 的收尾**（先做 #8 的前置）：给 `PaveFiller` 加 `SetGlue(BOPAlgo_GlueEnum)`（OCCT `BOPAlgo_Options.cxx`），再把 `BreppFill_Draft::Fuse` 的 `AddArgument`/`PerformWithFiller`/`BuildBOP`/`BuildBOP(states)` 与 gluer 全部换成字面调用；#9 按 `ds.argument_remap` 映射后启用 `merge_algorithm`。⚠ 动 `bop/**` ⇒ 八网格 + 全域复测。
  2. **★ `BRep_Tool::Tolerance` 的**24 份重复**收敛**（本轮只做了 6 份里的 5 份 + 一份真身）：按目录切片并行（`feat/loc_ope_*` 7 份、`shhealing/**` 8 份、`topalgo/**` 5 份、`fillet`/`hlr`/`kernel` 各 1 份），全部改为委托唯一样本并补上下限。⚠ 398 处调用点 + 多处跨模块 import ⇒ 分批 + 每批跑该域网格。
  3. **`bop/algo/builder.rs` 8012 行拆分**（<2000 行规范；按 OCCT 的 `BOPAlgo_Builder` / `BOPAlgo_BuilderShape` / `BOPAlgo_Tools` 划界拆子模块，父文件只留 re-export）。
