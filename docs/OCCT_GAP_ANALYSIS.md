...
3. Deepen fuzzy strategy with failure-class-aware escalation tuning (baseline adaptive policy now present; retries now also tune make-connected fallback aggressiveness by failure class; configurable conservative/adaptive/aggressive retry policies added).
