...
| Boolean framework | Fuse/Cut/Common/Section + imprint + splitter/cells + MakerVolume baseline + adaptive fuzzy retry with per-attempt diagnostics and retry-class-aware make-connected/glue tuning (including scoped seed-mode/seed-length/history-threshold/ring-depth retuning, bounded same-class retry rounds, plus per-attempt seed source/count/coverage and glue summaries) + iterative/growth-aware/capped make-connected cleanup (global+scoped, semantic seeds incl. tolerance-tagged + multi-PCurve + topology seam candidates, with history-informed seed-edge preference plus configurable ring-depth neighborhood expansion, low-coverage heuristic augmentation, scoped fallback to global cleanup on insufficient seed coverage by vertex-count, edge-coverage, or face-coverage criteria or on no-op scoped passes, independently tunable global fallback tolerance/pass ladder, and seed/fallback phase metadata + stable edge labels reporting) | TKBO | High | glue/deeper scoped connectivity rebuilding semantics, deeper failure recovery |
...
3. Deepen fuzzy strategy with failure-class-aware escalation tuning (baseline adaptive policy now present; retries now also retune make-connected scoped seed targeting, seed length, history augmentation thresholds, ring depth, fallback aggressiveness, and glue tolerance/use by failure class, with bounded repeated same-class retry rounds; configurable conservative/adaptive/aggressive retry policies added).
...
| Fuzzy tolerance option | Small | **Done (boolean/split options + adaptive retry policy baseline + per-attempt retry diagnostics)** |
| CellsBuilder (split-cell graph) | Large | **Done (baseline expression evaluator)** |
| MakeConnected baseline pass | Medium | **Done (iterative + growth-aware + tolerance-capped merge-near vertices + small-edge cleanup, including recovery after initial no-op passes when later tolerance growth can still help; global+scoped modes with semantic short/near-dup/tolerance-tagged/multi-PCurve/topology-seam/hybrid seeds, plus history-informed seed-edge preference with configurable ring-depth expansion, low-coverage heuristic augmentation, scoped fallback to global cleanup on insufficient vertex/edge/face coverage or no-op scoped passes, independently tunable global fallback tolerance/pass ladder, and scoped seed/fallback metadata + stable edge-label reporting)** |
