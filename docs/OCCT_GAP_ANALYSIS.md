...
- **Healing pipeline depth**: same-domain unification now has Phase 1 baseline (plane/cylinder/cone/torus/sphere) and Phase 2 double-validation (topological edge-continuity + geometric UV-region checks); healing orchestration now includes issue-driven pre-make-connected and SameRange/SameParameter parametric consistency prepass plus stalled fallback, but broader ShapeFix/ShapeProcess coverage remains missing.
...
| Healing and validation | SameParameter/SameRange + shell + wire diagnostics (P4 partial) + staged healing with issue-driven pre-make-connected, parametric consistency prepass, and make-connected-on-stall fallback | TKShHealing (10 packages) | High | ShapeUpgrade_UnifySameDomain, ShapeProcess, tolerance rules |
