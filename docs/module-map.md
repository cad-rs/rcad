...
| `shhealing/` | 7 个（最多：ShapeAnalysis_CanonicalRecognition/Edge、ShapeBuild_ReShape、ShapeConstruct_ProjectCurveOnSurface、ShapeFix_Shape、ShapeUpgrade_FaceDivide/UnifySameDomain） | `heal/`（checkshape/direct_faces/drop_small_edges/drop_small_solids/fix_face_size/fix_gaps/fix_shape/reshape/same_parameter/elementary_to_revolution…） | ≈486 脚本：**55 自包含可验收，435 数据依赖排除**（2026-09-09 计划口径；W0 复核修正为 **36 自包含/450 排除**——split_angle/continuity/surface_to_revolution 的 `cases.list` 别名网格与 drop_small_solids 共享 end 脚本均数据依赖，见 `docs/TKSHHEALING_PATH_B_DOCKET.md` §1） | `checkshape`、`fixshape`/`fixface`/`fixedge` 族、`sameparameter`、`unifysamedom` |
