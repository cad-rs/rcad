...
| `hlr/` | TKHLR | `HLRBRep`、`HLRAlgo`、`HLRAppli`、`HLRTopoBRep`、`Contap`、`Intrv`、`TopBas`、`TopCnx` | ◐ 精确线路 Stage 0-3 全关 + Stage 4 推进中（4a/4c 关，4d 断言就位：box 验收全绿 = OCCT ref_output 逐位；bug25813_1 combined 266.5264 精确、余 1 弧隐藏区间缺口；ptorus 余 IWalking 0 线缺口——机制级诊断见 tkhlr-port-plan.md session 12 追加段；poly 线路单列 Stage 5）。基线 algo lib 364+2i / builder 76+1 / pavefiller 26 / tktopalgo 36。**分 session runway 计划与进度勾选表见 `docs/tkhlr-port-plan.md`，每 session 开场先读它** |
