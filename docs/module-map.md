...
| `hlr/` | TKHLR | `HLRBRep`、`HLRAlgo`、`HLRAppli`、`HLRTopoBRep`、`Contap`、`Intrv`、`TopBas`、`TopCnx` | ◐ 精确线路 Stage 0-3 全关 + Stage 4 推进中（4a/4c 关，4d 断言就位：box 验收全绿 = OCCT ref_output 逐位；**bug25813_1 验收全绿 = 全树 nbshapes 30/15/5、36/18/6 全等 + mass 逐位（rel≤3.5e-8）**；ptorus 走线提前终止（42 碎片/246.4 vs OCCT 4 整线/389.62）——分歧在 IntWalk_PWalking/数值链，gxx wd 逻辑已逐行对齐；poly 线路单列 Stage 5）。基线 algo lib 368+1i / builder 76+1 / pavefiller 26 / tktopalgo 36 / tkbo 40 / tkgeom_algo 134+1。**分 session runway 计划与进度勾选表见 `docs/tkhlr-port-plan.md`，每 session 开场先读它** |
